...
Status: proposed RFC design. The abandoned [contextual-popover PR](https://github.com/PrairieLearn/PrairieLearn/pull/15552) and [tooltip PR](https://github.com/PrairieLearn/PrairieLearn/pull/15549) remain implementation references only; replacement work starts from `master` and follows the separate [implementation plan](./implementation-plan.md).
...
- Designing hover cards, peek/side-sheet navigation, or coachmarks/tours. The PL/PT audit found no current overlay requirement for these patterns.
...
- The native Popover API supplies display, top-layer, and light-dismiss mechanics but no content role or complete focus policy. Its January 2025 baseline availability is too recent for PrairieLearn's supported-browser horizon, so it is not an implementation option for this work.
...
Repository validation rules out a text-only contextual-help assumption. PrairieLearn's instructor-preview Tools help contains three links, and PrairieTest has 26 `IconWithPopover` callsites, including table and settings help with documentation links and a linked-assessment list containing an arbitrary number of links. PrairieTest also has roughly 130 shared `InlineEdit`/`CustomInlineEdit` callsites and 20 direct programmatic Bootstrap popover constructors, ranging from single-field editors to multi-field forms and a proctor-assignment table. These are concrete requirements for separate help, related-entity navigation, and anchored-task contracts. The audit found no current PL/PT overlay requirement for an entity hover card or coachmark/tour, so those speculative patterns are not part of the component taxonomy.
...
Static status indicators must remain understandable without hover or focus. Preserve compact visual conventions where they work, but include enough visible text for sighted users to understand the state: for example, the manual-grading table should show a static warning badge containing a triangle and “Open,” not a bare triangle whose meaning exists only in visually hidden text or a native `title`. A compact issue-count badge may keep its visible number when the table context makes the convention clear, with complete meaning in ordinary or visually hidden text and an optional strictly redundant native `title`. If a badge leads to the same destination as an adjacent link, it may be included in that link rather than becoming a separate control.
...
- Vanilla authoring remains ordinary Bootstrap markup and constructors. The shared behavior derives label-versus-description treatment from the trigger's visible and accessible naming when that distinction is unambiguous, repairs Bootstrap's duplicate announcement behavior, and provides a compatibility baseline for ambiguous legacy markup. Do not require `data-pl-*` attributes or a PrairieLearn browser API in course-authored or third-party content. Add an internal opt-in only if the implementation spike identifies a concrete first-party case that ordinary Bootstrap markup cannot represent.
...
- Related navigational links are permitted and must participate in the normal tab order. Prefer one specifically named documentation link, as Fluent recommends, but allow a deliberately small list when the use case genuinely requires it. An unbounded entity list, such as PrairieTest's linked-assessment list, is an inline-expansion, disclosure, or dedicated-navigation problem rather than contextual help.
...
Use a Carbon-style toggletip interaction model, informed by Fluent's content, naming, and density guidance: focus remains on the trigger, links enter ordinary sequential navigation, and the surface closes when focus leaves. Validate this selected contract with one private prototype before publishing an abstraction. The prototype must exercise actual PrairieLearn and PrairieTest content shapes: short text, text with one documentation link, the instructor-preview Tools help with its small link set, and a repeated form or table-header example. Record Tab/Shift+Tab destinations, Escape and outside-press behavior, focus restoration, screen-reader browse/swipe discoverability, collision, and zoom. Test representative current desktop and physical-mobile assistive-technology combinations through the shared verification strategy rather than running a four-pattern tournament on every platform.

`aria-expanded` and `aria-controls` alone must not be assumed to announce newly revealed text reliably. If the selected model loses focus, strands the virtual cursor, or makes the content undiscoverable in representative desktop or mobile testing, do not ship `ContextualHelp`; use visible helper text or a disclosure for that use case. Define the contract independently of either runtime, then implement and validate it in both React and vanilla against real callsites. The instructor-preview Tools help makes vanilla a first-class validation case rather than a later port.
...
The component should own its icon button and derive a subject-first name such as “Relative cost, more information.” It accepts general `ReactNode` content because legitimate help may need paragraphs, formatting, and a deliberately small set of links. The type system cannot usefully prevent interactive descendants, so usage guidance and review must prohibit forms, state-changing actions, menus, and unbounded navigation lists. One specifically named documentation link is the default; the instructor-preview Tools help may retain a small link set if the prototype shows that it remains usable.
...
An anchored dialog is appropriate for a small in-context task with a few controls, explicit completion and cancellation, no multi-step flow, and no information that must be compared across a large area. Carbon explicitly lists quick in-context editing as a toggletip use case, but that does not settle modality. The installed React Aria `Popover` deliberately contains focus and hides outside content from assistive technology by default, and PrairieTest's current global behavior already traps keyboard focus in interactive popovers. React Aria warns that its `isNonModal` escape hatch can harm the screen-reader experience. The default editor contract should therefore preserve contained focus rather than invent Tab-away cancellation. The repository audit found no anchored editor whose values persist immediately on change or blur: the typed `InlineEdit` and `CustomInlineEdit` families and the PrairieLearn question-points editor use explicit Save/Cancel or an explicit action plus Cancel, while PrairieTest's four `onlyCancel` cases are unavailable explanations rather than editors. This RFC therefore defines no nonmodal editor variant. This does not make every existing PrairieTest popover appropriate: the multi-field override editor, large proctor-assignment table, and other spacious or consequential workflows should be evaluated for a full modal, side panel, inline edit region, or dedicated page. High-impact, irreversible, multi-step, densely validated, or spacious work does not belong in an anchored editor.
...
The anchored editor is visually a popover but behaviorally a contained task dialog. That distinction avoids both accidental Tab-away cancellation and a persistent nonmodal dialog stranded elsewhere in the page. It also aligns the React path with React Aria's supported default and makes the vanilla requirement concrete: Bootstrap provides positioning and appearance, while PrairieLearn's controller must supply the dialog name and relationship, focus entry and containment, dirty dismissal, topmost Escape handling, restoration, modal accessibility boundary, nearest-modal container, and structural nested-widget ownership. If a future workflow genuinely persists changes immediately, its interaction model should be designed from that demonstrated case rather than exposed speculatively through this abstraction.
...
The PrairieLearn and PrairieTest audit found no concrete need for a focusable unavailable-action overlay component. Save restrictions belong in visible validation, reorder/delete restrictions belong in visible row or section state, PrairieTest's four `onlyCancel` explanations should become visible notes, and access-control prerequisites already have adjacent message patterns. Do not invent one-off `aria-disabled` popover behavior. If a future focusable unavailable-action component is independently justified, it must prevent the normal action through click, keyboard, and form-submission paths, preserve its visible accessible name, and point to already-visible explanatory text. It must not unexpectedly change meaning from “perform action” to “explain failure.”
...
- Programmatically created instances are a permanent requirement, not an edge case: PrairieTest currently has 20 direct Bootstrap popover constructors and PrairieLearn elements also construct instances. The behavior layer must enhance direct construction through Bootstrap's public delegated lifecycle events and `getInstance`, without requiring course-authored or third-party scripts to import a PrairieLearn API or depending on Bootstrap's private `_config`.
- Direct `new bootstrap.Tooltip(...)` and `new bootstrap.Popover(...)` calls in first-party, third-party, and course-authored scripts must continue to receive the compatibility baseline. New first-party code should prefer declarative Bootstrap markup when practical. Add an internal explicit override only if the implementation spike identifies a concrete first-party case whose semantics cannot be inferred from ordinary Bootstrap markup and accessible naming.
...
- Contextual help must use a real button trigger once that pattern exists. The vanilla contract must support links and formatting as well as plain text, but reject or reclassify forms and state-changing controls. Its host integration may use internal registration, but course and third-party content must not need a PrairieLearn-specific browser API.
...
- Declarative question content and programmatic course scripts continue to use ordinary Bootstrap markup and constructors. The shared behavior transparently distinguishes the common cases when naming makes the relationship unambiguous: an icon-only control whose tooltip matches its accessible name uses label semantics, while a visibly named control with distinct tooltip text uses description semantics. Ambiguous legacy markup remains operational through the compatibility baseline rather than requiring new `data-pl-*` attributes.
- Label treatment has one continuously available name source: the trigger's accessible name. The adapter renders that name visually, hides the duplicate bubble from the accessibility tree, and removes Bootstrap's redundant `aria-describedby`; it does not maintain two caller-authored strings that can drift. Dynamic accessible-name changes update an open bubble and future openings. Description treatment keeps the independently named trigger and associates its nonredundant tooltip text through `aria-describedby`.
- Do not expose a PrairieLearn browser-global tooltip/popover factory to course or third-party content. If a concrete first-party case cannot be represented after the implementation spike, keep any explicit override internal and narrowly typed rather than turning semantic modes into a second public authoring system.
...
- Equivalent ordinary markup produces equivalent naming, relationship, focus, dismissal, touch, and nested-modal behavior in PrairieLearn and PrairieTest. App bundles may still own CSS loading, MathJax/HTMX processing, and application-specific success handling.
...
The native Popover API is not a candidate within this plan because its January 2025 cross-browser baseline is too recent for PrairieLearn's permanent question-content compatibility surface. A future browser-support-policy change may make it a useful internal substrate, but any adoption must preserve the Bootstrap-shaped author contract rather than requiring course content to migrate again.
...
| Manual-grading issue counts and open-assessment indicators                  | Keep the issue count as a compact static number badge when table context makes it clear. Show the assessment-open state as a static warning badge containing the triangle and visible “Open” text. Do not turn either status into a button or tooltip         |
...
| PrairieTest settings/table help through `IconWithPopover`                   | Audit 26 callsites by content: keep short explanatory prose and small documentation-link sets press-accessible; move required rules into visible copy; move arbitrary linked-entity lists to inline expansion, disclosure, or dedicated navigation            |
...
5. Validate the selected Carbon-style contextual-help model in parallel and ship it only if the prototype passes representative physical-device and assistive-technology checks.
...
- Transparently inferred vanilla label treatment exposes one stable accessible name, keeps its visual label synchronized under dynamic updates, hides the generated bubble from the accessibility tree, and never adds a duplicate description.
...
- Ordinary declarative Bootstrap tooltips work when inserted dynamically, and direct programmatic Bootstrap construction receives the compatibility behavior without importing or bundling a second Bootstrap copy. Uninstall/reinstall leaves no stale instances or listeners.
...
- At least one current physical mobile screen-reader combination for any shipped contextual-help pattern, plus sighted checks on both iOS Safari and Android Chrome. Expand the matrix when a representative result is inconsistent or exposes a platform-specific risk.
...
- `IconButton` owns visual label tooltips and their continuously available accessible names; public `Tooltip` supplies only nonredundant descriptions. Vanilla behavior infers the equivalent relationship from ordinary Bootstrap markup and accessible naming when it is unambiguous.
...
- The tooltip foundation documentation describes only the shipped description `Tooltip`, label-tooltip `IconButton`, and vanilla contracts, and gives examples of information that must not be hidden in tooltips. `ContextualHelp` documentation is added only if that component ships after its prototype gate.
...
1. Does the selected Carbon-style contextual-help contract pass representative desktop and physical-mobile screen-reader testing for plain text, one link, a small link list, and repeated help triggers, and what DOM placement does it require?
...
4. Does the vanilla tooltip spike reveal any concrete first-party case whose label-versus-description semantics cannot be inferred from ordinary Bootstrap markup and accessible naming? If so, what is the narrowest internal override?
