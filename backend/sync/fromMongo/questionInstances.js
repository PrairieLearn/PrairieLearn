    ...
    fileToSQL: function(filename, callback) {
        ...
        sqldb.query(sql, [], function(err) {
            ...
            var sql
                ...
                + '                                 test_instance_id, test_question_id, auth_user_id, number)'
                + ' ('
                + '     SELECT qii.qiid, qii.date, qii.vid, qii.params, qii.true_answer, qii.options, ti.id, tq.id, u.id,'
                + '         CASE'
                + '             WHEN ti.qids @> \'[]\'::JSONB'
                + '                 THEN array_position(ARRAY(SELECT jsonb_array_elements_text(ti.qids)), qii.qid::text)'
                + '             ELSE tq.number'
                + '         END'
