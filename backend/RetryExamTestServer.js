...
exports.updateWithSubmission = function(tInstance, test, submission, options, callback) {
    ...
    if (!_(tInstance.qids).contains(submission.qid))
        return callback(new Error("QID is not in tInstance"));
    var question = tInstance.questionsByQID[submission.qid];
    if (question.nGradedAttempts >= question.points.length)
        return callback(new Error("Too many attempts at question"));
    if (question.correct)
        return callback(new Error("question is already correct"));
...
exports.grade = function(tInstance, test) {
    if (!tInstance.open)
