...
async function stopLaunchedTimeoutWorkspaces() {
  ...
  for (const workspace of workspaces) {
    logger.verbose(
      `workspaceTimeoutStop: launched timeout for workspace_id = ${workspace.id}`,
    );
...
async function stopHeartbeatTimeoutWorkspaces() {
  ...
  for (const workspace of workspaces) {
    logger.verbose(
      `workspaceTimeoutStop: heartbeat timeout for workspace_id = ${workspace.id}`,
    );
    await workspaceHelper.updateState(
      workspace.id,
      'stopped',
...
async function stopInLaunchingTimeoutWorkspaces() {
  ...
  for (const workspace of workspaces) {
    ...
    logger.error(
      `workspaceTimeoutStop: in-launching timeout for workspace_id = ${workspace.id}`,
    );
    await workspaceHelper.updateState(
      workspace.id,
      'stopped',
