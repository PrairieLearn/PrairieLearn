...
async function checkDBConsistency() {
    ...
    const db_hosts = new Set((await sqldb.queryAsync(sql.select_nonterminated_or_nonterminating_workspace_hosts, [])).rows.map(instance => instance.instance_id));
    const db_hosts_terminating = new Set((await sqldb.queryAsync(sql.select_running_or_terminating_workspace_hosts, [])).rows.map(instance => instance.instance_id));
    ...
    const not_in_db = set_difference(running_host_set, db_hosts_terminating);
    if (not_in_db.size > 0) {
    ...
    const not_in_ec2 = set_difference(db_hosts, running_host_set);
