...
CREATE OR REPLACE FUNCTION
    migration_111_array_increments_above_max(
...
CREATE OR REPLACE FUNCTION
    migration_111_instance_questions_calculate_stats(
...
second_calculation AS (
    SELECT migration_111_array_increments_above_max(submission_score_array_var) AS incremental_submission_score_array_var
...
SET
    ...
    incremental_submission_points_array = migration_111_array_product(incremental_submission_score_array_var, iq.points_list_original)
FROM
    first_calculation,
    second_calculation
...
SELECT migration_111_instance_questions_calculate_stats(iq.id)
FROM instance_questions AS iq;

DROP FUNCTION migration_111_array_product(DOUBLE PRECISION[],DOUBLE PRECISION[]);
DROP FUNCTION migration_111_array_increments_above_max(DOUBLE PRECISION[]);
DROP FUNCTION migration_111_instance_questions_calculate_stats(BIGINT);
