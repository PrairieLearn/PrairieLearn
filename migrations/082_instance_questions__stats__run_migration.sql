CREATE OR REPLACE FUNCTION
  migration_064_length_of_incorrect_streak (arr BOOLEAN[]) RETURNS INTEGER AS $$
     DECLARE
       i INTEGER := 1 ;
     BEGIN
       LOOP
...
BEGIN
    ...
    iq_stats AS (
      SELECT
        ...
        migration_064_length_of_incorrect_streak(attempts.attempts) AS length_of_incorrect_streak,
    ...
    SET some_correct_submission = iq_stats.some_correct_submission,
    ...
    FROM iq_stats
    WHERE iq.id = iq_stats.instance_question_id;
END;
...
DROP FUNCTION migration_064_length_of_incorrect_streak (BOOLEAN[]);
