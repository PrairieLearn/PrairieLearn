 ...
 */
module.exports.loadConfigForEnvironment = function (
  configDir,
  environment,
  callback,
) {
    ...
    [
      ...
      (callback) => {
        ...
        async.whilst(
          ...
          (callback) => {
            fs.readFile(
              path.join(configDir, `${parent}.yaml`),
              'utf-8',
              (err, data) => {
      ...
      (callback) => {
        ...
        for (const itemKey in configDescription) {
          ...
          if (configItem.default !== undefined) {
            ...
            callback(
              new Error(
                `${itemKey} is missing a default for environment ${environment}`,
              ),
            );
            return;
          }
          ...
          if (
            configItem.envVar !== undefined &&
            process.env[configItem.envVar] !== undefined
          ) {
            ...
            if (typeof configItem.default === 'number') {
              try {
                ...
                callback(
                  new Error(
                    `Unable to parse ${configItem.envVar}=${envValue} as a number`,
                  ),
                );
                return;
              }
              ...
              if (envValue.toLowerCase() === 'true' || envValue === 1) {
                ...
                callback(
                  new Error(
                    `Unable to parse ${configItem.envVar}=${envValue} as a boolean`,
                  ),
                );
                return;
              }
...
module.exports.loadConfig = function (configDir, opts, callback) {
  ...
  module.exports.loadConfigForEnvironment(
    configDir,
    env,
    (err, loadedConfig) => {
      if (ERR(err, callback)) return;
