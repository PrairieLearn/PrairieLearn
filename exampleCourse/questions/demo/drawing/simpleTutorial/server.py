...
def generate(data):
    ...
    alpha = (
        random.choice(allangles) if random.choice([0, 1]) else -random.choice(allangles)
    )
    ...
    if question == 1:
        text = (
            "Add a point to position $("
            + str(xP - x0)
            + ","
            + str(canvas_height - yP - y0)
            + ")$"
        )
        ...
        text += " The load should point " + str(orientation) + "."
        text += (
            " The magnitude (in absolute value) in $B$ is " + str(comp) + "than in $C$."
        )
