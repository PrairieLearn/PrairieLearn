...
An unavailable action that opens an explanation must remain focusable and pressable, so do not set `disabled` on its trigger. Use `aria-disabled="true"` to communicate that the action itself is unavailable; its visible label remains its accessible name.

```tsx
import { Popover } from '@prairielearn/ui';

<Popover content="This action is unavailable until the course has been synced.">
  <button type="button" className="btn btn-outline-secondary opacity-50" aria-disabled="true">
    Publish
  </button>
</Popover>;
```
