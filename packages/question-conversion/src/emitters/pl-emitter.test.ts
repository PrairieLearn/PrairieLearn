...
describe('PLEmitter', () => {
  ...
  describe('feedback rendering', () => {
    it('uses one field for general, correct, and incorrect feedback', () => {
      ...
      assert.notInclude(html, 'qti_import_correct');
      assert.notInclude(html, 'qti_import_incorrect');
...
`,
    ...
    it('supports one-sided outcome feedback', () => {
      ...
      assert.include(correctOnly.questionHtml, '{{#is_correct}}');
      assert.notInclude(correctOnly.questionHtml, '{{^is_correct}}');
      assert.notInclude(incorrectOnly.questionHtml, '{{#is_correct}}');
      assert.include(incorrectOnly.questionHtml, '{{^is_correct}}');
    });
    ...
    it('emits no submission panel and no grade() when feedback is absent', () => {
      const q = makeQuestion();
      const result = emitter.emit(makeAssessment([q]));
      assert.notInclude(result.questions[0].questionHtml, 'pl-submission-panel');
  ...
  describe('checkbox per-answer feedback', () => {
    ...
    it('emits independent selected-answer flags and one global field', () => {
      const q = makeQuestion({
        body: {
          type: 'checkbox',
          choices: [
      ...
      assert.include(result.questionHtml, '{{#feedback.qti_import}}');
      assert.include(result.questionHtml, '{{#is_correct}}');
      assert.include(result.questionHtml, '{{^is_correct}}');
      assert.equal(
        serverPy,
        `def grade(data):
