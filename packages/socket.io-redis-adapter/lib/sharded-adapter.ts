...
class ShardedRedisAdapter extends ClusterAdapter {
  ...
  override close(): Promise<void> | void {
    ...
    return Promise.all(channels.map((channel) => SUNSUBSCRIBE(this.subClient, channel))).then();
  }
  ...
  override publishResponse(requesterUid, response) {
    ...
    SPUBLISH(this.pubClient, `${this.channel}${requesterUid}#`, this.encode(response));
  }
