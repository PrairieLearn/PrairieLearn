...
export abstract class ClusterAdapter extends Adapter {
  ...
  protected async onMessage(message: ClusterMessage, offset?: string) {
    ...
    switch (message.type) {
      ...
      case MessageType.SOCKETS_JOIN:
        super.addSockets(decodeOptions(message.data.opts), message.data.rooms as string[]);
        break;

      case MessageType.SOCKETS_LEAVE:
        super.delSockets(decodeOptions(message.data.opts), message.data.rooms as string[]);
        break;

      case MessageType.DISCONNECT_SOCKETS:
        super.disconnectSockets(decodeOptions(message.data.opts), message.data.close as boolean);
        break;
  ...
  protected onResponse(response: ClusterResponse) {
    ...
    switch (response.type) {
      case MessageType.BROADCAST_CLIENT_COUNT: {
        this.ackRequests.get(requestId)?.clientCountCallback(response.data.clientCount as number);
        break;
      }
      ...
      case MessageType.FETCH_SOCKETS_RESPONSE: {
        ...
        request.current++;
        (response.data.sockets as any[]).forEach((socket) => request.responses.push(socket));
   ...
   * @private
   */
  private addOffsetIfNecessary(packet: any, opts: BroadcastOptions, offset: string) {
   ...
   * @protected
   */
  protected abstract publishResponse(requesterUid: string, response: ClusterResponse);
}
