...
-- BLOCK select_enrollment_reconciliation_audit_events
SELECT
  *
FROM
  audit_events
WHERE
  ...
  AND row_id = ANY ($enrollment_ids::bigint[])
  AND context ->> 'reason' = 'identity_reconciliation'
ORDER BY
  id;
...
-- BLOCK select_waiting_reconciliation_lock
SELECT
  1
FROM
  pg_stat_activity
