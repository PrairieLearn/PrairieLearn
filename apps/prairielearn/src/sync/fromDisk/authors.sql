...
SELECT d.name, d.email, d.orcid, d."originCourse"::bigint
FROM (
  SELECT DISTINCT author.name, author.email, author.orcid, author."originCourse"
  FROM jsonb_to_recordset($authors::jsonb) author(
    name text,
    email text,
    orcid text,
    "originCourse" text
...
LEFT JOIN authors a
  ON a.author_name   IS NOT DISTINCT FROM d.name
