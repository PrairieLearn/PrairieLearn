...
export function startOfDayDatetime(date?: Temporal.PlainDate, displayTimezone?: string): string {
  const d = date ?? Temporal.Now.plainDateISO(displayTimezone);
  return d.toPlainDateTime({ hour: 0, minute: 0, second: 1 }).toString({ smallestUnit: 'second' });
}

export function endOfDayDatetime(date?: Temporal.PlainDate, displayTimezone?: string): string {
  const d = date ?? Temporal.Now.plainDateISO(displayTimezone);
  return d
