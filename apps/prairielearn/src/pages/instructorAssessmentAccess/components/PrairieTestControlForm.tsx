...
type AfterCompleteVisibilityMode = 'show_both' | 'hide_questions' | 'hide_both';
...
const AFTER_COMPLETE_VISIBILITY_ITEMS: RichSelectItem<AfterCompleteVisibilityMode>[] = [
  ...
  {
    value: 'hide_questions',
    label: 'Hide questions, show score',
...
function getAfterCompleteVisibilityMode(
  ...
  if (!questionsHidden) return 'show_both';
  if (!scoreHidden) return 'hide_questions';
  return 'hide_both';
}
...
function ExamAfterCompleteFields({ index }: { index: number }) {
  ...
  const handleModeChange = (newMode: AfterCompleteVisibilityMode) => {
    questionsHiddenField.onChange(newMode !== 'show_both');
    scoreHiddenField.onChange(newMode === 'hide_both');
  };
    ...
    <div className="mt-3">
      ...
      {readOnly && (
        <Form.Text className="text-muted d-block mt-2">
          Read-only reservations always show questions and score, so these can't be hidden.
        </Form.Text>
      )}
    </div>
  );
                ...
                {...register(`mainRule.prairieTestExams.${index}.readOnly`, {
                  onChange: (e: React.ChangeEvent<HTMLInputElement>) => {
                    if (e.target.checked) {
                      ...
                      setValue(
                        `mainRule.prairieTestExams.${index}.afterCompleteScoreHidden`,
                        false,
                        { shouldDirty: true, shouldValidate: true },
                      );
                    }
