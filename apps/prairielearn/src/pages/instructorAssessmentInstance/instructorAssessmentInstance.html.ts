...
export function InstructorAssessmentInstance({ resLocals} : {resLocals: Record<string, any>}) {
    return html`
    <!DOCTYPE html>
    <html lang="en">
      <head>
      ...
      <body>
        ...
        ${renderEjs(__filename, "<%- include('../partials/assessmentSyncErrorsAndWarnings'); %>", {...resLocals})}
          <div class="card mb-4">
            <div class="card-header bg-primary text-white">
            ${resLocals.assessment_instance_label}
              Summary:
            ...
            <table class="table table-sm table-hover two-column-description">
              <tbody>
                ${ resLocals.instance_group  ?
                    html` 
                        <tr><th>Name</th><td colspan="2">${resLocals.instance_group.name}</td></tr>
                        <tr><th>Group Members</th><td colspan="2">${resLocals.instance_group_uid_list.join(', ')}</td></tr>
                    `
                    : html`
                        <tr><th>UID</th><td colspan="2">${resLocals.instance_user.uid}</td></tr>
                        <tr><th>Name</th><td colspan="2">${resLocals.instance_user.name}</td></tr>
                        <tr><th>Role</th><td colspan="2">${resLocals.instance_role}</td></tr>
                    `
                }
                <tr><th>Instance</th><td colspan="2">${resLocals.assessment_instance.number}
                    (<a href="${resLocals.plainUrlPrefix}/course_instance/${resLocals.course_instance.id}/assessment_instance/${resLocals.assessment_instance.id}">student view</a>)
                </td></tr>
                ...
                <tr>
                  ...
                  <td colspan="2">
                    ...
                    <span id="total-points">${(resLocals.assessment_instance.points).toString()}</span>
                    <small>/<span id="total-max-points" class="text-muted">${(resLocals.assessment_instance.max_points).toString()}</span></small>
                    ${ resLocals.authz_data.has_course_instance_permission_edit ?
                        html`
                            ...
                            data-content="${`${EditTotalPointsForm({resLocals, id: 'editTotalPointsButton'})}`}"
                ...
                <tr>
                  ...
                  <td class="align-middle" style="width: 20%;">
                    ${renderEjs(__filename, "<%- include('../partials/scorebar'); %>", {score: resLocals.assessment_instance.score_perc})}
                  </td>
                  <td class="align-middle" style="width: 100%;">
                    ${ resLocals.authz_data.has_course_instance_permission_edit ?
                        html`
                            ...
                            data-content="${`${EditTotalScorePercForm({resLocals, id: 'editTotalScorePercButton'})}`}"
                            data-trigger="manual" onclick="$(this).popover('show')">
                            <i class="fa fa-edit" aria-hidden="true"></i>
                            </button>
                        `
                ...
                <tr><th>Date started</th><td colspan="2">${resLocals.assessment_instance_date_formatted}</td></tr>
                <tr><th>Duration</th><td colspan="2">${resLocals.assessment_instance_duration}</td></tr>
              </tbody>
            </table>
          </div>
          ...
          <div class="card mb-4">
            ...
            <table id="instanceQuestionList" class="table table-sm table-hover">
              ...
              <tbody>
                ${resLocals.instance_questions.map((instance_question, i_instance_question) => {
                    return html`
                    ${ ( instance_question.start_new_zone && instance_question.zone_title) ? 
                            ...
                            <tr>
                                ...
                                ${ instance_question.zone_has_max_points ?? html`(maximum ${instance_question.zone_max_points} points)` }
                                ${ instance_question.zone_has_best_questions ?? html`(best ${instance_question.zone_best_questions} questions)` }
                                </th>
                            </tr>
                            `
                        : null
                    ...
                    <td>
                        ...
                        (<a href="${resLocals.plainUrlPrefix}/course_instance/${resLocals.course_instance.id}/instance_question/${instance_question.id}/">student view</a>)
                    </td>
                    <td>
                        I-${instance_question.instructor_question_number}.
                        ${instance_question.qid}
                        (<a href="${resLocals.urlPrefix}/question/${instance_question.question_id}/">instructor view</a>)
                    </td>
                    <td class="text-center">
                        ${renderEjs(__filename, "<%- include('../partials/instanceQuestionPoints') %>", {instance_question, component: 'auto'})}
                        ${ resLocals.authz_data.has_course_instance_permission_edit ?
                            html`
                                ...
                                data-content="${renderEjs(__filename, "<%= include('../partials/editQuestionPointsForm'); %>", {
                                                ...resLocals,
                                                id: 'editQuestionPointsAuto' + i_instance_question,
                                                field: 'auto_points',
                    ...
                    <td class="text-center">
                        ${renderEjs(__filename, "<%- include('../partials/instanceQuestionPoints'); %>", {instance_question, component: 'manual'})}
                        ${ resLocals.authz_data.has_course_instance_permission_edit ?
                            html`
                    ...
                    <td class="text-center">
                        ${renderEjs(__filename, "<%- include('../partials/instanceQuestionPoints'); %>", {instance_question, component: 'total'})}
                        ${ resLocals.authz_data.has_course_instance_permission_edit ? 
                            html`
                    ...
                    <td class="align-middle text-center text-nowrap">
                        ${renderEjs(__filename, "<%- include('../partials/scorebar'); %>", {score: instance_question.score_perc})}
                    </td>
                    <td class="align-middle" style="width: 1em;">
                        ${resLocals.authz_data.has_course_instance_permission_edit ?
                            html`
                                ...
                                data-content="${renderEjs(__filename, "<%= include('../partials/editQuestionScorePercForm');%>", {
                                    ...resLocals,
                                    id: 'editQuestionScorePerc' + i_instance_question,
                                    instance_question: instance_question})}"
                                data-trigger="manual" onclick="$(this).popover('show')">
                    ...
                    <td class="align-middle text-nowrap" style="width: 1em;">
                        ${ (resLocals.authz_data.has_course_instance_permission_edit && instance_question.status !== 'unanswered') ?
                            html`
                                <a class="btn btn-xs btn-secondary" href="${resLocals.urlPrefix}/assessment/${resLocals.assessment.id}/manual_grading/instance_question/${instance_question.id}">Manual grading</a>
                            `
                            : null
                        }
                    </td>
          ...
          <div class="card mb-4">
            ...
            <table id="instanceQuestionStatsTable" class="table table-sm table-hover tablesorter">
              ...
              <tbody>
              ${resLocals.assessment_instance_stats.map((row) => { 
                return html`
                    <tr>
                        <td>
                        I-${row.number}.
          ...
          <div class="card mb-4">
            ...
            <div class="card-body">
              <small>
                ...
                Download <a href="${resLocals.urlPrefix}/assessment_instance/${resLocals.assessment_instance.id}/${resLocals.logCsvFilename}">${resLocals.logCsvFilename}</a>.
                Uploaded student files are not shown above or in the CSV file.
                Student files can be downloaded on the <a href="${resLocals.urlPrefix}/assessment/${resLocals.assessment.id}/downloads">Downloads</a> tab.
              </small>
            </div>
                ...
                ${resLocals.log.map((row) => { 
                    return html`
                        ...
                        ${ row.client_fingerprint ?
                            html`
                                <td><a tabindex="0" class="badge color-${resLocals.fingerprint_colors[row.client_fingerprint_number % 6]} color-hover"
                                    role="button"
                        ...
                        <td>
                            ${ row.student_question_number ??
                                row.variant_id ?
                                    html`
                                        <a href="${resLocals.plainUrlPrefix}/course_instance/${resLocals.course_instance.id}/instance_question/${row.instance_question_id}/?variant_id=${row.variant_id}">
                        ...
                        ${ row.event_name !== 'External grading results' ? 
                            html`<td style="word-break: break-all;">${row.data != null ? JSON.stringify(row.data) : null }</td>`
                            : html`
