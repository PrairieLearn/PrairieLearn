...
export function ExamQuestionAvailablePoints({ row }: { row: ClientQuestionRow }) {
  ...
  const popoverContent = [
    ...
    bestScore > 0
      ? `<p>If you score less than ${bestScore}% on your next submission, then you will be awarded no additional points, but you will keep any awarded points that you already have.</p><p class="mb-0">If you score between ${bestScore}% and 100% on your next submission, then you will be awarded an additional <code>(${formatPoints(currentWeight)} * (score - ${bestScore})/100)</code> points.</p>`
