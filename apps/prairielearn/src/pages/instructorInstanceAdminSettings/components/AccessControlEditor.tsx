...
export function AccessControlEditor({
  ...
  // Check if we have accessControl rules (new system) - not used but kept for future reference
  // const hasAccessControlRules = courseInstance.access_control_published !== null ||
  ...
  const isArchived =
    ...
    new Date(courseInstance.access_control_published_end_date) < new Date();
  const isScheduled =
    hasStartDate &&
    courseInstance.access_control_published_start_date &&
    new Date(courseInstance.access_control_published_start_date) > new Date();
