...
function RubricItems({
  ...
  const ai_checked_rubric_item_ids = new Set(
    ai_rubric_items?.map((item) => item.id) || [],
  );

  return rubric_items?.map((item) =>
    RubricItem({
      item,
      ai_checked: ai_rubric_items ? ai_checked_rubric_item_ids.has(item.id) : undefined,
