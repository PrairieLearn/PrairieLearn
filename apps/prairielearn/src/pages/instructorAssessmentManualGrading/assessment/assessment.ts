...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (req.body.__action === 'assign_graders') {
      ...
      const totalPrecision = totalConfusionMatrix.truePositives / (totalConfusionMatrix.truePositives + totalConfusionMatrix.falsePositives);
      const totalRecall = totalConfusionMatrix.truePositives / (totalConfusionMatrix.truePositives + totalConfusionMatrix.falseNegatives);
      const totalPrecisionComplement = totalConfusionMatrix.trueNegatives / (totalConfusionMatrix.trueNegatives + totalConfusionMatrix.falsePositives);

      const totalRecallComplement = totalConfusionMatrix.trueNegatives / (totalConfusionMatrix.trueNegatives + totalConfusionMatrix.falseNegatives);
      const totalF1Score = 2 * (totalPrecision * totalRecall) / (totalPrecision + totalRecall);
      const totalF1ScoreComplement = 2 * (totalPrecisionComplement * totalRecallComplement) / (totalPrecisionComplement + totalRecallComplement);

      rows.push({
        assessment_question_id: 'total',
        number: -1,
        accuracy: (totalConfusionMatrix.truePositives + totalConfusionMatrix.trueNegatives) /
          (totalConfusionMatrix.truePositives + totalConfusionMatrix.trueNegatives +
            totalConfusionMatrix.falsePositives + totalConfusionMatrix.falseNegatives), 
