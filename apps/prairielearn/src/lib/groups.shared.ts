...
export function getRoleNamesForUser(
  roleAssignments: Record<string, { role_name: string }[]> | undefined,
  uid: string,
): string[] {
  return roleAssignments?.[uid]?.map((r) => r.role_name) ?? ['None'];
}

export function canUserAssignGroupRoles(
  groupRoles: { id: string; can_assign_roles: boolean | null }[] | undefined,
  roleAssignments: Record<string, { team_role_id: string; user_id: string }[]> | undefined,
  user_id: string,
): boolean {
  const assignerRoles =
    groupRoles?.filter((role) => role.can_assign_roles).map((role) => role.id) ?? [];
  const assignerUsers = Object.values(roleAssignments ?? {})
    .flat()
