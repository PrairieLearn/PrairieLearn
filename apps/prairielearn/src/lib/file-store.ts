...
function getFileStoreS3Bucket() {
  if (config.fileStoreS3Bucket === null) {
    throw new Error('fileStoreS3Bucket must be configured when using S3 file storage');
  }
...
export async function getFile(
  ...
  switch (file.storage_type) {
    ...
    case StorageTypes.S3: {
      const contents =
        data_type === 'buffer'
          ? await getFromS3(getFileStoreS3Bucket(), file.storage_filename, true)
          : await getFromS3(getFileStoreS3Bucket(), file.storage_filename, false);
