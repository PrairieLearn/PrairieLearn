...
async function updateInstanceQuestionFromCsvRow(
  ...
  return await sqldb.runInTransactionAsync(async () => {
    ...
    if (submission_data == null) {
      throw new Error(
        `Could not locate submission with id=${record.submission_id}, instance=${record.instance}, uid/team=${uid_or_team}, qid=${record.qid} for this assessment.`,
      );
    }
    if (uid_or_team !== null && submission_data.uid_or_team !== uid_or_team) {
      throw new Error(
        `Found submission with id=${record.submission_id}, but uid/team does not match ${uid_or_team}.`,
      );
    }
