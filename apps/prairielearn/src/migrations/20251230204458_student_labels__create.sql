...
-- The name of the student label must be unique within the course instance, but only for non-deleted labels.
CREATE UNIQUE INDEX student_labels_course_instance_id_name_unique ON student_labels (course_instance_id, name)
WHERE
  deleted_at IS NULL;
