...
ALTER TABLE course_instances
ADD CONSTRAINT course_instances_publishing_dates_consistency_check
CHECK (
  (publishing_publish_date IS NULL AND publishing_archive_date IS NULL) OR
  (publishing_publish_date IS NOT NULL AND publishing_archive_date IS NOT NULL)
);
