...
ALTER TABLE course_instances
ADD COLUMN publishing_unpublish_date TIMESTAMP WITH TIME ZONE;
...
-- Ensure that either both publish date and unpublish date are null, or neither are null
ALTER TABLE course_instances
ADD CONSTRAINT course_instances_publishing_dates_consistency_check CHECK (
  (
    publishing_publish_date IS NULL
    AND publishing_unpublish_date IS NULL
  )
  OR (
    publishing_publish_date IS NOT NULL
    AND publishing_unpublish_date IS NOT NULL
  )
);
