...
import { type Course, CourseSchema } from '../lib/db-types.ts';
import { createServerJob } from '../lib/server-jobs.ts';
import { getLockNameForCoursePath } from '../models/course.ts';
import { syncDiskToSqlWithLock } from '../sync/syncFromDisk.ts';

export default makeBatchedMigration({
  async getParameters() {
    const result = await queryScalar(
      'SELECT MAX(id) as max from courses;',
  ...
  async execute(min: bigint, max: bigint): Promise<void> {
    const courses = await queryRows(
      'SELECT * FROM courses WHERE id >= $min AND id <= $max AND deleted_at IS NULL',
      { min, max },
      CourseSchema,
    );
