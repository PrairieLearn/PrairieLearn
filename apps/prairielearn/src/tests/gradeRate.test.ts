...
describe(
  'Exam assessment with grade rate set',
  ...
  function () {
    ...
    test('check for enabled grade button on a question page before submission', async () => {
    ...
    test('check for disabled grade button on a question page after submission', async () => {
      const response = await helperClient.fetchCheerio(context.question1Url);
      assert.isTrue(response.ok);
    ...
    test('check for enabled grade button on another question page', async () => {
