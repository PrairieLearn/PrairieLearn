...
describe(
  'test random groups and delete groups',
  ...
  function () {
    ...
    test('get group-based homework assessment', async () => {
    ...
    test('check groups and users', async () => {
...
describe(
  'deleting a group closes its assessment instances',
  { timeout: 20_000, concurrent: false },
  function () {
    beforeAll(helperServer.before());
    afterAll(helperServer.after);
    ...
    test('deleting a single group closes its assessment instance', async () => {
    ...
    test('deleting all groups closes their assessment instances', async () => {
      const group = await createGroup({
        course_instance: courseInstance,
        assessment,
