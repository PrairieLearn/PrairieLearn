...
describe('features', () => {
  beforeAll(async function () {
    ...
    await queryRows('INSERT INTO users (name, uid) VALUES ($name, $uid);', {
      name: 'Test User',
  ...
  beforeEach(async () => {
    await queryRows('DELETE FROM feature_grants', {});
  });
  ...
  it('enables a feature flag via course options', async () => {
    ...
    await queryRows('UPDATE pl_courses SET options = $options WHERE id = 1', {
      options: {
