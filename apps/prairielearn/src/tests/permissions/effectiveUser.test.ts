...
describe('effective user', { timeout: 60_000 }, function () {
  ...
  test.sequential(
    ...
    async () => {
      const headers = {
        cookie:
          'pl_test_user=test_instructor; pl2_requested_date=1970-01-19T00:00:01; pl2_requested_uid=student@example.com',
      };
      const res = await helperClient.fetchCheerio(context.pageUrlStudent, { headers });
      assert.equal(res.status, 200);
    },
  ...
  test.sequential(
    ...
    async () => {
      const headers = {
        cookie:
          'pl_test_user=test_instructor; pl2_requested_date=1910-01-19T00:00:01; pl2_requested_uid=student@example.com',
      };
      const res = await helperClient.fetchCheerio(context.pageUrlStudent, { headers });
      assert.equal(res.status, 403);
    },
