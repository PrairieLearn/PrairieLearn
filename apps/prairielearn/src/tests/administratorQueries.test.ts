...
describe('AdministratorQuery page', { timeout: 60_000 }, function () {
  ...
  test('visit queries page', { concurrent: false }, async () => {
  ...
  test(
    'visit query page for a query without params (runs immediately)',
    { concurrent: false },
    async () => {
  ...
  test(
    'visit query page for a query with params (runs on submit)',
    { concurrent: false },
    async () => {
