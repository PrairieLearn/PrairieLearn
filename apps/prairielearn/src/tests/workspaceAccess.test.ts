...
describe(
  'Test workspace authorization access',
  ...
  function () {
    ...
    describe('workspaces created by instructors in a course instance', function () {
      ...
      test('create instructor workspace', async () => {
      ...
      test('can be accessed by the instructor', async () => {
        const url = baseUrl + `/workspace/${workspace_id}`;
        const response = await fetch(url);
        assert.equal(response.status, 200);
      });
      test("can't be accessed by enrolled student one", async () => {
      ...
      test("can't be accessed by enrolled student two", async () => {
      ...
      test("can't be accessed by an unenrolled user", async () => {
    ...
    describe('workspaces created by students', function () {
      let workspace_id: string | undefined;
      test('create student workspace', async () => {
        await withUser(studentOne, async () => {
      ...
      test('can be accessed by the student owner', async () => {
        await withUser(studentOne, async () => {
          const url = baseUrl + `/workspace/${workspace_id}`;
          const response = await fetch(url);
          assert.equal(response.status, 200);
      ...
      test("can't be accessed by another enrolled student", async () => {
        await withUser(studentTwo, async () => {
          const url = baseUrl + `/workspace/${workspace_id}`;
          const response = await fetch(url);
          assert.equal(response.status, 403);
