...
// TEST, merge into the above test that this is copied from?
describe('Course syncing for a public course', () => {
  before('set up testing database', helperDb.before);
  after('tear down testing database', helperDb.after);
  ...
  it('syncs for known features', async () => {
    ...
    console.log(courseData.course_instances); // TEST

    for (const course_instance of courseData.course_instances) {
      // Set the course to be public
      courseData.course_instance.shared_publicly = true
    }
