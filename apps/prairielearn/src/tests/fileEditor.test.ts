      ...
      const res = await new Promise<{ status: number; text: () => Promise<string> }>(
        (resolve, reject) => {
          ...
          const req = http.request(
            { hostname: 'localhost', port: config.serverPort, path: parsedUrl.path, method: 'GET' },
            (res) => {
              let data = '';
              ...
              res.on('end', () => {
                resolve({ status: res.statusCode ?? 500, text: () => Promise.resolve(data) });
              });
            },
          );
...
function writeAndPushFileInDev(fileName, fileContents) {
  describe(`write ${fileName} in courseDev and push to courseOrigin`, function () {
    ...
    it('should push', async () => {
      await execa('git', ['push'], { cwd: courseDevDir, env: process.env });
    });
  });
}
