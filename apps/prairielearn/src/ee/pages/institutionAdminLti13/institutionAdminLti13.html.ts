    ...
    <html lang="en">
      ...
      <body>
        ...
        <main class="container mb-4">
          ...
          <p>${lti13Instances.length} instance${lti13Instances.length === 1 ? '' : 's'} configured.
          </p>
          <hr/>
          ...
          <div class="row">
            ...
            <form method="POST">
              ...
              <button class="btn btn-outline-success btn-block my-4" type="submit"
                name="__action" value="add_instance">
          ...
          <div class="col-9">
            ...
            ${displayLTI13Instance(instance, resLocals, platform_defaults)}
...
const displayLTI13Instance = function (instance, resLocals, platform_defaults) {
  if (instance) {
    return html`
    <h3>
      ${instance.tool_platform_name} (ID #${instance.id})
    </h3>
    <p>
      Created at ${instance.created_at.toString()}
      <button class="btn btn-sm btn-secondary" type="button" data-toggle="modal" data-target="#instanceData">
      <i class="fa-solid fa-screwdriver-wrench"></i> Show details</button>
    </p>
    ...
    <p>
      ...
      (${instance.keystore?.keys ? instance.keystore.keys.length : 0} key${
    ...
    <input id="name" class="form-control" type="text" size="80" spellcheck="false"
      name="name" value="${
        instance.name
  ...
  <hr/>
    ...
    <label class="form-check-label">
      <input id="update_params" class="form-check-input" type="checkbox" name="platform_update" value="1" checked>
    ...
    <div class="form-group my-2">
    <label>Issuer params:
      <textarea class="form-control ml-2" id="issuer_params" name="issuer_params" cols="80" rows="8" spellcheck="false">${JSON.stringify(
        instance.issuer_params,
        null,
        3,
      )}</textarea>
    </label>
    ...
    <div class="form-group my-2">
      ...
      <input id="client_id" class="form-control" type="text" size="80" spellcheck="false" name="client_id"
      value="${instance.client_params?.client_id}" aria-describedby="client_idHelp">
      <small id="client_idHelp" class="form-text text-muted"> Get this unique ID from the LMS.
      </small>
    </div>
    ...
    <form method="POST">
      <input type="hidden" name="__csrf_token" value="${resLocals.__csrf_token}" />
      <button type="submit" name="__action" value="add_key" class="btn btn-success">Add key to keystore</button>
      <button type="submit" name="__action" value="delete_keys" class="btn btn-warning"
      onClick="return confirm('Really delete all keys from keystore?')">
      Delete all keys from keystore</button>
    </form>
