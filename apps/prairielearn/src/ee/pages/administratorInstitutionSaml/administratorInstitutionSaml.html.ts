...
export function AdministratorInstitutionSaml({
  ...
  lti13InstancesWithRosterSyncPermitted: Lti13Instance[];
  host: string;
  ...
  const hasRosterSyncPermitted = lti13InstancesWithRosterSyncPermitted.length > 0;
      ...
      <form method="POST" class="mb-3 js-configure-form" ${!hasSamlProvider ? 'hidden' : ''}>
        <div class="mb-3">
          ...
          <input
            ...
            aria-describedby="issuerHelp${hasRosterSyncPermitted
              ? ' saml-lti-roster-sync-dependencies'
              : ''}"
            ${hasRosterSyncPermitted ? 'readonly' : ''}
          />
...
${samlProvider?.certificate ?? '-----BEGIN CERTIFICATE-----\n-----END CERTIFICATE-----'}</textarea
        ...
        <div class="mb-3">
          ...
          <input
            ...
            aria-describedby="uinAttributeHelp${hasRosterSyncPermitted
              ? ' saml-lti-roster-sync-dependencies'
              : ''}"
            ${hasRosterSyncPermitted ? 'readonly' : ''}
          />
          ...
          ${hasRosterSyncPermitted
            ? html`
                <div class="alert alert-info mt-2 mb-0" id="saml-lti-roster-sync-dependencies">
                  <h4 class="h6">Roster syncing dependencies</h4>
                  <p>Roster syncing is permitted for the following LTI 1.3 instances:</p>
                  <ul>
                    ${lti13InstancesWithRosterSyncPermitted.map(
                      (instance) => html`
                        <li>
                  ...
                  <p class="mb-0">
                    The SAML issuer and UIN attribute are locked. Disable roster syncing permission
                    for every listed instance before changing them or deleting this configuration.
                  </p>
                </div>
              `
            : ''}
        ...
        ? html`
            ...
            <p>
              <button
                ...
                ${hasRosterSyncPermitted ? 'disabled' : ''}
              >
              ...
              ${hasRosterSyncPermitted
                ? html`
                    <small class="d-block text-muted">
                      Disable roster syncing permission for every listed LTI 1.3 instance first.
                    </small>
                  `
                : ''}
            </p>
