...
/**
 * Stub version of aiGrade that creates a server job and iterates through
 * submissions with progress tracking, but does NOT actually call any AI model.
 *
 * This is a temporary placeholder for the workflow engine integration.
 * To wire in real grading, replace the body of `gradeInstanceQuestionStub`
 * with actual AI grading logic (or swap this call back to `aiGrade`).
 */
export async function aiGradeStub({
  course,
  course_instance,
  assessment,
  assessment_question,
  ...
  const serverJob = await createServerJob({
    type: 'ai_grading',
    description: 'Perform AI grading (stub)',
    userId: user_id,
    authnUserId: authn_user_id,
    courseId: course.id,
    courseInstanceId: course_instance.id,
  ...
  serverJob.executeInBackground(async (job) => {
    job.info(
      `[STUB] AI grading — processing ${instance_questions.length} submissions without calling AI.`,
    );
    ...
    for (const instance_question of instance_questions) {
      ...
      item_statuses[instance_question.id] = JobItemStatus.complete;
      num_complete += 1;
      ...
      job.info(
        `[STUB] Processed submission ${num_complete}/${instance_questions.length} (instance_question ${instance_question.id})`,
      );
    }

    job.info(`[STUB] AI grading complete. ${num_complete} submissions processed.`);
  });
