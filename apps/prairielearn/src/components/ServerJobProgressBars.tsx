  ...
  useEffect(() => {
    ...
    for (const jobSequenceId of jobSequenceIds) {
      ...
      socket.on('serverJobProgressUpdate', (msg: StatusMessageWithProgress) => {
        ...
        const numFailed = Object.values(itemStatuses).filter(
        ...
        {!isComplete && (
          <>
            ...
            <div class="text-muted small text-nowrap">
              ...
              <span class="text-danger">{numFailed > 0 ? ` (${numFailed} failed)` : ''}</span>
            </div>
          </>
        )}
        {isComplete && (
          <div class="text-muted small text-nowrap">
            {hasFailed
              ? `${numTotal - numFailed}/${numTotal} ${itemNames} (${numFailed} failed)`
              : `${numTotal} ${itemNames}`}
          </div>
        )}
      </div>
