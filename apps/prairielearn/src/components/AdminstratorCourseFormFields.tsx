...
export function AdministratorCourseFormFields({
  ...
  }, [effectivePrefix, shortName, institutionId, prefixData, isDefaultInstitution, coursesRoot, setValue]);

  return (
    <>
      <div className="row g-3 mb-3">
        {/* Row 1: Institution | Timezone */}
        ...
        <div className="col-md-6">
          ...
          <div aria-live="polite" aria-atomic="true">
            {shortName && !/^[A-Z]+ [A-Z0-9]+$/.test(shortName) && (
              <div className="form-text text-warning">
                <i className="fa fa-exclamation-triangle" aria-hidden="true" /> The course rubric
                and number should be a series of upper case letters, followed by a space, followed
                by a series of numbers and/or letters.
              </div>
            )}
          </div>
        </div>
        ...
        <div className="col-md-6">
          ...
          <div aria-live="polite" aria-atomic="true">
            {!repoFormatValid && (
              <div className="form-text text-warning">
                <i className="fa fa-exclamation-triangle" aria-hidden="true" /> Repository name
                should follow the format <code>pl-&#123;institution&#125;-&#123;course&#125;</code>{' '}
                (e.g. <code>pl-uiuc-cs101</code>).
              </div>
            )}
            {!repoMatchesShortName && (
              <div className="form-text text-warning">
                <i className="fa fa-exclamation-triangle" aria-hidden="true" /> Repository name is
                out of sync with the short name. Expected <code>{expectedRepoShortName}</code>.
              </div>
            )}
          </div>
        </div>
        ...
        {/* AI suggestion output (full width) */}
