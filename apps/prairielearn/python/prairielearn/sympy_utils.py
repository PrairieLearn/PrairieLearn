...
from prairielearn.misc_utils import full_unidecode, get_uuid
...
class DisplayType(Enum):
    """Layout options for a rendered symbolic input."""
...
class RenderConfig:
    """Configuration for rendering a symbolic input."""
...
class SymbolicSubmissionParseSuccess:
    ...
    json: SympyJson | Literal[""]


type SymbolicSubmissionParseResult = SymbolicSubmissionParseSuccess | SympyParseFailure
...
def _build_formula_editor_tokens(
    ...
    *,
    ...
    tokens = (
        ...
        + list(_Constants.functions.keys())
        + list(custom_functions)
        + list(variables)
    )
    if allow_trig_functions:
        tokens += list(_Constants.trig_functions.keys())
    tokens += [
...
def _format_formula_editor_greek_character(character: str) -> str:
    transformed = greek_unicode_transform(character)
...
def try_parse_symbolic_submission(
    ...
    *,
    ...
    if formula_editor and submission is not None:
        submission = format_formula_editor_submission_for_sympy(
            submission,
            variable_list,
    ...
    if submission is None:
        return SympyParseFailure("No submitted answer.")
    formatted_submission, error = format_submission_for_sympy(
        submission, allow_sets=allow_sets
    )
    if error is not None:
        return SympyParseFailure(error)
...
def replace_imaginary_for_display(expr: sympy.Expr, imaginary_unit: str) -> sympy.Basic:
    """Replace SymPy's imaginary unit with the configured display symbol."""
...
def render_with_config(
    ...
    *,
    ...
    """Render a symbolic input using the provided Mustache template text."""
    operators: list[str] = list(STANDARD_OPERATORS)
    if config.allow_sets:
        operators.extend(SET_NOTATION_OPERATORS)
    operators.extend(config.custom_functions)
    operators.extend(_Constants.functions.keys())
    if config.allow_trig:
        operators.extend(_Constants.trig_functions.keys())
    if config.allow_sets:
        operators.extend(_Constants.set_functions.keys())

    constants = list(_Constants.variables.keys())
