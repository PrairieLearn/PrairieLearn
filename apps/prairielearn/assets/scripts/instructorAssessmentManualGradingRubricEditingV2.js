...
$(() => {
  ...
  document.querySelector('.js-rubric-edit').addEventListener('click', enableRubricItemEdit);
...
function enableRubricItemEdit(event) {
  if (!(event.currentTarget instanceof HTMLElement)) return;
  const card = event.currentTarget.closest('.card');
  if (!card) return;
  const table = card.querySelector('.js-rubric-items-table');
  if (!table) return;
  card.querySelectorAll('.js-rubric-item-disable').forEach((element) => (element.disabled = false));

  const editButton = card.querySelector('.js-rubric-edit');

  // Add cancel and save button
  const cancelButton = document.createElement('button');
  cancelButton.innerText = 'Cancel';
  cancelButton.classList.add('btn', 'btn-secondary', 'js-rubric-edit-cancel');
  cancelButton.addEventListener('click', disableRubricItemEdit);
  editButton.parentNode.appendChild(cancelButton);

  const saveButton = document.createElement('button');
  saveButton.innerText = 'Save';
  saveButton.classList.add('btn', 'btn-primary', 'js-rubric-edit-save');
...
function disableRubricItemEdit(event) {
  if (!(event.currentTarget instanceof HTMLElement)) return;
  const card = event.currentTarget.closest('.card');
  if (!card) return;
  const table = card.querySelector('.js-rubric-items-table');
  if (!table) return;
  card.querySelectorAll('.js-rubric-item-disable').forEach((element) => (element.disabled = true));

  const cancelButton = card.querySelector('.js-rubric-edit-cancel');
  const saveButton = card.querySelector('.js-rubric-edit-save');

  // Add edit button
  const editButton = document.createElement('button');
  editButton.classList.add('btn', 'btn-secondary', 'js-rubric-edit');
  editButton.innerText = 'Edit rubric';
  editButton.addEventListener('click', enableRubricItemEdit);
  cancelButton.parentNode.appendChild(editButton);
