...
function resetRubricItemRowsListeners() {
  ...
  document
    .querySelectorAll('.js-rubric-item-text-field')
    .forEach((button) => button.addEventListener('click', enableRubricItemTextField));
  document
    .querySelectorAll('.js-rubric-item-number-field')
    .forEach((button) => button.addEventListener('click', enableRubricItemNumberField));
  document
...
function enableRubricItemTextField(event) {
  if (!(event.currentTarget instanceof HTMLElement)) return;
  const container = event.currentTarget.closest('td');
  const label = container.querySelector('label'); // May be null
  const button = container.querySelector('button');
  ...
  input.classList.add('form-control');
  input.type = 'text';
  input.style.minWidth = '15rem';
  input.name = button.dataset.inputName;
  input.setAttribute('maxlength', 100);
  input.value = button.dataset.currentValue || '';
  input.required = true;
...
function enableRubricItemNumberField(event) {
  if (!(event.currentTarget instanceof HTMLElement)) return;
  const container = event.currentTarget.closest('td');
  const label = container.querySelector('label'); // May be null
  const button = container.querySelector('button');
