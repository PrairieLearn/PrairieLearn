...
export function initCalculator(storageKey: string, { drawer, fab, fabClose }: DrawerElements) {
  ...
  function addHistoryItem(dataHistoryItem: HistoryItem) {
    ...
    const inputField = ensureElement(inputRow.querySelector<MathfieldElement>('.pl-calculator-history-text'));
    inputField.value = input;

    // Set output text
    const outputRow = ensureElement(clone.querySelector<HTMLElement>('.history-output'));
    const outputField = ensureElement(outputRow.querySelector<MathfieldElement>('.pl-calculator-history-text'));
