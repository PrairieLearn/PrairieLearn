...
mechanicsObjects.LatexText = fabric.util.createClass(fabric.Object, {
  ...
  async gen_text(str, options) {
    ...
    // Temporarily disable inline line-breaking
    // We do this because pl-drawing expects a single SVG element
    // See https://docs.mathjax.org/en/latest/upgrading/whats-new-4.0/linebreaking.html#in-line-breaking
    const savedLinebreakOptions = MathJax.startup.output.options.linebreaks;
    MathJax.startup.output.options.linebreaks = { inline: false };

    const svgResult = await MathJax.tex2svgPromise(str, { display: false });

    MathJax.startup.output.options.linebreaks = savedLinebreakOptions;
