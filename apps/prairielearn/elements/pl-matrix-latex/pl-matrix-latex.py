...
def render(element_html: str, data: pl.QuestionData) -> str:
    ...
    if var_data is None:
        msg = f'No value in data["params"] for variable {var_name} in pl-matrix-latex element'
    ...
    if not isinstance(var_data, numbers.Number):
        ...
        if np.issubdtype(var_data.dtype, np.number):
            ...
            if var_data.ndim != 2:
                msg = f'Value in data["params"] for variable {var_name} in pl-matrix-latex element must be 2D array or scalar'
                raise ValueError(msg)
        else:
            msg = f'Value in data["params"] for variable {var_name} in pl-matrix-latex element must be numeric'
            raise ValueError(msg)
