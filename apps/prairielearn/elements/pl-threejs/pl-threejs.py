...
def get_objects(element, data):
    ...
    for child in element:
        ...
        if frame not in ["body", "space"]:
            msg = f'"frame" must be either "body" or "space": {frame:s}'
            raise Exception(msg)
        if frame == "body":
        ...
        try:
            ...
            msg = f'attribute "position" must have format [x, y, z]: {p:s}'
...
def render(element_html, data):
    ...
    if data["panel"] == "question":
        ...
        show_pose = pl.get_boolean_attrib(
            element, "show-pose-in-question", SHOW_POSE_IN_QUESTION_DEFAULT
        )
        ...
        html_params = {
            ...
            "tol_translation": "{:.2f}".format(
                pl.get_float_attrib(element, "tol-translation", 0.5)
            ),
            "tol_rotation": "{:.1f}".format(
                pl.get_float_attrib(element, "tol-rotation", 5)
            ),
        ...
        if partial_score is not None:
            html_params["error_in_translation"] = str(
                np.abs(np.round(partial_score["feedback"]["error_in_translation"], 2))
            )
            html_params["error_in_rotation"] = str(
                np.abs(np.round(partial_score["feedback"]["error_in_rotation"], 1))
            )
        ...
        show_pose = pl.get_boolean_attrib(
            element, "show-pose-in-correct-answer", SHOW_POSE_IN_CORRECT_ANSWER_DEFAULT
        )
        ...
        f = pl.get_string_attrib(
            element, "answer-pose-format", ANSWER_POSE_FORMAT_DEFAULT
        )
...
def grade(element_html, data):
    ...
    # Get tolerances
    tol_translation = pl.get_float_attrib(
        element, "tol-translation", TOL_TRANSLATION_DEFAULT
    )
    tol_rotation = pl.get_float_attrib(element, "tol-rotation", TOL_ROTATION_DEFAULT)
    if tol_translation <= 0:
        msg = f"tol_translation must be a positive real number: {tol_translation:g}"
        raise Exception(msg)
    if tol_rotation <= 0:
        msg = f"tol_rotation must be a positive real number (angle in degrees): {tol_rotation:g}"
        raise Exception(msg)

    # Check if angle is no greater than tolerance
    if (error_in_rotation <= tol_rotation) and (
        error_in_translation <= tol_translation
    ):
        data["partial_scores"][answer_name] = {
            "score": 1,
            "weight": weight,
            "feedback": {
                "error_in_rotation": error_in_rotation,
...
def parse_correct_answer(f, a):
    if f == "homogeneous":
        try:
            transform_mat = np.array(a, dtype=np.float64)
            ...
            if q.shape == (4,):
                ...
                if np.allclose(np.linalg.norm(axis), 1.0):
                    return (
                        np.reshape(p, (3,)),
                        np.roll(
                            pyquaternion.Quaternion(axis=axis, degrees=angle).elements,
                            -1,
                        ).tolist(),
                    )
                else:
...
def get_orientation(element, name_orientation, name_format):
    ...
    if f == "rpy":
        ...
        try:
            rot_mat = np.array(json.loads(s), dtype=np.float64)
            return np.roll(
                pyquaternion.Quaternion(matrix=rot_mat).elements, -1
            ).tolist()
        except Exception as err:
