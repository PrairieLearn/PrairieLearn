...
(() => {
  ...
  window.PLRTE = async function (uuid, options) {
    ...
    quill.on('text-change', function () {
      ...
      // We want our contents to also be considered valid Markdown, such that if
      // the content is parsed again it produces the same result. Most cases of
      // content generated by Quill is fine, the only concern is `<pre>` tags.
      // The content of these tags causes line breaks without any conversion,
      // which may cause content like the following to be processed when it
      // shouldn't:
      // ```
      // <p>Paragraph</p><pre>Code
      //
      // **code that looks like markdown but should not be treated as markdown**
      // </pre>
      // ```
      // To avoid this, we add two newlines just before the opening `<pre>` tag.
      // This will ensure that the block will be interpreted as an HTML block
      // and will not be processed as Markdown, as per:
      // https://spec.commonmark.org/0.31.2/#html-blocks
      //
      // While we don't expect students to use <textarea> in their content in
      // most cases, they may copy-paste the content from somewhere else, so we
      // need to handle this case. <style> tags are not a concern (we don't
      // expect quill to return them), and <script> tags are sanitized by
      // DOMPurify.
      contents = contents.replaceAll(/<pre/g, '\n\n<pre').replaceAll(/<textarea/g, '\n\n<textarea');
      inputElement.val(
        btoa(
          he.encode(contents, {
