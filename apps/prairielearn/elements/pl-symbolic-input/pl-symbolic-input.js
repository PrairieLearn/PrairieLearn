...
window.PLSymbolicInput = async function (name) {
  ...
  mf.inlineShortcuts = {
    ...
    // SymPy does not support |x| notation for absolute values, so replace with the correct function call
    '|': {
      value: '\\abs',
    },
...
function setUpSymbolicInputMacros(mf) {
  ...
  // Note that we include trigonometric functions in the macros, even if they are disabled for grading
