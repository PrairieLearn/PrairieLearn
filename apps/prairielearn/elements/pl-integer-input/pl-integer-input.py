...
def prepare(element_html: str, data: pl.QuestionData) -> None:
    ...
    if correct_answer is not None:
        try:
            ...
            raise ValueError(f"Correct answer is not a valid base {base} integer: {name}")

        #TODO get rid of this line
        #if is_not_within_limits(correct_answer):
        #    raise ValueError(
        #        f"correct answer is {correct_answer}, must be between -9007199254740991 and +9007199254740991"
        #        "(that is, between -(2^53 - 1) and +(2^53 - 1))."
        #    )
...
def render(element_html: str, data: pl.QuestionData) -> str:
    ...
    if data["panel"] == "question":
        ...
        if parse_error is None and name in data["submitted_answers"]:
            ...
            a_sub_parsed = pl.from_json(a_sub) if isinstance(a_sub, dict) else int(a_sub)
...
def grade(element_html: str, data: pl.QuestionData) -> None:
    ...
    a_tru_parsed = pl.string_to_integer(a_tru, base) if isinstance(a_tru, str) else int(a_tru)

    def grade_function(a_sub: Union[str, int]) -> Tuple[bool, None]:
        # If submitted answer is in a format generated by pl.to_json, convert it
        # back to a standard type (otherwise, do nothing)
        a_sub = pl.from_json(a_sub)

        a_sub_parsed = pl.string_to_integer(a_sub, base) if isinstance(a_sub, str) else int(a_sub)
