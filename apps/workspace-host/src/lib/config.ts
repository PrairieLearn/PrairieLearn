...
export const ConfigSchema = z
  .object({
    ...
    /** The mapped UID for the workspace container are optional here, but default to the owner UID/GID in transform. */
    workspaceMappedUid: z.number().optional(),
    workspaceMappedGid: z.number().optional(),
  ...
  .transform((data) => ({
    ...data,
    // If the mapped UID/GID is not provided, fall back to the owner UID/GID.
    workspaceMappedUid: data.workspaceMappedUid ?? data.workspaceJobsDirectoryOwnerUid,
    workspaceMappedGid: data.workspaceMappedGid ?? data.workspaceJobsDirectoryOwnerGid,
  }));
