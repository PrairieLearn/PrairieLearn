...
# This version has been customized for the alpha version of the symbolic
# matrix component input component. -Eric Huber 20200305

# Stuff that the extra to_json and from_json hacks at the bottom require
import numpy as np
import pandas
# Global constant (could thread this through function args later as an option)
_ALLOW_FLOAT = True
...
class CheckNumbersAllowingFloat(ast.NodeTransformer):
    def visit_Num(self, node):
        if isinstance(node.n, int):
            return ast.Call(func=ast.Name(id='_Integer', ctx=ast.Load()), args=[node], keywords=[])
        # elif isinstance(node.n, float):
...
def evaluate(expr, locals_for_eval={}):
    ...
    if _ALLOW_FLOAT:
        root = CheckNumbersAllowingFloat().visit(root)
    else:
...
def convert_string_to_sympy(a, variables, allow_hidden=False, allow_complex=False):
    ...
    # Do the conversion
    res = evaluate(a, locals_for_eval)
...
# These are copied from prairielearn.py in order to use the modified,
# float-accepting versions of sympy-related conversion functions above.
# -Eric Huber 20200306
