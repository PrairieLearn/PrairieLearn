...
element_names = {
    'controlledLine': 'Controlled Line',
    'vector': 'Force Vector',
    'arc_vector': 'Moment',
    'distTrianLoad': 'Distributed Triangular Load',
    'point': 'Point',
}

# Attributes for each element, automatically generated from doc.md
element_attributes = {
    "pl-drawing": ["preview", "answers-name", "width", "height", "grid-size", "snap-to-grid", "correct-answer", "tol",
                   "angle-tol", "show-tolerance-hint", "tolerance-hint", "hide-answer-panel"],
    "pl-drawing-initial": ["draw-error-box", "draw-error-box=true"],
    "pl-coordinates": ["x1", "y1", "width", "angle", "label", "offsetx", "offsety", "label-x", "offsetx-label-x",
                       "offsety-label-x", "label-y", "offsetx-label-y", "offsety-label-y", "color", "stroke-width",
                       "arrow-head-width", "arrow-head-length"],
    ...
    "pl-rod": ["x1", "y1", "x2", "y2", "width", "draw-pin", "label1", "offsetx1", "offsety1", "label2", "offsetx2",
               "offsety2", "color", "stroke-color", "stroke-width", "height"],
    "pl-3pointrod": ["x1", "y1", "x2", "y2", "x3", "y3", "width", "draw-pin", "label1", "offsetx1", "offsety1",
                     "label2", "offsetx2", "offsety2", "label3", "offsetx3", "offsety3", "color", "stroke-color", "stroke-width"],
    "pl-4pointrod": ["x1", "y1", "x2", "y2", "x3", "y3", "x4", "y4", "width", "draw-pin", "label1", "offsetx1",
                     "offsety1", "label2", "offsetx2", "offsety2", "label3", "offsetx3", "offsety3", "label4",
                     "offsetx4", "offsety4", "color", "stroke-color", "stroke-width"],
    "pl-collar-rod": ["x1", "y1", "x2", "y2", "width", "draw-pin", "label1", "offsetx1", "offsety1", "label2",
                      "offsetx2", "offsety2", "draw-collar-end1", "w1", "h1", "draw-collar-end2", "w2", "h2", "color",
    ...
    "pl-spring": ["x1", "y1", "width", "angle", "height", "interval", "x2", "y2", "stroke-color", "stroke-width",
                  "draw-pin"],
    "pl-pulley": ["x1", "y1", "x2", "y2", "x3", "y3", "alternative-path", "radius", "label", "offsetx", "offsety",
                  "color", "stroke-color", "stroke-width"],
    "pl-dimensions": ["x1", "y1", "width", "angle", "x2", "y2", "dim-offset", "dim-offset-angle", "start-support-line",
                      "end-support-line", "label", "offsetx", "offsety", "stroke-color", "stroke-width",
                      "draw-start-arrow", "draw-end-arrow", "arrow-head-width", "arrow-head-length", "color"],
    "pl-arc-dimensions": ["x1", "y1", "radius", "start-angle", "end-angle", "start-support-line", "end-support-line",
                          "draw-center", "draw-start-arrow", "draw-end-arrow", "label", "offsetx", "offsety", "stroke-color",
                          "stroke-width", "arrow-head-width", "arrow-head-length"],
    "pl-vector": ["x1", "y1", "anchor-is-tail", "width", "angle", "label", "offsetx", "offsety", "color",
    ...
    "pl-distributed-load": ["x1", "y1", "width", "spacing", "w1", "w2", "angle", "label1", "offsetx1",
                            "offsety1", "label2", "offsetx2", "offsety2", "color", "stroke-width", "arrow-head-width",
                            "arrow-head-length", "disregard-sense", "draw-error-box", "offset-forward", "offset-backward",
...
def prepare(element_html, data):
    ...
    if not prev:
        ...
        if n_ans_elements > 1:
            ...
            raise Exception("You do not have any 'pl-drawing-answer' inside `pl-drawing` where preview=False. You should either enter the `pl-drawing-answer` if you want to grade objects, or make preview=True ")
...
def render_drawing_items(elem, curid=1, defaults={}):
    ...
    def gen_controlledCurvedLine(el):
        ...
        # Here we make the following definitions for the javascript drawing:
        # obj['x1'] and obj['y1']: define the first end point, hence obj['x1']=x1 and obj['y1']=y1
        # obj['x3'] and obj['y3']: define the second end point, hence obj['x3']=x2 and obj['y3']=y2
        # obj['x2'] and obj['y2']: define the control (middle) point, hence obj['x2']=x3 and obj['y2']=y3
...
def render(element_html, data):
    ...
    if ((not (data['panel'] == 'question') and preview_mode) or
        data['panel'] == 'answer' and pl.get_boolean_attrib(element, 'hide-answer-panel', True)):
        return ''
...
def parse(element_html, data):
    ...
    try:
        ...
        if not 'objects' in data['submitted_answers'][name]:
...
def grade(element_html, data):
    ...
    if not 'objects' in student:
        data['format_errors'][name] = 'No submitted answer.'
        return data

    def relerr(x, xapp):
        return np.abs(x - xapp) / x
    ...
    def comp_controlledLine(ref, st):
        ...
        return_bool = (( abserr(ex1,rx1) <= ref['offset_x']+tol and
                        abserr(ey1,ry1) <= ref['offset_y']+tol and
                        abserr(ex2,rx2) <= ref['offset_x']+tol and
                        abserr(ey2,ry2) <= ref['offset_y']+tol ) or
                      ( abserr(ex1,rx2) <= ref['offset_x']+tol and
                        abserr(ey1,ry2) <= ref['offset_y']+tol and
                        abserr(ex2,rx1) <= ref['offset_x']+tol and
                        abserr(ey2,ry1) <= ref['offset_y']+tol ))
        return return_bool
    ...
    def comp_controlledCurvedLine(ref, st):
        ...
        return_bool = (( abserr(ex1,rx1) <= ref['offset_x']+tol and
                        abserr(ey1,ry1) <= ref['offset_y']+tol and
                        abserr(ex2,rx2) <= ref['offset_x']+tol and
                        abserr(ey2,ry2) <= ref['offset_y']+tol and
                        abserr(exm,rxm) <= ref['offset_control_x']+tol and
                        abserr(eym,rym) <= ref['offset_control_y']+tol) or
                      ( abserr(ex1,rx2) <= ref['offset_x']+tol and
                        abserr(ey1,ry2) <= ref['offset_y']+tol and
                        abserr(ex2,rx1) <= ref['offset_x']+tol and
                        abserr(ey2,ry1) <= ref['offset_y']+tol and
                        abserr(exm,rxm) <= ref['offset_control_x']+tol and
                        abserr(eym,rym) <= ref['offset_control_y']+tol))
        return return_bool
    ...
    def comp_vector(ref, st):
        ...
        # print("ref angles = ",rang,rang2,rang3,rang4)
        # print("sign ref angles = ",np.sign(rang))
        # print("st angle = ", eang)
        ...
        # Commenting this for now, since it does not seem necessary
        # # Adjust position if the vector is centered
        # if ref.get('originX', '') == 'center':
        #     fwd = np.array([np.cos(rang_rad), np.sin(rang_rad)])
        #     rpos -= fwd * np.float64(ref['width']) / 2
        ...
        basis = np.array([[np.cos(rang_rad), -np.sin(rang_rad)],
                          [np.sin(rang_rad), np.cos(rang_rad)]]).T
        epos_rel = basis @ (epos - rpos)
        rely, relx = epos_rel

        if (relx > tol or relx < -tol or
            rely > max_forward or rely < -max_backward):
            return False
    ...
    def comp_distLoad(ref, st):
        ...
        # Get position of student answer relative to reference answer
        basis = np.array([[-np.sin(rang_rad), -np.cos(rang_rad)],
                          [np.cos(rang_rad),  -np.sin(rang_rad)]]).T
        epos_rel = basis @ (epos - rpos)
        rely, relx = epos_rel
        if (relx > tol or relx < -tol or
            rely > max_forward or rely < -max_backward):
    ...
    def comp_arc_vector(ref, st):
        ...
        st_end_arrow = st['drawEndArrow']

        rpos = np.array([ref['left'], ref['top']])
        ref_start_arrow = ref['drawStartArrow']
        ref_end_arrow = ref['drawEndArrow']

        # Check if correct position
        relx, rely = epos - rpos
        if (relx > tol or relx < -tol or
            rely > tol or rely < -tol):
            return False
