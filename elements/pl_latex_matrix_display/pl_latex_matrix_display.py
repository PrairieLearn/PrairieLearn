...
def render(element_html, element_index, data):
    ...
    for child in element:
        ...
        if child.tag == 'variable':
            ...
            pl.check_attribs(child, required_attribs=['params_name'], optional_attribs=['digits'])
            ...
            latex_data += '$\\begin{bmatrix}'

            # Create string from 2D arrays
            if (var_data.ndim == 2):
                for i in range(var_data.shape[0]):
                    for j in range(var_data.shape[1]-1):
                        latex_data +=   pl.string_from_2darray(var_data[i,j],language='python', digits=digits) + ' & '
                    latex_data +=   pl.string_from_2darray(var_data[i,var_data.shape[1]-1],language='python', digits=digits) + ' \\newline '
            # Create string from 1D arrays
            elif (var_data.ndim == 1):
                for i in range(var_data.shape[0]):
                    latex_data +=   pl.string_from_2darray(var_data[i],language='python', digits=digits) + ' \\newline '

            latex_data += '\\end{bmatrix}$'
