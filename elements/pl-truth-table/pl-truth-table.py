...
def get_question_information(
    element: lxml.html.HtmlElement,
) -> Tuple[List[str], List[str], List[AnswerCol]]:
    variable_names = []
    rows = []
    cols = []
    ...
    for child in element:
        if child.tag == "pl-variable":
            ...
            raise ValueError(
                f"Tags inside of pl-truth-table must be one of pl-variable, pl-row, or pl-answer-column, not '{child.tag}'."
            )
...
def prepare(element_html: str, data: pl.QuestionData) -> None:
    ...
    expressions = [answer_col.expression for answer_col in answer_columns]
    ...
    display_variables = [
        {"key": str(i), "html": variable} for i, variable in enumerate(variable_names)
    ]
...
def parse(element_html: str, data: pl.QuestionData) -> None:
    ...
    if expected_num_answers != len(submitted_answers):
        data["format_errors"][
            name
        ] = "Number of submitted answers doesn't match number of rows."
        return
...
def render(element_html: str, data: pl.QuestionData) -> str:
    ...
    if data["panel"] == "question":
        ...
        variable_set = [
            {
                ...
                "name": get_form_name(name, variable["key"]),
            }
            for variable in display_variables
        ]

        ans_column_set = [
            {"html": ans_column["html"].strip()} for ans_column in display_ans_columns
        ]
        ...
        variable_set = [
            {"html": variable["html"].strip()} for variable in display_variables
        ]
        ...
        for i, row in enumerate(display_rows):
            ...
            for j in range(num_ans_columns):
                ...
                format_error = data["format_errors"].get(
                    get_form_name(name, answer_index)
                )

                if format_error is not None:
                    student_answer_set.append({"parse_error": format_error})
        ...
        variable_set = [{"html": variable["html"]} for variable in display_variables]
...
def grade(element_html: str, data: pl.QuestionData) -> None:
    ...
    def edc_grade_fn(submitted_answers: List[str]) -> Tuple[float, None]:
        num_correct = sum(
            ...
            for (submitted_answer, correct_answer) in zip(
...
def test(element_html: str, data: pl.ElementTestData) -> None:
    ...
    if result == "correct":
        ...
        if test_case == 1:
            ...
            data["format_errors"][
                get_form_name(name, random_index)
