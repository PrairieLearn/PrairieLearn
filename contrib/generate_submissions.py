...
def main():
    parser = argparse.ArgumentParser(
        description="Generate blank submissions for all students in an assessment."
    )
    parser.add_argument(
        "-i",
        "--course-instance",
        required=True,
        type=int,
        help="course instance where students are enrolled",
    )
    parser.add_argument(
        "-a",
        "--assessment",
        required=True,
        type=int,
        help="assessment ID where submissions should be added",
    )
    parser.add_argument(
        "--host",
        default="localhost:3000",
        help="Host part (e.g., 'localhost:3000') of URL where PrairieLearn is running",
    )
