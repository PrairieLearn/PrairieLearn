    ...
    def assignment(self, assignment_id, *, prompt_if_needed=False):
        ...
        if assignment_id:
            for assignment in self.request(
                f"{self.url_prefix}/assignments/{assignment_id}"
            ):
...
class CourseSubObject(Canvas):
    ...
    def update(self, data=None):
        ...
        if self.id:
            ...
            self.data = self.post(self.compute_base_url(), {self.route_name: self.data})
        self.id = self.compute_id()
        self.url_prefix = self.compute_url_prefix()
        return self
...
class Quiz(CourseSubObject):
    ...
    def update_question_group(self, group_id, group_data):
        ...
        if group_id:
            return self.put(
                f"{self.url_prefix}/groups/{group_id}", {"quiz_groups": [group_data]}
            )
    ...
    def update_question(self, question_id, question):
        ...
        if question_id:
            return self.put(
                f"{self.url_prefix}/questions/{question_id}", {"question": question}
            )
    ...
    def submission_questions(self, quiz_submission):
        ...
        for result in self.request(
            f"/quiz_submissions/{quiz_submission['id']}/questions"
        ):
            for question in result["quiz_submission_questions"]:
                questions[question["id"]] = question
        return questions

    def send_quiz_grade(self, quiz_submission, question_id, points, comments=None):
        """docstring"""
        self.put(
            f"{self.url_prefix}/submissions/{quiz_submission['id']}",
            {
                "quiz_submissions": [
                    {
