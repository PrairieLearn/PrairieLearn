...
A student's ability to install additional extensions depends on whether the workspace is configured to enable networking. With networking enabled, the student can install any extension from Open VSX. With networking disabled, the student cannot install additional extensions, but they can still use the preinstalled extensions included in the image. To block students from installing additional extensions while otherwise enabling networking, set the environment variable `EXTENSIONS_GALLERY` to the string `"{}"` in your question's `info.json` file.
