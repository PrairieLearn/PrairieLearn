...
describe('effective user', function() {
    ...
    before('insert users', async function() {
        ...
        await sqldb.callAsync('users_select_or_insert', ['staff03@illinois.edu', 'Staff Three', null, 'dev']);
        ...
        await sqldb.callOneRowAsync('course_permissions_insert_by_user_uid', [1, 'staff03@illinois.edu', 'Editor', 2]);
    ...
    step('instructor can override date (and becomes enrolled)', async () => {
        ...
        const headers = { cookie: 'pl_test_user=test_instructor; pl_requested_date=1700-01-19T00:00:01' };
        const response = await helperClient.fetchCheerio(context.pageUrlStudent, { headers });
        assert.isTrue(response.ok);
    ...
    step('instructor can emulate student and override date in range (expect success)', async () => {
        const headers = { cookie: 'pl_test_user=test_instructor; pl_requested_date=1900-01-19T00:00:01; pl_requested_uid=student@illinois.edu' };
        const response = await helperClient.fetchCheerio(context.pageUrlStudent, { headers });
        assert.isTrue(response.ok);
    });

    step('instructor can emulate student and override date out of range (expect failure)', async () => {
        const headers = { cookie: 'pl_test_user=test_instructor; pl_requested_date=1700-01-19T00:00:01; pl_requested_uid=student@illinois.edu' };
        const response = await helperClient.fetchCheerio(context.pageUrlStudent, { headers });
        assert.equal(response.status, 403);
    });
    ...
    step('cannot request uid of administrator when administrator access is inactive', async () => {
        const headers = { cookie: 'pl_test_user=test_instructor; pl_access_as_administrator=inactive; pl_requested_uid=dev@illinois.edu' };
        const response = await helperClient.fetchCheerio(context.pageUrlStudent, { headers });
        assert.equal(response.status, 403);
    });

    step('can request uid of course editor as course owner', async () => {
        const headers = { cookie: 'pl_test_user=test_instructor; pl_access_as_administrator=inactive; pl_requested_uid=staff03@illinois.edu' };
        const response = await helperClient.fetchCheerio(context.pageUrlCourseInstance, { headers });
        assert.isTrue(response.ok);
    });
    ...
    step('can request uid of student data viewer as student data editor', async () => {
        ...
        await sqldb.callOneRowAsync('course_instance_permissions_insert', [1, 3, 1, 'Student Data Viewer', 2]);
        const headers = { cookie: 'pl_test_user=test_instructor; pl_access_as_administrator=inactive; pl_requested_uid=staff03@illinois.edu' };
        const response = await helperClient.fetchCheerio(context.pageUrlCourseInstance, { headers });
        assert.isTrue(response.ok);
    });
    ...
    step('cannot request uid of student data editor as student data viewer', async () => {
        await sqldb.callOneRowAsync('course_instance_permissions_update_role', [1, 2, 1, 'Student Data Viewer', 2]);
        await sqldb.callOneRowAsync('course_instance_permissions_update_role', [1, 3, 1, 'Student Data Editor', 2]);
        const headers = { cookie: 'pl_test_user=test_instructor; pl_access_as_administrator=inactive; pl_requested_uid=staff03@illinois.edu' };
        const response = await helperClient.fetchCheerio(context.pageUrlCourseInstance, { headers });
        assert.equal(response.status, 403);
    });
    ...
    step('instructor can request lower course role', async () => {
        await sqldb.callAsync('course_permissions_update_role', [1, 2, 'Viewer', 1]);
        const headers = { cookie: 'pl_test_user=test_instructor; pl_access_as_administrator=inactive; pl_requested_course_role=Previewer' };
        const response = await helperClient.fetchCheerio(context.pageUrlCourseInstance, { headers });
        assert.isTrue(response.ok);
    });

    step('instructor cannot request higher course role', async () => {
        const headers = { cookie: 'pl_test_user=test_instructor; pl_access_as_administrator=inactive; pl_requested_course_role=Editor' };
        const response = await helperClient.fetchCheerio(context.pageUrlCourseInstance, { headers });
        assert.equal(response.status, 403);
    });
    ...
    step('instructor can request lower course instance role', async () => {
        await sqldb.callOneRowAsync('course_instance_permissions_update_role', [1, 2, 1, 'Student Data Editor', 2]);
        const headers = { cookie: 'pl_test_user=test_instructor; pl_access_as_administrator=inactive; pl_requested_course_instance_role=Student Data Viewer' };
        const response = await helperClient.fetchCheerio(context.pageUrlCourseInstance, { headers });
        assert.isTrue(response.ok);
    });
    ...
    step('instructor cannot request higher course instance role', async () => {
        await sqldb.callOneRowAsync('course_instance_permissions_update_role', [1, 2, 1, 'Student Data Viewer', 2]);
        const headers = { cookie: 'pl_test_user=test_instructor; pl_access_as_administrator=inactive; pl_requested_course_instance_role=Student Data Editor' };
        const response = await helperClient.fetchCheerio(context.pageUrlCourseInstance, { headers });
        assert.equal(response.status, 403);
    });

    step('instructor can request no role and be granted access to student page', async () => {
        const headers = { cookie: 'pl_test_user=test_instructor; pl_access_as_administrator=inactive; pl_requested_course_role=None; pl_requested_course_instance_role=None' };
        const response = await helperClient.fetchCheerio(context.pageUrlStudent, { headers });
        assert.isTrue(response.ok);
    });
    ...
    step('instructor can request no course role and be denied access to instructor page (course route)', async () => {
        const headers = { cookie: 'pl_test_user=test_instructor; pl_access_as_administrator=inactive; pl_requested_course_role=None' };
        const response = await helperClient.fetchCheerio(context.pageUrlCourse, { headers });
        assert.equal(response.status, 403);
    });
