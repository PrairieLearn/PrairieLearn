...
FROM
    ...
    JOIN questions AS q ON (q.id = aq.question_id) AND (q.qid = $qid)
    JOIN assessment_instances AS ai ON (ai.id = iq.assessment_instance_id) AND (ai.assessment_id = $assessment_id);
