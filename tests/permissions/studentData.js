...
describe('student data access', function() {
    ...
    before('insert users', async function() {
        await sqldb.callAsync('users_select_or_insert', ['instructor@illinois.edu', 'Instructor User', '100000000', 'dev']);
        await sqldb.callAsync('users_select_or_insert', ['student@illinois.edu', 'Student User', '000000001', 'dev']);
    ...
    step('student can access HW1/Q1', async () => {
        const headers = { cookie: 'pl_test_user=test_student' };
        const response = await helperClient.fetchCheerio(context.homeworkQuestionInstanceUrl, { headers });
        assert.isTrue(response.ok);
        const result = await sqldb.queryOneRowAsync(sql.select_variant, {assessment_id: context.homeworkAssessmentId});
    ...
    step('student can start E1 in exam mode', async () => {
        ...
        const result = await sqldb.queryOneRowAsync(sql.select_instance_question, { qid: 'addNumbers', assessment_id: context.examAssessmentId});
    ...
    step('student can access E1/Q* in exam mode', async () => {
        ...
        assert.isTrue(response.ok);
        const result = await sqldb.queryOneRowAsync(sql.select_variant, {assessment_id: context.examAssessmentId});
    ...
    step('instructor (no role) cannot view HW1 instance of student', async () => {
        const headers = { cookie: 'pl_test_user=test_instructor' };
        const response = await helperClient.fetchCheerio(context.homeworkAssessmentInstanceUrl, { headers });
    ...
    step('instructor (no role) cannot view E1 instance of student', async () => {
        const headers = { cookie: 'pl_test_user=test_instructor' };
        const response = await helperClient.fetchCheerio(context.examAssessmentInstanceUrl, { headers });
        assert.equal(response.status, 403);
    });
    ...
    step('instructor (student data viewer) cannot attach file to HW1 instance of student', async () => {
        const headers = { cookie: 'pl_test_user=test_instructor' };
        let response = await helperClient.fetchCheerio(context.homeworkAssessmentInstanceUrl, { headers });
        assert.isTrue(response.ok);
        ...
        response = await helperClient.fetchCheerio(context.homeworkAssessmentInstanceUrl, { method: 'POST', form, headers });
        assert.equal(response.status, 403);
    });

    step('instructor (student data viewer) cannot submit answer to HW1/Q1 instance of student', async () => {
        const headers = { cookie: 'pl_test_user=test_instructor' };
        let response = await helperClient.fetchCheerio(context.homeworkQuestionInstanceUrl, { headers });
        assert.isTrue(response.ok);
        const __csrf_token = response.$('span[id=test_csrf_token]').text();
        ...
        response = await helperClient.fetchCheerio(context.homeworkQuestionInstanceUrl, { method: 'POST', form, headers });
        assert.equal(response.status, 403);
    });

    step('instructor (student data viewer) cannot attach file to E1 instance of student', async () => {
        const headers = { cookie: 'pl_test_user=test_instructor' };
        let response = await helperClient.fetchCheerio(context.examAssessmentInstanceUrl, { headers });
        assert.isTrue(response.ok);
        const __csrf_token = response.$('span[id=test_csrf_token]').text();
        assert.lengthOf(response.$(`form[class=attach-text-form]`), 0);
        const form = {
        ...
        response = await helperClient.fetchCheerio(context.examAssessmentInstanceUrl, { method: 'POST', form, headers });
        assert.equal(response.status, 403);
    });
    ...
    step('instructor (student data viewer) cannot submit answer to E1/Q* instance of student', async () => {
        ...
        response = await helperClient.fetchCheerio(context.examQuestionInstanceUrl, { method: 'POST', form, headers });
        assert.equal(response.status, 403);
    });

    step('instructor (student data viewer) cannot emulate student', async () => {
        const headers = { cookie: 'pl_test_user=test_instructor; pl_requested_uid=student@illinois.edu' };
        const response = await helperClient.fetchCheerio(context.homeworkAssessmentInstanceUrl, { headers });
        assert.equal(response.status, 403);
    });

    step('instructor (student data editor) cannot attach file to HW1 instance of student', async () => {
        await sqldb.callOneRowAsync('course_instance_permissions_update_role', [1, 2, 1, 'Student Data Editor', 2]);
        const headers = { cookie: 'pl_test_user=test_instructor' };
        let response = await helperClient.fetchCheerio(context.homeworkAssessmentInstanceUrl, { headers });
        assert.isTrue(response.ok);
        const __csrf_token = response.$('span[id=test_csrf_token]').text();
    ...
    step('instructor (student data editor) can attach file to E1 instance of emulated student', async () => {
        const headers = { cookie: 'pl_test_user=test_instructor; pl_requested_uid=student@illinois.edu; pl_requested_mode=Exam' };
        let response = await helperClient.fetchCheerio(context.examAssessmentInstanceUrl, { headers });
        assert.isTrue(response.ok);
        helperClient.extractAndSaveCSRFToken(context, response.$, 'form[class=attach-text-form]');
        const form = {
    ...
    step('instructor (student data editor) can view gradebook', async () => {
        const headers = { cookie: 'pl_test_user=test_instructor' };
        let response = await helperClient.fetchCheerio(`${context.courseInstanceBaseUrl}/instructor/instance_admin/gradebook`, { headers });
        assert.isTrue(response.ok);
    });

    step('instructor (student data editor) can view gradebook raw data', async () => {
        const headers = { cookie: 'pl_test_user=test_instructor' };
        let response = await helperClient.fetchCheerio(`${context.courseInstanceBaseUrl}/instructor/instance_admin/gradebook/raw_data.json`, { headers });
        assert.isTrue(response.ok);
    });

    step('instructor (student data editor) can view homework assessment instances', async () => {
        const headers = { cookie: 'pl_test_user=test_instructor' };
        let response = await helperClient.fetchCheerio(`${context.courseInstanceBaseUrl}/instructor/assessment/${context.homeworkAssessmentId}/instances`, { headers });
        assert.isTrue(response.ok);
    });

    step('instructor (student data editor) can view homework assessment instances raw data', async () => {
        const headers = { cookie: 'pl_test_user=test_instructor' };
        let response = await helperClient.fetchCheerio(`${context.courseInstanceBaseUrl}/instructor/assessment/${context.homeworkAssessmentId}/instances/raw_data.json`, { headers });
        assert.isTrue(response.ok);
    });

    step('instructor (student data editor) can view exam assessment instances', async () => {
        const headers = { cookie: 'pl_test_user=test_instructor' };
        let response = await helperClient.fetchCheerio(`${context.courseInstanceBaseUrl}/instructor/assessment/${context.examAssessmentId}/instances`, { headers });
        assert.isTrue(response.ok);
    });

    step('instructor (student data editor) can view exam assessment instances raw data', async () => {
        const headers = { cookie: 'pl_test_user=test_instructor' };
        let response = await helperClient.fetchCheerio(`${context.courseInstanceBaseUrl}/instructor/assessment/${context.examAssessmentId}/instances/raw_data.json`, { headers });
        assert.isTrue(response.ok);
    });

    step('instructor (student data viewer) can view gradebook', async () => {
        const headers = { cookie: 'pl_test_user=test_instructor; pl_requested_course_instance_role=Student Data Viewer' };
        let response = await helperClient.fetchCheerio(`${context.courseInstanceBaseUrl}/instructor/instance_admin/gradebook`, { headers });
        assert.isTrue(response.ok);
    });
    ...
    step('instructor (no role) cannot view gradebook', async () => {
        const headers = { cookie: 'pl_test_user=test_instructor; pl_requested_course_instance_role=None' };
        let response = await helperClient.fetchCheerio(`${context.courseInstanceBaseUrl}/instructor/instance_admin/gradebook`, { headers });
        assert.equal(response.status, 403);
    });
