...
describe('editor library', () => {
  ...
  it('gets details for course instance info file', () => {
    const details = /** @type {import('../lib/editor').CourseInstanceInfo} */ (editor.getDetailsForFile('courseInstances/testinstance/infoCourseInstance.json'));
    assert.equal(details.type, 'courseInstance');
    assert.equal(details.ciid, 'testinstance');
  });

  it('gets details for question info', () => {
    const details = /** @type {import('../lib/editor').QuestionInfo} */ (editor.getDetailsForFile('questions/testquestion/info.json'));
    assert.equal(details.type, 'question');
    assert.equal(details.qid, 'testquestion');
  });

  it('gets details for assessment info file', () => {
    const details = /** @type {import('../lib/editor').AssessmentInfo} */ (editor.getDetailsForFile('courseInstances/testinstance/assessments/testassessment/infoAssessment.json'));
