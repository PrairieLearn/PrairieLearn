...
describe('Exam and homework assessment with active access restriction', function () {
  ...
  before('set up testing server', async function () {
    ...
    const resultsHomework = await sqldb.queryOneRowAsync(
      sql.select_homework8,
      [],
    );
  ...
  step('try to access the exam when it is not active', async () => {
    ...
    assert.match(
      msg.text(),
      /Assessment will become available on 2010-01-01 00:00:01/,
    );
  });
  ...
  step('simulate a time limit expiration', async () => {
    ...
    assert.equal(
      response.url,
      context.examInstanceUrl + '?timeLimitExpired=true',
    );
  ...
  step('try to access the homework when it is not active', async () => {
    ...
    assert.match(
      msg.text(),
      /Assessment will become available on 2020-01-01 00:00:01/,
    );
  });
  ...
  step('access the homework when it is active', async () => {
    ...
    const hwQuestionUrl = response
      .$('a:contains(' + questionTitlePrefix + ')')
      .attr('href');
  ...
  step(
    ...
    async () => {
      ...
      assert.match(
        msg.text(),
        /Assessment will become available on 2030-01-01 00:00:01/,
      );
