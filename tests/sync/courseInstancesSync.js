...
describe('Course instance syncing', () => {
  ...
  it('syncs access rules', async () => {
    ...
    const courseDir = await util.writeCourseToTempDirectory(courseData);
    await util.syncCourseData(courseDir);
    const syncedAccessRules = await util.dumpTable(
      'course_instance_access_rules',
    );
    assert.equal(
      syncedAccessRules.length,
      courseData.courseInstances[util.COURSE_INSTANCE_ID].courseInstance
        .allowAccess.length,
    );
  });
  ...
  it('soft-deletes and restores course instances', async () => {
    ...
    const originalCourseInstance =
      courseData.courseInstances[util.COURSE_INSTANCE_ID];
  ...
  it('syncs empty arrays correctly', async () => {
    ...
    courseData.courseInstances[
      util.COURSE_INSTANCE_ID
    ].courseInstance.allowAccess[0].uids = [];
    ...
    const syncedAccessRules = (
      await util.dumpTable('course_instance_access_rules')
    ).filter((ar) => ar.course_instance_id === syncedCourseInstance.id);
  ...
  it('records a warning if the same UUID is used in multiple course instances', async () => {
    const courseData = util.getCourseData();
    courseData.courseInstances['newinstance'] =
      courseData.courseInstances[util.COURSE_INSTANCE_ID];
    const courseDir = await util.writeCourseToTempDirectory(courseData);
    await util.syncCourseData(courseDir);
    const syncedCourseInstances = await util.dumpTable('course_instances');
  ...
  it('records an error if an allowAccess rule has a start date after the end date', async () => {
    ...
    courseData.courseInstances[
      util.COURSE_INSTANCE_ID
    ].courseInstance.allowAccess.push({
  ...
  it('records an error if a course instance directory is missing an infoCourseInstance.json file', async () => {
    ...
    const courseDir = await util.writeCourseToTempDirectory(courseData);
    await fs.ensureDir(
      path.join(courseDir, 'courseInstances', 'badCourseInstance'),
    );
    await util.syncCourseData(courseDir);
    const syncedCourseInstances = await util.dumpTable('course_instances');
    const syncedCourseInstance = syncedCourseInstances.find(
  ...
  it('records an error if a nested course instance directory does not eventually contain an infoCourseInstance.json file', async () => {
    ...
    const courseDir = await util.writeCourseToTempDirectory(courseData);
    await fs.ensureDir(
      path.join(courseDir, 'courseInstances', ...nestedCourseInstanceStructure),
    );
    await util.syncCourseData(courseDir);
    ...
    for (let i = 0; i < nestedCourseInstanceStructure.length - 1; i++) {
      const partialNestedCourseInstanceStructure =
        nestedCourseInstanceStructure.slice(0, i);
      const partialCourseInstanceId =
        partialNestedCourseInstanceStructure.join('/');
      const syncedCourseInstance = syncedCourseInstances.find(
  ...
  it('correctly handles a new course instance with the same short name as a deleted course instance', async () => {
    ...
    const syncedCourseInstance = syncedCourseInstances.find(
      (ci) =>
        ci.short_name === 'repeatedCourseInstance' && ci.deleted_at == null,
    );
    assert.equal(syncedCourseInstance.uuid, courseInstance.courseInstance.uuid);
    assert.equal(
      syncedCourseInstance.long_name,
      courseInstance.courseInstance.longName,
    );
  });
  ...
  it('does not modify deleted course instance long names', async () => {
    ...
    courseData.courseInstances['repeatedCourseInstance'] =
      originalCourseInstance;
    const courseDir = await util.writeAndSyncCourseData(courseData);

    // now change the UUID and long name of the course instance and re-sync
    const newCourseInstance = JSON.parse(
      JSON.stringify(originalCourseInstance),
    );
    newCourseInstance.courseInstance.uuid =
      '49c8b795-dfde-4c13-a040-0fd1ba711dc5';
    ...
    assert.equal(
      deletedCourseInstance.uuid,
      originalCourseInstance.courseInstance.uuid,
    );
    assert.equal(
      deletedCourseInstance.long_name,
      originalCourseInstance.courseInstance.longName,
    );
  });
  ...
  it('does not add errors to deleted course instances', async () => {
    ...
    newCourseInstance.courseInstance.uuid =
      '7902a94b-b025-4a33-9987-3b8196581bd2';
