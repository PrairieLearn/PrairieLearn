...
const getConflictPayload = ($page, type) => {
  ...
  return {
    submissionScore: $page(
      `div:contains("${wildcard}") > form input[name="submissionScore"]`
    ).val(),
    submissionNote: $page(
      `div:contains("${wildcard}") > form textarea[name="submissionNote"]`
    ).val(),
    instanceQuestionModifiedAt: $page(
      `div:contains("${wildcard}") > form > input[name="instanceQuestionModifiedAt"]`
    ).val(),
    __csrf_token: $page(`div:contains("${wildcard}") > form > input[name="__csrf_token"]`).val(),
    __action: $page(`div:contains("${wildcard}") > form > div > button[name="__action"]`).attr(
      'value'
    ),
    assessmentId: $page(`div:contains("${wildcard}") > form > input[name="assessmentId"]`).val(),
    assessmentQuestionId: $page(
      `div:contains("${wildcard}") > form > input[name="assessmentQuestionId"]`
    ).val(),

    // These only appear on conflict resolutions
    gradingJobId: $page(
      `div:contains("${wildcard}") > form > div > input[name="gradingJobId"]`
    ).val(),
    conflictDataSource: $page(
      `div:contains("${wildcard}") > form > div > input[name="conflictDataSource"]`
    ).val(),
  };
};

const gradeSubmission = async (iqManualGradeUrl, submissionNote, submissionScore) => {
  const $gradingPage = cheerio.load(await (await fetch(iqManualGradeUrl)).text());
...
const createGradingConflict = async (iqManualGradingUrl) => {
  ...
  const payload2 = getManualGradePayload(
    $iqManualGradingPage2,
    'Any message second grading job',
    95
  );
  ...
  describe(`Manual grading: '${action}' action type`, function () {
    ...
    before('create instructors and students', async () => {
      ...
      await sqlDb.callOneRowAsync('course_permissions_insert_by_user_uid', [
        1,
        mockInstructors[0].authUid,
        'Owner',
        2,
      ]);
      await sqlDb.callOneRowAsync('course_permissions_insert_by_user_uid', [
        1,
        mockInstructors[1].authUid,
        'Owner',
        3,
      ]);
    ...
    describe('student role: saving student submissions', () => {
      ...
      it('students should be able to save submissions on instance questions', async () => {
        ...
        for await (const student of mockStudents) {
          ...
          const hm1AddTwoNumbersUrl =
            siteUrl + $hm1Body('a:contains("HW1.1. Add two numbers")').attr('href');
          const hm1AddVectorsCartesianUrl =
            siteUrl +
            $hm1Body('a:contains("HW1.2. Addition of vectors in Cartesian coordinates")').attr(
              'href'
            );
          const hm1AdvantagesFossilFuelsUrl =
            siteUrl +
            $hm1Body('a:contains("HW1.3. Advantages of fossil fuels (radio)")').attr('href');

          await saveOrGrade(hm1AddTwoNumbersUrl, { c: 9999999 }, action);
          await saveOrGrade(
            hm1AddVectorsCartesianUrl,
            {
      ...
      it('db should contain 12 submissions (1 per question for each 4 students x 3 questions = 12 submissions)', async () => {
        ...
        context.rows.forEach((submission) => {
          if (!groupedByStudent[submission.auth_user_id]) {
            groupedByStudent[submission.auth_user_id] = [];
          }
        ...
        Object.keys(groupedByStudent).forEach((student) =>
          assert.lengthOf(groupedByStudent[student], 3, 'array has length of 3')
        );
      });
    });
    ...
    describe('instructor role: grading student submissions', () => {
      ...
      beforeEach('load manual grading page URL and get testing question rows', async () => {
        const instructorCourseInstanceUrl =
          baseUrl + '/course_instance/1/instructor/instance_admin/assessments';
        const instructorCourseInstanceBody = await (
          await fetch(instructorCourseInstanceUrl)
        ).text();

        manualGradingUrl =
          siteUrl +
          cheerio
            .load(instructorCourseInstanceBody)('a:contains("Homework for automatic test suite")')
            .attr('href') +
          'manual_grading';
      ...
      it('instructor sees "Ungraded" and "Graded" columns increment -/+ by one for each manual grading job', async () => {
        ...
        for (let i = 1; i <= mockStudents.length; i++) {
          ...
          let $nextGradingPage = cheerio.load(await nextPage.text());
          ...
          const $manualGradingPage = cheerio.load(await (await fetch(manualGradingUrl)).text());
          $fossilFuelsRow = cheerio.load(
            $manualGradingPage('.qid-value:contains("fossilFuelsRadio")').parent().html()
          );
      ...
      it('instructor(s) should appear in "Grading Contributors" column if has submitted manual grade', async () => {
        ...
        const contributorUids = $addVectorsRow('.grading-contributors-value')
          .text()
          .trim()
          .split(',');
      ...
      it('instructor should see "currently grading" warning message when a manual grading user is already assigned/grading instance question', async () => {
        ...
        assert.include(
          iqManualGradingBody,
          'Instructor 1 (instructor1@illinois.edu) is currently grading this question'
        );
      });
      it('instructor becomes the manual grading user when first within manual grading expiry time range', async () => {
        ...
        assert.include(
          iqManualGradingBody,
          'Instructor 2 (instructor2@illinois.edu) is currently grading this question'
        );
      });
      it('instructor loses manual grading user status when outside of time range', async () => {
        ...
        after('reset time', () => {
          config.manualGradingExpirySec = prevTime;
        });
      ...
      it('instructor should get grading conflict view if another instructor submits grade to same question first (if both viewing question simaltaneously)', async () => {
        ...
        const gradingJob = (
          await sqlDb.queryOneRowAsync(sql.get_conflict_grading_jobs_by_iq, {
            id: instanceQuestionId,
          })
        ).rows[0];
        ...
        const grading_job_user = (
          await sqlDb.queryOneRowAsync(sql.get_grading_job_manual_grader, {
            gradingJobId: gradingJob.id,
          })
        ).rows[0];
        const auth_user = (
          await sqlDb.queryOneRowAsync(sql.get_user, { uid: mockInstructors[1].authUid })
        ).rows[0];

        const $gradingConflictPage = cheerio.load(submission2Body);
        const existingGradePanelBody = $gradingConflictPage('div:contains("Current Grade")')
          .parent()
          .html();
        const incomingGradePanelBody = $gradingConflictPage('div:contains("Incoming Grade")')
          .parent()
          .html();
      ...
      it('instructor should NOT see questions that are NOT configured for manual grading', async () => {
        const question = (await sqlDb.queryAsync(sql.get_question, { qid: 'partialCredit1' }))
          .rows[0];
      ...
      it('grading conflict can be resolved by any instructor', async () => {
        const $gradingConflictPage = cheerio.load(await (await fetch(gradingConflictUrl)).text());
        ...
        const instanceQuestion = (
          await sqlDb.queryOneRowAsync(sql.get_instance_question, { id: instanceQuestionId })
        ).rows[0];
        const assessmentQuestion = (
          await sqlDb.queryOneRowAsync(sql.get_assessment_question, {
            id: instanceQuestion.assessment_question_id,
          })
        ).rows[0];

        // application layer back-end will divide payload score by 100
        assert.equal(
          instanceQuestion.points,
          (payload.submissionScore / 100) * assessmentQuestion.max_points
        );
      ...
      it('grading conflict resolution containing `submission` obj score and feedback should resolve conflict and NOT produce new grading job', async () => {
        ...
        const numGradingJobsBefore = (
          await sqlDb.queryAsync(sql.get_grading_jobs_by_iq, { id: instanceQuestionId })
        ).rows.length;
      ...
      it('consecutive grading conflicts can be resolved on same instance question', async () => {
        ...
        const $user2ConflictPage = cheerio.load(submission2Body);
        ...
        const $user1ConflictPage = cheerio.load(submission3Body);
        ...
        assert.equal(
          user1Payload.instanceQuestionModifiedAt,
          user2Payload.instanceQuestionModifiedAt
        );

        // user 2 submits successfully
        setUser(mockInstructors[1]);
        const resolution1Body = await (
          await fetch(submission2.url, {
            method: 'POST',
            headers: { 'Content-type': 'application/x-www-form-urlencoded' },
        ...
        const instanceQuestionId = parseInstanceQuestionId(iqManualGradingUrl);
        const conflictGradingJobs = (
          await sqlDb.queryAsync(sql.get_conflict_grading_jobs_by_iq, { id: instanceQuestionId })
        ).rows;
        ...
        assert.equal(
          instanceQuestion.points,
          (finalPayload.submissionScore / 100) * assessmentQuestion.max_points
        );
