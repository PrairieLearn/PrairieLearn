...
Before running the script, you must update the `config.json` file to ensure the script connect to
the correct instance of Canvas. The `api_url` value is based on your institution's Canvas URL. The
`access_token` is based on an access token associated to your account. The token can typically be
generated by opening your user's profile settings and selecting "Approved Integrations".
