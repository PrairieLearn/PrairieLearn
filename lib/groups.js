 ...
 */
module.exports.getGroupRoleReassignmentsAfterLeave = function (groupInfo, leavingUserId, groupId) {
  // Get leaving user's required roles
  const requiredRoles = groupInfo.rolesInfo.groupRoles.filter((role) => (role.minimum ?? 0) > 0);
  const groupRoleAssignments = Object.values(groupInfo.rolesInfo.roleAssignments).flat();
  const requiredGroupRoleIds = groupRoleAssignments
    .filter(({ user_id }) => user_id === leavingUserId.toString())
    .map(({ group_role_id }) => group_role_id)
    .filter((roleId) => requiredRoles.some((role) => role.id === roleId));

  // Get group user to group role assignments, excluding the leaving user
  const remainingGroupRoleAssignments = groupRoleAssignments.filter(
    ({ user_id }) => user_id !== leavingUserId.toString()
  );

  // Replace non-required group role assignments with required roles from leaving user
  const groupRoleAssignmentUpdates = remainingGroupRoleAssignments.map(
    ({ user_id, group_role_id }) => {
      const notRequired = !requiredRoles.some((role) => role.id === group_role_id);
      if (notRequired && requiredGroupRoleIds.length > 0) {
        return {
          user_id,
          group_role_id: requiredGroupRoleIds.pop(),
          group_id: groupId,
        };
      } else {
        return { user_id, group_role_id, group_id: groupId };
      }
    }
  );

  // Assign remaining required roles to another group user
  const assigneeUser = groupInfo.groupMembers.find((m) => m.user_id !== leavingUserId.toString());
  if (assigneeUser === undefined) {
    throw new Error("Couldn't find another user to reassign roles to");
  }
  for (const groupRoleId of requiredGroupRoleIds) {
  ...
  await sqldb.runInTransactionAsync(async () => {
    ...
    if (groupConfig.has_roles) {
      ...
      if (currentSize > 1) {
        ...
        const requiredRoles = groupInfo.rolesInfo.groupRoles.filter(
          (role) => (role.minimum ?? 0) > 0
        );
        if (currentSize - 1 <= requiredRoles.length) {
