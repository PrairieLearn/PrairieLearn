  ...
  if (groupInfo.isGroupMember) {
    ...
    if (groupInfo.hasRoles) {
      const rolesInfo = await getRolesInfo(
        assessmentId,
        groupInfo.groupMembers[0].group_id,
        userId
      );
      groupInfo.start =
        needSize <= 0 &&
...
async function getRolesInfo(assessmentId, groupId, userId) {
  const rolesInfo = {};
  ...
  result = await sqldb.queryAsync(sql.get_group_members, params);
  let groupSize = result.rows.length;
  ...
  rolesInfo.usersWithoutRoles = result.rows.filter(
