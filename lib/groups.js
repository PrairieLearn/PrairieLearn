...
async function getRolesInfo(assessmentId, userId, groupSize, groupMembers) {
  ...
  // TODO: Maybe rewrite in more JS to eliminate the sproc?
  params = [assessmentId, userId];
  result = await sqldb.callAsync('group_roles_validate', params);
  rolesInfo.validationErrors = result.rows;
  ...
  console.log(rolesInfo.disabledRoles);
  console.log(requiredRoleNames);
  console.log(optionalRoleNames);
