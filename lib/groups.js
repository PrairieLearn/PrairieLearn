...
async function getRolesInfo(assessmentId, groupId) {
  ...
  rolesInfo.validationErrors = rolesInfo.groupRoles.filter(
    (role) => (role.minimum && role.count < role.minimum) || 
    (role.maximum && role.count > role.maximum)
  );
  ...
  if (groupSize >= requiredRoleNames.length) {
    rolesInfo.rolesAreBalanced = Object.values(roleAssignments).every((roles) => roles.length === 1);
  } else {
