...
module.exports.getGroupInfo = function (assessmentId, userId, callback) {
  ...
  sqldb.query(sql.get_config_info, params, function (err, result) {
    ...
    const usingGroupRoles = result.rows[0].has_roles;

    sqldb.query(sql.get_group_info, params, async function (err, result) {
      if (ERR(err, callback)) return;
      ...
      let disabledRoles;
      let groupRoles;
      let rolesAreBalanced;

      if (isGroupMember) {
        join_code = group_info[0].name + '-' + group_info[0].join_code;
        let isValidRoleAssignment = true;
        ...
        if (usingGroupRoles) {
          ...
          isValidRoleAssignment = result.rowCount === 0;
          minimumSizeMet = needsize <= 0;
          validationErrors = result.rows;

          // Get info on all group roles for the assessment
          params = { assessment_id: assessmentId };
          groupRoles = await sqldb.queryAsync(sql.get_group_roles, params);
          ...
          disabledRoles = groupsize <= requiredRoleNames.length ? optionalRoleNames : [];
          ...
          if (groupsize >= requiredRoleNames.length) {
            rolesAreBalanced = group_info.every((role) => role.role_names.split(', ').length === 1);
          } else {
        ...
        start = minimumSizeMet && rolesAreBalanced && isValidRoleAssignment;
      }
