...
module.exports.createGroup = function (groupName, assessmentId, userId, authnUserId, callback) {
  ...
  if (groupName.length <= 30 && groupName.match(letters)) {
    ...
    sqldb.query(sql.get_assigner_role, params, function (err, result) {
      if (ERR(err, callback)) return;
      if (result.rows.length > 0) params.assigner_role_id = result.rows[0].id;
      const query = result.rows.length > 0 ? sql.create_group_with_roles : sql.create_group;
      sqldb.query(query, params, function (err, _result) {
        if (!err) {
          callback(null, true);
        } else {
          sqldb.query(sql.get_config_info, params, function (err, result) {
