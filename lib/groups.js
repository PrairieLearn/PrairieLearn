  ...
  await sqldb.runInTransactionAsync(async () => {
    ...
    if (groupConfig.has_roles) {
      ...
      if (currentSize <= requiredRoles.length && currentSize > 1) {
        // Grab another user from the group as assignee
        const assigneeUser = groupInfo.groupMembers.find((m) => m.user_id !== userId);
        const result = await sqldb.queryValidatedSingleColumnRows(
          sql.get_user_roles,
          {
            group_id: groupId,
            user_id: userId,
        ...
        // Give assignee all of the leaving user's roles
        for (const group_role_id of result) {
