...
module.exports.getConfigInfo = function (res, callback) {
  var params = {
...
module.exports.joinGroup = function (joinCode, res, callback) {
  ...
  if (validJoinCode) {
    ...
    if (join_code.length !== 4) {
      throw new Error('invalid length of join code');
    }
    let params = [
...
module.exports.createGroup = function (groupName, res, callback) {
  ...
  if (invalidGroupName) {
    sqldb.query(sql.get_config_info, params, function (err, result) {
      ...
      callback(null, invalidGroupName);
    });
  }
};
module.exports.leaveGroup = function (req, res, callback) {
  const params = {
    assessment_id: res.locals.assessment.id,
    user_id: res.locals.user.user_id,
    authn_user_id: res.locals.authn_user.user_id,
