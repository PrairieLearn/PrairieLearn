...
class PythonCaller {
    ...
    call(type, directory, file, fcn, args, callback) {
        ...
        if (type === 'question') {
            cwd = path.join(this.course.path, 'questions', directory);
            paths.push(path.join(this.course.path, 'serverFilesCourse'));
        } else if (type === 'course-element') {
            cwd = path.join(this.course.path, 'elements', directory);
            paths.push(path.join(this.course.path, 'serverFilesCourse'));
    ...
    _startChild() {
        ...
        const cmd = 'python3';
        const pythonTrampoline = path.join(__dirname, '..', 'python', 'trampoline.py');
