...
module.exports = {
        ...
        sqldb.queryOneRow(sql.select_assessment_info, params, function(err, result) {
            ...
            serverJobs.createJobSequence(options, function(err, job_sequence_id) {
                ...
                serverJobs.createJob(jobOptions, function(err, job) {
                    ...
                    var students = [];
                    sqldb.query(sql.select_enrollments, aid, function(err, resultlist) {
                        if (ERR(err, callback)) return;
                        ...
                        var not_assigned = [];
                        sqldb.query(sql.select_not_assigned, aid, function(err, resultlist2) {
                            if (ERR(err, callback)) return;
                            ...
                            for(let i = 0; i < num_group; i++){
                                ...
                                members.forEach(memeber => {
                                    return new Promise((resolve, _reject) => {
                                        const msg = 'Processing record ' + memeber;
                                        if (output == null) {
                                            output = msg;
                                        } else {
                                            output += '\n' + msg;
                                        }
                                        module.exports._updateGroups(assessment_id, teamname, memeber, (err) => {
                                            if (err) {
                                                errorCount++;
                                                const msg = String(err);
                                                if (output == null) {
    ...
    _updateGroups(assessment_id, group_name, uid, callback) {
        util.callbackify(async () => {
            const params = [
                assessment_id,
                group_name,
