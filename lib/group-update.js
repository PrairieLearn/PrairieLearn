...
module.exports = {
                      ...
                      (err, result) => {
                        ...
                        if (err) {
                          ...
                          if (inGroup) {
                            ...
                            job.verbose(
                              `ERROR: The following users are already in a group.`,
                            );
                        ...
                        namedLocks.releaseLock(lock, (lockErr) => {
                          if (lockErr) {
                            job.fail(
                              `ERROR: The lock ${lockName} was not released successfully`,
                            );
                            return;
                          }
                        });
                        ...
                        if (errorCount == 0) {
                          ...
                          job.verbose(
                            `Successfully updated groups for ${successCount} students`,
                          );
  ...
  autoGroups(
    ...
    if (
      ...
      return callback(
        new Error('Group Setting Requirements: max > 1; min > 0; max >= min'),
      );
    }
    let params = { assessment_id };
    sqldb.queryOneRow(
            ...
            namedLocks.tryLock(lockName, (err, lock) => {
              if (err || lock == null) {
                ...
                (async () => {
                  try {
                    job.verbose(
                      'Auto generate group settings for ' + assessmentLabel,
                    );
                    job.verbose(`----------------------------------------`);
                    job.verbose(`Fetching the enrollment lists...`);
                    const aid = { assessment_id };
                    const students = [];
                    const resultList = await sqldb.queryAsync(
                      sql.select_enrollments,
                      aid,
                    );
                    ...
                    var notAssigned = [];
                    const resultList2 = await sqldb.queryAsync(
                      sql.select_not_assigned,
                      aid,
                    );
                    ...
                    job.verbose(
                      numNotAssigned + ' of them have not been in a group',
                    );
                    job.verbose(`----------------------------------------`);
                    job.verbose(
