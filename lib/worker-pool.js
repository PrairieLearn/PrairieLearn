...
/*
  ...
  messages from worker to manager look like {id, err, retArgs}
*/
...
module.exports = {
    ...
    init(nproc, callback) {
        ...
        async.times(nproc, (n, callback) => {
            const worker = child_process.fork(__dirname + '/worker');
            workers[n] = worker;
            worker.on('error', (err) => {
            ...
            worker.on('exit', (code, signal) => {
                logger.error(`ERROR: worker ${n} exited`, {code, signal});
                process.exit(1);
            });
            worker.on('message', (message) => {
                ...
                calls[message.id].callback(message.err, message.retArgs);
