...
class CourseInstanceCopyEditor extends Editor {
    ...
    write(callback) {
        ...
        async.waterfall([
            (callback) => {
                debug('Get all existing long names and generate a number');
                sqldb.query(sql.select_course_instances_with_course, {course_id: this.course.id}, (err, result) => {
                    ...
                    this.order_by = (_.max(_.map(result.rows, 'order_by')) || 1) + 1;
                    callback(null);
                });
            },
            (callback) => {
