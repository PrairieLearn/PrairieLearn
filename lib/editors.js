...
class FileDeleteEditor extends Editor {
  ...
  canEdit(callback) {
    if (!parentContainsChild(this.container.rootPath, this.deletePath)) {
    ...
    const found = this.container.invalidRootPaths.find((invalidRootPath) =>
      parentContainsChild(invalidRootPath, this.deletePath)
    );
    if (found) {
      let err = new Error('Invalid file path');
      err.info =
...
class FileRenameEditor extends Editor {
  ...
  canEdit(callback) {
    ...
    if (!parentContainsChild(this.container.rootPath, this.oldPath)) {
      let err = new Error('Invalid file path');
      err.info =
    ...
    if (!parentContainsChild(this.container.rootPath, this.newPath)) {
      let err = new Error('Invalid file path');
      err.info =
    ...
    found = this.container.invalidRootPaths.find((invalidRootPath) =>
      parentContainsChild(invalidRootPath, this.oldPath)
    );
    if (found) {
      let err = new Error('Invalid file path');
      err.info =
    ...
    found = this.container.invalidRootPaths.find((invalidRootPath) =>
      parentContainsChild(invalidRootPath, this.newPath)
    );
    if (found) {
      let err = new Error('Invalid file path');
      err.info =
...
class FileUploadEditor extends Editor {
  ...
  canEdit(callback) {
    if (!parentContainsChild(this.container.rootPath, this.filePath)) {
      let err = new Error('Invalid file path');
      err.info =
        `<p>The file path</p>` +
        `<div class="container"><pre class="bg-dark text-white rounded p-2">${this.oldPath}</pre></div>` +
    ...
    const found = this.container.invalidRootPaths.find((invalidRootPath) =>
      parentContainsChild(invalidRootPath, this.filePath)
    );
    if (found) {
      let err = new Error('Invalid file path');
      err.info =
