...
class Editor {
    ...
    /**
     * Delete a question, assessment, course instance, etc. based on its ID.
     * Will automatically remove empty parent directories to prevent sync issues.
     * @param rootDirectory Root directory that the items are being stored in.
     * @param id Item to delete, relative from the root directory.  This is a path that
     may contain slashes to indicate subdirectories.
     * @callback {function(err)} Function to call once this has finished.
     */
    deleteFolderAndPrecedingSubfolders(rootDirectory, id, callback) {
        const basePath = path.join(this.course.path, 'questions');
        const deletePath = path.join(rootDirectory, this.question.qid);
        const qidSplit = this.question.qid.split(path.sep);
        ...
        fs.remove(deletePath, (err) => {
            ...
            const reverseFolders = qidSplit.slice(0, -1).reverse();
            ...
            async.eachOfSeries(reverseFolders, (folder, index, callback) => {
                if (!seenNonemptyFolder) {
                    const delPath = path.join(rootDirectory, ...qidSplit.slice(0, qidSplit.length - 1 - index));
...
class AssessmentDeleteEditor extends Editor {
    ...
    write(callback) {
        ...
        this.deleteFolderAndPrecedingSubfolders(deletePath, this.assessment.tid, (err) => {
            if (ERR(err, callback)) return;
            this.pathsToAdd = [
...
class CourseInstanceDeleteEditor extends Editor {
    ...
    write(callback) {
        ...
        this.deleteFolderAndPrecedingSubfolders(deletePath, this.course_instance.short_name, (err) => {
            if (ERR(err, callback)) return;
            this.pathsToAdd = [
...
class QuestionDeleteEditor extends Editor {
    ...
    write(callback) {
        ...
        this.deleteFolderAndPrecedingSubfolders(path.join(this.course.path, 'questions'), this.question.qid, (err) => {
            if (ERR(err, callback)) return;
            this.pathsToAdd = [
