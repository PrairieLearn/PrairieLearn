...
class Editor {
  ...
  canEdit(callback) {
    ...
    if (this.course.example_course) {
      return callback(
        new Error(`Access denied (cannot edit the example course)`),
      );
    }
        ...
        serverJobs.createJob(jobOptions, (err, job) => {
          ...
          syncFromDisk._syncDiskToSqlWithLock(
            ...
            (err, result) => {
              if (
                ERR(err, (e) =>
                  logger.error('Error in _syncDiskToSqlWithLock()', e),
                )
              ) {
      ...
      const _reloadQuestionServers = () => {
        ...
        serverJobs.createJob(jobOptions, (err, job) => {
          ...
          requireFrontend.undefQuestionServers(coursePath, job, (err) => {
            if (
              ERR(err, (e) =>
                logger.error('Error in undefQuestionServers()', e),
              )
            ) {
              job.fail(err);
            } else {
      ...
      (folder, index, callback) => {
        if (!seenNonemptyFolder) {
          const delPath = path.join(
            rootDirectory,
            ...idSplit.slice(0, idSplit.length - 1 - index),
          );
  ...
  getNamesForCopy(oldShortName, shortNames, oldLongName, longNames) {
    ...
    function getNumberShortName(basename, oldnames) {
      ...
      oldnames.forEach((oldname) => {
        const found = oldname.match(
          new RegExp(`^${escapeRegExp(basename)}_copy([0-9]+)$`),
        );
        if (found) {
    ...
    function getNumberLongName(basename, oldnames) {
      ...
      oldnames.forEach((oldname) => {
        ...
        const found = oldname.match(
          new RegExp(`^${escapeRegExp(basename)} \\(copy ([0-9]+)\\)$`),
        );
        if (found) {
          const foundNumber = parseInt(found[1]);
          if (foundNumber >= number) {
    ...
    const number =
      numberShortName > numberLongName ? numberShortName : numberLongName;
    return {
    ...
    async.waterfall(
      [
        ...
        (callback) => {
          ...
          this.courseInstancePath = path.join(
            courseInstancesPath,
            this.short_name,
          );
  ...
  write(callback) {
    ...
    async.waterfall(
      [
        ...
        (callback) => {
          debug(
            `Find all assessments (in all course instances) that contain ${this.question.qid}`,
          );
          sqldb.query(
    ...
    async.waterfall(
      [
        (callback) => {
          ...
          fs.readJson(
            path.join(this.from_path, 'info.json'),
            (err, infoJson) => {
              if (ERR(err, callback)) return;
    ...
    fs.readFile(this.filePath, (err, contents) => {
      if (err) {
        ...
        if (oldHash == newHash) {
          ...
          debug(
            'new contents are different from old contents, so continue with upload',
          );
          callback(null, true);
        }
      }
