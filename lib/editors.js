...
class Editor {
    ...
    canEdit(callback) {
        ...
        if (!this.authz_data.has_course_permission_edit) return callback(error.make(403, 'Access denied (must be course editor)'));
        ...
        serverJobs.createJobSequence(options, (err, job_sequence_id) => {
            ...
            const _syncFromDisk = () => {
                ...
                serverJobs.createJob(jobOptions, (err, job) => {
                    ...
                    syncFromDisk._syncDiskToSqlWithLock(this.course.path, this.course.id, job, (err, result) => {
                                ...
                                zone.questions.forEach((question) => {
                                    if (question.alternatives) {
                                        question.alternatives.forEach((alternative) => {
                                            if (idsEqual(alternative.id, this.question.qid)) {
                                                alternative.id = this.qid_new;
                                                found = true;
                                            }
                                        });
                                    } else if (idsEqual(question.id, this.question.qid)) {
