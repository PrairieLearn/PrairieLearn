...
// class QuestionFilenameLocationEditor extends Editor {
//   constructor(params) {
//       super(params);
//       this.filename_location_new = params.filename_location_new;
//       this.description = `Rename question's filename location to ${this.filename_location_new}`;
//   }
//
//   canEdit(callback) {
//       super.canEdit((err) => {
...
//
//   write(callback) {
//       debug('ChangeFilenameLocationEditor: write()');
...
//                   },
//                   (infoJson, callback) => {
//                       debug(`Find/replace filename_location in ${infoPath}`);
//                       if (infoJson.thumbnail) {
//                           infoJson.thumbnail.location = this.filename_location_new;
//                       } else {
//                           infoJson.thumbnail = {filename: 'None', location: this.filename_location_new};
//                       }
//                       // infoJson.thumbnail.location = this.filename_location_new;
//                       fs.writeJson(infoPath, infoJson, {spaces: 4}, (err) => {
//                           if (ERR(err, callback)) return;
...
// }
//
// class QuestionThumbnailFilenameEditor extends Editor {
//     constructor(params) {
//         super(params);
//         this.thumbnail_filename_new = params.thumbnail_filename_new;
//         this.description = `Rename question's thumbnail filename to ${this.thumbnail_filename_new}`;
//     }
//
//     canEdit(callback) {
//         super.canEdit((err) => {
...
//
//     write(callback) {
//         debug('ChangeThumbnailFilenameEditor: write()');
//         const infoPath = path.join(this.course.path, 'questions', this.question.qid, 'info.json');
//         async.waterfall([
//             (callback) => {
//                 async.waterfall([
...
//                     },
//                     (infoJson, callback) => {
//                         debug(`Find/replace thumbnail_filename in ${infoPath}`);
//                         if (infoJson.thumbnail) {
//                             infoJson.thumbnail.filename = this.thumbnail_filename_new;
//                         } else {
//                             intoJson.thumbnail = {filename: this.thumbnail_filename_new, location: question};
//                         }
//                         // infoJson.thumbnail.filename = this.thumbnail_filename_new;
//                         fs.writeJson(infoPath, infoJson, {spaces: 4}, (err) => {
//                             if (ERR(err, callback)) return;
                ...
                async.waterfall([
                    ...
                    (infoJson, callback) => {
                        ...
                        if (infoJson.thumbnail) {
                            ...
                            intoJson.thumbnail = {filename: this.thumbnail_filename_new, location: this.filename_location_new};
                        }
                        fs.writeJson(infoPath, infoJson, {spaces: 4}, (err) => {
                            if (ERR(err, callback)) return;
                            callback(null);
