...
class Editor {
    ...
    removeEmptyPrecedingSubfolders(rootDirectory, id, callback) {
        ...
        async.eachOfSeries(reverseFolders, (folder, index, callback) => {
    ...
    getExistingShortNames(rootDirectory, infoFile, callback) {
        ...
        const walk = (relativeDir, callback) => {
            fs.readdir(path.join(rootDirectory, relativeDir), (err, directories) => {
                ...
                async.each(directories, (dir, callback) => {
    ...
    write(callback) {
        ...
        fs.remove(path.join(deletePath, this.assessment.tid), (err) => {
            ...
            this.removeEmptyPrecedingSubfolders(deletePath, this.assessment.tid, (err) => {
                if (ERR(err, callback)) return;
                this.pathsToAdd = [
                    path.join(deletePath, this.assessment.tid),
                ];
...
class AssessmentRenameEditor extends Editor {
    ...
    write(callback) {
        ...
        const basePath = path.join(this.course.path, 'courseInstances', this.course_instance.short_name, 'assessments');
...
class CourseInstanceCopyEditor extends Editor {
    ...
    write(callback) {
        ...
        async.waterfall([
            ...
            (callback) => {
                debug('Get all existing short names');
                this.getExistingShortNames(courseInstancesPath, 'infoCourseInstance.json', (err, filenames) => {
                    if (ERR(err, callback)) return;
                    this.oldNamesShort = filenames;
                    callback(null);
                });
    ...
    write(callback) {
        ...
        fs.remove(path.join(deletePath, this.course_instance.short_name), (err) => {
            ...
            this.removeEmptyPrecedingSubfolders(deletePath, this.course_instance.short_name, (err) => {
                if (ERR(err, callback)) return;
                this.pathsToAdd = [
                    path.join(deletePath, this.course_instance.short_name),
                ];
...
class CourseInstanceRenameEditor extends Editor {
    ...
    write(callback) {
        ...
        fs.move(oldPath, newPath, {overwrite: false}, (err) => {
            if (ERR(err, callback)) return;
            this.removeEmptyPrecedingSubfolders(path.join(this.course.path, 'courseInstances'), this.course_instance.short_name, (err) => {
                if (ERR(err, callback)) return;
...
class QuestionAddEditor extends Editor {
    ...
    write(callback) {
        ...
        async.waterfall([
            ...
            (callback) => {
                const fromPath = path.join(__dirname, '..', 'exampleCourse', 'questions', 'demo', 'calculation');
...
class QuestionDeleteEditor extends Editor {
    ...
    write(callback) {
        ...
        fs.remove(path.join(this.course.path, 'questions', this.question.qid), (err) => {
            if (ERR(err, callback)) return;
            this.removeEmptyPrecedingSubfolders(path.join(this.course.path, 'questions'), this.question.qid, (err) => {
                if (ERR(err, callback)) return;
                this.pathsToAdd = [
                    path.join(this.course.path, 'questions', this.question.qid),
                ];
...
class QuestionEditThumbnailEditor extends Editor {
    constructor(params) {
        super(params);
        this.thumbnail_filename_new = params.thumbnail_filename_new;
        this.filename_location_new = params.filename_location_new;
        this.description = `Edit thumbnail information to {${this.thumbnail_filename_new}, ${this.filename_location_new}}`;
    }
    ...
    write(callback) {
        debug('EditThumbnail: write()');
        const infoPath = path.join(this.course.path, 'questions', this.question.qid, 'info.json');
        async.waterfall([
            (callback) => {
                debug(`Read ${infoPath}`);
                fs.readJson(infoPath, (err, infoJson) => {
            ...
            (infoJson, callback) => {
                debug(`Find/replace thumbnail_filename and filename_location in ${infoPath}`);
                if (this.thumbnail_filename_new == '' && this.filename_location_new == '') {
                    delete infoJson['thumbnail'];
                } else {
                    if (infoJson.thumbnail) {
                        infoJson.thumbnail.filename = this.thumbnail_filename_new;
                        infoJson.thumbnail.location = this.filename_location_new;
                    } else {
                        infoJson.thumbnail = {filename: this.thumbnail_filename_new, location: this.filename_location_new};
                    }
                }
                fs.writeJson(infoPath, infoJson, {spaces: 4}, (err) => {
                    if (ERR(err, callback)) return;
...
class ThumbnailUploadEditor extends FileUploadEditor {
    constructor(params) {
        super(params);
        this.thumbnail_filename_new = params.thumbnail_filename_new;
        this.filename_location_new = params.filename_location_new;
        this.description = `Upload thumbnail to ${path.relative(this.container.rootPath, this.filePath)}`;
    }
    ...
    shouldEdit(callback) {
      ...
      fs.readFile(this.filePath, (err, contents) => {
          if (err) {
              ...
              debug('newHash: ' + newHash);
              if (oldHash == newHash) {
                  if (this.question.thumbnail_filename == this.thumbnail_filename_new && this.question.filename_location == this.filename_location_new) {
                      debug('new contents are the same as old contents, so abort upload');
                      callback(null, false);
                  } else {
                      debug('new contents are the same as old contents but the thumbnail info is different, so continue with upload');
                      callback(null, true);
                  }
              } else {
                  debug('new contents are different from old contents, so continue with upload');
    ...
    write(callback) {
        debug('ThumbnailUploadEditor: write()');
        const infoPath = path.join(this.course.path, 'questions', this.question.qid, 'info.json');
        async.parallel([
            (callback) => {
                async.series([
