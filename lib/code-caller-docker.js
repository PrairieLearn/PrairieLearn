...
class DockerCaller {
    constructor(options = { questiontTimeoutMlliseconds: 5000 }) {
        /** @type {CallerState} */
        this.state = CREATED;
        this.uuid = uuidv4();
    ...
    call(type, directory, file, fcn, args, callback) {
        ...
        this.ensureChild().then(() => {
            ...
            // unresponsive.
            const containerTimeout = this.options.questiontTimeoutMlliseconds * 1.5;
    ...
    async ensureChild() {
        ...
        // This will only execute if we're in `container` mode.
        ...
        await this.ensureChildMutex.runExclusive(async () => {
            ...
            await ensureImage();
            await this._ensureDirectoryIfNeeded();
            const binds = [`${this.hostDirectory.path}:/course:ro`];
    ...
    async _ensureDirectoryIfNeeded() {
        await this.ensureDirectoryMutex.runExclusive(async () => {
            if (config.workersExecutionMode === 'container' && !this.hostDirectory) {
    ...
    async _createAndAttachContainer(binds) {
        ...
        this.container = await docker.createContainer({
            ...
            Env: [
                ...
                `QUESTION_TIMEOUT_MILLISECONDS=${this.options.questiontTimeoutMlliseconds}`,
            ],
        });
