...
class DockerCaller {
    ...
    this.ensureChild()
      .then(() => {
        if (!this._checkState([WAITING])) {
          callback(new Error('invalid DockerCaller state'));
          return;
        }

        if (!this._checkReadyForCall()) {
          callback(new Error('not ready for call'));
          return;
        }
   ...
   */
  callAsync(type, directory, file, fcn, args) {
    return new Promise((resolve, reject) => {
      this.call(type, directory, file, fcn, args, (err, data, output) => {
        if (err) return reject(err);
        return resolve({ result: data, output });
      });
    });
  }
   ...
   */
  restart(callback) {
    util.callbackify(this.restartAsync)(callback);
  }

  async restartAsync() {
    this.debug('enter restart()');
    if (!this._checkState([CREATED, WAITING, EXITING, EXITED])) {
      throw new Error('Unexpected DockerCaller state');
    }
    ...
    if (this.callCount === 0) {
      ...
      const { result } = await this.callAsync('restart', null, null, 'restart', []);
      this.coursePath = null;
      this.callCount = 0;
  ...
  _callCallback(err, data, output) {
    ...
    if (parseInt(process.versions.node.split('.')[0]) >= 16) {
      ...
      // @ts-expect-error -- The types no longer reflect this form of the function.
      c({}, err, data, output);
    }
  ...
  _callIsFinished() {
    ...
    let data = null;
    let err = null;
    console.log(`__output__: ${this.outputStdout}`);
    try {
      data = JSON.parse(this.outputStdout);
      console.log(`__parsed_data__: ${JSON.stringify(data)}`);
      if (data.error) {
