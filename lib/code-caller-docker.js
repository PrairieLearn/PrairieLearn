...
const WORKER_IMAGE = 'prairielearn/executor';
...
class DockerCaller {
        ...
        util.callbackify(async () => {
            ...
            if (this.course) {
                if (this.course.path === course.path) {
                    ...
                    callback(new Error(`DockerCaller is already allocated for course "${this.course.path}"`));
                    return;
                }
            }
     ...
     */
    async _ensureImage() {
        const image = docker.getImage(WORKER_IMAGE);
        try {
            await image.inspect();
        } catch (e) {
            if (e.statusCode === 404) {
                await docker.pull(WORKER_IMAGE, {});
            } else {
                throw e;
            }
        }
    ...
    async _handleContainerExit(err, code) {
        ...
        if (this.state == WAITING) {
            this._logError(`DockerCaller container exited while in state = WAITING, code = ${code}`);
            this.container = null;
            this.state = EXITED;
            ...
            this.container = null;
            this.state = EXITED;
            this._callCallback(new Error(`DockerCaller container exited unexpectedly, code = ${code}`));
 ...
 * fails. It'll check for any dicretories in the OS tmp directory that match the
