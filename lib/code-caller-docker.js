...
function getExecutorImageName() {
    const imageName = `prairielearn/executor:${executorImageVersion}`;
...
class DockerCaller {
    ...
    prepareForCourse(coursePath, callback) {
        util.callbackify(async () => {
            ...
            if (this.coursePath) {
                if (this.coursePath === coursePath) {
                    // Same course as before; we can reuse the existing setup
                    return;
                } else if (config.workersExecutionMode === 'container') {
                    throw new Error(`DockerCaller is already allocated for course "${this.coursePath}"`);
                }
            }
            ...
            if (config.workersExecutionMode === 'container') {
                ...
                let courseDir;
                if (coursePath === '/PrairieLearn/exampleCourse') {
                    ...
                    const originalDirectory = originalNames.get(coursePath);
                    courseDir = config.courseDirsHost[originalDirectory];
                    if (!courseDir) {
                        throw new Error(`No host directory specified for course at ${coursePath}`);
                    }
                }
     ...
     */
    async _ensureImage() {
        this.debug('enter _ensureImage()');
        const imageName = getExecutorImageName();
        const image = docker.getImage(imageName);
        try {
            await image.inspect();
            this.debug(`image already exists`);
            this.debug('exit _ensureImage()');
        } catch (e) {
            if (e.statusCode === 404) {
                this.debug('pulling image');
                ...
                this.debug('successfully pulled image');
                this.debug('exit _ensureImage()');
            } else {
                throw e;
            }
    ...
    async _handleContainerExit(err, code) {
        ...
        if (this.state == WAITING) {
            this._logError(`DockerCaller container exited while in state = WAITING, code = ${JSON.stringify(code)}, err = ${err}`);
            this.container = null;
            this.state = EXITED;
            ...
            this.container = null;
            this.state = EXITED;
            this._callCallback(new Error(`DockerCaller container exited unexpectedly, code = ${JSON.stringify(code)}, err = ${err}`));
