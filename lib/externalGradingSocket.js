...
module.exports.connection = function(socket) {
    ...
    socket.on('getResults', (msg, callback) => {
        ...
        module.exports.renderPanels(msg.grading_job_id, msg.urlPrefix, (err, panels) => {
            if (ERR(err, (err) => logger.error(err))) return;
            callback({
                grading_job_id: msg.grading_job_id,
...
module.exports.renderPanels = function(grading_job_id, urlPrefix, callback) {
    question.renderPanelsForGradingJob(grading_job_id, urlPrefix, (err, results) => {
        if (ERR(err, callback)) return;
