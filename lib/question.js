...
/**
 * Internal error type for tracking errors resulting from parsing student submissions.
 */
class SubmissionParsingError extends Error {
    constructor(message) {
        super(message);
...
/**
 * Internal error type thrown when a fatal question error occurs when grading.
 */
class FatalGradingError extends Error {
    constructor(message) {
        super(message);
        this.name = 'FatalGradingError';
    }
}
...
module.exports = {
    ...
    */
     ...
     * @param {Object} submission - The submission to grade (for regrading only -- pass in null if not regrading).
     * @param {function} callback - A callback(err, result) function. If this grading request must be 
     *                            fulfilled by an external autograder, then the grading_job id will be stored in 
     *                            result.grading_job_id. This request should only be submitted once the transaction 
     *                            of this client is committed. If regrading a submission, the submission's
     *                            score and points are stored in result.score and result.points, respectively.
     */
    _gradeVariantWithClient(client, variant, check_submission_id, question, course, authn_user_id, overrideGradeRateCheck, submission, callback) {
        debug('_gradeVariantWithClient()');
        let questionModule, courseIssues, data, grading_job;
        const result = {};
        const regrading = (submission != null);
        async.series([
            (callback) => {
                if (regrading) return callback(null);
                var params = [
                    variant.id,
                    check_submission_id,
                ];
            ...
            (callback) => {
                if (regrading || overrideGradeRateCheck) return callback(null);
                var params = [
            ...
            (callback) => {
                ...
                questionModule.parse(submission, variant, question, course, (err, ret_courseIssues, ret_data) => {
                    ...
                    debug('_gradeVariantWithClient()', 'completed parse()');

                    if (!data.gradable) {
                        debug('_gradeVariantWithClient()', 'error when parsing');
                        return callback(new SubmissionParsingError());
                    }
                    ...
                    debug('_gradeVariantWithClient()', 'submitted_answer:', submission.submitted_answer);
            ...
            (callback) => {
                ...
                sqldb.callWithClientOneRow(client, 'grading_jobs_insert', params, (err, result) => {
                    if (ERR(err, callback)) return;
                    if (data.broken) {
                        debug('_gradeVariantWithClient()', 'Fatal question error occurred when grading');
                        const error = new FatalGradingError();
                        error.courseIssues = courseIssues;
                    ...
                    if (!grading_job.gradable) {
                        debug('_gradeVariantWithClient()', 'submission not gradable');
            ...
            (callback) => {
                ...
                sqldb.queryWithClientOneRow(client, sql.select_instance_question_points_by_variant_id, params, (err, result) => {
                    if (ERR(err, callback)) return;
            ...
            if (regrading) {
                if (err instanceof NoSubmissionError) return callback(null);
                if (ERR(err, callback)) return;
                result.score = data.score;
                result.points = data.points;
            } else {    
                // catch errors that we are using to exit with no action
                if (err instanceof FatalGradingError) {
                    debug('_gradeVariantWithClient()', 'Fatal question error occurred when grading');
                    err = null;
                } else if (err instanceof NoSubmissionError) {
                ...
                if (data && !data.broken && grading_job && grading_job.grading_method == 'External') {
                    ...
                    result.grading_job_id = grading_job.id;
                }
            }
    ...
    gradeVariant(variant, check_submission_id, question, course, authn_user_id, overrideGradeRateCheck, callback) {
        ...
        sqldb.beginTransaction((err, client, done) => {
            ...
            async.series([
                ...
                // in the presence of errors.
                (callback) => {
                    this._gradeVariantWithClient(client, variant, check_submission_id, question, course, authn_user_id, overrideGradeRateCheck, null, (err, result) => {
                        if (ERR(err, callback)) return;
    ...
    regradeQuestion(authn_user_id, course_id, instance_question_id, keep_highest_score, callback) {
        ...
        serverJobs.createJobSequence(options, (err, job_sequence_id) => {
            ...
            serverJobs.createJob(jobOptions, (err, job) => {
                ...
                // We want to first check if there is any submissions to regrade for this question instance.
                // If there is no submission, we can just return.
                sqldb.query(sql.select_submission_data_for_regrading, {instance_question_id: instance_question_id}, (err, result) => {
                    if (err) {
                        job.fail(`Error in select_submission_data_for_regrading: ${err}`);
                        logger.error(`Error in select_submission_data_for_regrading: ${err}`);
                        return;
                    }
                    if (result.rowCount === 0) {
                        job.verbose(`No submissions to regrade. Exiting.`);
                        job.succeed();
                        return;
                    }
                    ...
                    sqldb.beginTransaction((err, client, done) => {
                        if (err) {
                            job.fail(`Error in beginTransaction(): ${err}`);
                        ...
                        async.series([
                            (callback) => {
                                ...
                                sqldb.queryWithClientOneRow(client, sql.select_course, params, (err, result) => {
                                    if (ERR(err, callback)) return;
                            ...
                            (callback) => {
                                ...
                                sqldb.queryWithClientOneRow(client, sql.select_question, params, (err, result) => {
                                    if (ERR(err, callback)) return;
                            ...
                            (callback) => {
                                ...
                                sqldb.queryWithClient(client, sql.log_regrading_request, params, (err) => {
                                    if (ERR(err, callback)) return;
                                    callback(null);
                                });
                            },
                            ...
                            (callback) => {
                                ...
                                async.eachSeries(submissions, ((submission, callback) => {
                                    ...
                                    sqldb.queryWithClientOneRow(client, sql.select_variant, {variant_id: submission.variant_id}, (err, result) => {
                                        ...
                                        this._gradeVariantWithClient(client, variant, null, question, course, submission.auth_user_id, true, submission, (err, result) => {
                                            if (err instanceof FatalGradingError) {
                                                err.courseIssues.forEach((courseErr) => {
                                                    job.verbose('Encountered fatal grading issue when regrading:');
                                                    job.error(chalk.red(courseErr));
                                                    job.verbose('-------------------------------------------Console Log-------------------------------------------');
                                                    job.verbose(courseErr.data.outputStderr);
                                                    job.verbose('-------------------------------------------Stack-------------------------------------------');
                                                    job.verbose(courseErr.data.stack);
                                                    job.verbose('-------------------------------------------------------------------------------------------------');
                                                });
                                                return callback('Encountered fatal error when running grading script');
                                            } else if (err instanceof SubmissionParsingError) {
                                                job.error(chalk.yellow('⚠ Error when trying to parse this submission, skipping.'));
                                                return callback(null);  // just move on to the next submission
                                            }

                                            if (ERR(err, callback)) return;
                                            if (result) {
                                                job.verbose(chalk.green(`✓ Submission successfully regraded. New score: ${result.score}.`));
                                                job.verbose(chalk.green(`Points earned for this instance question so far: ${result.points}.`));

                                                if (!variant.open) {
                                                    job.verbose(chalk.yellow('Note: this submission\'s score does not count toward the question score because the variant is closed.'));
                                                }
                                            }
                                            callback(null);
                                        });
                            ...
                            (callback) => {
                                ...
                                job.verbose(chalk.yellow(`The regraded score (${regraded_instance_question.points} points) is lower than the `
                                    + `original score (${original_instance_question.points} points). Restoring the original instance question.`));
                            ...
                            (callback) => {
                                ...
                                job.verbose(chalk.yellow('Restoring original variants.'));
                            ...
                            // Log regrading rollback (if performed)
                            (callback) => {
                                if (!keep_highest_score || original_instance_question.points <= regraded_instance_question.points) {
                                    callback(null);
                                    return;
                                ...
                                const params = {
                                    ...
                                    auth_user_id: authn_user_id,
                                    instance_question_id: instance_question_id,
                                    points: original_instance_question.points,
                                    score_perc: original_instance_question.score_perc,
                                };
                                sqldb.queryWithClient(client, sql.log_regrading_rollback, params, (err) => {
                                    if (ERR(err, callback)) return;
                                    callback(null);
                                });
                            },
                            // Final recalculation and logging of assessment score
                            (callback) => {
                                const params = [
                                    ...
                                    !keep_highest_score,  // allow_decrease
                                ];
    ...
    saveAndGradeSubmission(submission, variant, question, course, overrideGradeRateCheck, callback) {
        ...
        sqldb.beginTransaction((err, client, done) => {
            ...
            async.series([
                ...
                (callback) => {
                    this._gradeVariantWithClient(client, variant, submission_id, question, course, submission.auth_user_id, overrideGradeRateCheck, null, (err, result) => {
                        if (ERR(err, callback)) return;
                        grading_job_id = result.grading_job_id;
    ...
    _testVariantWithClient(client, variant, question, course, test_type, authn_user_id, callback) {
        ...
        async.series([
            ...
            (callback) => {
                this._gradeVariantWithClient(client, variant, test_submission_id, question, course, authn_user_id, true, null, (err) => {
                    if (ERR(err, callback)) return;
    ...
    _buildLocals(variant, question, instance_question, assessment, assessment_instance, assessment_question, overlay_grading_interface, authz_result) {
        const locals = {};
    ...
    getAndRenderVariant(variant_id, variant_seed, locals, callback) {
        async.series([
            ...
            (callback) => {
                ...
                const newLocals = this._buildLocals(variant, question, instance_question, assessment, assessment_instance, assessment_question, overlayGradingInterface, authz_result);
