...
module.exports = {
    ...
    _compareSubmissions(expected_submission, test_submission, callback) {
        ...
        const checkEqual = (name, var1, var2) => {
            if (!_.isEqual(var1, var2)) courseErrs.push(new Error(`"${name}" mismatch: expected "${var1}" but got "${var2}"`));
        };
        ...
        checkEqual('format_errors', expected_submission.format_errors, test_submission.format_errors);
     ...
     * @param {Object} course - The course for the variant.
     * @param {number} authn_user_id - The currently authenticated user.
     * @param {function} callback - A callback(err, variant_id) function.
     */
    testQuestion(question, course, authn_user_id, callback) {
        debug('testQuestion()');
        let expected_submission_id, expected_submission, test_submission_id;
        sqldb.beginTransaction((err, client, done) => {
            if (ERR(err, callback)) return;
            async.series([
                (callback) => {
                ...
                (callback) => {
                    ...
                    this._testVariantWithCLient(client, variant, question, course, authn_user_id, (err) => {
                        if (ERR(err, callback)) return;
                        debug('testQuestion()', 'tested variant');
                        callback(null);
                    });
                },
            ], (err) => {
     ...
     * 
     * @param {Object} count - The number of times to test.
     * @param {Object} question - The question for the variant.
     * @param {Object} course - The course for the variant.
     * @param {number} authn_user_id - The currently authenticated user.
     * @param {function} callback - A callback(err, job_sequence_id) function.
     */
    startTestQuestion(count, question, course, authn_user_id, callback) {
        const options = {
            course_id: course.id,
        ...
        serverJobs.createJobSequence(options, (err, job_sequence_id) => {
            ...
            serverJobs.createJob(jobOptions, (err, job) => {
                ...
                job.verbose('Testing ' + question.qid);
                this.testQuestion(question, course, authn_user_id, (err, variant_id) => {
                    if (ERR(err, () => {})) {
                        return job.fail(err);
                    }

                    sqldb.query(sql.select_errors_for_variant, {variant_id}, (err, result) => {
                        if (ERR(err, () => {})) {
                            return job.fail(err);
                        }

                        if (result.rowCount > 0) {
                            job.verbose(`ERROR: ${result.rowCount} errors encountered during test. See the "Errors" page for details.`);
                        } else {
                            job.verbose('Success: no errors during test');
                        }
                        job.succeed();
                    });
                });
