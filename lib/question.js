      ...
      [
        ...
        async () => {
          if (locals.instance_question) {
            locals.rubric_mustache_data = {
              correct_answers: locals.variant?.true_answer,
              params: locals.variant?.params,
            };
            // Only first three submissions are rendered initially
            await async.eachSeries(locals.submissions.slice(0, 3), async (submission) => {
              submission.rubric_data_manual = await manualGrading.selectRubricGradingData(
                locals.assessment_question.id,
                locals.assessment_question.manual_rubric_id,
                submission.manual_rubric_grading_id,
                locals.rubric_mustache_data
              );
              submission.rubric_data_auto = await manualGrading.selectRubricGradingData(
                locals.assessment_question.id,
                locals.assessment_question.auto_rubric_id,
                submission.auto_rubric_grading_id,
                locals.rubric_mustache_data
              );
            });
          }
      ...
      async.parallel(
        [
          async () => {
            ...
            const htmls = await util.asyncify(module.exports._render)(
              renderSelection,
              variant,
              question,
              submission,
            ...
            submission.feedback_manual_html = await util.asyncify(markdown.processContent)(
            ...
            panels.submissionPanel = await util.asyncify(ejs.renderFile)(
              templatePath,
              renderParams
            );
          },
