    ...
    */
    ...
    _regradeVariantWithClient(client, variant, submission, question, course, authn_user_id, serverJob, callback) {
        ...
        async.series([
            ...
            serverJob.verbose(chalk.green(`✓ Submission successfully regraded. New score: ${data.score}.`));
            serverJob.verbose(chalk.green(`Points earned for this instance question so far: ${data.points}.`));

            if (!variant.open) {
                debug('_regradeVariantWithClient()', 'variant closed');
                serverJob.verbose(chalk.yellow('Note: this submission\'s score does not count toward the question score because the variant is closed.'));
            } else {
