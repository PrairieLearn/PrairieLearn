...
module.exports = {
    ...
    */
    ...
    saveSubmission(submission, variant, question, course, callback) {
        ...
        sqldb.beginTransaction((err, client, done) => {
            ...
            async.series([
                ...
                // in the presence of errors.
                (callback) => {
                    util.callbackify(async () => this._saveS3FileUploads(submission, variant, question))((err, file_ids) => {
                        if (ERR(err, callback)) return;
     ...
     * @return {Array} file_ids of uploaded S3 files from Files table.
     */ 
    
    async _saveS3FileUploads(submission, variant, question) {
        ...
        const s3Files = [];
            ...
            for await (const file of files) {
                ...
                if (Buffer.byteLength(file.contents, 'base64') >= config.fileUploadS3MinBytes) {
                    const file_id = await fileStore.upload(file.name, file.contents, 'student_upload', assessment_instance_id, instance_question_id, question_id, authn_user_id, user_id, fileStore.storageTypes.S3);
