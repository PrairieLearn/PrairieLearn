...
module.exports = {
    ...
    */
     ...
     */
    _saveSubmissionWithClient(client, submission, variant, question, course, instanceQuestion, callback) {
        ...
        async.series([
            ...
            (callback) => { // if we have a workspace, get the files
                if (workspace_id == null) return callback(null);
                util.callbackify(workspace.getGradedFiles)(workspace_id, (err, result) => {
                    if (ERR(err, callback)) return;
                    zipPath = result;
                    callback(null);
                });
     ...
     * @param {Object} question - The question for the variant.
     * @param {Object} course - The course for the variant.
     * @param {Object} assessment - The assessment for the variant
     * @param {function} callback - A callback(err, submission_id) function.
     */
    saveSubmission(submission, variant, question, course, assessment, instanceQuestion, callback) {
        let submission_id;
        ...
        sqldb.beginTransaction((err, client, done) => {
            ...
            async.series([
                ...
                // in the presence of errors.
                (callback) => {
                    // this._saveS3FileUploads(submission, client, variant, question, course);
                    this._saveSubmissionWithClient(client, submission, variant, question, course, instanceQuestion, (err, ret_submission_id) => {
                        if (ERR(err, callback)) return;
    ...
    /**
     * NEEDS DOCUMENTATION
     * Saves file to S3 and then removes it from object to be saved to database.
     */ 
    
    
    // res.locals.instance_question.id, res.locals.user.user_id, res.locals.instance_questions.authn_user_id, 
    _saveS3FileUploads(submission, client, instanceQuestion, assessment) {

        // MAJOR NOTE: Instructor view sends assessment in form of an array, while student view sends full assessment object. 
        // MUST RESOLVE THIS ISSUE TO KNOW THE FLOW

        // ALSO, Instructor view does not send an instance_question object, as it does not occur on res.locals. 

        // What is the intended logic here? Try to understand.

        const keys = Object.keys(submission.submitted_answer);
        keys.forEach((key) => {
            if (key.indexOf('_file_upload_') > -1) {
                // Need to add in logic for switching to S3 on front-end setup
                const files = JSON.parse(submission.submitted_answer[key]);
                    files.forEach((files) => {
                        fileStore.upload(files.name, files.contents, 'student_upload', assessment.id, instanceQuestion.question_id, instanceQuestion.user_id, instanceQuestion.authn_user_id, fileStore.modes.S3);
                    });
            }
        });
    },
     ...
     * @param {function} callback - A callback(err, submission_id) function.
     */
    saveAndGradeSubmission(submission, variant, question, course, instanceQuestion, callback) {
        ...
        sqldb.beginTransaction((err, client, done) => {
            ...
            async.series([
                (callback) => {
                    // parse out any files to be stored and save them remotely to S3
                    // inspect point
                    // as each of the three files lead to this inspection point, the modifications will go here for all of them.
                    console.log(submission);
                    // MAJOR NOTE: Still need the assessment down below entered as null in _saveS3FileUplkoads(,,,null)
                    // _saveS3FileUploads(body); submission, client, instanceQuestion, assessment 
                    if (submission.submitted_answer.__s3_upload) this._saveS3FileUploads(submission, client, instanceQuestion, null);
     ...
     * @param {function} callback - A callback(err) function.
     */
    _testVariantWithClient(client, variant, question, course, test_type, authn_user_id, instanceQuestion, callback) {
     ...
     * @param {function} callback - A callback(err, variant) function.
     */
    _testQuestion(question, course_instance, course, test_type, authn_user_id, callback) {
        debug('_testQuestion()');
     ...
     */
    _runTest(logger, showDetails, question, course_instance, course, test_type, authn_user_id, callback) {
        let variant, expected_submission, test_submission, success = true;
        async.series([
            (callback) => {
                logger.verbose('Testing ' + question.qid);
                this._testQuestion(question, course_instance, course, test_type, authn_user_id, (err, ret_variant, ret_expected_submission, ret_test_submission) => {
                    if (ERR(err, callback)) return;
                    variant = ret_variant;
                    expected_submission = ret_expected_submission;
                    test_submission = ret_test_submission;
        ...
        serverJobs.createJobSequence(options, (err, job_sequence_id) => {
            ...
            serverJobs.createJob(jobOptions, (err, job) => {
                ...
                async.eachSeries(_.range(count * test_types.length), (iter, callback) => {
                    ...
                    this._runTest(job, showDetails, question, course_instance, course, type, authn_user_id, (err, ret_success) => {
                        if (ERR(err, callback)) return;
