...
module.exports = {    
     ...
     */
    _makeVariant(question, course, options, student_identity, callback) {
        debug('_makeVariant()');
        var variant_seed;
        ...
        questionServers.getModule(question.type, (err, questionModule) => {
            if (ERR(err, callback)) return;
            questionModule.generate(question, course, variant_seed, student_identity, (err, courseIssues, data) => {
                if (ERR(err, callback)) return;
    ...
    */
     ...
     */
    _makeAndInsertVariantWithClient(client, question_id, instance_question_id, user_id, authn_user_id, group_work, course_instance_id, course, options, student_identity, callback) {
        console.log("make and insert variant");
        console.log(`user id is ${user_id}`);
        this._selectQuestionWithClient(client, question_id, instance_question_id, (err, question) => {
            if (ERR(err, callback)) return;
            this._makeVariant(question, course, options, student_identity, (err, courseIssues, variant) => {
                if (ERR(err, callback)) return;
                const params = [
     ...
     * @param {function} callback - A callback(err, variant) function.
     */
    _ensureVariantWithClient(client, question_id, instance_question_id, user_id, authn_user_id, group_work, course_instance_id, course, options, require_open, student_identity, callback) {
        if (instance_question_id != null) {
            ...
            sqldb.callWithClient(client, 'instance_questions_select_variant', params, (err, result) => {
                ...
                this._makeAndInsertVariantWithClient(client, question_id, instance_question_id, user_id, authn_user_id, group_work, course_instance_id, course, options, student_identity, (err, variant) => {
                    if (ERR(err, callback)) return;
     ...
     * @param {function} callback - A callback(err, variant) function.
     */
    ensureVariant(question_id, instance_question_id, user_id, authn_user_id, group_work, course_instance_id, course, options, require_open, student_identity, callback) {
        let variant;
        sqldb.beginTransaction((err, client, done) => {
            ...
            async.series([
                ...
                (callback) => {
                    this._ensureVariantWithClient(client, question_id, instance_question_id, user_id, authn_user_id, group_work, course_instance_id, course, options, require_open, student_identity, (err, ret_variant) => {
                        if (ERR(err, callback)) return;
                        variant = ret_variant;
                        callback(null);
                    });
     ...
     *                            committed.
     */
    _gradeVariantWithClient(client, variant, check_submission_id, question, course, course_instance, authn_user_id, overrideGradeRateCheck, student_identity, callback) {
        console.log('in grade variant with client');
        ...
        async.series([
            ...
            (callback) => {
                if (question.grading_method == 'Internal') {
                    ...
                    questionModule.grade(submission, variant, question, course, student_identity, (err, ret_courseIssues, ret_data) => {
                        if (ERR(err, callback)) return;
                        courseIssues = ret_courseIssues;
                        data = ret_data;
                        const hasFatalIssue = _.some(_.map(courseIssues, 'fatal'));
     ...
     * @param {function} callback - A callback(err, submission_id) function.
     */
    saveAndGradeSubmission(submission, variant, question, course, overrideGradeRateCheck, student_identity, callback) {
        console.log('in save and grade submission');
        ...
        sqldb.beginTransaction((err, client, done) => {
            ...
            async.series([
                ...
                (callback) => {
                    this._gradeVariantWithClient(client, variant, submission_id, question, course, submission.auth_user_id, overrideGradeRateCheck, student_identity, (err, ret_grading_job_id) => {
                        if (ERR(err, callback)) return;
                        grading_job_id = ret_grading_job_id;
        ...
        sqldb.beginTransaction((err, client, done) => {
            ...
            async.series([
                (callback) => {
                    ...
                    this._ensureVariantWithClient(client, question.id, instance_question_id, authn_user_id, authn_user_id, group_work, course_instance_id, course, options, require_open, undefined, (err, ret_variant) => {
                        if (ERR(err, callback)) return;
                        variant = ret_variant;
    ...
    getAndRenderVariant(variant_id, variant_seed, locals, callback) {
        console.log('locals.user is ', JSON.stringify(locals.user));
        console.log('locals.course instance is ', JSON.stringify(locals.course_instance));
        console.log('locals.question is', JSON.stringify(locals.question))
        async.series([
            (callback) => {
                if (variant_id != null) {
                    ...
                    this.ensureVariant(locals.question.id, instance_question_id, locals.user.user_id, locals.authn_user.user_id, assessmentGroupWork, course_instance_id, locals.course, options, require_open, locals.user, (err, variant) => {
                        if (ERR(err, callback)) return;
     ...
     */
    renderPanelsForSubmission(submission_id, urlPrefix, questionContext, csrfToken, authorizedEdit, callback) {
        const params = {
            submission_id,
        };
