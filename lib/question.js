  ...
  getAndRenderVariant(variant_id, variant_seed, locals, callback) {
    async.series(
      [
        ...
        async () => {
          // if barcodes found, get available pdf files
          const barcodedSubmissions = locals.submissions
            .slice(Math.max(locals.submissions.length - 5, 0)) // only last 5 displayed
          ...
          // place the pdf on the submission so the submission.ejs view can load it
