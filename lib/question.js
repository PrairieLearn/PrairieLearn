...
module.exports = {
    ...
    */
    ...
    async _saveS3FileUploads(submission, client, variant, question, course) {
        ...
        const s3Files = [];
            ...
            for await (const file of files) {
                ...
                if (Buffer.byteLength(file.contents, 'base64') >= config.fileUploadS3MinBytes) {
                    const file_id = await fileStore.upload(file.name, file.contents, 'student_upload', assessment_instance_id, instance_question_id, authn_user_id, user_id, fileStore.storageTypes.S3);
