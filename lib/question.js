...
module.exports = {
    ...
    */
     ...
     * @param {function} callback - A callback(err) function.
     */
    _regradeVariantWithClient(client, variant, submission, question, course, authn_user_id, callback) {
        ...
        async.series([
            ...
            (callback) => {
                ...
                this._writeCourseIssues(client, courseIssues, variant, submission.auth_user_id, studentMessage, courseData, (err) => {
                    if (ERR(err, callback)) return;
                    debug('_regradeVariantWithClient()', `wrote courseIssues: ${courseIssues.length}`);
                    callback(null);
                });
            },
            (callback) => {
            ...
            // catch errors we are using to exit with no action
            if (err instanceof NoSubmissionError) {
                debug('_regradeVariantWithClient()', 'submission not gradable');
                return callback(null);
            }
                            ...
                            (callback) => {
                                ...
                                async.eachSeries(submissions, ((submission, callback) => {
                                    ...
                                    sqldb.queryWithClientOneRow(client, sql.select_variant, {variant_id: submission.variant_id}, (err, result) => {
                                        ...
                                        this._regradeVariantWithClient(client, variant, submission, question, course, submission.auth_user_id, (err, data) => {
                                            if (err instanceof FatalGradingError) {
                                                const courseIssues = err.courseIssues;
                                                courseIssues.forEach((courseErr) => {
                                            ...
                                            if (ERR(err, callback)) return;
                                            if (data) {
                                                job.verbose(chalk.green(`✓ Submission successfully regraded. New score: ${data.score}.`));
                                                job.verbose(chalk.green(`Points earned for this instance question so far: ${data.points}.`));
