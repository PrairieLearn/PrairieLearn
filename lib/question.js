...
module.exports = {    
    ...
    */
    ...
    _gradeVariantWithClient(client, variant, check_submission_id, question, course, authn_user_id, overrideGradeRateCheck, callback) {
        ...
        let questionModule, courseIssues, data, submission, grading_jobs;
        async.series([
            (callback) => {
                var params = [
                    variant.id,
            ...
            (callback) => {
                if (question.grading_method_internal) {
            ...
            if (data && !data.broken && grading_jobs) {
                // If we have an external grading job (should be at most one)
                // We'll need to queue that job for grading once this
                // transaction completes
                for (let i = 0; i < grading_jobs.length; i++) {
                    if (grading_jobs[i].grading_method === 'External') {
                        callback(null, grading_jobs[i].id);
                        break;
                    } else if (i === grading_jobs.length - 1) {
                        callback(null);
                    }
                }
            } else {
    ...
    _createTestSubmissionWithClient(client, variant, question, course, test_type, authn_user_id, callback) {
        ...
        let questionModule, courseIssues, data, submission_id, grading_jobs;
        async.series([
            (callback) => {
                questionServers.getModule(question.type, (err, ret_questionModule) => {
                    if (ERR(err, callback)) return;
            ...
            (callback) => {
                ...
                sqldb.callWithClientOneRow(client, 'grading_jobs_insert', params, (err, result) => {
                    if (ERR(err, callback)) return;
                    grading_jobs = result.rows;
                    grading_jobs.forEach(grading_job => 
        ...
        async.series([
            ...
            (callback) => {
                ...
                const submissionKeys = ['broken', 'correct', 'feedback', 'format_errors', 'gradable',
                                        ...
                                        'partial_scores', 'raw_submitted_answer',
                ...
                sqldb.query(sql.select_submissions, params, (err, result) => {
                    ...
                    if (result.rowCount >= 1) {
                        locals.submissions = result.rows.map((s) => ({
                            ...
                            grading_status: this._buildGradingStatus(s, s.grading_jobs),
                            ...s,
                        }));
    ...
    /**
     * Translates a submission's grading_jobs objects into grading status information.
     * Relates to grading logic defined by new gradingMethods on infoQuestion.json scehema build
     * for manual grading.
     * @param {Object} submission object tied to grading_jobs
     * @param {Array<Object>} grading_jobs array of grading_jobs tied to submission
     * @return {Object} An object outlining grading status information
     */
    _buildGradingStatus(submission, grading_jobs) {
        const filter = (method) => {
            for (let i = 0; i < grading_jobs.length; i++) {
                if (grading_jobs[i].method === method) return true;
            }
        };
        console.log('submission in buildGradingStatus', submission);
        return {
            isManuallyGraded: filter('Manual'),
            isInternallyGraded: filter('Internal'),
            isExternallyGraded: filter('External'),
            isGradingMethodsQuestion: (() => {
                return (submission.grading_method_internal == true && (submission.grading_method_external == true || submission.grading_method_manual == true));
            })(),
        };
    },
