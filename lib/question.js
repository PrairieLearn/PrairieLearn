...
module.exports = {
    ...
    */
    ...
    _subFilesWithReferences(submission, fileUploadKey, file_ids) {
        debug('_subFileReferences()');
        delete submission.submitted_answer[fileUploadKey];
        submission.submitted_answer['_file_upload_storage'] = {fileUploadKey, file_ids};
    },
    ...
    async _saveS3FileUploads(submission, client, variant) {
        ...
        // pl-file-upload creates _file_upload key
        if (fileUploadKey) {
            const files = JSON.parse(submission.submitted_answer[fileUploadKey]);
            for await (const file of files) {
                // NEED
                //display_filename, contents, type, assessment_instance_id, instance_question_id, user_id, authn_user_id, storage_type
        ...
        this._subFilesWithReferences(submission, fileUploadKey, fileIds);
        return fileIds;
    },
    ...
    saveAndGradeSubmission(submission, variant, question, course, callback) {
        ...
        sqldb.beginTransaction((err, client, done) => {
            ...
            async.series([
                (callback) => {
                    // const saveFilesToS3 = util.callbackify();
                    util.callbackify(async () => this._saveS3FileUploads(submission, client, variant, question, course))((err, file_ids) => {
                        if (ERR(err, callback)) return;
                        debug('saveAndGradeSubmission()', 'file_ids', file_ids);
                        callback(null);
                    });
                },
                (callback) => {                    
