...
module.exports = {
    ...
    */
    ...
    saveSubmission(submission, variant, question, course, callback) {
        ...
        sqldb.beginTransaction((err, client, done) => {
            ...
            async.series([
                ...
                // in the presence of errors.
                (callback) => {
                    util.callbackify(async () => this._saveS3FileUploads(submission, client, variant, question, course))((err, file_ids) => {
                        if (ERR(err, callback)) return;
                        debug('saveSubmission()', 'file_ids', file_ids);
                        callback(null);
                    });
                },
                (callback) => {
    ...
    async _saveS3FileUploads(submission, client, variant) {
        ...
        // RESOLUTION: DO NOT TOUCH ANY OF THAT LOGIC AND WORK WITH WHAT WE HAVE
        ...
        const s3Files = [];
            ...
            // small encoded files can remain in DB for now.
            // large files go to s3
            if (fileUploadKey) {
            const files = JSON.parse(submission.submitted_answer[fileUploadKey]);

            // might not need this await. check it out
            // also might want to include promise-all for db/s3 uploads
            for await (const file of files) {
                console.log(console.log('file size in bytes', Buffer.byteLength(file.contents, 'base64')));
                Buffer.byteLength(file.contents, 'base64') >= config.fileUploadS3MinBytes ?  console.log('Going to S3') : console.log('Going to DB'); 
                if (Buffer.byteLength(file.contents, 'base64') >= config.fileUploadS3MinBytes) {
                    const file_id = await fileStore.upload(file.name, file.contents, 'student_upload', null, instance_question_id, authn_user_id, user_id, fileStore.storageTypes.S3);

                    // we need this for downloading files on front-end
                    s3Files.push({ name: file.name, id: file_id });
                } else {
            ...
            submission.submitted_answer['_file_upload_s3'] = {[fileUploadKey]: s3Files};
        }

        return s3Files.map((file) => file.id);
    },
            ...
            async.series([
                (callback) => {
                    util.callbackify(async () => this._saveS3FileUploads(submission, client, variant, question, course))((err, file_ids) => {
                        ...
                        debug('saveAndGradeSubmission()', 'file_ids uploaded to S3', file_ids);
                        callback(null);
                    });
                },
                (callback) => {                    
                    ...
                    // if (submission.submitted_answer.__s3_upload) this._saveS3FileUploads(submission, client, null);
                    this._saveSubmissionWithClient(client, submission, variant, question, course, (err, ret_submission_id) => {
                        if (ERR(err, callback)) return;
                        submission_id = ret_submission_id;
