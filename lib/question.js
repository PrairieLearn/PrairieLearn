...
const barcodeGenerator = require('../lib/barcodeGenerator');
  ...
  getAndRenderVariant(variant_id, variant_seed, locals, callback) {
    async.series(
      [
        ...
        (callback) => {
          if (locals.submissions.length) {
            // we only display the last 5 submissions; want to display file if valid barcode included
            const submissions = locals.submissions.slice(Math.max(locals.submissions.length - 5, 0))
              .filter((submission) => submission.submitted_answer && barcodeGenerator.isValid(submission.submitted_answer._pl_artifact_barcode));
            const barcodes = submissions.map((submission) => submission.submitted_answer._pl_artifact_barcode);
            const query = sql.get_barcode_file_ids.replace('$match', `b.barcode = '${barcodes.join("' OR b.barcode = '")}'`);
            sqldb.queryAsync(query, {})
              .then((query) => {
                const barcodeFileIds = query.rows;
                const fileQueries = barcodeFileIds.map((barcodeFileId) => {
                  return fileStore.get(barcodeFileId.file_id)
                    .then((file) => {
              ...
              .then((fileQueries) => {
                for (let i = 0; i < fileQueries.length; i++) {
                  for (let j = 0; j < locals.submissions.length; j++) {
                    if (locals.submissions[j].submitted_answer._pl_artifact_barcode === fileQueries[i].barcode) {
                      locals.submissions[j]['pdf_artifact_upload'] = fileQueries[i].file.contents.toString('base64');
                    }
                    if (locals.submission.submitted_answer._pl_artifact_barcode === fileQueries[i].barcode) {
                      locals.submission['pdf_artifact_upload'] = fileQueries[i].file.contents.toString('base64');
                    }
                  }
                }
                callback(null);
