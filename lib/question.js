...
module.exports = {
    ...
    */
    ...
    regradeQuestion(authn_user_id, course_id, instance_question_id, callback) {
        const options = {
            course_id: course_id,
            ...
            serverJobs.createJob(jobOptions, (err, job) => {
                ...
                sqldb.beginTransaction((err, client, done) => {
                    ...
                    sqldb.queryWithClient(client, sql.select_all_submissions_and_other_data, {instance_question_id: instance_question_id}, (err, result) => {
                        // if (ERR(err, callback)) return;
                        if (err) {
                            job.verbose('Error in select_all_submissions_and_other_data');
                        ...
                        if (result.rowCount === 0) {
                            // callback(null);
                            job.verbose(`No submission found for question '${qid}'.`);
                            job.succeed();
                        } else {
