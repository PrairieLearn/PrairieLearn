...
module.exports = {
    ...
    */
     ...
     * @param {function} callback - A callback(err, submission_id) function.
     */
    async saveSubmission(submission, variant, question, course, callback) {
        let submission_id;
        const fileUploads = this._extractFileUploads(submission);
        await this._uploadFilesToS3(submission, variant, question, fileUploads.s3Files);
        sqldb.beginTransaction((err, client, done) => {
            if (ERR(err, callback)) return;
            async.series([
                // Even though we only have a single series function,
    ...
    _extractFileUploads(submission) {
        // We should probably change this to send all files to S3
        const fileUploadKey = Object.keys(submission.submitted_answer).find((key) => key.indexOf('_file_upload_') > -1);
        const dbFiles = [];
        const s3Files = [];

        // small encoded files can still go to DB
        if (fileUploadKey) {
            const files = JSON.parse(submission.submitted_answer[fileUploadKey]);

            for (const file of files) {
                const fileBuffer = Buffer.from(file.contents, 'base64');
                console.log('file size in bytes', Buffer.byteLength(fileBuffer));
                Buffer.byteLength(fileBuffer) >= config.fileUploadS3MinBytes ?  console.log('Going to S3') : console.log('Going to DB'); 
                if (Buffer.byteLength(fileBuffer) >= config.fileUploadS3MinBytes) {
                    // we need this for downloading files on front-end
                    s3Files.push({ name: file.name, contents: fileBuffer });
                } else {
                    // re-compiling files without s3 data
                    dbFiles.push({ name: file.name, contents: fileBuffer });
                    // We probably want to add these db files
                    // to S3 and not have to deal with this.
                }
            }
            
            submission.submitted_answer[fileUploadKey] = JSON.stringify(dbFiles);
            submission.submitted_answer['_file_storage_s3'] = {[fileUploadKey]: s3Files};
        }

        return {s3Files, dbFiles};
    },

    /**
     * Saves file to S3 and then removes it from object to be saved to database.
     * @param {Object} submission - The submission data.
     * @param {Object} variant - Variant with instance question id.
     * @param {Object} course - Question object if instructor.
    //  * @param {number} assessment - Assessment object if instructor.
     * @param {Object} s3Files - Uploaded files ready to be sent to S3 bucket.
     * @return {Array} ids of uploaded files from Files table.
     */ 
    
    async _uploadFilesToS3(submission, variant, question, s3Files) {
        debug('_uploadFilesToS3()');
        // instructor view
        const assessment_id = submission.assessment_id; // not sure how to get this one
        const question_id = question.id;

        // student view
        const assessment_instance_id = submission.assessment_instance_id;
        const instance_question_id = variant.instance_question_id;

        const authn_user_id = variant.authn_user_id;
        const user_id = variant.user_id;

        const file_ids = [];

        for await (const file of s3Files) {
            const file_id = await fileStore.upload(file.name, file.contents, 'student_upload', assessment_id, assessment_instance_id, instance_question_id, question_id, authn_user_id, user_id, fileStore.storageTypes.S3);
            file_ids.push(file_id);
            debug('_uploadFilesToS3() file uploaded', file_id);
        }
        return file_ids;
    },
     ...
     * @param {function} callback - A callback(err, submission_id) function.
     */
    async saveAndGradeSubmission(submission, variant, question, course, callback) {
        debug('saveAndGradeSubmission()');
        const fileUploads = this._extractFileUploads(submission);
        const fileIds = await this._uploadFilesToS3(submission, variant, question, fileUploads.s3Files);
    ...
    getAndRenderVariant(variant_id, variant_seed, locals, callback) {
        async.series([
            ...
            (callback) => {
                // if (locals.submissions[0].submitted_answer['_file_upload_storage']) {
                //     this._subReferencesWithFiles(locals.submissions[0]);
                // }

                if (locals.submissions[0] && locals.submissions[0].submitted_answer['_file_upload_storage']) {
                    var params = {
                        file_ids: locals.submissions[0].submitted_answer['_file_upload_storage']['file_ids'],
                    };

                    sqldb.query(sql.select_files, params, (err, result) => {
                        if (ERR(err, callback)) return;
                        if (result.rowCount >= 1) {
                            // Do I want to fetch the file through S3 or send hyperlinks back?
                            // Decide based on size of file
                            // Maybe we should only use S3 when declared in the pl-file-upload elem && 
                            // if over a particular size? encoding big files may be back to send to the 
                            // browser
