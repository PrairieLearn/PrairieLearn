    ...
    questionServers.getModule(question.type, (err, questionModule) => {
      ...
      questionModule.generate(question, course, variant_seed, (err, courseIssues, data) => {
        ...
        if (question.workspace_image !== null) {
          ...
          ).filter((file) => !fg.isDynamicPattern(file));
        ...
        async () => {
          ...
          try {
            ...
            if (err.isFormatError) {
              if (!submission.format_errors) submission.format_errors = {};
              if (!submission.format_errors._files) submission.format_errors._files = [];
