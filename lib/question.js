...
module.exports = {
    ...
    */
    ...
    /**
     * Save and grade a new submission in a separate worker process.
     *
     * @param {Object} submission - The submission to save (should not have an id property yet).
     * @param {Object} variant - The variant to submit to.
     * @param {Object} question - The question for the variant.
     ...
     * @param {function} callback - A callback(err, submission_id) function.
     */
    workerSaveAndGradeSubmission(submission, variant, question, course, callback) {
        if (workerPool.initialized) {
            workerPool.call('saveAndGradeSubmission', {submission, variant, question, course}, (err, retArgs) => {
                if (ERR(err, callback)) return;
                callback(null, retArgs.submission_id);
            });
        } else {
            this.saveAndGradeSubmission(submission, variant, question, course, (err, submission_id) => {
                if (ERR(err, callback)) return;
                callback(null, submission_id);
            });
        }
    ...
    /**
     * Save a new submission in a separate worker process.
     *
     * @param {Object} submission - The submission to save (should not have an id property yet).
     * @param {Object} variant - The variant to submit to.
     * @param {Object} question - The question for the variant.
     ...
     * @param {function} callback - A callback(err, submission_id) function.
     */
    workerSaveSubmission(submission, variant, question, course, callback) {
        if (workerPool.initialized) {
            workerPool.call('saveSubmission', {submission, variant, question, course}, (err, retArgs) => {
                if (ERR(err, callback)) return;
                callback(null, retArgs.submission_id);
            });
        } else {
            this.saveSubmission(submission, variant, question, course, (err, submission_id) => {
                if (ERR(err, callback)) return;
                callback(null, submission_id);
            });
        }
    ...
    /**
     * Render all information in a separate worker process.
     *
     * @param {?number} variant_id - The variant to render, or null if it should be generated.
     * @param {Object} locals - The current locals structure to read/write.
     * @param {function} callback - A callback(err) function.
     */
    workerGetAndRenderVariant(variant_id, locals, callback) {
         if (workerPool.initialized) {
            workerPool.call('getAndRenderVariant', {variant_id, locals}, (err, retArgs) => {
                if (ERR(err, callback)) return;
                _.assign(locals, retArgs.locals);
                callback(null);
            });
        } else {
            this.getAndRenderVariant(variant_id, locals, (err) => {
                if (ERR(err, callback)) return;
                callback(null);
            });
        }
