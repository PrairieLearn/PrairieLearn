...
module.exports = {
      ...
      questionModule.generate(
        ...
        (err, courseIssues, data) => {
          ...
          if (question.workspace_image !== null) {
            ...
            variant.params['_required_file_names'] = variant.params[
              '_required_file_names'
            ].concat(question.workspace_graded_files);
          }
          if (variant.broken) {
            return callback(null, courseIssues, variant);
          }
          questionModule.prepare(
            question,
            course,
            variant,
            (err, extraCourseIssues, data) => {
              if (ERR(err, callback)) return;
  ...
  _selectQuestionWithClient(
    ...
    if (question_id != null) {
      ...
      if (instance_question_id == null)
        return callback(
          new Error('question_id and instance_question_id cannot both be null'),
        );
      sqldb.callWithClientOneRow(
        client,
  ...
  _makeAndInsertVariantWithClient(
    ...
    this._selectQuestionWithClient(
      client,
      question_id,
      instance_question_id,
      (err, question) => {
      ...
      [
        ...
        (callback) => {
          ...
          if (
            ...
            debug(
              '_saveSubmissionWithClient()',
              'not getting workspace graded files',
            );
            return callback(null);
          }
          util.callbackify(workspaceHelper.getGradedFiles)(
            workspace_id,
            (err, result) => {
              if (ERR(err, callback)) return;
              zipPath = result;
        ...
        (callback) => {
          questionServers.getModule(
            question.type,
            (err, ret_questionModule) => {
              if (ERR(err, callback)) return;
    ...
    async.series(
        ...
        (callback) => {
          ...
          sqldb.callWithClientZeroOrOneRow(
            ...
            (err, result) => {
              ...
              if (result.rows[0].allow_grade_left_ms > 0)
                return callback(new NoSubmissionError());
              callback(null);
            },
          );
      ...
      (err) => {
        ...
        if (err instanceof NoSubmissionError) {
          debug(
            '_gradeVariantWithClient()',
            'no submissions for grading, skipping',
          );
          err = null;
        }
        if (ERR(err, callback)) return;
        ...
        if (
          ...
          !data.broken &&
          grading_job &&
          grading_job.grading_method == 'External'
        ) {
  ...
  _createTestSubmissionWithClient(
    ...
    if (question.type != 'Freeform')
      return callback(new Error('question.type must be Freeform'));
  ...
  _compareSubmissions(expected_submission, test_submission, callback) {
    ...
    const checkEqual = (name, var1, var2) => {
      ...
      if (!_.isEqual(var1, var2))
        courseIssues.push(
          new Error(
            `"${name}" mismatch: expected "${json1}" but got "${json2}"`,
          ),
        );
    };
    ...
    if (expected_submission.broken) {
      courseIssues.push(
        new Error('expected_submission is broken, skipping tests'),
      );
    ...
    checkEqual(
      'gradable',
      expected_submission.gradable,
      test_submission.gradable,
    );
    checkEqual(
      'format_errors keys',
        ...
        [
          (callback) => {
            ...
            const course_instance_id =
              (course_instance && course_instance.id) || null;
            const options = {};
        ...
        (callback) => {
          ...
          if (_.isObject(expected_submission)) {
            logger.verbose(
              'expected_submission:\n' +
                jsonStringifySafe(
                  _.pick(expected_submission, submissionKeys),
                  null,
                  '    ',
                ),
            );
          ...
          if (_.isObject(test_submission)) {
            logger.verbose(
              'test_submission:\n' +
                jsonStringifySafe(
                  _.pick(test_submission, submissionKeys),
                  null,
                  '    ',
                ),
            );
          ...
          sqldb.query(
            ...
            (err, result) => {
              ...
              if (result.rowCount > 0) {
                success = false;
                logger.verbose(
                  `ERROR: ${result.rowCount} issues encountered during test.`,
                );
              } else {
      ...
      serverJobs.createJob(jobOptions, (err, job) => {
        ...
        async.eachSeries(
          ...
          (iter, callback) => {
            ...
            job.verbose(
              `Test ${Math.floor(iter / test_types.length) + 1}, type ${type}`,
            );
            this._runTest(
              job,
              showDetails,
          ...
          (err) => {
            ...
            if (!success)
              return job.fail(
                'Some tests failed. See the "Errors" page for details.',
              );
            job.succeed();
          },
        );
    ...
    questionServers.getModule(question.type, (err, questionModule) => {
      ...
      questionModule.render(
        ...
        (err, courseIssues, htmls) => {
          ...
          const user_id =
            locals && locals.authn_user ? locals.authn_user.user_id : null;
          this._writeCourseIssues(
            null,
            courseIssues,
            variant,
  ...
  _buildQuestionUrls(
    ...
    if (!assessment) {
      ...
      urls.calculationQuestionGeneratedFileUrl =
        questionUrl + 'generatedFilesQuestion';
    ...
    if (!assessment) {
      ...
      if (assessment.type == 'Homework') {
        ...
        if (!question.single_variant) {
          ...
          locals.variantAttemptsLeft =
            assessment_question.tries_per_variant - variant.num_tries;
      ...
      if (assessment.type == 'Exam') {
        if (assessment_instance.open && instance_question.open) {
          ...
          locals.variantAttemptsTotal =
            instance_question.points_list_original.length;
        } else {
      ...
      [
        (callback) => {
          if (variant_id != null) {
            ...
            const require_open =
              locals.assessment && locals.assessment.type != 'Exam';
            ...
            const assessmentGroupWork = locals.assessment
              ? locals.assessment.group_work
              : false;
            this.ensureVariant(
              locals.question.id,
              instance_question_id,
        ...
        (callback) => {
          questionServers.getEffectiveQuestionType(
        ...
        (callback) => {
          var questionJson = JSON.stringify({
            ...
            questionGeneratedFilePath:
              locals.calculationQuestionGeneratedFileUrl,
            ...
            submittedAnswer:
              locals.showSubmissions && locals.submission
                ...
                : null,
            feedback:
              locals.showFeedback && locals.submission
                ? locals.submission.feedback
                : null,
            trueAnswer: locals.showTrueAnswer
              ? locals.variant.true_answer
              : null,
            submissions: locals.showSubmissions ? locals.submissions : null,
          });
          var encodedJson = encodeURIComponent(questionJson);
          locals.questionJsonBase64 =
  ...
  _buildGradingJobStats(job) {
    if (job) {
      ...
      return {
        submitDuration: formatDiff(
          job.grading_requested_at,
          job.grading_submitted_at,
        ),
        queueDuration: formatDiff(
          job.grading_submitted_at,
          job.grading_received_at,
        ),
        prepareDuration: formatDiff(
          job.grading_received_at,
          job.grading_started_at,
        ),
        runDuration: formatDiff(
          job.grading_started_at,
          job.grading_finished_at,
        ),
        reportDuration: formatDiff(job.grading_finished_at, job.graded_at),
        totalDuration: formatDiff(
          job.grading_requested_at,
          job.graded_at,
          false,
        ),
          ...
          (callback) => {
            ...
            this._render(
              ...
              (err, htmls) => {
                ...
                submission.grading_job_stats =
                  this._buildGradingJobStats(grading_job);
