        ...
        (callback) => {
          ...
          sqldb.query(sql.select_submissions, params, (err, result) => {
            ...
            if (result.rowCount >= 1) {
              ...
              const manual_feedback = locals.submissions.filter((s) => s.feedback?.manual);
              if (manual_feedback.length > 0) {
                const renderer = new showdown.Converter({
                  literalMidWordUnderscores: true,
                  literalMidWordAsterisks: true,
                });
                locals.manual_feedback_list = manual_feedback.map((s) => ({
                  submission: s,
                  feedback_html: renderer.makeHtml(s.feedback.manual.toString()),
                }));
              }
