...
module.exports = {
    ...
    */
    ...
    // Might want to do some refactoring since most of the code is the same from _gradeVariantWithClient()
        ...
        async.series([
            ...
            (callback) => {
                if (!data.gradable) {
                    ...
                    callback(new RegradeParsingError());
                    return;
                }
                // Instead of update the submission in database, we directly modify the submission obj
            ...
            // catch NoSubmissionError as we are just using it to exit with no action
            if (err instanceof NoSubmissionError) {
                debug('_regradeVariantWithClient()', 'no submissions for grading, skipping');
                err = null;
            }
            // catch RegradeParsingError as we are just using it to exit with no action
            if (err instanceof RegradeParsingError) {
                serverJob.error(chalk.yellow('⚠ Error when trying to parse this submission, skipping.'));
                debug('_regradeVariantWithClient()', 'error when parsing');
                callback(null);
            } else {
                if (ERR(err, callback)) return;
                serverJob.verbose(chalk.green(`✓ Regrade submission successful, new submission score: ${data.score}`));
        ...
        serverJobs.createJobSequence(options, (err, job_sequence_id) => {
            ...
            serverJobs.createJob(jobOptions, (err, job) => {
                ...
                sqldb.beginTransaction((err, client, done) => {
                    ...
                    // If there is no submission, we can just call the callback
                    sqldb.queryWithClient(client, sql.select_all_submissions_and_other_data, {instance_question_id: instance_question_id}, (err, result) => {
                        // if (ERR(err, callback)) return;
                        if (err) {
                            job.fail('Error in select_all_submissions_and_other_data');
                            logger.error('Error in select_all_submissions_and_other_data', err);
                            return;
                        }
                            ...
                            async.series([
                                ...
                                (callback) => {
                                    ...
                                    sqldb.queryWithClientOneRow(client, sql.select_instance_question, params, (err, result) => {
                                        ...
                                        job.verbose(`The original points is: ${original_instance_question.points}`);
                                        callback(null);
                                    });
                                },
                                // reset the question instance to original state
                                (callback) => {
                                    const params = {
                                ...
                                // reset the assessment score to 0 if keep_highest_score is false
                                // so when we regrade the question, the assessment score will be updated
                                // (callback) => {
                                ...
                                //     };
                                //     sqldb.queryWithClient(client, sql.reset_instance_assessment, params, (err) => {
                                //         if (err) {
                                //             callback(err);
                                //         } else {
                                //             job.verbose(`Assessment '${assessment_instance_id}' successfully reset.`);
                                //             callback(null);
                                //         }
                                //     });
                                // },
                                (outerCallback) => {
                                    ...
                                    async.eachSeries(submissions, ((submission, innerCallback) => {
                                        job.verbose(chalkDim(`Regrading submission ${idx}...`));
                                        idx += 1;

                                        // ********* Maunally mark submission to be not broken and gradable *********
                                        submission.broken = false;  
                                        submission.gradable = true;
                                        // **************************************************************************

                                        // ******** Maunally reset submitted_answer and clear format_errors *********
                                        ...
                                        sqldb.queryWithClientOneRow(client, sql.select_variant, {variant_id: submission.variant_id}, (err, result) => {
                                            if (ERR(err, innerCallback)) return;
                                            variant = result.rows[0];
                                            this._regradeVariantWithClient(client, variant, submission, question, course, submission.auth_user_id, job, innerCallback);
                                        });   
                                    }), (err) => {
                                        if (ERR(err, outerCallback)) return;
                                        outerCallback(null);
                                    });
                                },
                                ...
                                (callback) => {
                                    ...
                                    sqldb.queryWithClientOneRow(client, sql.select_instance_question, params, (err, result) => {
                                        ...
                                        job.verbose(`The new points is: ${regraded_instance_question.points}`);
                                        callback(null);
                                    });
                                },
                                // if user choose to keep highest score and the regraded score is lower, restore the stat back to pre-regrade stage
                                (callback) => {
                                    if (!keep_highest_score || original_instance_question.points <= regraded_instance_question.points) {
                                        callback(null);
                                        return;
                                    }
                                    job.verbose(chalk.yellow('The new score is lower than the original score, restore the question.'));
                                ...
                                sqldb.endTransaction(client, done, err, (err) => {
                                    if (err) {
                                        ...
                                        job.fail(`Regrading failed due to error in one of the submissions.`);
                                        return;
                                    } else {
