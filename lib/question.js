    ...
    _saveSubmissionWithClient(client, submission, variant, question, course, callback) {
        ...
        async.series([
            ...
            (callback) => { // if we have a workspace, get the files
                ...
                sqldb.queryZeroOrOneRow(sql.select_workspace_graded_files, {workspace_id}, (err, result) => {
                    if (ERR(err, callback)) return;
                    let workspace_graded_files = [];
                    if (result.rowCount > 0) {
                        workspace_graded_files = result.rows[0].workspace_graded_files;
                    }
                    submission.submitted_answer['_required_file_names'] = workspace_graded_files;
