...
module.exports = {
    ...
    */
     ...
     * @param {Object} course - The course for the variant.
     * @param {number} authn_user_id - The currently authenticated user.
     * @param {boolean} overrideSubmissionRateCheck - Whether to override submission rate limits.
     ...
     *                            committed.
     */
    _gradeVariantWithClient(client, variant, check_submission_id, question, course, authn_user_id, overrideSubmissionRateCheck, callback) {
        ...
        async.series([
            ...
            (callback) => {
                if (overrideSubmissionRateCheck) return callback(null);
                var params = [
                    variant.instance_question_id,
                ];
                sqldb.callWithClientZeroOrOneRow(client, 'instance_question_next_submission', params, (err, result) => {
                    if (ERR(err, callback)) return;
                    debug('_gradeVariantWithClient()', 'checked submission rate', 'next_submission_next_ms:', result.rows[0].next_submission_left_ms);
                    if (result.rows[0].next_submission_left_ms > 0) return callback(new NoSubmissionError());
                    callback(null);
                });
            },
            (callback) => {
     ...
     * @param {function} callback - A callback(err) function.
     */
    gradeVariant(variant, check_submission_id, question, course, authn_user_id, overrideSubmissionRateCheck, callback) {
        let grading_job_id;
        sqldb.beginTransaction((err, client, done) => {
            if (ERR(err, callback)) return;
            async.series([
                ...
                // in the presence of errors.
                (callback) => {
                    this._gradeVariantWithClient(client, variant, check_submission_id, question, course, authn_user_id, overrideSubmissionRateCheck, (err, ret_grading_job_id) => {
                        if (ERR(err, callback)) return;
     ...
     * @param {function} callback - A callback(err, submission_id) function.
     */
    saveAndGradeSubmission(submission, variant, question, course, overrideSubmissionRateCheck, callback) {
        ...
        sqldb.beginTransaction((err, client, done) => {
            ...
            async.series([
                ...
                (callback) => {
                    this._gradeVariantWithClient(client, variant, submission_id, question, course, submission.auth_user_id, overrideSubmissionRateCheck, (err, ret_grading_job_id) => {
                        if (ERR(err, callback)) return;
                        grading_job_id = ret_grading_job_id;
    ...
    _buildLocals(variant, question, instance_question, assessment, assessment_instance, assessment_question) {
        const locals = {};
        ...
        if (!assessment) {
            ...
            if (instance_question.next_submission_left_ms > 0) {
    ...
    getAndRenderVariant(variant_id, variant_seed, locals, callback) {
        async.series([
            ...
            (callback) => {
                ...
                const newLocals = this._buildLocals(variant, question, instance_question, assessment, assessment_instance, assessment_question);
