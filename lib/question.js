...
module.exports = {
   ...
   */
  _saveSubmission(submission, variant, question, course, callback) {
    debug('_saveSubmission()');
    ...
    async.series(
      [
        ...
        (callback) => {
          ...
          if (
            ...
            debug('_saveSubmission()', 'not getting workspace graded files');
            return callback(null);
          }
          util.callbackify(workspaceHelper.getGradedFiles)(workspace_id, (err, result) => {
            if (ERR(err, callback)) return;
            zipPath = result;
            debug('_saveSubmission()', `saved graded files: ${zipPath}`);
            callback(null);
          });
        },
        async () => {
        ...
        (callback) => {
          questionServers.getModule(question.type, (err, ret_questionModule) => {
            ...
            debug('_saveSubmission()', 'loaded questionModule');
            callback(null);
          });
        },
        (callback) => {
          questionModule.parse(
            ...
            (err, ret_courseIssues, ret_data) => {
              ...
              debug('_saveSubmission()', 'completed parse()');
              callback(null);
            }
          );
        },
        (callback) => {
          ...
          this._writeCourseIssues(
            ...
            (err) => {
              if (ERR(err, callback)) return;
              debug('_saveSubmission()', `wrote courseIssues: ${courseIssues.length}`);
              callback(null);
            }
          );
        },
        (callback) => {
          ...
          sqldb.callOneRow('submissions_insert', params, (err, result) => {
            ...
            debug('_saveSubmission()', 'inserted', 'submission_id:', submission_id);
            callback(null);
          });
        },
      ],
      (err) => {
        if (ERR(err, callback)) return;
        debug('_saveSubmission()', 'returning', 'submission_id:', submission_id);
  ...
  saveSubmission(submission, variant, question, course, callback) {
    this._saveSubmission(submission, variant, question, course, (err, submission_id) => {
      if (ERR(err, callback)) return;
      callback(null, submission_id);
    });
  },
      ...
      [
        ...
        (callback) => {
          ...
          const params = [
            ...
            data.feedback,
            data.format_errors,
            // TODO: actually set `received_time`, `start_time`, and `finish_time` here.
            // These should use timestamps from the DB to avoid clock skew issues.
            null,
            null,
            null,
            data.gradable,
        ...
        (callback) => {
          sqldb.queryOneRow(
            ...
            (err, result) => {
              ...
              // TODO: delete `null` client param here.
  ...
  saveAndGradeSubmission(submission, variant, question, course, overrideGradeRateCheck, callback) {
    ...
    async.series(
      [
        (callback) => {
          this._saveSubmission(submission, variant, question, course, (err, ret_submission_id) => {
            if (ERR(err, callback)) return;
