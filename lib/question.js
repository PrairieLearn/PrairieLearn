...
module.exports = {
    ...
    */
        ...
        sqldb.beginTransaction((err, client, done) => {
            ...
            async.series([
                ...
                (callback) => {
                    ...
                    util.callbackify(async () => this._saveS3FileUploads(submission, client, variant, question, course))((err, file_id) => {
                        if (ERR(err, callback)) {
                            console.log(err);
                            return;
                        }
    ...
    async _saveS3FileUploads(submission, client, variant, question, course) {
        ...
        const fileUploadKey = Object.keys(submission.submitted_answer).find((key) => {
        ...
        console.log('succesful file id uploads ', fileIds);
        // keys.forEach((key) => {
        //     if (key.indexOf('_file_upload_') > -1) {
        //         const files = JSON.parse(submission.submitted_answer[key]);
        //             files.forEach(async (files) => {
        //                 await fileStore.upload(files.name, files.contents, 'student_upload', assessment.id, null, null, null, fileStore.storageTypes.S3);
        //             });
        //     }
        // });
    },
