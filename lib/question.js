...
const scrapPaperMaker = require('../lib/scrap-paper/generate');
  ...
  getAndRenderVariant(variant_id, variant_seed, locals, callback) {
    async.series(
      [
        ...
        async () => {
          // For submissions with valid barcodes attached to them, we'll load
          // the corresponding files from the file store. These will then be
          // rendered in an iframe by the sugmission partial.
          const barcodedSubmissions = locals.submissions
            .filter((s) => s.submitted_answer && s.submitted_answer._pdf_barcode_scan)
            .filter((s) => scrapPaperMaker.isValid(s.submitted_answer._pdf_barcode_scan));

          if (barcodedSubmissions.length === 0) return;

          const barcodes = barcodedSubmissions.map((s) => s.submitted_answer._pdf_barcode_scan);
          const query = sql.get_barcode_file_ids.replace(
            '$match',
            `b.barcode = '${barcodes.join("' OR b.barcode = '")}'`
          );
          const barcodeFileIds = (await sqldb.queryAsync(query, {})).rows;
          const fileQueries = barcodeFileIds.map((barcodeFileId) => {
            return fileStore.get(barcodeFileId.file_id).then((file) => {
              return { barcode: barcodeFileId.barcode, file };
            });
          });
          const files = Promise.all(fileQueries);

          // The submission partial will render the PDF in an ifram if `pdf_barcode_scan`
          // is present on the submission.
          for (let i = 0; i < files.length; i++) {
            for (let j = 0; j < locals.submissions.length; j++) {
              if (locals.submissions[j].submitted_answer._pdf_barcode_scan === files[i].barcode) {
                locals.submissions[j]['pdf_barcode_scan'] =
                  files[i].file.contents.toString('base64');
              }
              if (locals.submission.submitted_answer._pdf_barcode_scan === files[i].barcode) {
                locals.submission['pdf_barcode_scan'] = files[i].file.contents.toString('base64');
              }
            }
          }
        },
