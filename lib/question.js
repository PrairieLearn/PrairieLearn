...
module.exports = {
    ...
    /**
     * Test a question variant. Errors will be inserted into the errors table.
     * 
     * @param {Object} client - SQL client that must be inside a transaction.
     * @param {Object} variant - The variant to submit to.
     * @param {Object} question - The question for the variant.
     * @param {Object} course - The course for the variant.
     * @param {number} authn_user_id - The currently authenticated user.
     * @param {function} callback - A callback(err, result) function.
     */
    testVariant(client, variant, question, course, authn_user_id, callback) {
        debug('testVariant()');
        let expected_submission_id, expected_submission, ;
        sqldb.beginTransaction((err, client, done) => {
            if (ERR(err, callback)) return;
            async.series([
                (callback) => {
                ...
                (callback) => {
                    sqldb.callWithClientOneRow('submissions_select', [expected_submission_id], (err, result) => {
                        if (ERR(err, callback)) return;
                        expected_submission = result.rows[0];
                        debug('testVariant()', 'selected test submission, id:', expected_submission.id);
                        callback(null);
                    });
                },
                (callback) => {
                    const submission = {
                        ...
                        auth_user_id: authn_user.user_id,
                        submitted_answer: expected_submission.submitted_answer,
                    };
            question.saveAndGradeSubmission(submission, variant, res.locals.question, res.locals.course, (err) => {
                ...
                sqldb.endTransaction(client, done, err, (err) => {
                    if (ERR(err, callback)) return;
                    debug('testVariant()', 'returning submission_id:', submission_id);
                    callback(null, submission_id);
                });
            });
        });
