      ...
      [
        ...
        (callback) => {
          if (locals.submissions.length) {
            ...
            const submissions = locals.submissions
              .slice(Math.max(locals.submissions.length - 5, 0))
              .filter(
                (submission) =>
                  submission.submitted_answer &&
                  barcodeGenerator.isValid(submission.submitted_answer._pl_artifact_barcode)
              );
            const barcodes = submissions.map(
              (submission) => submission.submitted_answer._pl_artifact_barcode
            );
