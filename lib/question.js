    ...
    */
    ...
    getAndRenderVariant(variant_id, variant_seed, locals, callback) {
        async.series([
            ...
            (callback) => {
                if (locals.submission && locals.submission.file_id && locals.submission.file_type == 'artifact_upload') {
                    fileStore.get(locals.submission.file_id)
                        .then((file) => {
                            locals.submission['artifact'] = file.contents.toString('base64');
                            callback(null);
                        })
                        .catch((err) => {
                            if (ERR(err, callback)) return;
