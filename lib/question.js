      ...
      [
        ...
        (callback) => {
          ...
          const barcodedSubmissions = locals.submissions
            ...
            .filter((s) => s.submitted_answer && s.submitted_answer._pl_artifact_barcode)
            .filter((s) => barcodeGenerator.isValid(s.submitted_answer._pl_artifact_barcode));

          if (barcodedSubmissions.length) {
            const barcodes = barcodedSubmissions.map(
              (s) => s.submitted_answer._pl_artifact_barcode
            );
              ...
              .then((fileQueries) => {
                ...
                for (let i = 0; i < fileQueries.length; i++) {
                  for (let j = 0; j < locals.submissions.length; j++) {
                    if (
                      locals.submissions[j].submitted_answer._pl_artifact_barcode ===
                      fileQueries[i].barcode
                    ) {
                      locals.submissions[j]['pdf_artifact_upload'] =
                        fileQueries[i].file.contents.toString('base64');
                    }
                    if (
                      locals.submission.submitted_answer._pl_artifact_barcode ===
                      fileQueries[i].barcode
                    ) {
                      locals.submission['pdf_artifact_upload'] =
                        fileQueries[i].file.contents.toString('base64');
                    }
                  }
                }
