      ...
      [
        ...
        (callback) => {
          if (locals.submissions.length) {
            // we only display the last 5
            const last5BarcodeSubmissions = locals.submissions.slice(Math.max(locals.submissions.length - 5, 0))
              .filter((submission) => submission.file_id);
            const getAvailFiles = last5BarcodeSubmissions.map((submission) => {
              return fileStore.get(submission.file_id)
                .then((file) => {
                  return {submission_id: submission.id, file};
                });
            }
              );
            Promise.all(getAvailFiles)
              .then((files) => {
                for (let i = 0; i < files.length; i++) {
                  for (let j = 0; j < locals.submissions.length; j++) {
                    if (locals.submissions[j].id === files[i].submission_id) {
                      locals.submissions[j]['pdf_artifact_upload'] = files[i].file.contents.toString('base64');
                    }
                  }
                }
              })
              .then(() => {
                return fileStore
                .get(locals.submission.file_id)
                .then((file) => {
                  locals.submission['pdf_artifact_upload'] = file.contents.toString('base64');
                  callback(null);
                });
              });
          } else {
          ...
          if (
            locals.submission &&
            locals.submission.file_id &&
            locals.submission.file_type === 'pdf_artifact_upload'
          ) {
            fileStore
              .get(locals.submission.file_id)
              .then((file) => {
