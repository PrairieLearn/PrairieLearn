        ...
        serverJobs.createJobSequence(options, (err, job_sequence_id) => {
            ...
            serverJobs.createJob(jobOptions, (err, job) => {
                ...
                sqldb.beginTransaction((err, client, done) => {
                    ...
                    sqldb.queryWithClient(client, sql.select_graded_submissions_and_other_data, {instance_question_id: instance_question_id}, (err, result) => {
                        if (err) {
                            // TODO: sqldb.endTransaction?
                        ...
                        if (result.rowCount === 0) {
                            ...
                            async.series([
                                ...
                                // reset all variants for the instance question
                                (callback) => {
                                    const params = {
                                        instance_question_id: instance_question_id,
                                    };
                                ...
                                // close all variants for the instance question
                                (callback) => {
                                    const params = {
                                        instance_question_id: instance_question_id,
                                    };
                                    sqldb.queryWithClient(client, sql.close_variants, params, (err) => {
                                        if (err) {
                                            callback(err);
                                        } else {
                                            job.verbose(`Closed all variants for question '${qid}'.`);
                                            callback(null);
                                        }
                                    });
                                },
