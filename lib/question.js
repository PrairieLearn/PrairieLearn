...
const { eventBuffer, runWithEventBuffer } = require('@prairielearn/async-local-event-buffer');
...
module.exports = {
    ...
    questionServers.getModule(question.type, (err, questionModule) => {
      ...
      questionModule.generate(question, course, variant_seed, (err, courseIssues, data) => {
        ...
        questionModule.prepare(question, course, variant, (err, extraCourseIssues, data) => {
          eventBuffer.push('prepare complete');
          if (ERR(err, callback)) return;
  ...
  _makeAndInsertVariantWithClient(
    ...
    this._selectQuestionWithClient(client, question_id, instance_question_id, (err, question) => {
      eventBuffer.push('_selectQuestionWithClient complete');
      if (ERR(err, callback)) return;
      this._makeVariant(question, course, options, (err, courseIssues, variant) => {
        eventBuffer.push('_makeVariant complete');
        if (ERR(err, callback)) return;
        const params = [
        ...
        sqldb.callWithClientOneRow(client, 'variants_insert', params, (err, result) => {
          ...
          this._writeCourseIssues(
            ...
            courseData,
            (err) => {
              eventBuffer.push('_writeCourseIssues complete');
              if (ERR(err, callback)) return;
  ...
  _ensureVariantWithClient(
    ...
    if (instance_question_id != null) {
      ...
      sqldb.callWithClient(client, 'instance_questions_select_variant', params, (err, result) => {
        eventBuffer.push('instance_questions_select_variant complete', { variant_id: variant?.id });
        if (ERR(err, callback)) return;
        const variant = result.rows[0].variant;
        if (variant != null) {
        ...
        this._makeAndInsertVariantWithClient(
          ...
          options,
          (err, variant) => {
            eventBuffer.push('_makeAndInsertVariantWithClient complete', {
      ...
      this._makeAndInsertVariantWithClient(
        ...
        options,
        (err, variant) => {
          eventBuffer.push('_makeAndInsertVariantWithClient complete', { variant_id: variant?.id });
          if (ERR(err, callback)) return;
          callback(null, variant);
        }
      );
  ...
  ensureVariant(
    ...
    runWithEventBuffer(() => {
      const timeoutId = setTimeout(() => {
        ...
        eventBuffer.flush().forEach((event) => logger.error(event));
      }, 5 * 1000);
  ...
  saveSubmission(submission, variant, question, course, callback) {
    ...
    runWithEventBuffer(() => {
      const timeoutId = setTimeout(() => {
        ...
        eventBuffer.flush().forEach((event) => console.error(event));
      }, 60 * 1000);
  ...
  _testQuestion(question, group_work, course_instance, course, test_type, authn_user_id, callback) {
    ...
    runWithEventBuffer(() => {
      ...
      const timeoutId = setTimeout(() => {
        ...
        eventBuffer.flush().forEach(() => logger.error(eventBuffer));
      }, 60 * 1000);
      ...
      [
        (callback) => {
          if (variant_id != null) {
            ...
            sqldb.callOneRow('variants_select', params, (err, result) => {
              eventBuffer.push('variants_select complete');
              if (ERR(err, callback)) return;
            ...
            this.ensureVariant(
              ...
              require_open,
              (err, variant) => {
                eventBuffer.push('ensureVariant complete');
                if (ERR(err, callback)) return;
        ...
        (callback) => {
          ...
          sqldb.query(sql.select_submissions, params, (err, result) => {
            eventBuffer.push('select_submissions complete');
            if (ERR(err, callback)) return;
        ...
        (callback) => {
          questionServers.getEffectiveQuestionType(locals.question.type, (err, eqt) => {
            eventBuffer.push('getEffectiveQuestionType complete');
            if (ERR(err, callback)) return;
        ...
        (callback) => {
          ...
          this._render(
            ...
            locals,
            (err, htmls) => {
              eventBuffer.push('render complete');
              if (ERR(err, callback)) return;
        ...
        (callback) => {
          ...
          sqldb.query(sql.select_issues, params, (err, result) => {
            eventBuffer.push('select_issues complete');
            if (ERR(err, callback)) return;
