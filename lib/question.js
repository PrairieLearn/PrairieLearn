...
module.exports = {
    ...
    */
    ...
    _saveS3FileUploads(submission, client, assessment) {
        ...
        keys.forEach((key) => {
            if (key.indexOf('_file_upload_') > -1) {
                ...
                const files = JSON.parse(submission.submitted_answer[key]);
                    files.forEach((files) => {
                        fileStore.upload(files.name, files.contents, 'student_upload', assessment.id, null, null, null, fileStore.modes.S3);
                    });
            }
        });
    },
