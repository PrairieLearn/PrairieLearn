        ...
        async.series([
            ...
            (callback) => {
                ...
                sqldb.callWithClientOneRow(client, 'grading_jobs_insert', params, (err, result) => {
                    ...
                    /* If the submission was marked invalid during grading the sproc will return a null record,
                       and we should bail here to prevent LTI updates. */
                    if (results.rows[0].id === null) return callback(new NoSubmissionError());
        ...
        sqldb.queryOneRow(sql.select_submission_info, params, (err, results) => {
            ...
            async.parallel([
                (callback) => {
                    ...
                    // the panel shouldn't be rerendered.  Check if 'feedback' is set because
                    // that will indicate the grading code was run (even if it is ungradable)
