...
module.exports = {
    ...
    */
    ...
    _saveSubmissionWithClient(client, submission, variant, question, course, callback) {
        ...
        logger.info(`submission: ${JSON.stringify(submission)}`);
        ...
        async.series([
            (callback) => { // if workspace, get workspace_id
                if (question.workspace_image !== null) {
                    ...
                    sqldb.queryOneRow(sql_select_workspace_id, params, (err, result) => {
                        if (ERR(err, callback)) return;
            ...
            (callback) => { // if workspace, download graded files
                if (question.workspace_image !== null) {
                    const action = 'grade';
                    logger.info(`... calling callbackified controlContainer(${workspace_id}, '${action}')`);
                    util.callbackify(workspace.controlContainer)(workspace_id, action, (err, result) => {
                        if (ERR(err, callback)) return;
            ...
            (callback) => { // if workspace, encode graded files into _files
                if (question.workspace_image !== null) {
                    logger.info(`... unzipping w-${workspace_id}-graded-files.zip`);
                    const zip = new admZip(`w-${workspace_id}-graded-files.zip`);
                    ...
                    zipEntries.forEach((zipEntry) => {
                        ...
                        // FIXME: keep original path (currently stripped as workaround for autograder)
                        name = name.split('/').pop();

                        requiredFileNames.push(name);
                        console.log(`... pushed zipEntry: ${zipEntry.entryName}`);

                        const hash = crypto.createHash('sha1').update(name).digest('hex');
                        const contents = zipEntry.getData().toString('utf8'); 
                        const encodedContents = Buffer.from(contents).toString('base64');

                        submission.submitted_answer[`_workspace_${hash}`] = encodedContents;
            ...
            (callback) => {
                questionModule.parse(submission, variant, question, course, (err, ret_courseIssues, ret_data) => {
                    ...
                    if (question.workspace_image !== null) {
                        ...
                        logger.info(`... adding _required_file_names`);
                        if (!('_required_file_names' in data['params'])) {
                            data['params']['_required_file_names'] = [];
                        }
                        requiredFileNames.forEach((name) => {
                            data['params']['_required_file_names'].push(name);
                        });
                    }
                    logger.info(`data: ${JSON.stringify(data)}`);
