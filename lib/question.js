    ...
    */
    ...
    _testVariantWithClient(client, variant, question, course, authn_user_id, callback) {
        ...
        async.series([
            ...
            (callback) => {
                this._gradeVariantWithClient(client, variant, test_submission_id, question, course, authn_user_id, (err, grading_job_id) => {
                    if (ERR(err, callback)) return;
                    
                    if (grading_job_id !== undefined) {
                        debug('testVariant()', 'starting external grading job, id:', grading_job_id);
                        externalGrader.beginGradingJob(grading_job_id, (err) => {
                            if (ERR(err, callback)) return;
                            callback(null);
                        });
