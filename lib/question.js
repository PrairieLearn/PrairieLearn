      ...
      [
        ...
        async () => {
          if (locals.instance_question) {
            const manualResult = await sqldb.callAsync('rubric_gradings_select_data', [
              locals.assessment_question.manual_rubric_id,
              locals.instance_question.manual_rubric_grading_id,
            ]);
            locals.rubric_data_manual = manualResult.rows[0];

            const autoResult = await sqldb.callAsync('rubric_gradings_select_data', [
              locals.assessment_question.auto_rubric_id,
              locals.instance_question.auto_rubric_grading_id,
            ]);
            locals.rubric_data_auto = autoResult.rows[0];
          }
        },
        (callback) => {
        ...
        async () => {
          ...
          (locals.rubric_data_auto?.rubric_items || [])
            .concat(locals.rubric_data_manual?.rubric_items || [])
            .forEach((item) => {
              item.rubric_item.short_text_rendered = mustache.render(
                item.rubric_item.short_text || '',
                mustache_data
              );
              item.rubric_item.staff_instructions_rendered = mustache.render(
                item.rubric_item.staff_instructions || '',
                mustache_data
              );
              item.rubric_item.description_rendered = mustache.render(
