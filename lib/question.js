    ...
    getAndRenderVariant(variant_id, locals, callback) {
        async.series([
            ...
            (callback) => {
                ...
                sqldb.callOneRow('format_date_full_compact', [locals.variant.date, locals.course_instance.display_timezone], (err, result) => {
                    if (ERR(err, callback)) return;
                    locals.variant.formatted_date = result.rows[0].format_date_full_compact;
                    callback(null);
                });
            },
        ], (err) => {
