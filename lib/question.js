...
module.exports = {
    ...
    */
     ...
     * @param {Object} question - The question for the variant.
     * @param {Object} course - The course for the variant.
     * @param {boolean} autosave - If the save operation was triggered by an autosave functionality.
     * @param {function} callback - A callback(err, submission_id) function.
     */
    _saveSubmissionWithClient(client, submission, variant, question, course, autosave, callback) {
     ...
     * @param {Object} question - The question for the variant.
     * @param {Object} course - The course for the variant.
     * @param {boolean} autosave - If the save operation was triggered by an autosave operation.
     * @param {function} callback - A callback(err, submission_id) function.
     */
    saveSubmission(submission, variant, question, course, autosave, callback) {
        let submission_id;
        sqldb.beginTransaction((err, client, done) => {
            if (ERR(err, callback)) return;
            async.series([
                ...
                // in the presence of errors.
                (callback) => {
                    this._saveSubmissionWithClient(client, submission, variant, question, course, autosave, (err, ret_submission_id) => {
                        if (ERR(err, callback)) return;
    ...
    saveAndGradeSubmission(submission, variant, question, course, callback) {
        ...
        sqldb.beginTransaction((err, client, done) => {
            ...
            async.series([
                (callback) => {
                    this._saveSubmissionWithClient(client, submission, variant, question, course, false, (err, ret_submission_id) => {
                        if (ERR(err, callback)) return;
                        submission_id = ret_submission_id;
