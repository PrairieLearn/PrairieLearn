...
module.exports = {    
    ...
    */
    ...
    regradeQuestion(instance_question_id, callback) {
        sqldb.beginTransaction((err, client, done) => {
            if (ERR(err, callback)) return;
            let submissions;
            ...
            sqldb.queryWithClient(client, sql.select_all_submissions_and_variants, {instance_question_id: instance_question_id}, (err, result) => {
                if (ERR(err, callback)) return;
                if (result.rowCount === 0) {
                    callback(null);
                } else {
                    ...
                    async.series([
                        ...
                        //
                        // No need to update the old question, we can use the grade() to calculate the score
                        //
                        // async () => {
                        //     await Promise.all(submissions.map(async (submission) => {
                        //         // First we need to update the variant. We can do that by creating a new variant with the same seed
                        //         this._makeVariant(question, course, {variant_seed: submission.variant_seed}, (err, _, variant) => {
                        //             if (err) logger.error(err);
                        //             if (ERR(err, callback)) return;
                        //             logger.info(JSON.stringify(variant));
                        //             new_true_answer = variant.true_answer;
                        //         });
                        //     }));
                        //     return null;
                        // },
                        ...
                        (outerCallback) => {
                            async.eachSeries(submissions, ((submission, innerCallback) => {
                                this._regradeVariantWithClient(client, submission, submission, question, course, submission.auth_user_id, innerCallback);
                            }), (err) => {
