...
module.exports = {
    ...
    */
        ...
        this._selectQuestionWithClient(client, question_id, instance_question_id, (err, question) => {
            ...
            this._makeVariant(question, course, options, (err, courseErrs, variant) => {
                ...
                sqldb.callWithClientOneRow(client, 'variants_insert', params, (err, result) => {
                    ...
                    sqldb.queryWithClient(client, 'SELECT * FROM variants WHERE id = $1', [variant.id], (err, result) => {
                        if (ERR(err, callback)) return;
                        console.log('question.qid', question.qid, 'result.rows', result.rows);
    ...
    _render(renderSelection, variant, question, submission, submissions, course, course_instance, locals, callback) {
        questionServers.getModule(question.type, (err, questionModule) => {
            if (ERR(err, callback)) return;
            questionModule.render(renderSelection, variant, question, submission, submissions, course, course_instance, locals, (err, courseErrs, htmls) => {
                if (ERR(err, callback)) return;
