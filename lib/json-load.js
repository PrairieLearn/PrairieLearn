...
module.exports.validateJSONWithSchema = function(json, schemaFilename, callback) {
    const absSchemaFilename = path.join(__dirname, '..', schemaFilename);
    module.exports.readJSON(absSchemaFilename, (err, schema) => {
        if (err) return callback(err);
        module.exports.validateJSON(json, schema, (err) => {
            if (err) return callback(err);
            callback(null);
        });
    });
