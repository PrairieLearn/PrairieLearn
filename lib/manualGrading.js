...
async function _updateInstanceQuestionScoreWithClient(
  ...
  if (!update_result.modified_at_conflict) {
    const result = (
      await sqldb.queryWithClientOneRowAsync(client, sql.select_assessment_instance_id, {
        instance_question_id,
      })
    ).rows[0];
    await util.promisify(ltiOutcomes.updateScore)(result.assessment_instance_id);
  }
