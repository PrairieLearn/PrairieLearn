...
/** Populates the locals objects for rubric data. Assigns values to `rubric_data_auto` and
 * `rubric_data_auto` in the locals.
 *
...
async function populateRubricData(locals) {
  ...
  if (locals.assessment_question.auto_rubric_id) {
    locals.rubric_data_auto = (
      await sqldb.queryZeroOrOneRowAsync(sql.select_rubric_data, {
        assessment_question_id: locals.assessment_question.id,
        rubric_id: locals.assessment_question.auto_rubric_id,
      })
    ).rows[0];
  }
  ...
  await async.eachLimit(
    [
      ...(locals.rubric_data_manual?.rubric_items || []),
      ...(locals.rubric_data_auto?.rubric_items || []),
    ],
    3,
    async (item) => {
      item.short_text_rendered = await markdown.processContentInline(
        mustache.render(item.short_text || '', mustache_data)
      );
      item.staff_instructions_rendered = await markdown.processContent(
        mustache.render(item.staff_instructions || '', mustache_data)
      );
      item.description_rendered = await markdown.processContent(
 ...
 * `manual_rubric_grading_id` and/or `auto_rubric_grading_id` field, typically a submission or
 * grading job. Updates the object in-place with `rubric_grading_manual` and `rubric_grading_auto`
 * values, as well as the rendered version of the manual feedback.
 *
...
async function populateManualGradingData(submission) {
  if (submission.manual_rubric_grading_id) {
    submission.rubric_grading_manual = (
      await sqldb.queryZeroOrOneRowAsync(sql.select_rubric_grading_data, {
        rubric_grading_id: submission.manual_rubric_grading_id,
      })
    ).rows[0];
  }
  if (submission.auto_rubric_grading_id) {
    submission.rubric_grading_auto = (
      await sqldb.queryZeroOrOneRowAsync(sql.select_rubric_grading_data, {
        rubric_grading_id: submission.auto_rubric_grading_id,
      })
    ).rows[0];
  }
 ...
 * @returns {Promise<{id: number, computed_points: number}>} The ID and points of the created rubric grading.
 */
async function insertRubricGrading(rubric_id, rubric_items, adjust_points) {
  return rubric_id
    ? sqldb.runInTransactionAsync(async (client) =>
        _insertRubricGradingWithClient(client, rubric_id, rubric_items, adjust_points)
      )
    : null;
}
...
async function _insertRubricGradingWithClient(client, rubric_id, rubric_items, adjust_points) {
  ...
  return { id: rubric_grading_result.id, computed_points };
}
...
async function updateInstanceQuestionScore(
  ...
  return sqldb.runInTransactionAsync(async (client) =>
...
async function _updateInstanceQuestionScoreWithClient(
  ...
  const current_submission = (
    await sqldb.queryWithClientOneRowAsync(client, sql.select_submission_for_score_update, {
      assessment_id,
      instance_question_id,
      submission_id,
      check_modified_at,
  ...
  if (current_submission.auto_rubric_id && score?.auto_rubric_data?.rubric_id) {
    auto_rubric_grading = await _insertRubricGradingWithClient(
      client,
      score?.auto_rubric_data?.rubric_id,
      score?.auto_rubric_data?.applied_rubric_items,
      score?.auto_rubric_data?.adjust_points
    );
    score.auto_points = auto_rubric_grading.computed_points;
    score.auto_score_perc = null;
  } else if (
    current_submission.auto_rubric_id &&
    score?.partial_scores == null &&
    score?.auto_points == null &&
    score?.auto_score_perc == null
  ) {
    // If there is a rubric, and the auto_points will not be updated, keep the current rubric grading.
    auto_rubric_grading = { id: current_submission.auto_rubric_grading_id };
  } else {
    // If the auto_points will be updated and the rubric grading has not been set, clear the rubric grading.
    auto_rubric_grading = null;
  }
  ...
  if (score?.auto_score_perc != null) {
    ...
    new_auto_score_perc = score.auto_score_perc;
    new_auto_points = (score.auto_score_perc * current_submission.max_auto_points) / 100;
  } else if (score?.auto_points != null) {
    if (score?.points != null) {
      throw new Error('Cannot set both auto_points and points');
    }
    new_auto_points = score.auto_points;
    new_auto_score_perc =
  ...
  if (score?.manual_score_perc != null) {
    ...
    new_manual_points = (score.manual_score_perc * current_submission.max_manual_points) / 100;
    ...
    new_manual_points = score.manual_points;
    ...
    new_score_perc = score.score_perc;
  ...
  if (
    ...
    (idsEqual(current_submission.submission_id, submission_id) ||
      new_score_perc != null ||
      score?.feedback ||
      score?.partial_scores)
  ) {
    console.log(1, new_auto_score_perc, new_manual_points, new_auto_points);
    const grading_job_result = await sqldb.queryWithClientOneRowAsync(
      client,
      sql.insert_grading_job,
      {
        ...
        auto_rubric_grading_id: auto_rubric_grading?.id,
      }
    );
    grading_job_id = grading_job_result.rows[0].id;

    console.log(2, new_auto_score_perc, new_manual_points, new_auto_points);
    if (!current_submission.modified_at_conflict && current_submission.submission_id) {
      await sqldb.queryWithClientOneRowAsync(client, sql.update_submission_score, {
        submission_id: current_submission.submission_id,
        feedback: score?.feedback,
        partial_scores: score?.partial_scores,
        manual_rubric_grading_id: manual_rubric_grading?.id,
  ...
  if (new_score_perc != null && !current_submission.modified_at_conflict) {
    await sqldb.queryWithClientAsync(client, sql.update_instance_question_score, {
    ...
    console.log(4);
    await sqldb.callWithClientAsync(client, 'assessment_instances_grade', [
    ...
    await util.promisify(ltiOutcomes.updateScore)(current_submission.assessment_instance_id);
  }
