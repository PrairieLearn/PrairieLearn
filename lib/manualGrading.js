...
module.exports = {
  ...
  updateAssessmentQuestionRubric: async (
    ...
    await sqldb.runInTransactionAsync(async (client) => {
      ...
      if (use_rubrics) {
        ...
        module.exports._recomputeInstanceQuestion(
          client,
          assessment_question_id,
          rubric_type,
          authn_user_id
  ...
  /**
   */
  _recomputeInstanceQuestion: async (
    client,
    assessment_question_id,
    rubric_type,
    authn_user_id
    ...
    async.eachOfSeries(instance_questions, async (instance_question) => {
      const new_rubric_grading = (
        await sqldb.callWithClientAsync(client, 'rubric_gradings_insert', [
          instance_question.rubric_id,
          instance_question.applied_rubric_items,
          instance_question.adjust_points,
        ])
      ).rows[0];

      // TODO Call instance_questions_update_score
    });
  },
};
