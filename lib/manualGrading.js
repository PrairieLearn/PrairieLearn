 ...
 * @param {number} max_points - number of points assigned as the maximum number of auto or manual points to the assessment question.
 ...
 */
async function insertRubricGrading(rubric_id, max_points, rubric_items, adjust_points) {
  return sqldb.runInTransactionAsync(async () => {
    ...
    const computed_points =
      Math.min(
        ...
        max_points + rubric_data.max_extra_points
...
async function updateInstanceQuestionScore(
  ...
  return sqldb.runInTransactionAsync(async () => {
    ...
    if (current_submission.auto_rubric_id && score?.auto_rubric_data?.rubric_id) {
      ...
      score.auto_score_perc = undefined;
    } else if (
