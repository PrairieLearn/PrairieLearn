...
module.exports.stop = async function () {
  ...
  // TODO: wait for any current processing to actually complete.
 ...
 * @returns {string} The URL of the results queue.
 */
async function loadQueueUrl(sqs) {
  logger.verbose(
    `External grading results queue ${config.externalGradingResultsQueueName}: getting URL...`
  );
  const data = await sqs.getQueueUrl({ QueueName: config.externalGradingResultsQueueName }).promise;
  logger.verbose(
