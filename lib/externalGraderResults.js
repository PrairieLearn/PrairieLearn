...
//                        logger.info('externalGraderResults: before sqs.receiveMessage');
                        sqs.receiveMessage(params, (err, data) => {
                            if (ERR(err, done)) return;
                            if (!data.Messages) {
//                                logger.info('externalGraderResults: no data.Messages');
                                return done(null, null);
                            }
                            logger.info('externalGraderResults: sqs.receiveMessage', data);
...
function logAtTimeout(jobId, logBundle) {
    logger.info('externalGradingResults timeout logs for ' + jobId);
    logger.info(logBundle);
}
...
function processMessage(data, callback) {
    ...
    var logBundle = [];
    var loggingTimeout = setTimeout(logAtTimeout, 3000, jobId, logBundle)

    logBundle.push({ timestamp: Date.now(), msg: `externalGraderResults: ${jobId} processMessage ${data}`});
    logger.verbose('Processing external grading job result message', {grading_job_id: jobId, ...data});
    if (data.event === 'job_received') {
        logger.info(`${jobId} processMessage, job_received`);
        logBundle.push({ timestamp: Date.now(), msg: `${jobId} processMessage, job_received`});
        const params = {
        ...
        logger.info(`externalGraderResults: ${jobId} processMessage, grading_result`);
        logBundle.push({ timestamp: Date.now(), msg: `${jobId} processMessage, grading_result`});
        ...
        async.series([
            ...
            (callback) => {
                ...
                if (data.data) {
                    // We have the data!
                    logBundle.push({ timestamp: Date.now(), msg: `${jobId} processMessage, data in data`});
                    processResults(jobId, data.data);
                    logBundle.push({ timestamp: Date.now(), msg: `${jobId} processMessage, callback`});
                    clearTimeout(loggingTimeout);
                    callback(null);
                    ...
                    logBundle.push({ timestamp: Date.now(), msg: `processMessage, fetching from s3`});
...
//    logger.info(`externalGraderResults: ${jobId} processResults`);
