...
function processMessage(data, callback) {
    ...
    logger.info(`externalGraderResults: ${jobId} processMessage`, data);
    logger.verbose('Processing external grading job result message', {grading_job_id: jobId, ...data});
    if (data.event === 'job_received') {
        logger.info(`externalGraderResults: ${jobId} processMessage, job_received`);
        const params = {
        ...
        async.series([
            ...
            (callback) => {
                ...
                if (data.data) {
                    // We have the data!
                    logger.info(`externalGraderResults: ${jobId} processMessage, data in data`);
                    processResults(jobId, data.data);
                    logger.info(`externalGraderResults: ${jobId} processMessage, callback`);
                    callback(null);
                    ...
                    logger.info(`externalGraderResults: ${jobId} processMessage, fetching from s3`);
