    ...
    async.forever((next) => {
      async.waterfall(
        ...
        (err) => {
          if (
            ERR(err, (err) =>
              logger.error('Error handling grader results', err),
            )
          );
          next();
        },
...
function processMessage(data, callback) {
  ...
  try {
    ...
    return callback(
      error.makeWithData(
        'Message does not contain a valid grading job id.',
        data,
      ),
    );
  }
  ...
  if (data.event === 'job_received') {
      ...
      [
        (callback) => {
          ...
          sqldb.queryOneRow(sql.get_job_details, params, (err, result) => {
            if (err) {
              clearTimeout(loggingTimeout);
              return callback(
                error.makeWithData(`Job ${jobId} could not be found`, data),
              );
            }
            if (result.rows[0].was_graded) {
              clearTimeout(loggingTimeout);
              return callback(
                error.makeWithData(`Job ${jobId} was already graded`, data),
              );
            }
        ...
        (callback) => {
          ...
          if (data.data) {
            ...
            new AWS.S3(config.awsServiceGlobalOptions).getObject(
              params,
              (err, s3Data) => {
                if (ERR(err, callback)) return;
                logBundle.push({
    ...
    return callback(
      error.makeWithData(`Unknown grading event: ${data.event}`, data),
    );
  }
}
