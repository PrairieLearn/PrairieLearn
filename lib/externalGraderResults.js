...
function processMessage(data, callback) {
    ...
    logger.info('externalGraderResults: processMessage ', data);
    logger.verbose('Processing external grading job result message', {grading_job_id: jobId, ...data});
    if (data.event === 'job_received') {
        logger.info('externalGraderResults: processMessage, job_received');
        const params = {
        ...
        logger.info('externalGraderResults: processMessage, grading_result');
        ...
        async.series([
            ...
            (callback) => {
                ...
                if (data.data) {
                    // We have the data!
                    logger.info('externalGraderResults: processMessage, data in data');
                    ...
                    logger.info('externalGraderResults: processMessage, fetching from s3');
            ...
            if (ERR(err, callback)) return;
            logger.info('externalGraderResults: processMessage, finishing up');
            callback(null);
        });
    } else {
...
function processResults(jobId, data) {
    logger.info('externalGraderResults: processResults ', jobId);
