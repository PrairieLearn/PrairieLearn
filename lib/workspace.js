...
module.exports = {
    ...
    async initialize(workspace_id) {
        ...
        const {course_id, question_id, qid} = (await sqldb.queryOneRowAsync(sql.select_workspace_paths, {workspace_id})).rows[0];
        const course_path = chunks.getRuntimeDirectoryForCourse({id: course_id});
        const chunksToLoad = [
            {
                'type': 'question',
        ...
        await chunks.ensureChunksForCourseAsync(course_id, chunksToLoad);
