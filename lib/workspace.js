...
module.exports = {
    ...
    async initialize(workspace_id) {
        ...
        const {workspace, question, course} = (await sqldb.queryOneRowAsync(sql.select_workspace_data, {workspace_id})).rows[0];
    ...
    async getGradedFilesFromS3(workspace_id) {
        ...
        const result = await sqldb.queryOneRowAsync(sql.select_workspace_version_and_required_file_names, { workspace_id });
        const { workspace_version, workspace_required_file_names } = result.rows[0];
        const timestamp = new Date().toISOString().replace(/[-T:.]/g, '-');
        ...
        await async.eachLimit(workspace_required_file_names, config.workspaceJobsParallelLimit, async (file) => {
            try {
