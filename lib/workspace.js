...
module.exports = {
  ...
  async initialize(workspace_id) {
    ...
    const dynamicFiles = (
      await async.mapSeries(variant_params._files || [], async (file) => {
        try {
          // Ignore files without a name
          if (!file.name) return null;
          // Discard names with directory traversal outside the home directory
          if (!parentContainsChild(remotePath, path.join(remotePath, file.name))) return null;
          ...
          if (file.questionFile) {
            ...
            if (!parentContainsChild(questionBasePath, localPath)) return null;
            // Discard paths that do not exist
            if (!(await fse.exists(localPath))) return null;
            return {
              name: file.name,
              localPath,
            };
