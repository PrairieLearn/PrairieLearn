...
module.exports = {
  ...
  async getGradedFilesFromFileSystem(workspace_id) {
    ...
    await async.eachLimit(
      await fg(workspace_graded_files, { cwd: remoteDir }),
      ...
      async (file) => {
        try {
          ...
          if (stat.isFile()) {
            ...
            debug(`Graded file ${file} is not a file`);
          }
        } catch (err) {
  ...
  async getGradedFilesFromS3(workspace_id) {
    ...
    const gradedFiles = workspace_graded_files.some(fg.isDynamicPattern)
      ? await awsHelper.listFilesFromS3Async(
