...
module.exports = {
  ...
  connection(socket) {
    ...
    socket.on('visibilityTimeout', async (msg) => {
      const workspace_id = msg.workspace_id;
      await module.exports
        .updateState(workspace_id, 'stopped', 'Stopped due to inactivity')
        .catch((err) => {
          Sentry.captureException(err);
        });

      const meter = metrics.getMeter('prairielearn');
      const visibilityTimeoutCounter = getCounter(meter, 'workspace.stopped.visibility_timeout', {
