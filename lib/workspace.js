...
const { contains } = require('./instructorFiles');
...
module.exports = {
  ...
  /** Default options for calls to fast-glob. Set as an object so other related libraries may use
   * the same values.
   * @type {object}
   */
  fastGlobDefaultOptions: { extglob: false, braceExpansion: false },
  ...
  async updateMessage(workspace_id, message, toDatabase = true) {
    ...
    if (toDatabase) await sqldb.queryAsync(sql.update_workspace_message, { workspace_id, message });
  ...
  async updateState(workspace_id, state, message = '') {
    ...
    await sqldb.queryAsync(sql.update_workspace_state, { workspace_id, state, message });
  ...
  async initialize(workspace_id) {
    ...
    const localPathExists = await fse.pathExists(localPath);

    /** @type {{name: string; localPath?: string; buffer?: Buffer }[]} */
    const dynamicFiles = (
      await async.mapSeries(variant_params._workspace_files || [], async (file) => {
        try {
  ...
  async getGradedFilesFromFileSystem(workspace_id) {
    ...
    const gradedFiles = (
      await fg(workspace_graded_files, {
        cwd: remoteDir,
        stats: true,
    ...
    if (gradedFiles.length > config.workspaceMaxGradedFilesCount) {
      throw new SubmissionFormatError(
        `Cannot submit more than ${config.workspaceMaxGradedFilesCount} files from the workspace.`
      );
    }
    if (_.sumBy(gradedFiles, (file) => file.stats.size) > config.workspaceMaxGradedFilesSize) {
      throw new SubmissionFormatError(
        `Workspace files exceed limit of ${filesize(config.workspaceMaxGradedFilesSize, {
          base: 2,
        })}.`
      );
    }
