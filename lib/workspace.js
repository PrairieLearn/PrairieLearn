  ...
  await new Promise((resolve, reject) => {
    stream
      .on('open', () => {
        ...
        if (dynamic) {
          dynamic.forEach(({ name, buffer }) => archive.append(buffer, { name }));
        }
...
module.exports = {
  ...
  async initialize(workspace_id) {
    ...
    const dynamicFiles = Array.isArray(variant_params?._files)
      ? variant_params._files
          .map((file) => {
            try {
              if (!file.name) {
                logger.warning(`Dynamic file included without a name. Ignoring file.`, err);
                return null;
              }
              return {
                name: file.name,
                buffer: Buffer.from(file.contents ?? '', file.encoding ?? 'base64'),
              };
            } catch (err) {
              logger.warning(
    ...
    if (workspace.homedir_location === 'S3') {
      ...
      if (dynamicFiles) {
        await async.eachSeries(dynamicFiles, async ({ name, buffer }) => {
