...
async function uploadDirectoryToS3Async(s3Path, bucket_name) {
    const s3 = new AWS.S3();

    async function walk_directory(directory) {
        const files = await fsPromises.readdir(directory);
        await async.each(files, async (file) => {
            const file_path = path.join(directory, file);
            const stat = await fsPromises.stat(file_path);
            if (stat.isFile()) {
                const file_body = await fsPromises.readFile(file_path);
                const bucket_path = file_path.substring(s3Path.length + 1);
                const params = {
                    Bucket: bucket_name,
                    Key: bucket_path,
                    Body: file_body,
                ...
                try {
                    ...
                    logger.info(`[workspace.js] synced ${bucket_path} to ${bucket_name}`);
                } catch (err) {
                    logger.error(`[workspace.js] error syncing ${bucket_path}:\n ${err}`);
                }
            } else {
                return await walk_directory(file_path);
            }
        });
    }

    console.log(s3Path);
    await walk_directory(s3Path);
}
