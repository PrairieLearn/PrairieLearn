  ...
  async initialize(workspace_id) {
    ...
    const staticFiles = (
      await async
        ...
        .catch(() => {
          // Template path does not exist or is not accessible, do nothing
          return [];
        })
    ).filter((file) => file);
      ...
      await async
        .mapSeries(klaw(templatePath), async (file) => {
          try {
            ...
            return {
              name: path.relative(templatePath, file.path),
            ...
            return { name: path.relative(templatePath, file.path), localPath: file.path };
          }
        })
        .catch(() => {
