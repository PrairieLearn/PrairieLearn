...
module.exports = {
  ...
  connection(socket) {
    ...
    socket.on('heartbeat', (msg, callback) => {
      const workspace_id = msg.workspace_id;
      sqldb.queryOneRow(
        sql.update_workspace_heartbeat_at_now,
        { workspace_id },
        (err, result) => {
          if (ERR(err, callback)) return;
  ...
  async updateMessage(workspace_id, message, toDatabase = true) {
    ...
    if (toDatabase)
      await sqldb.callAsync('workspaces_message_update', [
        workspace_id,
        message,
      ]);
  ...
  async updateState(workspace_id, state, message = '') {
    // TODO: add locking
    debug(
      `Setting workspaces.state='${state}', workspaces.message='${message}'`,
    );
    await sqldb.callAsync('workspaces_state_update', [
      workspace_id,
      state,
      message,
    ]);
  ...
  async controlContainer(workspace_id, action, options = {}) {
    ...
    if (action === 'getGradedFiles') {
      ...
      if (!match)
        throw new Error(
          `Content-Disposition format error: ${contentDisposition}`,
        );
    ...
    if (state == 'uninitialized' || state == 'stopped') {
      ...
      while (true) {
        ...
        if (attempt > config.workspaceLaunchingRetryAttempts) {
          throw new Error(
            'Time exceeded to deploy more computational resources',
          );
        }
        ...
        await util.promisify(setTimeout)(
          config.workspaceLaunchingRetryIntervalSec * 1000,
        );
        attempt++;
      }
      await module.exports.updateMessage(
        workspace_id,
        'Sending launch command to host',
      );
  ...
  async initialize(workspace_id) {
    ...
    if (workspace.homedir_location == 'S3') {
      ...
      throw new Error(
        `Unknown backing file storage: ${workspace.homedir_location}`,
      );
    }
  },
  ...
  async assignHost(workspace_id) {
    ...
    const result = await sqldb.callOneRowAsync(
      'workspace_hosts_assign_workspace',
      params,
    );
    const workspace_host_id = result.rows[0].workspace_host_id;
    debug(
      `assignHost(): workspace_id=${workspace_id}, workspace_host_id=${workspace_host_id}`,
    );
  ...
  async getGradedFiles(workspace_id) {
    ...
    if (zipPath == null) {
      if (workspace.homedir_location == 'S3') {
        ...
        zipPath = await module.exports.getGradedFilesFromFileSystem(
          workspace_id,
        );
      } else {
        throw new Error(
  ...
  async getGradedFilesFromFileSystem(workspace_id) {
    ...
    await new Promise((resolve, reject) => {
      stream
        ...
        .on('finish', () => {
          debug(
            `Zipped graded files as ${zipPath} (${archive.pointer()} total bytes)`,
          );
          resolve(zipPath);
        });
    });
