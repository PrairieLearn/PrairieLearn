...
module.exports = {
  ...
  async controlContainer(workspace_id, action, options = {}) {
    ...
    if (action === 'getGradedFiles') {
      if (!res.ok) {
        throw { ...(await res.json()), isFormatError: true };
      }
  ...
  async getGradedFiles(workspace_id) {
    ...
    if (workspace.state === 'running') {
      ...
      try {
        ...
        if (err.isFormatError) throw err;
      }
    }
  ...
  async getGradedFilesFromFileSystem(workspace_id) {
    ...
    const gradedFiles = await fg(workspace_graded_files, { cwd: remoteDir, stats: true });

    if (gradedFiles.length > config.workspaceMaxGradedFilesCount) {
      throw { message: 'Too many files submitted in workspace.', isFormatError: true };
    }
    if (_.sumBy(gradedFiles, (file) => file.stats.size) > config.workspaceMaxGradedFilesSize) {
      throw { message: 'Workspace files are larger than allowed.', isFormatError: true };
    }
  ...
  async getGradedFilesFromS3(workspace_id) {
    ...
    await async.eachLimit(
      workspace_graded_files, // wildcards are not supported in S3, as it is not in use in production
