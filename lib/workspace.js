...
const { parentContainsChild } = require('./file-paths');
...
/**
 * @param {string} source - Path of the original workspace directory
 * @param {Object[]} dynamic - List of dynamic files to add to archive
 * @param {string} dynamic[].name - Name of the file in the archive
 * @param {string?} dynamic[].localPath - Path of the file to copy to the specific name
 * @param {Buffer?} dynamic[].buffer - Data to save to the file
 * @param {string} zip - Path of the zip file to be created
 * @return {Promise<void>}
 */
  ...
  await new Promise((resolve, reject) => {
    stream
      .on('open', () => {
        ...
        if (dynamic) {
          dynamic.forEach(({ name, localPath, buffer }) => {
            if (localPath) {
...
module.exports = {
  ...
  async initialize(workspace_id) {
    const { workspace, variant_params, question, course } = (
    ...
    const questionBasePath = `${course_path}/questions/${question.qid}`;
    const localPath = `${questionBasePath}/workspace`;
    ...
    const dynamicFiles = Array.isArray(variant_params?._files)
      ? variant_params._files
          .map((file) => {
            try {
              if (!file.name) {
                // Dynamic file included without a name. Ignoring file.
                return null;
              }
              if (file.questionFile) {
                return {
                  name: file.name,
                  localPath: path.join(questionBasePath, file.questionFile),
                };
              }
              return {
                name: file.name,
          ...
          .filter((file) => {
            // Discard files ignored in previous step
            if (!file) return false;
            // Discard names with directory traversal outside the home directory
            if (!parentContainsChild(remotePath, path.join(remotePath, file.name))) return false;
            if (file.localPath) {
              // Discard paths with directory traversal outside the question
              if (!parentContainsChild(questionBasePath, file.localPath)) return false;
              // Discard paths that do not exist
              if (!fse.existsSync(file.localPath)) return false;
            }
            return true;
          })
      : null;
    ...
    if (workspace.homedir_location === 'S3') {
      await zipDirectory(localPathExists ? localPath : null, dynamicFiles, localZipPath);
      ...
      if (dynamicFiles) {
        await async.eachSeries(dynamicFiles, async ({ name, localPath, buffer }) => {
          await awsHelper.uploadToS3Async(
      ...
      if (dynamicFiles) {
        await async.eachSeries(dynamicFiles, async ({ name, localPath, buffer }) => {
          await fse.ensureDir(path.join(sourcePath, path.dirname(name)));
          if (localPath) {
            await fse.copy(localPath, path.join(sourcePath, name));
          } else {
            await fse.writeFile(path.join(sourcePath, name), buffer);
          }
        });
      }

      if (localPathExists || dynamicFiles) {
