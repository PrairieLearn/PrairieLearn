...
module.exports = {
    ...
    async updateMessage(workspace_id, message, toDatabase=true) {
        ...
        module.exports.emitMessageForWorkspace(workspace_id, 'change:message', {workspace_id, message});
    },
    ...
    async updateState(workspace_id, state, message='') {
        ...
        module.exports.emitMessageForWorkspace(workspace_id, 'change:state', {workspace_id, state, message});
    },
    ...
    async initialize(workspace_id) {
        ...
        const {course_path, qid} = (await sqldb.queryOneRowAsync(sql.select_workspace_paths, {workspace_id})).rows[0];
        const workspace = (await sqldb.queryOneRowAsync(sql.select_workspace, {workspace_id})).rows[0];
        const workspace_version = workspace.version;
        ...
        await sqldb.queryOneRowAsync(sql.update_workspace_homedir_location, { workspace_id, homedir_location });

        /* local workspace files */
        const localPath = `${course_path}/questions/${qid}/workspace`;

        /* base workspace directory wherever we are uploading to */
        const remoteDirName = `workspace-${workspace_id}-${workspace_version}`;
        ...
        const localZipPath = path.join(config.workspaceMainZipsDirectory, `${remoteDirName}-${now}.zip`);

        /* if we don't actually have any local files, just create an empty zip */
        let localPathExists;
        try {
            await fsPromises.stat(localPath);
            localPathExists = true;
            debug(`Zipping ${localPath} as ${localZipPath}`);
        } catch (err) {
        ...
        if (workspace.homedir_location == 'S3') {
            ...
            await fse.ensureDir(path.join(root, remoteDirName));

            await fsPromises.copyFile(localZipPath, path.join(root, remoteZipPath));
            if (localPathExists) {
                debug(`Syncing ${localPath} to ${remotePath}`);
                ...
                await fsPromises.chown(path.join(root, remotePath), config.workspaceJobsDirectoryOwnerUid, config.workspaceJobsDirectoryOwnerGid);
    ...
    async getGradedFilesFromFileSystem(workspace_id) {
        const { workspace_version, workspace_graded_files } = (await sqldb.queryOneRowAsync(sql.select_workspace_version_and_graded_files, { workspace_id })).rows[0];
        const timestamp = new Date().toISOString().replace(/[-T:.]/g, '-');
        const zipPath = path.join(config.workspaceMainZipsDirectory, `workspace-${workspace_id}-${workspace_version}-${timestamp}.zip`);

        const archive = archiver('zip');
        const remoteName = `workspace-${workspace_id}-${workspace_version}`;

        /* Zip files from filesystem to zip file */
        await async.eachLimit(workspace_graded_files, config.workspaceJobsParallelLimit, async (file) => {
            try {
    ...
    async getGradedFilesFromS3(workspace_id) {
        ...
        const result = await sqldb.queryOneRowAsync(sql.select_workspace_version_and_graded_files, { workspace_id });
