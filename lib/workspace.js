...
module.exports = {
  ...
  async getGradedFilesFromFileSystem(workspace_id) {
    ...
    const gradedFiles = _.flatten(
      async.mapSeries(workspace_graded_files, async (file) =>
        glob.hasMagic(file)
          ? await async.promisify(glob)(file, { cwd: remoteDir, root: remoteDir })
          : [file]
      )
    );
    ...
    await async.eachLimit(gradedFiles, config.workspaceJobsParallelLimit, async (file) => {
      try {
        ...
        const stat = await fsPromises.lstat(remotePath);
        if (stat.isDirectory()) {
          debug(`Graded file ${file} is a directory, skipping`);
          // debug(`Zipping graded directory ${remotePath} into ${zipPath}`);
          // archive.directory(file, false);
        } else if (stat.isFile()) {
          debug(`Zipping graded file ${remotePath} into ${zipPath}`);
          archive.file(remotePath, { name: file });
        } else {
          debug(`Graded file ${file} is not a file or directory`);
        }
      } catch (err) {
  ...
  async getGradedFilesFromS3(workspace_id) {
    ...
    const archive = archiver('zip');
    const s3Name = path.join(`workspace-${workspace_id}-${workspace_version}`, 'current');

    const gradedFiles = _.flatten(
      async.mapSeries(workspace_graded_files, async (file) =>
        glob.hasMagic(file)
          ? await awsHelper.listFilesFromS3Async(config.workspaceS3Bucket, `${s3Name}/`, file)
          : [file]
      )
    );
    ...
    await async.eachLimit(
      ...
      async (file) => {
        try {
          ...
          const s3Path = path.join(s3Name, file);
          const options = {
