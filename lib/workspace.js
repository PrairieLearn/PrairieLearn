...
module.exports = {
            ...
            socket.join(workspace_id, () => {
                ...
                sqldb.queryOneRow(sql.select_workspace, {workspace_id}, (err, result) => {
                    ...
                    if (workspace.state == 'uninitialized') {
                        util.callbackify(module.exports.initAndLaunch)(workspace_id, (err) => {
                            if (err) {
                                logger.error(`Error from initAndLaunch(): ${err}`);
    ...
    async controlContainer(workspace_id, action) {
        const results = await sqldb.queryZeroOrOneRowAsync(sql.select_workspace_host, {workspace_id});
        if (results.rowCount == 0) {
            throw new Error(`No host for workspace_id=${workspace_id}`);
        }
        const workspace_host = results.rows[0];
        const postJson = {
    ...
    async initAndLaunch(workspace_id) {
        ...
        await module.exports.updateState(workspace_id, 'running');
    },
    ...
    async assignHost(workspace_id) {
        ...
        const results = await sqldb.queryAsync(sql.select_workspace_hosts, {});
        logger.info(`controlContainer workspace_hosts query: ${JSON.stringify(results.rows)}`);
        const workspace_hosts = results.rows;
