...
module.exports = {
    ...
    async getGradedFilesFromS3(workspace_id) {
        ...
        // query workspace version
        result = await sqldb.queryOneRowAsync(sql.select_workspace_version, { workspace_id });
        const { workspace_version } = result.rows[0];

        // define zip names
        const timestamp = new Date().toISOString().replace(/[-T:.]/g, '-');
        const zipDir = `workspace-${workspace_id}-${workspace_version}-${timestamp}`;
        const zipPath = path.join(config.workspaceMainZipsDirectory, `${zipDir}.zip`);
        ...
        for (const file of workspace_graded_files) {
            try {
                const localPath = path.join(config.workspaceMainZipsDirectory, zipDir, file);
                ...
                debug(`Zipping ${localPath} into ${zipPath}`);
...
module.exports.changeWorkspaceFileOwner = function (filepath, callback) {
...
module.exports.changeWorkspaceFileOwnerAsync = util.promisify(module.exports.changeWorkspaceFileOwner);
