...
const zipPrefix = config.workspaceGradedFilesReceiveDirectory;
            ...
            await async.each(files, async (file) => {
                ...
                if (stat.isFile()) {
                    ...
                    try {
                        await s3.putObject({
                            Bucket: bucketName,
                            Key: bucketPath + '/', /* Put a forward slash at the end to create a directory */
                            Body: '',
                        }).promise();
                        logger.info(`synced directory ${bucketPath} to ${bucketName}`);
                    } catch (err) {
                        logger.error(`error syncing ${bucketPath}:\n ${err}`);
                    }
