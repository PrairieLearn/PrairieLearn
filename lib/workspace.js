...
module.exports = {
  async init() {
    module.exports._namespace = socketServer.io.of(/^\/workspace-\d+$/);
  ...
  async connection(socket) {
    ...
    const workspaceId = namespace.name.replace('/workspace-', '');
    console.log(`Workspace ${workspaceId} connected`);
    socket.join(`workspace-${workspaceId}`);

    socket.on('startWorkspace', async () => {
      const result = await sqldb.queryOneRowAsync(sql.select_workspace, {
        workspace_id: workspaceId,
      });
      const workspace = result.rows[0];

      // Immediately send the state back to the client, but continue starting
      // the workspace in the background.
      module.exports.emitMessageForWorkspace(workspaceId, 'change:state', {
        workspace_id: workspaceId,
        state: workspace.state,
      });

      module.exports.startup(workspaceId, workspace.state).catch(async (err) => {
        logger.error(`Error starting workspace ${workspaceId}`, err);
    ...
    // Immediately tell the client the current state of the workspace.
    const result = await sqldb.queryOneRowAsync(sql.select_workspace, {
      workspace_id: workspaceId,
    });
    const workspace = result.rows[0];
