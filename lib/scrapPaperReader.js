...
const processScrapPaperPdf = async (pdfBuffer, originalName, userId, logger) => {
  ...
  const result = await decodeBarcodes(pdfBuffer, originalName, logger);
  logger.info(`Decoder results ${JSON.stringify(result), null, '\t'}`);
  // 2. filter only the barcode pages that we could read
  const decodedPdfPages = result.filter((decodedPdfPage) => decodedPdfPage.barcode);
