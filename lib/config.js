...
config.useCloudWatchLogging = true;
...
config.getInstanceInformation = async () => {
    logger.info('Getting EC2 instance metadata');
    const MetadataService = new AWS.MetadataService();
    const document = await util.promisify(MetadataService.request)('/latest/dynamic/instance-identity/document');
    const data = JSON.parse(document);
    logger.info('instance-identity', data);
    AWS.config.update({'region': data.region});
    config.instanceIdentity = data;
    config.instanceId = data.instanceId;
};

config.getConfigFromSecretsManager = async () => {
    // Look up a special tag and use its value to find a secret
    // containing configuration data. The secret data must be JSON
    // with a single object, which is the same format (same key names)
    // as our "config" object. We will assign all data from the secret
    // object into our config, overwriting existing properties.
    logger.info('Getting instance tags');
    const ec2 = new AWS.EC2();
    const params = {
        Filters: [
            {Name: 'resource-id', Values: [config.instanceId]},
        ],
    };
    const data = await util.promisify(ec2.describeTags)(params);
    logger.info('Instance tags', data.Tags);
    const confSecretTag = _.find(data.Tags, {Key: 'ConfSecret'});
    if (confSecretTag != null) {
        const SecretId = confSecretTag.Value;
        logger.info(`SecretId: ${SecretId}`);
        const secretsManager = new AWS.SecretsManager();
        const data = await util.promisify(secretsManager.getSecretValue)({SecretId});
        ...
        if (data.SecretString != null) {
            ...
            _.assign(config, secretConfig);
        }
    }
};

config.getAutoScalingGroup = async () => {
    // detect an AutoScalingGroup we are inside (if any)
    logger.info('Detecting AutoScalingGroup...');
    config.autoScalingGroupName = null;
    var autoscaling = new AWS.AutoScaling();
    var params = {InstanceIds: [config.instanceId]};
    const data = util.promisify(autoscaling.describeAutoScalingInstances)(params);
    if (data.AutoScalingInstances.length == 1) {
        config.autoScalingGroupName = data.AutoScalingInstances[0].AutoScalingGroupName;
        logger.info(`Running inside AutoScalingGroup: ${config.autoScalingGroupName}`);
    } else {
        logger.info('Not running inside an AutoScalingGroup');
    }
};

config.initCloudWatchLogging = async () => {
    if (config.useCloudWatchLogging) {
        logger.info('Enabling CloudWatch logging...');
        const groupName = config.globalLogGroup;
        const streamName = config.instanceId;
        logger.initCloudWatchLogging(groupName, streamName);
        logger.info(`CloudWatch logging enabled. Logging to ${groupName}/${streamName}`);
    }
};

config.awsSetupAsync = async () => {
    if (!config.runningInEc2) return;
    await config.getInstanceInformation();
    await config.getConfigFromSecretsManager();
    await config.getAutoScalingGroup();
    await config.initCloudWatchLogging();
};
config.awsSetup = util.callbackify(config.awsSetupAsync);
