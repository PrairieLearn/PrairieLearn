...
config.cronIntervalStopStaleWorkspacesSec = 10 * 60;
config.cronIntervalWorkspacesLoadSec = 20;
config.cronIntervalWorkspacesStateTransitionSec = 10;
...
config.getDBConfigValueAsync = async function(key, defaultVal = null) {
    const result = await sqldb.call('config_select', {key});
...
config.getDBConfigValue = function(key, callback) {
    sqldb.queryZeroOrOneRow(sql.get_value, {key}, (err, result) => {
        if (ERR(err, callback)) return;
        if (result.rows.length == 0) {
            return callback(new Error(`No key: ${key}`));
        }
