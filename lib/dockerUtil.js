...
dockerUtil.pullAndPushToECR = function(image, dockerAuth, job, callback) {
    ...
    docker.createImage({}, params, (err, stream) => {
        ...
        docker.modem.followProgress(stream, (err) => {
                    ...
                    confirmOrCreateECRRepo(repositoryName, job, (err) => {
                        ...
                        pushImage.push({}, (err, stream) => {
                            ...
                            docker.modem.followProgress(stream, (err) => {
                                ...
                                job.info(JSON.stringify(output));
                                if ('progressDetail' in output && output.progressDetail.total) {
                                    const info = `${output.status} ${output.id} ${output.progressDetail.total}`;
                                    if (!printedInfos.has(info)) {
            ...
            if ('progressDetail' in output && output.progressDetail.total) {
                const info = `${output.status} ${output.id} (${output.progressDetail.total} bytes)`;
                if (!printedInfos.has(info)) {
                    printedInfos.add(info);
                    job.info(info);
                }
