...
dockerUtil.logProgressOutput = function(output, job, printedInfos) {
    let info = null;
    if ('status' in output && 'id' in output && 'progressDetail' in output && output.progressDetail.total) {
                    ...
                    confirmOrCreateECRRepo(repositoryName, job, (err) => {
                        ...
                        pushImage.push({}, (err, stream) => {
                            ...
                            docker.modem.followProgress(stream, (err) => {
                                ...
                                dockerUtil.logProgressOutput(output, job, printedInfos);
                            });
                        }, dockerAuth);
                    });
                });
