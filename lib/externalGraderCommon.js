...
module.exports.buildDirectory = function(dir, submission, variant, question, course, callback) {
    async.series([
        ...
        (callback) => {
            // if (submission.submitted_answer._file_storage_s3) {
                
            // }

            if (submission.submitted_answer._files) {
                // NOTE: Might want to look at submitting another parameter with the file buffer contents and
                // filename; then integrate it here. We do not want to re-download it from S3 when
                ...
                async.each(submission.submitted_answer._files, (file, callback) => {
                    ...
                    // Files are expected to be Buffers
                    let decodedContents = file.contents;
