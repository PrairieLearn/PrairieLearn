        ...
        (callback) => {
            if (submission.submitted_answer._files) {
                async.each(submission.submitted_answer._files, (file, callback) => {
                    ...
                    if (relPath.split(path.sep).includes('..') || path.isAbsolute(relPath)) {
                        callback(new Error('Invalid filename'));
                        return;
                    }
                    // We need to make sure that the directories are created first.
                    // Here fs is actually fs-extra, so we can use ensureDirSync.
                    const targetDir = path.dirname(fullPath);
                    if (!ensureDirAsync(targetDir)) {
                        callback(new Error(`ensureDir failed to create ${targetDir}`));
                        return;
                    }
                    fs.writeFile(fullPath, decodedContents, callback);
                }, function(err) {
                    if (ERR(err, callback)) return;
                    callback(null);
                });
...
async function ensureDirAsync(dir) {
    try {
        // The version of ensureDir from fs-extra works recursively
        // by default to recreate the intermediate paths in dir
