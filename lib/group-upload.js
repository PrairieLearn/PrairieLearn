...
module.exports = {
    ...
    uploadInstanceGroups(assessment_id, csvFile, user_id, authn_user_id, callback) {
        ...
        sqldb.queryOneRow(sql.select_assessment_info, params, function(err, result) {
            ...
            const options = {
                ...
                description: 'Upload group arrangement for ' + assessment_label,
            };
            serverJobs.createJobSequence(options, function(err, job_sequence_id) {
                ...
                serverJobs.createJob(jobOptions, function(err, job) {
                    ...
                    job.verbose('Uploading group arrangement for ' + assessment_label);
                    ...
                    csvConverter
                        ...
                        .then(() => {
                            ...
                            job.verbose(`----------------------------------------`);
                            if (errorCount == 0) {
                                job.verbose(`Successfully updated arrangement for ${successCount} students, with no errors`);
                                job.succeed();
                            } else {
                                job.verbose(`Successfully updated arrangement for ${successCount} students`);
    ...
    _updateGroupsFromJson(json, assessment_id, authn_user_id, callback) {
        util.callbackify(async () => {
            ...
            await sqldb.callAsync('instance_groups_update', params);
            /*
            const params = [
                assessment_id,
