                ...
                serverJobs.createJob(jobOptions, function(err, job) {
                    ...
                    csvConverter
                        ...
                        .then(() => {
                            ...
                            job.verbose(`----------------------------------------`);
                            if (errorCount == 0) {
                                job.verbose(`Successfully updated arrangement for ${successCount} groups, with no errors`);
                                job.succeed();
                            } else {
                                job.verbose(`Successfully updated arrangement for ${successCount} groups`);
                                job.fail(`Error updating ${errorCount} groups`);
                            }
                        })
                        .catch((err) => {
                            if (output != null) {
