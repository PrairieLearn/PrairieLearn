    ...
    const worker_function = async (job_sequence_id) => {
      ...
      if (client === null) {
        ...
        await module.exports._runJobAsync(
          { type: 'exit_early', description: 'Nothing to do, exiting...', last_in_sequence: true },
          job_sequence_id,
          authn_user.user_id,
          async () => {
            return;
      ...
      await module.exports._runJobAsync(
        { type: 'create_repo', description: 'Creating repository from template' },
        job_sequence_id,
        authn_user.user_id,
        async (job) => {
      ...
      await module.exports._runJobAsync(
        ...
        authn_user.user_id,
        async (job) => {
          let { sha: sha, contents: courseInfo } = await module.exports.getFileFromRepoAsync(
            client,
            options.repo_short_name,
            'infoCourse.json'
          );
      ...
      if (options.github_user) {
        await module.exports._runJobAsync(
          { type: 'add_instructor', description: 'Adding instructor to repository' },
          job_sequence_id,
          authn_user.user_id,
          async (job) => {
            try {
      ...
      await module.exports._runJobAsync(
        { type: 'update_commit_hash', description: 'Update course commit hash' },
        job_sequence_id,
        authn_user.user_id,
        async () => {
