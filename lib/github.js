...
module.exports = {
  ...
  createRepoFromTemplateAsync: async function (client, repo, template) {
    ...
    while (!repo_up) {
      try {
        ...
        logger.debug(
          `${repo} is not ready yet, polling again in ${poll_time_ms} ms`,
        );
      }
  ...
  getFileFromRepoAsync: async function (client, repo, path) {
    ...
    return {
      sha: file.data.sha,
      contents: Buffer.from(file.data.content, file.data.encoding).toString(
        'ascii',
      ),
    };
  },
  ...
  createCourseRepoJob: function (options, authn_user, callback) {
    const worker_function = async (job_sequence_id) => {
      ...
      await module.exports._runJobAsync(
        ...
        async (job) => {
          ...
          if (branches.length != 1) {
            throw new Error(
              `New repo has ${branches.length} branches, expected one.`,
            );
          }
      ...
      await module.exports._runJobAsync(
        ...
        authn_user.user_id,
        async (job) => {
          let { sha: sha, contents: courseInfo } =
            await module.exports.getFileFromRepoAsync(
              client,
              options.repo_short_name,
              'infoCourse.json',
            );
      ...
      await module.exports._runJobAsync(
        ...
        async (job) => {
          ...
          inserted_course = (
            await sqldb.callAsync('courses_insert', sql_params)
          ).rows[0];
      ...
      await module.exports._runJobAsync(
        ...
        async () => {
          ...
          await sqldb.queryOneRowAsync(
            sql.set_course_owner_permission,
            sql_params,
          );
        },
      );
