    ...
    createCourseRepoJob: function(options, authn_user, callback) {
        const worker_function = async (id) => {
            ...
            if (client === null) {
                ...
                await module.exports._runJobAsync({ 'type': 'exit_early', 'description': 'Nothing to do, exiting...', last_in_sequence: true }, id, authn_user.user_id, async () => { return; });
                return;
            }

            /* As a debug step, show the course information given to us since the info can be changed by an admin before the job is run */
            await module.exports._runJobAsync({'type': 'info', 'description': 'Show course information'}, id, authn_user.user_id, async (job) => {
                job.info(`Creating course ${options.short_name}`);
                job.info(JSON.stringify(options, null, 4));
            });

            /* Create base github repo from template */
            await module.exports._runJobAsync({'type': 'create_repo', 'description': 'Creating repository from template'}, id, authn_user.user_id, async (job) => {
            ...
            await module.exports._runJobAsync({'type': 'update_info_course', 'description': 'Updating infoCourse.json'}, id, authn_user.user_id, async (job) => {
            ...
            await module.exports._runJobAsync({'type': 'add_machine', 'description': 'Adding machine team to repo'}, id, authn_user.user_id, async (job) => {
            ...
            if (options.github_user) {
                await module.exports._runJobAsync({'type': 'add_instructor', 'description': 'Adding instructor to repository'}, id, authn_user.user_id, async (job) => {
                    try {
            ...
            let inserted_course;
            await module.exports._runJobAsync({'type': 'update_courses', 'description': 'Adding course to database'}, id, authn_user.user_id, async (job) => {
                const sql_params = [
            ...
            await module.exports._runJobAsync({'type': 'update_', 'description': 'Giving user owner permission'}, id, authn_user.user_id, async () => {
                const sql_params = {
            ...
            await module.exports._runJobCommandAsync({
                ...
                env: git_env,
            }, id, authn_user.user_id);
            await module.exports._runJobAsync({'type': 'update_commit_hash', 'description': 'Update course commit hash'}, id, authn_user.user_id, async () => {
            ...
            }, id, authn_user.user_id, async function (job) {
