...
module.exports = {
    ...
    createCourseRepoAsync: async function(short_name, title, instructor_github) {
        ...
        logger.info(`Creating GitHub repository for ${repo}`);
    ...
    createAndAddCourseFromRequestAsync: async function(course_request_id, authn_user) {
        /* Grab the course request */
        let sql_params = {
            course_request_id
        };
        const course_request = await sqldb.queryOneRowAsync(sql.get_course_request, sql_params);
