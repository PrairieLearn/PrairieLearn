...
module.exports = {
    ...
    _runJobAsync: function(options, id, user_id, func) {
        return new Promise((resolve, reject) => {
            ...
            serverJobs.createJob(options, function(err, job) {
                if (err) {
                    ...
                    serverJobs.failJobSequence(id);
                    reject(err);
                    return;
                }
                func(job)
                    .then(() => { job.succeed(); resolve(); })
                    .catch((err) => { job.fail(err); reject(err); });
            });
        });
    },

    _runJobCommandAsync: function(options, id, user_id) {
        return new Promise((resolve, reject) => {
            options = _.defaults({
                user_id: user_id,
                authn_user_id: user_id,
            ...
            serverJobs.spawnJob(options);
        });
    },

    createCourseRepoJob: function(options, authn_user, callback) {
        const worker_function = async (id) => {
            const client = module.exports.getGithubClient();
            if (client === null) {
                /* If we are running locally and don't have a client, then just exit early */
                await module.exports._runJobAsync({ 'type': 'exit_early', 'description': 'Nothing to do, exiting...', last_in_sequence: true }, id, authn_user.id, async () => { return; });
                return;
            }

            /* As a debug step, show the course information given to us since the info can be changed by an admin before the job is run */
            await module.exports._runJobAsync({'type': 'info', 'description': 'Show course information'}, id, authn_user.id, async (job) => {
                job.info(`Creating course ${options.short_name}`);
                job.info(JSON.stringify(options, null, 4));
            });

            /* Create base github repo from template */
            await module.exports._runJobAsync({'type': 'create_repo', 'description': 'Creating repository from template'}, id, authn_user.id, async (job) => {
            ...
            await module.exports._runJobAsync({'type': 'update_info_course', 'description': 'Updating infoCourse.json'}, id, authn_user.id, async (job) => {
            ...
            await module.exports._runJobAsync({'type': 'add_machine', 'description': 'Adding machine team to repo'}, id, authn_user.id, async (job) => {
            ...
            if (options.github_user) {
                await module.exports._runJobAsync({'type': 'add_instructor', 'description': 'Adding instructor to repository'}, id, authn_user.id, async (job) => {
                    try {
            ...
            let inserted_course;
            await module.exports._runJobAsync({'type': 'update_courses', 'description': 'Adding course to database'}, id, authn_user.id, async (job) => {
                const sql_params = [
            ...
            await module.exports._runJobAsync({'type': 'update_', 'description': 'Giving user owner permission'}, id, authn_user.id, async () => {
                const sql_params = {
                    'course_id': inserted_course.id,
                    'course_request_id': options.course_request_id,
                };
                const results = await sqldb.queryOneRowAsync(sql.set_course_owner_permission, sql_params);
            });

            /* Automatically sync the new course.  This part is shamelessly stolen from pages/shared/syncHelpers.js */
            const git_env = process.env;
            if (config.gitSshCommand != null) {
                git_env.GIT_SSH_COMMAND = config.gitSsCommand;
            }
            ...
            await module.exports._runJobAsync({'type': 'update_commit_hash', 'description': 'Update course commit hash'}, id, authn_user.id, async () => {
            ...
            }, id, authn_user.id, async function (job) {
                await syncFromDisk.syncDiskToSqlAsync(inserted_course.path, inserted_course.id, job);
            });
        };
