...
const segmentOptions = [
  // TODO: Add/remove segmentation possibilities once we have real PDF uploads to analyze.
 ...
 * @property {import('./server-jobs').Job} logger
...
class Queue {
   ...
   * @param {import('./server-jobs').Job} logger
   * @returns {Promise<T>}
   */
  static enqueue(task, logger) {
 ...
 * @returns {Promise<ReturnType<import('pdfjs-dist').getDocument>['promise']>} pdf.js wrapper
 */
 ...
 * @param {string} userId authn user id to be stored in file-api alongside file metadata
...
const _uploadPages = async (decodedPdfs, fileMetadata, userId) => {
  ...
  for (let i = 0; i < decodedPdfs.length; i++) {
    for (let j = 0; j < fileMetadata.length; j++) {
      if (
        fileMetadata[j].barcode === decodedPdfs[i].barcode &&
        ...
        const fileId = await fileStore.upload(
          ...
          'pdf_barcode_scan',
          fileMetadata[j].assessment_instance_id,
          fileMetadata[j].instance_question_id,
          userId,
          userId,
          'S3'
        );
 ...
 * @returns {void}
 */
const processScrapPaperPdf = (pdfBuffer, originalName, userId, logger) => {
...
const _processScrapPaperPdf = async (pdfBuffer, originalName, userId, logger) => {
  ...
  try {
    ...
    const uploadedFiles = await _uploadPages(readBarcodedPages, fileMetadata.rows, userId);
...
const _decodeJpegs = async (jpegs, logger) => {
  for (let i = 0; i < jpegs.length; i++) {
    ...
    logger.info(
      `_decodeJpegs() - took ${secondsElapsed}s to decode ${jpegs[i].jpegFilename} / page ${i}`
    );
  }
  return jpegs;
};
 ...
 * @param {ConvertPdfResult} jpeg jpeg file object from arr produced by _convertPdf()
 ...
 * @returns {Promise<string>} a barcode string or null
 */
 ...
 * @returns {Promise<string[]>} contains filepaths referencing jpeg segments
 */
const _segmentJpeg = async (pageNum, jpegFilepath, workingDir, logger, patchSizeNum) => {
  ...
  const filenames = await fs.readdir(segmentsDir);
...
/**
 * @typedef {Object} ConvertPdfResult
 * @property {number} pageNum
 * @property {number} secondsElapsed
 * @property {string} jpegFilename
 * @property {string} jpegFilepath
 ...
 * @param {object} logger
 * @returns {Promise<ConvertPdfResult[]>} jpeg type arr
 */
 ...
 * @param {object} logger
 * @returns {Promise<Array<object>>} Decoded jpeg data
 */
const decodeBarcodes = async (pdfBuffer, workingDir, originalFilename, logger) => {
  const start = Date.now();
  logger.info('decodeBarcodes() - start}');

  const pdf = await readPdf(pdfBuffer);
