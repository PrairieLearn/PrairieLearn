...
functions.saveSubmission = (args, callback) => {
    question.saveSubmission(args.submission, args.variant, args.question, args.course, (err, submission_id) => {
        callback(err, {submission_id});
    });
};

functions.getAndRenderVariant = (args, callback) => {
    question.getAndRenderVariant(args.variant_id, args.locals, (err) => {
        callback(err, args);
    });
};
