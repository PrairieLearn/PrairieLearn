  ...
  handleGradingRequest(grading_job, submission, variant, question, course) {
    ...
    async.waterfall(
      [
        ...
        (callback) => {
          externalGraderCommon.buildDirectory(
            dir,
            submission,
            variant,
            question,
        ...
        (callback) => {
          if (
            question.external_grading_entrypoint.includes('serverFilesCourse')
          ) {
            ...
            exec(
              `chmod +x ${path.join(
                dir,
                question.external_grading_entrypoint.slice(6),
              )}`,
              (err) => {
                if (err) {
                  logger.error(
          ...
          docker.createContainer(
            {
              ...
              Env: Object.entries(question.external_grading_environment).map(
                ([k, v]) => (v === null ? k : `${k}=${v}`),
              ),
              AttachStdout: true,
              AttachStderr: true,
        ...
        (callback) => {
          ...
          sqldb.query(sql.update_job_output, outputParams, (err, _result) => {
            if (
              ERR(err, (err) =>
                logger.error('Error from update_job_output', err),
              )
            )
              return;
            callback(null);
          ...
          if (results.succeeded) {
            fs.readFile(
              ...
              (err, data) => {
                ...
                return callback(
                  null,
                  externalGraderCommon.makeGradingResult(
                    grading_job.id,
                    results,
                  ),
                );
            ...
            return callback(
              null,
              externalGraderCommon.makeGradingResult(grading_job.id, results),
            );
          }
        },
      ],
