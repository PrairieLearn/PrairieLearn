    ...
    if (config.externalGradingUseAws) {
        ...
        async.waterfall([
            ...
            (container, callback) => {
                ...
                sqldb.query(sql.local_update_grading_start_time, params, function(err) {
                    if (ERR(err, callback)) return;
                    externalGradingSocket.gradingLogStatusUpdated(grading_job.id);
                    callback(null, container);
                });
            },
            (container, callback) => {
