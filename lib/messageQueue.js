...
module.exports.sendToGradingQueue = function(grading_log, submission, variant, question, course) {
    ...
    if (config.externalGradingUseAws) {
        ...
        async.series([
            ...
            (callback) => {
                updateJobSubmissionTime(grading_log.id, (err) => {
                    if (ERR(err, callback)) return;
                    callback(null);
                });
            }
...
function buildDirectory(dir, submission, question, course, callback) {
    async.series([
        ...
        (callback) => {
            ...
            console.log('copydir', 'environmentDir', environmentDir, dir);
            fs.copy(environmentDir, dir, (err) => {
                if (ERR(err, callback)) return;
                callback(null);
            });
        },
        (callback) => {
            if (question.external_grading_autograder) {
                ...
                console.log('copydir', 'agDir', agDir, path.join(dir, 'shared'));
                fs.copy(agDir, path.join(dir, 'shared'), (err) => {
                    if (ERR(err, callback)) return;
                    callback(null);
                });
            } else {
        ...
        (callback) => {
            ...
            fs.access(testsDir, fs.constants.R_OK, (err) => {
                if (!err) {
                    console.log('copydir', 'testsDir', testsDir, path.join(dir, 'tests'));
...
function updateJobSubmissionTime(grading_id, callback) {
    var params = {
        grading_log_id: grading_id,
    };
