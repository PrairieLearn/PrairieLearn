...
module.exports.init = function(processGradingResult, callback) {
    ...
    if (config.externalGraderUseAws) {
    ...
    if (!question.external_grading_enabled) {
        ...
        let ret = {
            ...
            grading: {
                ...
                feedback: {
                    succeeded: true,
                    message: "Autograder is not enabled :(",
                },
            },
        };
    ...
    if (config.externalGraderUseAws) {
        ...
        async.series([
            ...
            (callback) => {
                ...
                const params = {
                    Bucket: config.externalGraderJobsS3Bucket,
...
function buildDirectory(dir, submission, question, course, callback) {
    async.series([
        ...
        (callback) => {
            if (question.external_grading_autograder !== undefined) {
...
function createAndRegisterJobDefinition(jobId, question, callback) {
    const params = {
        ...
        containerProperties: {
            image: question.external_grading_image,
            jobRoleArn: config.externalGraderJobRole,
            memory: 512,
            vcpus: 1,
        },
...
function submitGradingJobAWS(jobId, callback) {
    const params = {
        ...
        jobQueue: config.externalGraderJobQueue,
        containerOverrides: {
            environment: [
                {
                    name: "JOB_ID",
                ...
                {
                    name: "S3_JOBS_BUCKET",
                    value: config.externalGraderJobsS3Bucket,
                },
                {
                    name: "S3_RESULTS_BUCKET",
                    value: config.externalGraderResultsS3Bucket,
                },
                {
                    name: "S3_ARCHIVES_BUCKET",
                    value: config.externalGraderArchivesS3Bucket,
                },
                {
                    name: "WEBHOOK_URL",
                    value: config.externalGraderWebhookUrl,
                },
                {
                    name: "CSRF_TOKEN",
