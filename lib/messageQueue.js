...
module.exports.sendToGradingQueue = function(grading_job, submission, variant, question, course) {
    ...
    if (config.externalGradingUseAws) {
        ...
        async.series([
            ...
            (callback) => {
                createAndRegisterECSTaskDefinition(grading_job.id, question, (err, data) => {
                    if (ERR(err, callback)) return;
                    logger.info(`Successfully registered task defition for job ${grading_job.id} with AWS ECS`);
                    logger.info(data);
                    callback(null);
                });
            },
            (callback) => {
                runECSTask(grading_job.id, question, (err, data) => {
                    if (ERR(err, callback)) return;
                    logger.info(`Successfully submitted grading job ${grading_job.id} to AWS ECS`);
                    logger.info(data);
                    callback(null);
                });
            },
...
function getTaskDefinitionFamilyForQuestion(question) {
    // For now, this just uses the image name as the task definition name
    // This will create a new revision for every job using a specified
    return SHA256(question.external_grading_image).toString();
}

function createAndRegisterECSTaskDefinition(jobId, question, callback) {
    const params = {
        ...
        family: getTaskDefinitionFamilyForQuestion(question),
        taskRoleArn: config.externalGradingJobRole,
    };

    const ecs = new AWS.ECS();
    ecs.registerTaskDefinition(params, callback);
}

function runECSTask(jobId, question, callback) {
    logger.info(`Using cluster "${config.externalGradingECSCluster}"`);
    const params = {
        cluster: config.externalGradingECSCluster,
        taskDefinition: getTaskDefinitionFamilyForQuestion(question),
        overrides: {
            containerOverrides: [
                {
                    name: 'grade',
