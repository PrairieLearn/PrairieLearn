    ...
    if (config.autograderUseAws) {
        ...
        if (question.autograding_enabled != true) {
          ...
          // Send the grade out for processing and display
            ...
            // Now that we've built up our directory, let's zip it up to be sent
            // off to S3

            let s3 = new AWS.S3();
            s3.createBucket({Bucket: config.autograderJobsS3Bucket}, (err, data) => {
                if (err) {
                    logger.error('Unable to create S3 bucket for autograder jobs');
                    logger.error(err);
                    return;
                }
