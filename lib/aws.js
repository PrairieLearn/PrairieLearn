...
async function metadataServiceRequest(path) {
...
async function getInstanceMetadata() {
  ...
  if (!cachedInstanceMetadata) {
    const document = await metadataServiceRequest('/latest/dynamic/instance-identity/document');
...
module.exports.loadConfigSecrets = async function () {
  ...
  const hostname = await metadataServiceRequest('/latest/meta-data/local-hostname');
 ...
 *
 * @param {string} s3Bucket - The S3 bucket name.
 * @param {string} s3Prefix - The S3 source path of the base directory for searching.
 * @param {string} pattern - The pattern of files to be retrieved, using minimatch rules.
 * @return {string[]}
 */
module.exports.listFilesFromS3Async = async function (s3Bucket, s3Prefix, pattern) {
  const s3 = new AWS.S3();
  let params = {
    Bucket: s3Bucket,
    Prefix: s3Prefix,
  ...
  do {
    const data = await util.promisify(s3.listObjects)(params);
    files = files.concat(data.Contents.map((entry) => entry.Key).filter(minimatch.filter(pattern)));
    params.ContinuationToken = data.NextContinuationToken;
  } while (data.IsTruncated);

  return files;
};
module.exports.listFilesFromS3 = util.callbackify(module.exports.listFilesFromS3Async);
