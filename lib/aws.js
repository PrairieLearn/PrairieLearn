...
module.exports.downloadFromS3Async = async function (s3Bucket, s3Path, localPath, watcherUpdateQueue=null, forWorkspace=true) {
    ...
    if (localPath.endsWith('/')) {
        ...
        if (forWorkspace) await workspaceHelper.changeWorkspaceFileOwner(localPath);
        ...
        if (forWorkspace) await workspaceHelper.changeWorkspaceFileOwner(path.dirname(localPath));
    }
    ...
    return new Promise((resolve, reject) => {
        ...
        s3Stream.pipe(fileStream).on('error', function(err) {
            ...
            if (forWorkspace) {
                workspaceHelper.changeWorkspaceFileOwnerSync(localPath, (err) => {
