...
module.exports.init = async function () {
  ...
  if (process.env.AWS_ENDPOINT) {
    config.awsServiceGlobalOptions.endpoint = new AWS.Endpoint(
...
module.exports.loadConfigSecrets = async function () {
  ...
  const document = await request_promise(
  ...
  const secret_value = await secretsManager
...
module.exports.uploadToS3Async = async function (
  ...
  if (localPath) {
    logger.verbose(
      `Uploaded localPath=${localPath} to s3://${s3Bucket}/${s3Path}`,
    );
  } else {
    ...
    await async.each(files, async (file) => {
      ...
      if (stat.isFile()) {
        ...
        try {
          ...
          logger.verbose(
            `Uploaded file ${localFilePath} to s3://${s3Bucket}/${s3FilePath}`,
          );
        } catch (err) {
          logger.verbose(
        ...
        const s3DirPath = s3FilePath.endsWith('/')
          ? s3FilePath
          : `${s3FilePath}/`;
        try {
          await s3
            ...
            .promise();
          logger.verbose(
            `Uploaded directory ${localFilePath} to s3://${s3Bucket}/${s3DirPath}`,
          );
        } catch (err) {
          logger.verbose(
...
module.exports.uploadDirectoryToS3 = util.callbackify(
  this.uploadDirectoryToS3Async,
);
 ...
 */
module.exports.downloadFromS3Async = async function (
  s3Bucket,
  s3Path,
  localPath,
  options = {},
  ...
  if (localPath.endsWith('/')) {
    debug(
      `downloadFromS3: bypassing S3 and creating directory localPath=${localPath}`,
    );
  ...
  debug(
    `downloadFromS3: creating containing directory for file localPath=${localPath}`,
  );
 ...
 */
module.exports.deleteFromS3Async = async function (
  s3Bucket,
  s3Path,
  isDirectory = false,
) {
