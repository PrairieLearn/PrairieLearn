        ...
        await async.each(files, async (file) => {
            ...
            if (stat.isFile()) {
                ...
                try {
                    ...
                    logger.verbose(`Did not sync file ${localFilePath} to $s3://{s3Bucket}/${s3FilePath}: ${err}`);
                }
                ...
                try {
                    ...
                    logger.verbose(`Did not sync directory ${localFilePath} to $s3://{s3Bucket}/${s3DirPath}: ${err}`);
                }
