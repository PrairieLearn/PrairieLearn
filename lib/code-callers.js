...
/** @typedef {{ (err: Error | null, caller: CodeCaller): void, __load_uuid: string }} InternalCallback */

/**
 * FIFO queue for jobs/callbacks waiting for an available pythonCaller
 * @type {Array<[string, InternalCallback]>}
 */
...
module.exports = {
  init() {
    ...
    cancelWarmup = module.exports._warmUpWorkers();
  },
  ...
  returnPythonCaller(pc, callback) {
    ...
    pc.restart((err, success) => {
      ...
      if (needsFullRestart) {
        const existingIndex = pythonCallers.findIndex((p) => p.uuid === pc.uuid);
        ...
        pythonCallers[existingIndex] = pc;
        load.startJob('python', pc.uuid);
      }
  ...
  /**
   * Calls `ensureChild()` on each caller in the pool, waiting between each one.
   *
   * Returns a function that can be used to cancel warmup.
   */
  _warmUpWorkers() {
    ...
    const startNextCaller = () => {
      ...
      const caller = pythonCallers[iCaller];
      debug(`_warmUpWorkers(): running ensureChild() for worker number ${iCaller}`);
      ...
      caller
        .ensureChild()
        .catch((err) => logger.error(`Failed to ensure child for worker number ${iCaller}`, err));
