  ...
  regradeAssessmentInstance(
    ...
    sqldb.queryOneRow(
      ...
      function (err, result) {
        ...
        const assessment_instance_label =
          result.rows[0].assessment_instance_label;
        let jobInfo;
        ...
        var options = {
          ...
          description:
            'Regrade ' + assessment_instance_label + ' for ' + jobInfo,
        };
        serverJobs.createJobSequence(options, function (err, job_sequence_id) {
          ...
          serverJobs.createJob(jobOptions, function (err, job) {
            ...
            job.verbose(
              'Regrading ' + assessment_instance_label + ' for ' + jobInfo,
            );
              ...
              function (err, result) {
                if (ERR(err, function () {})) {
                  ...
                  if (result.rowCount != 1) {
                    job.fail(
                      new Error('Incorrect rowCount: ' + result.rowCount),
                    );
                  }
      ...
      function (err, result) {
        ...
        serverJobs.createJobSequence(options, function (err, job_sequence_id) {
          ...
          serverJobs.createJob(jobOptions, function (err, job) {
            ...
            job.verbose(
              'Regrading all assessment instances for ' + assessment_label,
            );
              ...
              function (err, result) {
                ...
                async.eachSeries(
                  ...
                  function (err) {
                    ...
                    job.verbose('Regrading complete');
                    job.verbose(
                      'Number of assessment instances updated: ' +
                        updated_count,
                    );
                    if (error_count > 0) {
                      ...
                      job.fail(
                        new Error(
                          'Errors occurred while regrading, see output for details',
                        ),
                      );
                    } else {
                      job.succeed();
