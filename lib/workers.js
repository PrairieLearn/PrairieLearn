...
module.exports = {
  ...
  async withPythonCaller(fn) {
    return new Promise((resolve, reject) => {
      module.exports.getPythonCaller(async (err, pc) => {
        if (err) {
          reject(err);
          return;
        }
        ...
        // Return the python caller; only after that should we resolve or reject.
        module.exports.returnPythonCaller(pc, (pcErr) => {
          if (pcErr ?? fnErr) {
            reject(pcErr ?? fnErr);
            return;
          }
