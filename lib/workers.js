...
// These are used in 'native' and 'internal' execution modes, when workers can be used for any course
...
// This is used in 'container' execution mode, when each course has a dedicated worker
let courseCallers = {}; // Maps a course directory to its caller and a queue of waiting callbacks
...
module.exports.init = function() {
    ...
    if (workersExecutionMode === 'native' || workersExecutionMode === 'container') {
        CodeCaller = DockerCaller;
    } else if (workersExecutionMode === 'internal') {
        CodeCaller = PythonCaller;
    }
    if (config.workersExecutionMode === 'container') {
        debug('using course allocation strategy');
        const numWorkers = config.courseDirs.length;
        for (let i = 0; i < numWorkers; i++) {
            // The full resolved path is what's stored in the database,
            // so we normalize to that here so we can match against it
            const coursePath = path.resolve(config.courseDirs[i]);
            const pc = new CodeCaller();
            pc.number = i;
            pc.__course_path = coursePath;
            courseCallers[coursePath] = {
                pc,
                available: true,
                waitingCallbacks: [],
            };
        ...
        debug('using random allocation strategy');
        let numWorkers = config.workersCount;
        if (numWorkers == null) {
...
module.exports.finish = function(callback) {
    ...
    const testFinished = () => {
        if (config.workersExecutionMode === 'container') {
            const numAvailable = Object.values(courseCallers).filter(c => c.available === true).length;
            if (numAvailable < config.courseDirs.length) {
                ...
                Object.values(courseCallers).forEach(c => c.done());
                courseCallers = {};
...
module.exports.getPythonCaller = function(course, callback) {
    debug('getPythonCaller()');
    if (config.workersExecutionMode === 'container') {
        const coursePath = course.path;
        const courseCaller = courseCallers[coursePath];
        if (courseCaller.available) {
            courseCaller.available = false;
            const { pc } = courseCaller;
            load.endJob('python_worker_idle', pc.uuid);
            debug(`getPythonCaller(): got ${pc.number}`);
            load.startJob('python_worker_active', pc.uuid);
            pc.prepareForCourse(course.path, (err) => {
            ...
            courseCaller.waitingCallbacks.push([course, callback]);
        }
    } else {
        ...
        if (availablePythonCallers.length > 0) {
            ...
            if (config.workerUseQueue) {
                ...
                callback.__load_uuid = uuidv4();
                load.startJob('python_callback_waiting', callback.__load_uuid);
                waitingCallbacks.push([course, callback]);
            } else {
...
module.exports.returnPythonCaller = function(pc, callback) {
    ...
    pc.restart((err, success) => {
        ...
        if (config.workersExecutionMode === 'container') {
            const coursePath = pc.__course_path;
            const courseCaller = courseCallers[coursePath];
            ...
            if (needsFullRestart) {
                ...
                pc.number = i;
                pc.__course_path = coursePath;
                courseCallers[coursePath].pc = pc;
            }

            // by this point either the restart succeeded or we have a brand new PythonCaller
            if (courseCaller.waitingCallbacks.length > 0) {
                debug('returnPythonCaller(): passing to a waiting callback');
                const [course, cb] = courseCaller.waitingCallbacks[0];
                courseCaller.waitingCallbacks = courseCaller.waitingCallbacks.slice(1);
                ...
                debug('returnPythonCaller(): marking available');
                load.startJob('python_worker_idle', pc.uuid);
                courseCaller.available = true;
            }
        } else {
            if (needsFullRestart) {
                const i = pc.number;
...
module.exports._warmUpWorkers = function() {
    ...
    let callers;
    if (config.workersExecutionMode === 'container') {
        callers = Object.values(courseCallers).map(c => c.pc);
    } else {
        callers = availablePythonCallers;
    }
    let iCaller = 0;
    ...
    const startNextCaller = () => {
        ...
        for (let i = 0; i < availablePythonCallers.length; i++) {
            if (callers[i].number == iCaller) {
                debug(`_warmUpWorkers(): running availablePythonCallers[${i}].ensureChild() for worker number ${iCaller}`);
                ...
                callers[i].ensureChild().catch(err => logger.error(err));
                break;
            }
        }
        iCaller++;
        if (iCaller < callers.length) {
