...
module.exports.init = function() {
    ...
    for (let i = 0; i < numWorkers; i++) {
        const pc = new CodeCaller({ questionTimeoutMilliseconds: config.questionTimeoutMilliseconds });
        pc.number = i;
    ...
    const startNextCaller = () => {
        ...
        for (let i = 0; i < availablePythonCallers.length; i++) {
            if (callers[i].number == iCaller) {
                ...
                callers[i].ensureChild().catch(err => logger.error('Failed to ensure child for worker', err));
                break;
            }
        }
        iCaller++;
