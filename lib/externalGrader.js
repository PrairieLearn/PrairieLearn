...
function updateJobStartTime(grading_job_id, startTime, callback) {
    var params = {
        grading_job_id: grading_job_id,
        grading_started_at: startTime,
    };
    sqldb.query(sql.update_grading_started_time, params, (err, _result) => {
        if (ERR(err, callback)) return;
        externalGradingSocket.gradingJobStatusUpdated(grading_job_id);
        callback(null);
    });
