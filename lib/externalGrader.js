...
module.exports.beginGradingJob = function (grading_job_id, callback) {
  ...
  sqldb.queryOneRow(sql.select_grading_job_info, params, (err, result) => {
    ...
    const { grading_job, submission, variant, question, course } =
      result.rows[0];

    module.exports._beginGradingJob(
      grading_job,
      submission,
      variant,
      question,
...
module.exports._beginGradingJob = function (
  grading_job,
  submission,
  variant,
  question,
  ...
  gradeRequest.on('received', (receivedTime) => {
    ...
    updateJobReceivedTime(grading_job.id, receivedTime, (err) => {
      if (
        ERR(err, (err) =>
          logger.errror('Error updating job received time', err),
        )
      )
        return;
    });
