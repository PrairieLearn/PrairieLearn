...
class CodeCallerContainer {
  ...
  async _createAndAttachContainer() {
    ...
    // Wait for the health check to pass before continuing. In production
    // environments, it can take on the order of dozens of seconds for the
    // image to start completely as we wait for EBS to load all the image's data.
    ...
    for await (const event of eventStream) {
      const msg = JSON.parse(event.toString('utf8'));
      if (msg.status === 'health_status: healthy') {
