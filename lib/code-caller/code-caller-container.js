...
class CodeCallerContainer {
  ...
  async call(type, directory, file, fcn, args) {
    ...
    this.callId = uuidv4();
    this.debug(`call ID: ${this.callId}`);

    const callData = { type, directory, file, fcn, args, callId: this.callId };
  ...
  _callIsFinished() {
    ...
    try {
      ...
      if (data.id != null && data.id !== this.callId) {
        err = new Error(`Mismatched call ID: expected ${this.callId}, got ${data.id}`);
      } else if (data.error) {
