...
class CodeCallerNative {
  ...
  constructor(
    ...
    this.outputData = '';
    this.outputRestart = '';
    this.socketData = Buffer.alloc(0);
  ...
  _handleSocketData(data) {
    ...
    this._checkState([IN_CALL, EXITING]);
    if (this.state === IN_CALL) {
      console.log('Got socket data with length', data.length);
      this.socketData = Buffer.concat([this.socketData, data]);
      const length = this.socketData.readUInt32BE(0);
      if (this.socketData.length >= length + 4) {
        this._callIsFinished();
      }
    }
  ...
  _callIsFinished() {
    ...
    try {
      if (this.outputData) {
        ...
        data = JSON.parse(this.socketData.subarray(4).toString('utf-8'));
      }
    } catch (e) {
