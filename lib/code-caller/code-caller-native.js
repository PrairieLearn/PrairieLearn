...
class CodeCallerNative {
  ...
  constructor(
    ...
    /** @type {CodeCallerNativeChildProcess} */
    this.child = null;
    this.callback = null;
    this.timeoutID = null;
   ...
   * @param {string} fcn
   * @param {any[]} args
   * @param {import('./code-caller-shared').CallOptions | undefined} [options]
   * @returns {Promise<{ result: any, output: string }>}
   */
  call(type, directory, file, fcn, args, options) {
    ...
    this.timeoutID = setTimeout(
      ...
      options?.timeout ?? this.options.questionTimeoutMilliseconds
    );
  ...
  async restart(options) {
    ...
    if (this.state === CREATED) {
      ...
      const { result } = await this.call('restart', null, null, 'restart', [], options);
      ...
      return new Promise((resolve, reject) => {
        ...
        this.timeoutID = setTimeout(this._restartTimeout.bind(this), options.timeout ?? 1000);
  ...
  _startChild() {
    ...
    this.child = /** @type {CodeCallerNativeChildProcess} */ (
