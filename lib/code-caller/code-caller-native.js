...
class CodeCallerNative {
  ...
  async call(type, directory, file, fcn, args) {
    ...
    if (this.span?.name.startsWith('elementFunction')) {
      this.span.addEvent('WRITING_DATA');
    }
  ...
  _handleStdio3Data(data) {
    ...
    if (!this.previousIteration) {
      this.previousIteration = Date.now();
    }
    if (this.state === IN_CALL) {
      ...
      console.log(
        `it has been ${Date.now() - this.previousIteration}ms since end of last iteration`
      );
      this.previousIteration = Date.now();
    }
