...
/** RandomGenerator constructor.
    ...
    @param {String} seed (Optional, default uses current time) Seed value for the generator, must be an integer.
*/
...
RandomGenerator.prototype.randBool = function(trueProb) {
    ...
    if (this._mersenne.genrand_real2() < trueProb)
        return true;
    return false;
};
...
RandomGenerator.prototype.randSign = function(posProb) {
    ...
    if (this._mersenne.genrand_real2() < posProb)
        return 1;
    return -1;
};
...
RandomGenerator.prototype.randCategoryChoices = function(nSel, cats) {
    ...
    _(extras).each(function(i) {sels[i]++;});
    return sels;
};
...
RandomGenerator.prototype.randFunc = function() {
    ...
    case 1: return {fcn: 'poly', data: this.randPoly()};
    case 2: return {fcn: 'exp', data: this.randExp()};
    case 3: return {fcn: 'trig', data: this.randTrig()};
    }
};
...
RandomGenerator.prototype.solveDifficulty = function(answerInd, givenInds, lhs) {
    var unknownInds = _.chain(_.range(0, lhs[0].length)).difference(givenInds).without(answerInd).value();

    if (_(lhs).some(function(row) {
        return row[answerInd] !== 0 && _(unknownInds).every(function(i) {return row[i] === 0;});
    })) {
        return 1;
    }

    if (_(unknownInds).some(function(i) {
        var extraUnknownInds = _(unknownInds).without(i);
        return _(lhs).some(function(row) {
            return row[answerInd] !== 0 && row[i] !== 0 && _(extraUnknownInds).every(function(i) {return row[i] === 0;});
...
RandomGenerator.prototype.maxSolveDifficulty = function(answerInds, givenInds, lhs) {
    var that = this;
    return _.chain(answerInds).map(function(answerInd) {
...
RandomGenerator.prototype.chooseGiven = function(difficulty, varGroups, answerGroup, lhs) {
    ...
    do {
        givenGroups = [];
        let remainingGroups = _(_.range(0, varGroups.length)).without(answerGroup);
        givenInds = [];
