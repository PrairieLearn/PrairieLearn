...
module.exports.sendSlackAlmaMessage = (msg, channel, options, callback) => {
    ...
    request(default_options, (err, res, body) => {
        if (ERR(err, callback)) return;
        if (!body.ok) {
            callback(error.makeWithData(`Error sending message to ${default_options.json.channel}`, {body}));
            return;
        }
        callback(null, res, body);
    });
...
module.exports.sendCourseRequestMessage = (msg, callback) => {
    module.exports.sendSlackAlmaMessage(msg, config.secretSlackCourseRequestChannel, {}, (err, res) => {
        if (ERR(err, callback)) return;
        callback(null, res);
    });
};
