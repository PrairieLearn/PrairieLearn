...
module.exports.processQuestion = function(html) {
    return html.replace(regex, (_match, originalContents) => {
        ...
        // By default, all text gets wrapped with <p>. If the input text only
        // consists of one line, we'll assume this is trying to be used in
        // an "inline" fashion, like <pl-answer><markdown>**Hi**</markdown></pl-answer>.
        // In that case' we'll remove the surrounding <p> tags.
        const lineCount = originalContents.split('\n').length;
        let { contents } = res;
        if (lineCount === 1 && contents.startsWith('<p>') && contents.endsWith('</p>')) {
