...
const sql = require('../prairielib/lib/sql-loader').loadSqlEquiv(__filename);
...
class LoadEstimator {
  ...
  startJob(id) {
    ...
    if (_.has(this.currentJobs, id)) {
      logger.error(
        `load.startJob(): ${this.jobType} id already running: ${id}`,
      );
    }
  ...
  close() {
    ...
    if (this.timeoutID != null) {
      debug(
        `LoadEstimator.close(): jobType = ${this.jobType}, clearing timeout`,
      );
  ...
  _getAndResetLoadEstimate() {
    debug(
      `LoadEstimator._getAndResetLoadEstimate(): jobType = ${this.jobType}`,
    );
    this._addIntegratedLoad();
    const nowMS = Date.now();
  ...
  _reportLoad() {
    ...
    sqldb.query(sql.insert_load, params, (err) => {
      ...
      debug(
        `LoadEstimator._reportLoad(): jobType = ${
          this.jobType
        }, scheduling next call for ${config.reportIntervalSec * 1000} ms`,
      );
      this.timeoutID = setTimeout(
        this._reportLoad.bind(this),
  ...
  _warnOldJobs() {
    ...
    _.forEach(this.currentJobs, (info, id) => {
      if (
        nowMS - info.startMS > config.maxResponseTimeSec * 1000 &&
        !info.warned
      ) {
        const details = {
        ...
        logger.error(
          `load._warnOldJobs(): job exceeded max response time`,
          details,
        );
        info.warned = true;
      }
...
module.exports = {
  initEstimator(jobType, maxJobCount, warnOnOldJobs) {
    ...
    estimators[jobType] = new LoadEstimator(
      jobType,
      maxJobCount,
      warnOnOldJobs,
    );
  ...
  startJob(jobType, id, maxJobCount) {
    debug(
      `startJob(): jobType = ${jobType}, id = ${id}, maxJobCount = ${maxJobCount}`,
    );
    if (!_.has(estimators, jobType)) {
