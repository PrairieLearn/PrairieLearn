...
module.exports.describe = function (options, callback) {
  ...
  if (!options.databaseName)
    return callback(
      new Error('you must specify a database name with databaseName'),
    );
  if (
    ...
    return callback(
      new Error(`'${options.outputFormat}' is not a valid output format`),
    );
  }
      ...
      (callback) => {
        ...
        sqldb.query(sql.get_tables, [], (err, results) => {
          ...
          if (options.ignoreTables && _.isArray(options.ignoreTables)) {
            tables = _.filter(
              tables,
              (table) => options.ignoreTables.indexOf(table.name) == -1,
            );
          }
                  ...
                  sqldb.query(
                    ...
                    (err, results) => {
                      ...
                      const rows = _.filter(results.rows, (row) => {
                        return (
                          (ignoreColumns[table.name] || []).indexOf(row.name) ==
                          -1
                        );
                      });
                      ...
                      if (options.outputFormat === 'string') {
                        ...
                        output.tables[table.name] += rows
                          .map((row) => {
                            var rowText = formatText(
                              `    ${row.name}`,
                              colors.bold,
                            );
                            rowText +=
                              ':' + formatText(` ${row.type}`, colors.green);
                            if (row.notnull) {
                    ...
                    (err, results) => {
                      ...
                      if (options.outputFormat === 'string') {
                        ...
                        output.tables[table.name] += results.rows
                          .map((row) => {
                            const using = row.indexdef.substring(
                              row.indexdef.indexOf('USING '),
                            );
                            var rowText =
                              formatText(`    ${row.name}`, colors.bold) + ':';
                            // Primary indexes are implicitly unique, so we don't need to
                            // capture that explicitly.
                            if (row.isunique && !row.isprimary) {
                              if (
                                !row.constraintdef ||
                                row.constraintdef.indexOf('UNIQUE') === -1
                              ) {
                  ...
                  sqldb.query(
                    ...
                    (err, results) => {
                      ...
                      if (options.outputFormat === 'string') {
                        ...
                        output.tables[table.name].foreignKeyConstraints =
                          results.rows;
                      }
                      callback(null);
                    },
                  ...
                  sqldb.query(
                    ...
                    (err, results) => {
                      ...
                      if (
                        ...
                        _.isArray(options.ignoreTables)
                      ) {
        ...
        sqldb.query(sql.get_enums, [], (err, results) => {
          ...
          rows.forEach((row) => {
            if (options.outputFormat == 'string') {
              ...
              output.enums[row.name] = formatText(
                values.join(', '),
                colors.gray,
              );
            } else {
              output.enums[row.name] = pgArray
