...
module.exports.describe = function (options, callback) {
  ...
  async.series(
    [
                      ...
                      output.tables[table.name] += results.rows
                        .map((row) => {
                          ...
                          rowText += row.constraintdef
                            ? formatText(` ${row.constraintdef}`, colors.green)
                            : '';
      ...
      (callback) => {
        // Get the names of the views
        sqldb.query(sql.get_views, [], (err, results) => {
          if (ERR(err, callback)) return;
          let views = results.rows;

          // Filter out ignored views (uses same option as tables)
          if (options.ignoreTables && _.isArray(options.ignoreTables)) {
            views = _.filter(views, (view) => options.ignoreTables.indexOf(view.viewname) === -1);
          }

          if (options.outputFormat === 'string') {
            views.forEach((view) => (output.views[view.viewname] = view.definition));
          } else {
            views.forEach((view) => (output.views[view.viewname] = view.definition));
          }
      ...
      (callback) => {
        ...
        if (options.outputFormat === 'string') {
          ...
          output.views = _.mapValues(output.views, (item) => item + '\n');
        }
        callback(null);
      },
      (callback) => {
