...
module.exports = {
  generateToken: function (data, secretKey) {
    ...
    var signature = base64url.encode(
      hmacSha256(checkString, secretKey).toString(),
    );
    debug(
  ...
  getCheckedData: function (token, secretKey, options) {
    ...
    var checkSignature = base64url.encode(
      hmacSha256(checkString, secretKey).toString(),
    );
    if (checkSignature !== tokenSignature) {
      debug(
        `getCheckedData(): FAIL - signature mismatch: checkSig=${checkSignature} != tokenSig=${tokenSignature}`,
      );
      return null;
    }
    ...
    if (options.maxAge != null) {
      ...
      try {
        ...
        debug(
          `getCheckedData(): FAIL - could not parse date: ${tokenDateString}`,
        );
        return null;
      }
      var currentTime = options.req_date
        ? Date.parse(options.req_date)
        : Date.now();
      var elapsedTime = currentTime - tokenDate.getTime();
      if (elapsedTime > options.maxAge) {
        debug(
          `getCheckedData(): FAIL - too old: elapsedTime=${elapsedTime} > maxAge=${options.maxAge}`,
        );
        return null;
      }
    }
    ...
    try {
      ...
      debug(
        `getCheckedData(): FAIL - could not base64 decode: ${tokenDateString}`,
      );
      return null;
    }
    try {
