...
function checkInfoValid(idName, info, infoFile, courseInfo, logger, cache) {
        ...
        if (_(info).has('allowAccess')) {
            _(info.allowAccess).forEach(function(rule) {
                ...
                if (('role' in rule) && (rule.role != 'Student')) {
                    logger.warn(`WARNING: ${infoFile}: access rule with role ${rule.role} will be ignored`);
                }
            });
        }
        if (myError) {
    ...
    if (idName == 'ciid') {
        ...
        if (_(info).has('userRoles')) {
            logger.warn(`WARNING: ${infoFile}: userRoles will be ignored - should set course and course instance permissions on course admin access page`);
        }
    }
