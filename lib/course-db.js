...
module.exports.loadInfoDB = function(db, idName, parentDir, infoFilename, defaultInfo, schema, optionSchemaPrefix, courseInfo, logger, callback) {
    var that = this;
...
module.exports.loadFullCourse = function(courseDir, logger, callback) {
    ...
    async.series([
        ...
        function(callback) {
            const questionLoadStart = new Date();
            that.loadInfoDB(course.questionDB, 'qid', course.courseInfo.questionsDir, 'info.json', defaultQuestionInfo, schemas.infoQuestion, 'questionOptions', course.courseInfo, logger, function(err) {
                logger.info(`Question info elapsed time: ${(new Date()) - questionLoadStart}ms`);
                if (ERR(err, callback)) return;
                callback(null);
            });
        },
        function(callback) {
            that.loadInfoDB(course.courseInstanceDB, 'ciid', course.courseInfo.courseInstancesDir, 'infoCourseInstance.json', defaultCourseInstanceInfo, schemas.infoCourseInstance, null, course.courseInfo, logger, function(err) {
                if (ERR(err, callback)) return;
                callback(null);
            });
        },
    ], function(err) {
        logger.info(`Elapsed time: ${(new Date()) - startTime}ms`);
        if (ERR(err, callback)) return;
        async.forEachOf(course.courseInstanceDB, function(courseInstance, courseInstanceDir, callback) {
            ...
            fs.lstat(assessmentsDir, function(err, stats) {
                ...
                // ENOTDIR: `assessments` is not a directory
                    ...
                    that.loadInfoDB(courseInstance.assessmentDB, 'tid', assessmentsDir, 'infoAssessment.json', defaultAssessmentInfo, schemas.infoAssessment, null, course.courseInfo, logger, function(err) {
                        if (ERR(err, callback)) return;
                        callback(null);
                    });
                }
