...
module.exports.loadFullCourse = function(courseDir, logger, callback) {
    ...
    async.series([
        ...
        function(callback) {
            loadInfoDB('qid', course.courseInfo.questionsDir, 'info.json', defaultQuestionInfo, schemas.infoQuestion, 'questionOptions', course.courseInfo, logger, function(err, questionDB) {
                if (ERR(err, callback)) return;
        ...
        function(callback) {
            loadInfoDB('ciid', course.courseInfo.courseInstancesDir, 'infoCourseInstance.json', defaultCourseInstanceInfo, schemas.infoCourseInstance, null, course.courseInfo, logger, function(err, courseInstanceDB) {
                if (ERR(err, callback)) return;
        ...
        async.forEachOf(course.courseInstanceDB, function(courseInstance, courseInstanceDir, callback) {
            ...
            fs.lstat(assessmentsDir, function(err, stats) {
                ...
                // ENOTDIR: `assessments` is not a directory
                    ...
                    loadInfoDB('tid', assessmentsDir, 'infoAssessment.json', defaultAssessmentInfo, schemas.infoAssessment, null, course.courseInfo, logger, function(err, assessmentDB) {
                        if (ERR(err, callback)) return;
