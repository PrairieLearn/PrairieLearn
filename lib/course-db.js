...
function loadInfoDB(db, idName, parentDir, infoFilename, defaultInfo, schema, optionSchemaPrefix, courseInfo, logger, recursive, callback) {
        ...
        fs.readdir(path.join(parentDir, relativeDir), function(err, files) {
            ...
            async.each(files, function(dir, callback) {
                ...
                jsonLoad.readInfoJSON(infoFile, schema, function(err, info) {
                    ...
                    if (recursive && err && err.code && (err.code == 'ENOENT') && err.path === infoFile) {
                        /* This is a directory, lets try to recurse into it. */
                        walk(path.join(relativeDir, dir), callback);
                        return;
                    }
                    if (ERR(err, callback)) return;
                ...
                if (ERR(err, callback)) return;
                logger.debug('successfully loaded info from ' + path.join(parentDir, relativeDir) + ', number of items = ' + _.size(db));
                callback(null);
            });
        });
    };
        ...
        function(callback) {
            ...
            fs.lstat(course.courseInfo.questionsDir, function(err, stats) {
                ...
                // ENOTDIR: `questions` is not a directory
                    ...
                    loadInfoDB(course.questionDB, 'qid', course.courseInfo.questionsDir, 'info.json', defaultQuestionInfo, schemas.infoQuestion, 'questionOptions', course.courseInfo, logger, true, function(err) {
                        if (ERR(err, callback)) return;
                        callback(null);
                    });
                }
        ...
        function(callback) {
            ...
            fs.lstat(course.courseInfo.courseInstancesDir, function(err, stats) {
                ...
                // ENOTDIR: `questions` is not a directory
                    ...
                    loadInfoDB(course.courseInstanceDB, 'ciid', course.courseInfo.courseInstancesDir, 'infoCourseInstance.json', defaultCourseInstanceInfo, schemas.infoCourseInstance, null, course.courseInfo, logger, false, function(err) {
                        if (ERR(err, callback)) return;
                        callback(null);
                    });
                }
        ...
        async.forEachOf(course.courseInstanceDB, function(courseInstance, courseInstanceDir, callback) {
            ...
            fs.lstat(assessmentsDir, function(err, stats) {
                ...
                // ENOTDIR: `assessments` is not a directory
                    ...
                    loadInfoDB(courseInstance.assessmentDB, 'tid', assessmentsDir, 'infoAssessment.json', defaultAssessmentInfo, schemas.infoAssessment, null, course.courseInfo, logger, false, function(err) {
                        if (ERR(err, callback)) return;
                        callback(null);
                    });
                }
