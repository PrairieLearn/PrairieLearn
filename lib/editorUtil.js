...
module.exports.getDetailsForFile = function(filePath) {
    ...
    } else if (pathComponents.length >= 5 && pathComponents[0] == 'courseInstances' && pathComponents[2] === 'assessments' && pathComponents[pathComponents.length - 1] === 'infoAssessment.json') {
        const ciid = pathComponents[1];
        const aid = pathComponents.slice(3, pathComponents.length - 1).join(path.posix.sep);
