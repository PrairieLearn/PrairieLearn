...
module.exports.getDetailsForFile = function (filePath) {
  ...
  if (pathComponents.length === 1 && pathComponents[0] === 'infoCourse.json') {
    ...
    const ciid = pathComponents
      .slice(1, pathComponents.length - 1)
      .join(path.posix.sep);
 ...
 */
module.exports.getErrorsAndWarningsForFilePath = async function (
  courseId,
  filePath,
) {
