...
module.exports.getDetailsForFile = function(filePath) {
    ...
    if (pathComponents.length === 1 && pathComponents[0] === 'infoCourse.json') {
        return { type: 'course' };
    } else if (pathComponents.length >= 3 && pathComponents[0] === 'courseInstances' && pathComponents[pathComponents.length - 1] === 'infoCourseInstance.json') {
        const ciid = pathComponents.slice(1, pathComponents.length - 1).join(path.posix.sep);
        return { type: 'courseInstance', ciid };
    } else if (pathComponents.length >= 3 && pathComponents[0] === 'questions' && pathComponents[pathComponents.length - 1] === 'info.json') {
        const qid = pathComponents.slice(1, pathComponents.length - 1).join(path.posix.sep);
        return { type: 'question', qid };
    } else if (pathComponents.length >= 5 && pathComponents[0] === 'courseInstances' &&
        ...
        const aid = pathComponents.slice(assessment_index + 1, pathComponents.length - 1).join(path.posix.sep);
