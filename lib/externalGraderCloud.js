...
const externalGraderRabbitMq = require('./externalGraderRabbitMq');
...
function sendJobToSqsQueue(message, config, callback) {
    ...
    async.series([
        (callback) => {
            if (QUEUE_URL) {
                ...
                const params = {
                    QueueName: config.externalGradingQueueName,
                };
...
function sendJobToRabbitMqQueue(message, config, callback) {
    externalGraderRabbitMq.sendToQueue(config.externalGradingQueueName, message, callback);
}
...
function sendJobToQueue(jobId, question, config, callback) {
    ...
    switch (config.externalGradingQueueType) {
        case 'sqs':
            return sendJobToSqsQueue(message, config, callback);
        case 'rabbitmq':
            return sendJobToRabbitMqQueue(message, config, callback);
        default:
            return callback(new Error(`Unknown queue type: ${config.externalGradingQueueType}`));
    }
}
