...
class PLConstantValue(NumberSymbol, metaclass=Singleton):

    def __new__(self, sympy_impl=None, latex_symbol=None, unicode_symbol=None, text_symbol=None, docstr=""):
        ...
        self.sympy_impl = sympy.sympify(sympy_impl).evalf()
    ...
    def __doc__(self):
        docstr = "%s ≈ %1.5f (%s)" % (self.unicode_symbol, self._eval_evalf(nprec=6), self.docstr)
        return docstr
    ...
    def _eval_expand_func(self, **hints):
        ...
        if isinstance(self.sympy_impl, Function):
            isFunc = True
        return self.sympy_impl.eval_expand_func(function=isFunc)

    def _eval_evalf(self, nprec):
        numericalConstApprox = self._eval_expand_func()._eval_evalf(nprec + 6)
        return Float(numericalConstApprox, precision=nprec)
...
class PLConstants:
    ...
    NAMED_CONSTANTS_DICT = {
        ...
        'pi'           : {
            ...
            'latex_symbol'   : r'\pi',
            'sympy_impl'     : 'S.Pi',
            'docstr'         : 'The classical PI constant',
        },
        'e'            : {
            'unicode_symbol' : u'ℯ',
            'latex_symbol'   : r'e',
            'sympy_impl'     : 'S.E',
            'docstr'         : 'Euler\'s number - exp(1)',
        },
        'golden_ratio' : {
            'unicode_symbol' : u'𝞍',
        ...
        'cbrt_omega'   : {
            ...
            'sympy_impl'     : '(1 + S.I * sympy.sqrt(3)) / 2',
    ...
    def ResolveNamedConstantsFromDict(namedConstsDict):
        ...
        for constNameKey in namedConstsDict.keys():
            ...
            constParamsDict['text_symbol'] = constNameKey
            sympyConstObj = PLConstantValue(**constParamsDict)
            namedConstsList += [ (constNameKey, sympyConstObj) ]
        return namedConstsList
...
def GetConstantVariablesByGroup(cvarGrpId=None):
    ...
    for varGrp in varGroupsToAdd:
        ...
        varsList += allVarGroupsDict[varGrp]()
    return varsList
