...
module.exports = {
   ...
   * log.
   * @returns {Promise<Array<{event_name: string,
                              event_color: string,
                              event_date: Date,
                              ...
                              instructor_question_number: string}>>} the results of the log query.
   */
  async selectAssessmentInstanceLog(assessment_instance_id, include_files) {
    return (
      await sqldb.queryAsync(sql.assessment_instance_log, { assessment_instance_id, include_files })
    ).rows;
  },
};
