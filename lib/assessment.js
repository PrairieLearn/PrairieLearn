...
module.exports = {
    ...
    /**
     * Render the `html` or `text` property of an assessment.
     *
     * @param {import('../sync/course-db').Assessment} assessment - The assessment to render the text for.
     * @param {string} urlPrefix - The current server urlPrefix.
     * @param {function} callback - A callback(err, html) function.
     */
    renderHtmlOrText(assessment, urlPrefix) {
        if (assessment.html) {
            const context = {
                client_files_course: urlPrefix + '/clientFilesCourse',
                client_files_course_instance: urlPrefix + '/clientFilesCourseInstance',
                client_files_assessment: urlPrefix + '/assessment/' + assessment.id + '/clientFilesAssessment',
            };
            return mustache.render(assessment.html, context);
        }
