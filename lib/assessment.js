...
module.exports = {
    ...
    /**
     * Internal worker for gradeAllAssessmentInstances(). Do not call directly.
     * @protected
     *
     * @param {Object} client - SQL client that must be inside a locked transaction.
     * @param {number} assessment_instance_id - The assessment instance to grade.
     ...
     * @param {function} callback - A callback(err) function.
     */
    _gradeAllAssessmentInstancesWithClient(client, assessment_id, authn_user_id, close, overrideGradeRate, callback) {
        debug('_gradeAllAssessmentInstancesWithClient()');
        let rows;
        let grading_job_ids = [];
        async.series([
            (callback) => {
                ...
                sqldb.queryWithClient(client, sql.select_and_lock_instances_to_grade, params, (err, result) => {
                    if (ERR(err, callback)) return;
                    rows = result.rows;
                    debug('_gradeAllAssessmentInstancesWithClient()', 'selected assessment instances', 'count:', rows.length);
                    callback(null);
                });
            },
            (callback) => {
                async.eachSeries(rows, (row, callback) => {
                    debug('_gradeAssessmentInstanceWithClient()', 'loop', 'assessment.id:', row.assessment_instance_id);
                    this._gradeAssessmentInstanceWithClient(client, row.assessment_instance_id, authn_user_id, close, overrideGradeRate, (err, instance_grading_job_ids) => {
                        if (ERR(err, callback)) return;
                        if (instance_grading_job_ids !== undefined) {
                            grading_job_ids = grading_job_ids.concat(instance_grading_job_ids);
                        }
                        callback(null);
                    });
                }, (err) => {
                    if (ERR(err, callback)) return;
                    debug('_gradeAllAssessmentInstancesWithClient()', 'finished grading');
                    callback(null);
                });
            },
        ], (err) => {
            if (ERR(err, callback)) return;
            debug('_gradeAllAssessmentInstancesWithClient()', 'success');
            callback(null, grading_job_ids);
        });
    },
     ...
     * @param {function} callback - A callback(err) function.
     */
    gradeAllAssessmentInstances(assessment_id, authn_user_id, close, overrideGradeRate, callback) {
        ...
        sqldb.beginTransaction((err, client, done) => {
            ...
            async.series([
                (callback) => {
                    this._gradeAllAssessmentInstancesWithClient(client, assessment_id, authn_user_id, close, overrideGradeRate, (err, ret_grading_job_ids) => {
                        if (ERR(err, callback)) return;
                        grading_job_ids = ret_grading_job_ids;
                        debug('gradeAllAssessmentInstances()', 'finished _gradeAllAssessmentInstancesWithClient()');
                        callback(null);
                    });
                },
            ], (err) => {
                sqldb.endTransaction(client, done, err, (err) => {
                    if (ERR(err, callback)) return;
                    debug('gradeAllAssessmentInstances()', 'success');
                    if (grading_job_ids.length > 0) {
                        // We need to submit these grading jobs to be graded
                        externalGrader.beginGradingJobs(grading_job_ids, (err) => {
                            if (ERR(err, callback)) return;
