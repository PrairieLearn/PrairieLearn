...
module.exports = {
    ...
    _gradeAssessmentInstanceWithClient(client, assessment_instance_id, authn_user_id, close, instructorOverride, callback) {
        ...
        let rows;
        let overrideGradeRateCheck = close || instructorOverride;
        ...
        async.series([
            ...
            (callback) => {
                async.eachSeries(rows, (row, callback) => {
                    ...
                    question._gradeVariantWithClient(client, row.variant, check_submission_id, row.question, row.course, authn_user_id, overrideGradeRateCheck, (err, grading_job_id) => {
                        if (ERR(err, callback)) return;
