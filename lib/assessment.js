...
module.exports = {
  ...
  async updateAssessmentStatisticsForCourseInstance(course_instance_id) {
    ...
    for (const row of result.rows) {
      await module.exports.updateAssessmentStatistics(row.assessment_id, false);
    }
  },

  /**
   * Updates statistics for an assessment, optionally checking whether the
   * update is needed.
   *
   * @param {number} assessment_id - The assessment ID.
   * @param {boolean} [checkUpdateNeeded=true] - Whether to check if an update is needed.
   */
  async updateAssessmentStatistics(assessment_id, checkUpdateNeeded = true) {
