      ...
      [
        async () => {
          await sqldb.runInTransactionAsync(async (client) => {
            await sqldb.callWithClientAsync(client, 'assessment_instances_lock', [
              assessment_instance_id,
            ]);
            const updateResult = await sqldb.callWithClientOneRowAsync(
              client,
              'assessment_instances_update',
              [assessment_instance_id, authn_user_id]
            );
            let updated = updateResult.rows[0].updated;
            if (!updated) return null; // skip if not updated

            // if updated, regrade to pick up max_points changes, etc.
            await sqldb.callWithClientOneRow(client, 'assessment_instances_grade', [
              assessment_instance_id,
              authn_user_id,
              null, // credit
