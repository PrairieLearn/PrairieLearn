  ...
  renderText(assessment, urlPrefix, callback) {
    ...
    var context = {
      ...
      clientFilesAssessment:
        urlPrefix + '/assessment/' + assessment.id + '/clientFilesAssessment',
    };
          ...
          (callback) => {
            this._gradeAssessmentInstanceWithClient(
              ...
              (err, ret_grading_job_ids) => {
                ...
                debug(
                  'gradeAssessmentInstance()',
                  'finished _gradeAssessmentInstanceWithClient()',
                );
                callback(null);
              },
            );
      ...
      function (err, result) {
        ...
        serverJobs.createJobSequence(options, function (err, job_sequence_id) {
          ...
          const jobOptions = {
            ...
            type: 'grade_all_assessment_instances',
            description:
              'Grade all assessment instances for ' + assessment_label,
              ...
              (err, result) => {
                ...
                async.eachSeries(
                  ...
                  (err) => {
                    if (err) {
                      job.fail(
                        error.newMessage(err, 'Error grading instances'),
                      );
                    } else {
      ...
      [
        (callback) => {
          if (!_(content.grading).isObject()) {
            return callback(
              error.makeWithData('invalid grading', { content: content }),
            );
          }
          if (
            _(content.grading).has('feedback') &&
            !_(content.grading.feedback).isObject()
          ) {
            return callback(
          ...
          const succeeded = !!_.get(
            content,
            'grading.feedback.results.succeeded',
            true,
          );
          const gradable = !!_.get(
            content,
            'grading.feedback.results.gradable',
            true,
          );
          if (!succeeded) {
