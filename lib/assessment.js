    ...
    processGradingResult(content) {
        async.series([
            (callback) => {
                ...
                let succeeded = true;
                let gradable = true;
                if (_(content.grading.feedback).has('results') &&
                    _(content.grading.feedback.results).isObject()) {
                    let results = content.grading.feedback.results;

                    if (_(results).has('succeeded')) {
                        succeeded = results.succeeded;
                    }
                    if (!succeeded) {
                        content.grading.score = 0;
                    }
                    if (succeeded && _(results).has('gradable')) {
