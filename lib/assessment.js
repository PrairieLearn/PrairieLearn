...
module.exports = {
     ...
     * @param {boolean} instructorOverride - Whether to override submission rate limits.
     * @param {function} callback - A callback(err) function.
     */
    _gradeAssessmentInstanceWithClient(client, assessment_instance_id, authn_user_id, close, instructorOverride, callback) {
        debug('_gradeAssessmentInstanceWithClient()');
        let rows;
        let overrideSubmissionRateCheck = close || instructorOverride;
        ...
        async.series([
            ...
            (callback) => {
                async.eachSeries(rows, (row, callback) => {
                    ...
                    question._gradeVariantWithClient(client, row.variant, check_submission_id, row.question, row.course, authn_user_id, overrideSubmissionRateCheck, (err, grading_job_id) => {
                        if (ERR(err, callback)) return;
     ...
     * @param {function} callback - A callback(err) function.
     */
    gradeAssessmentInstance(assessment_instance_id, authn_user_id, close, instructorOverride, callback) {
        ...
        sqldb.beginTransaction((err, client, done) => {
            ...
            async.series([
                ...
                (callback) => {
                    this._gradeAssessmentInstanceWithClient(client, assessment_instance_id, authn_user_id, close, instructorOverride, (err, ret_grading_job_ids) => {
                        if (ERR(err, callback)) return;
