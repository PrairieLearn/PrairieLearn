        ...
        async.series([
            ...
            (callback) => {
                async.eachSeries(rows, (row, callback) => {
                    ...
                    question._gradeVariantWithClient(client, row.variant, check_submission_id, row.question, row.course, authn_user_id, overrideGradeRate, null, (err, result) => {
                        if (ERR(err, callback)) return;
                        if (result.grading_job_id !== undefined) {
                            grading_job_ids.push(result.grading_job_id);
                        }
                        callback(null);
                    });
                }, (err) => {
