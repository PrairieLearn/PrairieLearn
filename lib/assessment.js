...
module.exports = {
  ...
  _gradeAssessmentInstance(
    ...
    debug('_gradeAssessmentInstance()');
    let rows;
    ...
    async.series(
      [
        ...
        (callback) => {
          sqldb.call(
            ...
            (err, result) => {
              if (ERR(err, callback)) return;
              rows = result.rows;
              debug('_gradeAssessmentInstance()', 'selected variants', 'count:', rows.length);
              callback(null);
            }
          );
        },
        (callback) => {
          async.eachSeries(
            rows,
            (row, callback) => {
              debug('_gradeAssessmentInstance()', 'loop', 'variant.id:', row.variant.id);
              const check_submission_id = null;
              question._gradeVariant(
                row.variant,
                check_submission_id,
                row.question,
                row.course,
            ...
            (err) => {
              if (ERR(err, callback)) return;
              debug('_gradeAssessmentInstance()', 'finished grading');
              callback(null);
            }
          );
        },
      ...
      (err) => {
        if (ERR(err, callback)) return;
        debug('_gradeAssessmentInstance()', 'success');
  ...
  gradeAssessmentInstance(
    ...
    this._gradeAssessmentInstance(
      ...
      (err, ret_grading_job_ids) => {
        ...
        if (ret_grading_job_ids.length > 0) {
          // We need to submit these grading jobs to be graded
          externalGrader.beginGradingJobs(ret_grading_job_ids, (err) => {
            if (ERR(err, callback)) return;
            callback(null);
          });
        } else {
        ...
        debug('gradeAssessmentInstance()', 'finished _gradeAssessmentInstance()');
        callback(null);
      }
    );
  },
