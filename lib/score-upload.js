...
module.exports = {
    ...
    _updateInstanceQuestionFromJson(json, assessment_id, authn_user_id, callback) {
        ...
        sqldb.queryOneRow(sql.select_instance_question, params, function(err, result) {
            ...
            if (!_.has(json, 'score_perc') && !_.has(json, 'points')) {
            ...
            if (feedback_json != null) {
                ...
                try {
                    ...
                    return callback(new Error(`Unable to parse "feedback_json" field as JSON: ${e}`));
                }
                if (!_.isPlainObject(feedback_obj)) {
                    return callback(new Error(`Parsed "feedback_json" is not a JSON object: ${feedback_obj}`));
                }
