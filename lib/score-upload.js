...
module.exports = {
            ...
            csvConverter
              ...
              .subscribe((json, number) => {
                return new Promise((resolve, _reject) => {
                  ...
                  const msg = `Processing CSV record ${number}: ${JSON.stringify(
                    json,
                  )}`;
                  if (output == null) {
                    output = msg;
              ...
              .then(() => {
                ...
                if (errorCount == 0) {
                  job.verbose(
                    `Successfully updated scores for ${successCount} questions, with no errors`,
                  );
                  job.succeed();
                } else {
                  job.verbose(
                    `Successfully updated scores for ${successCount} questions`,
                  );
            ...
            csvConverter
              ...
              .then(() => {
                ...
                if (errorCount == 0) {
                  ...
                  job.verbose(
                    `Successfully updated scores for ${successCount} assessment instances`,
                  );
  ...
  _getFeedbackOrNull(json) {
    const feedback_string = module.exports._getJsonPropertyOrNull(
      json,
      'feedback',
    );
    const feedback_json = module.exports._getJsonPropertyOrNull(
      json,
      'feedback_json',
    );
    let feedback = null;
    ...
    if (feedback_json != null) {
      ...
      if (!_.isPlainObject(feedback_obj)) {
        throw new Error(
          `Parsed "feedback_json" is not a JSON object: ${feedback_obj}`,
        );
      }
  ...
  _getPartialScoresOrNull(json) {
    const partial_scores_json = module.exports._getJsonPropertyOrNull(
      json,
      'partial_scores',
    );
    ...
    if (partial_scores_json != null) {
      ...
      if (!_.isPlainObject(partial_scores)) {
        throw new Error(
          `Parsed "partial_scores" is not a JSON object: ${partial_scores}`,
        );
      }
    }
  ...
  _updateInstanceQuestionFromJson(
    json,
    assessment_id,
    authn_user_id,
    callback,
  ...
  _updateAssessmentInstanceFromJson(
    json,
    assessment_id,
    authn_user_id,
    callback,
    ...
    sqldb.queryOneRow(query, params, function (err, result) {
      if (err)
        return callback(
          new Error(`unable to locate instance ${json.instance} for ${id}`),
        );
      ...
      if (_.has(json, 'score_perc')) {
        ...
        return callback(
          new Error('must specify either "score_perc" or "points"'),
        );
      }
    });
  },
