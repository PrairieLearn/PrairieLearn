...
module.exports = {
                    ...
                    csvConverter
                        ...
                        .subscribe((json, number) => {
                            return new Promise((resolve, _reject) => {
                                ...
                                module.exports._updateAssessmentInstanceFromJson(json, assessment_id, authn_user_id, create_on_error, (err) => {
                                    if (err) {
                                        errorCount++;
                                        const msg = String(err);
                                        if (output == null) {
    ...
    _updateAssessmentInstanceFromJson(json, assessment_id, authn_user_id, create_on_error, callback) {
        ...
        sqldb.queryOneRow(query, params, function(err, result) {
            ...
            if (err) {
				...
				if (create_on_error) sqldb.call('assessment_instances_insert', iparams, (err, _result) => {
                    if (ERR(err, callback)) return;
                    tmp_id = _result.rows[0].assessment_instance_id;
                });
				else return callback(new Error(`unable to locate instance ${json.instance} for ${id}`));
			}
            
            if (tmp_id === null) {
                tmp_id = result.rows[0].assessment_instance_id;
            }
            const assessment_instance_id = tmp_id;
