...
module.exports = {
    ...
    uploadQuestionScores(assessment_id, csvFile, user_id, authn_user_id, callback) {
        debug('uploadQuestionScores()');
        if (csvFile == null) {
        ...
        sqldb.queryOneRow(sql.select_assessment_info, params, function(err, result) {
            ...
            serverJobs.createJobSequence(options, function(err, job_sequence_id) {
                ...
                serverJobs.createJob(jobOptions, function(err, job) {
                    ...
                    job.verbose('Parsing CSV into JSON...');
                    csvStream = streamifier.createReadStream(csvFile.buffer, {encoding: 'utf8'});
                    ...
                    csvConverter
                        ...
                        .subscribe((json, lineNumber) => {
                                return new Promise((resolve, _reject) => {
                                    job.verbose(`Processing CSV line ${lineNumber}: ${JSON.stringify(json)}`);
                                    resolve();
                                });
                        })
                        .then(() => {
