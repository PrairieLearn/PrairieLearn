...
module.exports = {
                    ...
                    csvConverter
                        ...
                        .subscribe((json, number) => {
                            return new Promise((resolve, _reject) => {
                                ...
                                module.exports._updateAssessmentInstanceFromJson(json, assessment_id, authn_user_id, score_only, (err) => {
                                    if (err) {
                                        errorCount++;
                                        const msg = String(err);
                                        if (output == null) {
    ...
    _updateAssessmentInstanceFromJson(json, assessment_id, authn_user_id, score_only, callback) {
        let query, id;
        if (_.has(json, 'uid')) {
        ...
        sqldb.queryZeroOrOneRow(query, params, function(err, result) {
            ...
            if (result.rowCount == 0) {
                if (!score_only) return callback(new Error(`Unable to locate instance ${json.instance} for ${id}`));
            } else {
