...
module.exports = {
    ...
    uploadAssessmentInstanceScores(assessment_id, csvFile, user_id, authn_user_id, callback) {
        ...
        sqldb.queryOneRow(sql.select_assessment_info, params, function(err, result) {
            ...
            const course_instance_id = result.rows[0].course_instance_id;
            const course_id = result.rows[0].course_id;
            const score_only = result.rows[0].score_only;
            const mode = result.rows[0].asst_mode;
                    ...
                    csvConverter
                        ...
                        .subscribe((json, number) => {
                            return new Promise((resolve, _reject) => {
                                ...
                                module.exports._updateAssessmentInstanceFromJson(json, assessment_id, authn_user_id, score_only, mode, (err) => {
                                    if (err) {
                                        errorCount++;
                                        const msg = String(err);
                                        if (output == null) {
    ...
    _updateAssessmentInstanceFromJson(json, assessment_id, authn_user_id, score_only, mode, callback) {
        let query, id, group_work;
        if (_.has(json, 'uid')) {
        ...
        sqldb.queryZeroOrOneRow(query, params, function(err, result) {
            ...
            let assessment_instance_id = null;
            ...
            if (result.rowCount == 0) {
                ...
                if (score_only) sqldb.callOneRow('assessment_instances_insert', iparams, (err, _result) => {
                    if (ERR(err, callback)) return;
                    assessment_instance_id = _result.rows[0].assessment_instance_id;
                });
                else return callback(new Error(`Unable to locate instance ${json.instance} for ${id}`));
            } else {
