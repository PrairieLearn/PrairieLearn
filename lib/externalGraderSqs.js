    ...
    async.series([
        (callback) => {
            if (QUEUE_URL) {
                ...
                logger.verbose(`External grading jobs queue ${config.externalGradingJobsQueueName}: getting URL...`);
                const params = {
                ...
                sqs.getQueueUrl(params, (err, data) => {
                    ...
                    logger.verbose(`External grading jobs queue ${config.externalGradingJobsQueueName}: got URL ${QUEUE_URL}`);
                    callback(null);
                });
            }
        },
        (callback) => {
            ...
            sqs.sendMessage(params, (err) => {
                if (ERR(err, callback)) return;
                logger.verbose('Queued external grading job', {grading_job_id: jobId, ...params});
                callback(null);
            });
        },
    ], (err) => {
