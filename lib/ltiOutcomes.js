...
function updateScore(ai_id, callback) {
  ...
  sqldb.queryZeroOrOneRow(sql.get_score, { ai_id }, (err, result) => {
    ...
    request.post(post_params, function (err, httpResponse, body) {
      ...
      debug(httpResponse);
      debug(body);
      const bodyObject = parser.parse(body);
      ...
      if (imsx_codeMajor === 'success') {
        logger.info(
          `ltiOutcomes.updateScore() ai_id=${ai_id} score=${score} returned ${bodyObject.imsx_POXEnvelopeResponse.imsx_POXHeader.imsx_POXResponseHeaderInfo.imsx_statusInfo.imsx_codeMajor}`
        );
        debug(
          `ltiOutcomes.updateScore() ai_id=${ai_id} score=${score} returned ${bodyObject.imsx_POXEnvelopeResponse.imsx_POXHeader.imsx_POXResponseHeaderInfo.imsx_statusInfo.imsx_codeMajor}`
        );
