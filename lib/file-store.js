 ...
 */
module.exports.upload = async (display_filename, buffer, type, assessment_instance_id, instance_question_id, submission_id, user_id, authn_user_id, storage_type) => {
    ...
    if (storage_type === this.storageTypes.S3) {
        ...
        if (config.fileStoreS3Bucket == null) throw new Error('config.fileStoreS3Bucket is null, which does not allow uploads');

        const res = await uploadToS3Async(config.fileStoreS3Bucket, storage_filename, null, false, buffer);
...
module.exports.get = async (file_id, data_type = 'buffer') => {
    ...
    if (result.rows[0].storage_type === this.storageTypes.FileSystem) {
        ...
        if (data_type === 'buffer') {
            ...
            debug(`get(): createReadStream ${filename} and return object with contents stream and file object`);
            readStream = fs.createReadStream(filename);
        }
    }

    if (result.rows[0].storage_type === this.storageTypes.S3) {
        if (config.fileStoreS3Bucket == null) throw new Error(`config.fileStoreS3Bucket must be configured to get file_id ${file_id} from file store`);

        if (data_type === 'buffer') {
            debug(`get(): s3 fetch file ${result.rows[0].storage_filename} from ${config.fileStoreS3Bucket} and return object with contents buffer and file object`);
            buffer = await getFromS3Async(config.fileStoreS3Bucket, result.rows[0].storage_filename);
        } else {
            debug(`get(): s3 fetch stream ${result.rows[0].storage_filename} from ${config.fileStoreS3Bucket} and return object with contents stream and file object`);
            readStream = await getFromS3Async(config.fileStoreS3Bucket, result.rows[0].storage_filename, false);
        }
    }
