...
module.exports.upload = async (display_filename, buffer, type, assessment_instance_id, instance_question_id, user_id, authn_user_id, storage_type) => {
    ...
    if (!storage_type) storage_type = config.fileStoreStorageDefault;
...
module.exports.getStream = async (file_id) => {
    ...
    const file = await this.get(file_id);
 ...
 */
module.exports.get = async (file_id, getStream = false) => {
    debug(`get(): file_id=${file_id}`);
    ...
    if (result.rows[0].storage_type === this.storageTypes.S3) {
        ...
        if (getStream) {
            readStream = await getFromS3Async(config.fileStoreS3Bucket, result.rows[0].storage_filename);
        } else {
            buffer = await getFromS3Async(config.fileStoreS3Bucket, result.rows[0].storage_filename, true);
        }
    }
