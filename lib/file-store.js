...
module.exports.upload = async (display_filename, contents, type, assessment_instance_id, instance_question_id, user_id, authn_user_id, storage_type) => {
    ...
    const dir = storage_type === this.storageTypes.fileSystem ? path.join(config.filesRoot, relDir) : relDir;
    ...
    if (storage_type === this.storageTypes.FileSystem) {
        ...
        debug(`uploadFile(): writeFile ${storage_filename}`);
        await fsPromises.writeFile(storage_filename, contents, {mode: 0o600});
    }
