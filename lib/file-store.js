...
module.exports.upload = async (display_filename, contents, type, assessment_instance_id, instance_question_id, user_id, authn_user_id, storage_type) => {

    if (config.filesRoot == null) throw new Error('config.filesRoot is null, which does not allow uploads');

    // default S3
    if (!storage_type) storage_type = this.storageTypes.S3;
    ...
    if (storage_type === this.storageTypes.S3) {
        ...
        const options = {
            ...
            body: Buffer.from(contents, 'basex64'),
        };
        const res = await uploadToS3Async(options.bucket, options.key, null, null, options.body);
 ...
 */
module.exports.get = async (file_id, display_filename) => {
    debug(`get(): file_id=${file_id} display_filename=${display_filename}`);
    const params = { file_id, display_filename };
    ...
    if (result.rows[0].storage_type === this.storageTypes.S3) {
        ...
        return getFromS3Async(config.fileUploadS3Bucket, result.rows[0].storage_filename);
    }
