...
/**
 * Determines if the two chunk metadata objects are equal.
 *
 * @param {ChunkMetadata} a
 * @param {ChunkMetadata} b
 */
module.exports.chunkMetadataEqual = function (a, b) {
  switch (a.type) {
    case 'elements':
    ...
    case 'clientFilesCourseInstance':
      return a.type === b.type && a.courseInstanceName === b.courseInstanceName;
    case 'clientFilesAssessment':
      return (
        a.type === b.type &&
        a.courseInstanceName === b.courseInstanceName &&
        a.assessmentName === b.assessmentName
      );
    case 'question':
      return a.type === b.type && a.questionName === b.questionName;
  }
};

/**
 * Determines if the two chunk objects are equal.
 *
 * @param {Chunk} a
 * @param {Chunk} b
 */
module.exports.chunksEqual = function (a, b) {
  switch (a.type) {
    case 'elements':
    case 'elementExtensions':
    case 'clientFilesCourse':
    case 'serverFilesCourse':
      return b.type === a.type;
    case 'clientFilesCourseInstance':
      // Use `==` instead of `===` since IDs may be either strings or numbers.
      return a.type === b.type && a.courseInstanceId == b.courseInstanceId;
    case 'clientFilesAssessment':
      return (
        // Use `==` instead of `===` since IDs may be either strings or numbers.
        a.type === b.type &&
        a.courseInstanceId == b.courseInstanceId &&
        a.assessmentId == b.assessmentId
      );
    case 'question':
      // Use `==` instead of `===` since IDs may be either strings or numbers.
      return a.type === b.type && a.questionId == b.questionId;
  }
};
...
module.exports.chunkMetadataFromDatabaseChunk = function (chunk) {
  switch (chunk.type) {
    ...
    case 'clientFilesCourseInstance':
      return {
        type: chunk.type,
        courseInstanceName: chunk.courseInstanceName,
      };
    case 'clientFilesAssessment':
      return {
        type: chunk.type,
        courseInstanceName: chunk.courseInstanceName,
        assessmentName: chunk.assessmentName,
      };
    case 'question':
      return {
        type: chunk.type,
        questionName: chunk.questionName,
      };
  }
};

/**
 * Constructs a {@link Chunk} object from the given {@link DatabaseChunk} object.
 *
 * @param {DatabaseChunk} chunk
 * @returns {Chunk}
 */
module.exports.chunkFromDatabaseChunk = function (chunk) {
  switch (chunk.type) {
    case 'elements':
    case 'elementExtensions':
    case 'clientFilesCourse':
    ...
    case 'clientFilesCourseInstance':
      return {
        type: chunk.type,
        courseInstanceId: chunk.course_instance_id,
      };
    case 'clientFilesAssessment':
      return {
        type: chunk.type,
...
module.exports.deleteChunks = async function (courseId, chunksToDelete) {
  await sqlDb.callAsync('chunks_delete', [courseId, chunksToDelete]);
};
...
module.exports.ensureChunksForCourseAsync = async (courseId, chunks) => {
  ...
  console.log('chunks', dbChunks.rows);
  ...
  await Promise.all(
    chunks.map(async (chunk) => {
      // The `chunks` array that the caller passed in contains `Chunk` objects, which
      // reference chunks by their entity IDs. However, we need to end up with
      // `ChunkMetadata` objects that reference chunks by their human-readable entity
      // names, which correspond to paths on disk.
      const databaseChunk = dbChunks.rows.find((c) =>
        module.exports.chunksEqual(module.exports.chunkFromDatabaseChunk(c), chunk)
      );

      if (databaseChunk) {
        // Blindly remove this chunk from disk - if it doesn't exist, `fs.remove`
        // will silently no-op.
        const chunkMetadata = module.exports.chunkMetadataFromDatabaseChunk(databaseChunk);
