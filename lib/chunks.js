...
/**
 * Determines if the two chunk metadata objects are equal. Note that this only
 ...
 */
module.exports.coursePathForChunk = function (coursePath, chunk) {
  switch (chunk.type) {
    case 'elements':
    case 'elementExtensions':
    case 'clientFilesCourse':
    case 'serverFilesCourse':
      return path.join(coursePath, chunk.type);
    case 'question':
      return path.join(coursePath, 'questions', chunk.questionName);
    case 'clientFilesCourseInstance':
      return path.join(
        coursePath,
        'courseInstances',
...
module.exports.ensureChunksForCourseAsync = async (courseId, chunks) => {
  ...
  // the `elements` directory does not exist inside the course's unpacked
  // directory.
  //
  ...
  // the unpacked chunk from the course's directory.
  //
  ...
  await Promise.all(
    chunks.map(async (chunk) => {
      const databaseHasChunk =
        dbChunks.rows.findIndex((c) => module.exports.chunkMetadataEqual(c, chunk)) !== -1;
      if (!databaseHasChunk) {
        // Blindly remove this chunk from disk - if it doesn't exist, `fs.remove`
        // will silently no-op.
        await fs.remove(module.exports.coursePathForChunk(courseChunksDirs.course, chunk));
      }
    })
  );
};
