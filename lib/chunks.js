    ...
    try {
        for (let i = 0; i < course_ids.length; i++) {
            ...
            try {
                ...
                await job.failAsync(String(err));
                throw err;
            }
        }
    } catch (err) {
        try {
            await serverJobs.failJobSequence(job_sequence_id);
        } catch (err) {
...
module.exports._generateAllChunksForCourseWithJob = async (course_id, authn_user_id, job) => {
    ...
    job.info(chalkDim(`Release lock`));

    job.info(chalk.green(`Successfully generate chunks for course ID = ${course_id}`));
};
