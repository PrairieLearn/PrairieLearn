...
/** @typedef {"elements" | "elementExtensions" | "clientFilesCourse" | "serverFilesCourse" | "clientFilesCourseInstance" | "clientFilesAssessment" | "question" | "assessment"} ChunkType */
 ...
 * @property {Set<string>} clientFilesAssessments
...
/**
 * Questions and assessments can be nested arbitrarily deep in directories. In
 * order to identify which portion of a path corresponds to a question/assessment
 * ID, we need to brute-force it and try progressively larger parts of the path
 * until we find one that corresponds with a question/assessment ID.
 * 
 * @param {string[]} pathComponents 
 * @param {{ [id: string]: any }} candidateIds 
 * @returns {string | null}
 */
module.exports.findIdForPathComponents = (pathComponents, candidateIds) => {
    for (let i = 1; i < pathComponents.length; i++) {
        const candidateId = path.join(...pathComponents.slice(0, i));
...
module.exports.identifyChunksFromChangedFiles = (changedFiles, courseData) => {
    ...
    changedFiles.forEach(changedFile => {
        ...
        if (changedFile.startsWith('questions/')) {
            ...
            const questionId = module.exports.findIdForPathComponents(pathComponents, courseData.questions);
            if (questionId) {
        ...
        if (changedFile.startsWith('courseInstances/')) {
            ...
            if (clientFilesCourseInstanceIndex >= 0) {
                ...
                if (courseData.courseInstances[courseInstanceId]) {
                    if (!courseChunks.courseInstances[courseInstanceId]) {
                        courseChunks.courseInstances[courseInstanceId] = {
                            assessments: new Set(),
                            clientFilesAssessments: new Set(),
                ...
                // Important: if the courseInstanceId in the path didn't match a known course
                // instance, fall through to account for weird things like people putting
            ...
            if (assessmentsIndex >= 0) {
                ...
                if (courseData.courseInstances[courseInstanceId]) {
                    const assessmentId = module.exports.findIdForPathComponents(
                        pathComponents.slice(assessmentsIndex + 1),
                        courseData.courseInstances[courseInstanceId].assessments,
                    );
                    if (assessmentId) {
                        ...
                        // NOTE: originally, we only generated chunks for `clientFilesAssessment`, not
                        // each assessment directory as a whole. However, since chunking was introduced,
                        // we added support for an `assessment.html` file adjacent to `infoAssessment.json`
                        // that must be made available via chunks. We added an `assessment` chunk type for
                        // the entire assessment directory, and we'll need to sync both in parallel during
                        // the transition period.
                        // TODO: drop `clientFilesAssessment` chunks at some point in the future once
                        // we can safely cut over to only using the `assessment` chunks.
                        if (clientFilesAssessmentIndex >= 0 && clientFilesAssessmentIndex > assessmentsIndex) {
                            courseChunks.courseInstances[courseInstanceId].clientFilesAssessments.add(assessmentId);
                        }
                    }
                }
            }
...
module.exports.identifyChunksToGenerate = async ({ coursePath, courseId, courseData, changedFiles }) => {
        ...
        switch (courseChunk.type) {
            ...
            case 'clientFilesAssessment': {
                ...
                if (courseChunk.type === 'clientFilesCourseInstance') {
                    ...
                    existingCourseChunks.courseInstances[courseInstanceName].clientFilesAssessments.add(assessmentName);
                }
                break;
            }
        }
    ...
    await async.each(Object.entries(courseData.courseInstances), async ([ciid, courseInstanceInfo]) => {
        ...
        await async.each(Object.entries(courseInstanceInfo.assessments), async ([tid, assessmentInfo]) => {
            const assessmentPath = path.join(coursePath, 'courseInstances', ciid, 'assessments', tid);
            const hasAssessmentDirectory = await fs.pathExists(assessmentPath);
            const hasClientFilesAssessmentDirectory = await fs.pathExists(path.join(assessmentPath, 'clientFilesAssessment'));
            if (hasAssessmentDirectory && (
                (
                    !existingCourseChunks.courseInstances[ciid] ||
            ...
            if (hasClientFilesAssessmentDirectory && (
                (
                    !existingCourseChunks.courseInstances[ciid] ||
                    !existingCourseChunks.courseInstances[ciid].clientFilesAssessments ||
                    !existingCourseChunks.courseInstances[ciid].clientFilesAssessments.has(tid)
                ) || (
                    changedCourseChunks.courseInstances[ciid] &&
                    changedCourseChunks.courseInstances[ciid].clientFilesAssessments &&
                    changedCourseChunks.courseInstances[ciid].clientFilesAssessments.has(tid)
                )
            )) {
                chunksToGenerate.push({
