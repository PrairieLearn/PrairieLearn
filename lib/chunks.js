...
module.exports.getRuntimeDirectoryForCourse = (course) => {
    if (globalConfig.chunksEnabled && globalConfig.chunksConsumer) {
...
module.exports.ensureChunksForCourseAsync = async (courseId, chunks) => {
    if (!globalConfig.chunksEnabeld || !globalConfig.chunksConsumer) {
    ...
    // place on disk.
    await async.each(dbChunks.rows, async (chunk) => {
