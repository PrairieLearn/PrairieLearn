 ...
 */
module.exports.createChunksSymlinks = async ({ coursePath, courseId, course }) =>  {
    ...
    await Promise.all(Object.keys(course.courseInstanceDB).map(async (courseInstanceDir) => {
        await relativeSymlink(path.join('courseInstances', courseInstanceDir, 'clientFilesCourseInstance'));
        await Promise.all(Object.keys(course.courseInstanceDB[courseInstanceDir].assessmentDB).map(async (assessmentDir) => {
            await relativeSymlink(path.join('courseInstances', courseInstanceDir, 'assessments', assessmentDir, 'clientFilesAssessment'));
        }));
    }));

    // Handle questions
    await Promise.all(Object.keys(course.questionDB).map(async (questionDir) => {
...
const ensureChunk = async (courseId, chunk) => {
    ...
    switch (chunk.type) {
        ...
        case 'clientFilesCourse':
            ...
            break;
        case 'clientFilesCourseInstance':
            targetDirectory = path.join(courseDir, 'courseInstances', chunk.course_instance_name, 'clientFilesCourseInstance');
            break;
        case 'clientFilesAssessment':
            targetDirectory = path.join(courseDir, 'courseInstances', chunk.course_instance_name, 'assessments', chunk.assessment_name, 'clientFilesAssessment');
            break;
        case 'question':
...
module.exports.ensureChunksForCourse = async (courseId, chunks) => {
    ...
    // First, query the database to identify the UUID of each desired chunk
    const dbChunks = await sqlDb.queryAsync(sql.select_uuid_for_chunks, {
        course_id: courseId,
