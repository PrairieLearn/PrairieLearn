...
module.exports.coursePathForChunk = function (coursePath, chunkMetadata) {
  switch (chunkMetadata.type) {
    ...
    case 'question':
      return (() =>{ console.trace(); return path.join(coursePath, 'questions', chunkMetadata.questionName) })();
    case 'clientFilesCourseInstance':
      return path.join(
        coursePath,
        'courseInstances',
 ...
 */
module.exports.getRuntimeDirectoryForCourse = (course, question) => {
  console.log(globalConfig.chunksConsumer);
  if (globalConfig.chunksConsumer) {
    // TODO: handle this case. How can I get the course.id for the course that
    // shared the question?
    ...
    if (question?.directory && question.directory[0] == '@') {
      ...
      const coursePath = question.directory.substring(1, firstSlash);
      const questionDirectory = question.directory.substring(firstSlash + 1, question.directory.length);

      // FIXME: dirty mutation of question object here! Not sure if this has undesirable side effects
      // question.directory = questionDirectory;
      console.log("Question shared from course:", coursePath);
      return '/PrairieLearn/' + coursePath; // FIXME: this hack probably won't always work
    } else {
      return course.path;
    }
  }
...
const ensureChunk = async (courseId, chunk) => {
  ...
  switch (chunk.type) {
    ...
    case 'clientFilesAssessment':
      ...
      break;
    case 'question':
      relativeTargetPath = (() =>{ console.trace(); return path.join('questions', chunk.question_name) })();
      break;
    default:
