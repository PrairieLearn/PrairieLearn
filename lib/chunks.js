...
module.exports.identifyChunksFromChangedFiles = (changedFiles, courseData) => {
  ...
  changedFiles.forEach((changedFile) => {
    ...
    if (changedFile.startsWith('courseInstances/')) {
      ...
      const clientFilesCourseInstanceIndex = pathComponents.indexOf(
        'clientFilesCourseInstance',
      );
      const assessmentsIndex = pathComponents.indexOf('assessments');
      const clientFilesAssessmentIndex = pathComponents.indexOf(
      ...
      if (
        ...
        const courseInstanceId = path.join(
          ...pathComponents.slice(0, assessmentsIndex),
        );
        const assessmentId = path.join(
          ...pathComponents.slice(
            assessmentsIndex + 1,
            clientFilesAssessmentIndex,
          ),
        );
        ...
        if (
          ...
          courseChunks.courseInstances[courseInstanceId].assessments.add(
            assessmentId,
          );
        }
      }
...
module.exports.identifyChunksToGenerate = async ({
  ...
  await async.each(
    ...
    async (
      ...
      const hasChunkDirectory = await fs.pathExists(
        path.join(coursePath, chunkType),
      );
      if (
        hasChunkDirectory &&
        ...
        async ([tid, assessmentInfo]) => {
          ...
          if (
            ...
            (!existingCourseChunks.courseInstances[ciid] ||
              ...
              !existingCourseChunks.courseInstances[ciid].assessments.has(
                tid,
              ) ||
              (changedCourseChunks.courseInstances[ciid] &&
 ...
 */
module.exports.createAndUploadChunks = async (
  coursePath,
  courseId,
  chunksToGenerate,
) => {
  ...
  await async.eachLimit(
    chunksToGenerate,
    globalConfig.chunksMaxParallelUpload,
    async (chunk) => {
      let chunkDirectory;
      switch (chunk.type) {
        case 'elements':
...
module.exports.getChunksDirectoriesForCourseId = (courseId) => {
  const baseDirectory = path.join(
    globalConfig.chunksConsumerDirectory,
    `course-${courseId}`,
  );
  return {
    base: baseDirectory,
...
module.exports.updateChunksForCourse = async ({
  ...
  let changedFiles = [];
  if (oldHash && newHash) {
    changedFiles = await module.exports.identifyChangedFiles(
      coursePath,
      oldHash,
      newHash,
    );
  ...
  await module.exports.createAndUploadChunks(
    coursePath,
    courseId,
    chunksToGenerate,
  );
 ...
 */
module.exports.generateAllChunksForCourseList = async (
  course_ids,
  authn_user_id,
) => {
...
module.exports._generateAllChunksForCourseListWithJobSequence = async (
  ...
  try {
    for (let i = 0; i < course_ids.length; i++) {
      ...
      const jobOptions = {
        course_id:
          null /* Set the job's course_id to null so we can find it from the admin page */,
        type: 'generate_all_chunks',
      ...
      try {
        await module.exports._generateAllChunksForCourseWithJob(
          course_id,
          authn_user_id,
          job,
        );
 ...
 * @param {object} job
 */
module.exports._generateAllChunksForCourseWithJob = async (
  course_id,
  authn_user_id,
  job,
) => {
  ...
  job.info(
    chalk.green(`Successfully generated chunks for course ID = ${course_id}`),
  );
};
...
const ensureChunk = async (courseId, chunk) => {
  const courseChunksDirs =
    module.exports.getChunksDirectoriesForCourseId(courseId);
  const downloadPath = path.join(
    courseChunksDirs.downloads,
    `${chunk.uuid}.tar.gz`,
  );
  ...
  const relativeUnpackPath = path.relative(
...
module.exports.ensureChunksForCourseAsync = async (courseId, chunks) => {
  ...
  await async.eachLimit(
    dbChunks.rows,
    globalConfig.chunksMaxParallelDownload,
    async (chunk) => {
...
module.exports.ensureChunksForCourse = util.callbackify(
  module.exports.ensureChunksForCourseAsync,
);
