...
Job.prototype.finish = function (code, signal) {
  ...
  sqldb.queryOneRow(sql.update_job_on_close, params, function (err, result) {
    ...
    if (that.options.job_sequence_id != null) {
      socketServer.io
        .to('jobSequence-' + that.options.job_sequence_id)
        .emit('update');
    }
    if (ERR(err, function () {})) {
      ...
      if (status == 'Success') {
        ...
        if (that.options.on_error) {
          that.options.on_error(
            that.id,
            new Error('Job terminated with error'),
          );
        }
      }
    }
...
module.exports.connection = function (socket) {
  socket.on('joinJob', function (msg, callback) {
    ...
    sqldb.queryOneRow(sql.select_job, params, function (err, result) {
      if (err)
        return logger.error(
          'socket.io joinJob error selecting job_id ' + msg.job_id,
          err,
        );
      ...
      if (module.exports.liveJobs[msg.job_id]) {
        output = ansiUp.ansi_to_html(
          module.exports.liveJobs[msg.job_id].output,
        );
      } else {
  ...
  socket.on('joinJobSequence', function (msg, callback) {
    ...
    sqldb.queryOneRow(sql.select_job_sequence, params, function (err, result) {
      if (err)
        return logger.error(
          'socket.io joinJobSequence error selecting job_sequence_id ' +
            msg.job_sequence_id,
          err,
        );
...
module.exports.createJob = function (options, callback) {
  ...
  sqldb.queryOneRow(sql.insert_job, params, function (err, result) {
    ...
    if (job.options.job_sequence_id != null) {
      socketServer.io
        .to('jobSequence-' + job.options.job_sequence_id)
        .emit('update');
    }
    callback(null, job);
  });
...
module.exports.killJob = function (job_id, callback) {
  ...
  if (!job.proc)
    return callback(
      new Error('job_id ' + job_id + ' does not have proc to kill'),
    );
  job.proc.kill();
        ...
        if (!_.has(module.exports.liveJobs, row.id)) {
          ...
          sqldb.query(sql.update_job_on_error, params, function (err, _result) {
            ...
            if (row.job_sequence_id != null) {
              socketServer.io
                .to('jobSequence-' + row.job_sequence_id)
                .emit('update');
            }
            if (ERR(err, function () {})) {
              logger.error(
                'errorAbandonedJobs: error updating job on error',
                err,
              );
            }
            callback(null);
...
module.exports.errorAbandonedJobsAsync = util.promisify(
  module.exports.errorAbandonedJobs,
);
...
module.exports.createJobSequenceAsync = async (options) => {
  // we write this explicitly so that typescript knows what's going on
  const job_sequence_id = await util.promisify(
...
module.exports.failJobSequence = function (job_sequence_id, callback) {
  ...
  sqldb.query(sql.fail_job_sequence, params, function (err, _result) {
    ...
    if (ERR(err, () => {})) {
      logger.error(
        'error failing job_sequence_id ' + job_sequence_id + ' with error',
        err,
      );
      if (callback) return callback(err);
    } else {
...
module.exports.getJobSequence = function (
  job_sequence_id,
  course_id,
  callback,
) {
...
module.exports.getJobSequenceAsync = util.promisify(
  module.exports.getJobSequence,
);
 ...
 */
module.exports.getJobSequenceWithFormattedOutput = function (
  job_sequence_id,
  course_id,
  callback,
) {
