...
module.stop = async function () {
  return new Promise((resolve) => {
    // Poll until all jobs are finished.
    const intervalId = setInterval(() => {
      if (Object.keys(module.exports.liveJobs).length === 0) {
  ...
  sqldb.query(sql.select_running_jobs, {}, function (err, result) {
    ...
    async.eachSeries(
      ...
      function (row, callback) {
        // Filter out rows that correspond to running jobs that we know about.
        if (_.has(module.exports.liveJobs, row.id)) {
          return callback(null);
        }

        // This job *might* be abandoned by the server, but it's also possible
        // that in between `select_running_jobs` and now, it actually finished
        // normally. `update_job_on_error` is written such that it will only
        // modify rows that still have `status = 'Running'`, so we can safely
        // execute it no matter what.
        logger.debug('Job possibly abandoned by server, id: ' + row.id);
        const params = {
          job_id: row.id,
          stderr: null,
          stdout: null,
