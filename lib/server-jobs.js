...
Job.prototype.finish = function(code, signal) {
    ...
    sqldb.queryOneRow(sql.update_job_on_close, params, function(err, result) {
        ...
        if (ERR(err, function() {})) {
            // In this case, we have no result. So, we cannot know whether or not
            // on_error has already been called. We have no choice but to call it
            ...
            // In this case, we check the result. If result.rows[0].update_job is false
            // this means the job status was no longer 'Running' - the only way for this
            // to be is if an 'error' event had already been handled. We don't want to
            // handle it again, so we only proceed if update_job is true.
            if (result.rows[0].update_job) {
...
module.exports.spawnJob = function(options, callback) {
    module.exports.createJob(options, function(err, job) {
        ...
        // when a process exits, first 'exit' is fired with stdio
