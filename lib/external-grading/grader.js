...
const externalGradingSocket = require('./socket');
const fileStoreFactory = require('./file-store-factory');
    ...
    async.series([
        ...
        (callback) => {
            ...
            queue.submitJob(message, (err) => {
                if (ERR(err, callback)) return;
                logger.info(`Successfully submitted external grading job ${grading_job.id}`);
                callback(null);
            });
        },
        (callback) => {
