  ...
  let barcode = null;
    ...
    for (let i = 0; i < patchSizes.length; i++) {
      for (let j = 0; j < segmentOptions.length; j++) {
        ...
        const segmentFilepaths = await _segmentJpeg(jpeg.pageNum, jpeg.jpegFilepath, jpeg.workingDir, logger, j);
        for (let k = 0; k < segmentFilepaths.length; k++) {
          barcode = await _decodeJpegSegment(segmentFilepaths[k], patchSizes[i]);
          if (barcode) {
            logger.info(`_decodeJpeg() found barcode, ${barcode}, with segmentation option  ${j} and framesize '${patchSizes[i]}'`);
            return barcode;
          }
        }
      }
