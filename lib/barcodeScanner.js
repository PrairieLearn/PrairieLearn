...
const _decodeJpegs = async (jpegs, logger) => {
  for (let i = 0; i < jpegs.length; i++) {
    ...
    logger.info(`_decodeJpegs() - start decoding of ${jpegs[i].jpegFilename} / page ${i + 1}`)
    jpegs[i]['barcode'] = await _decodeJpeg(jpegs[i], logger);
    const secondsElapsed = (new Date() - start) / 1000;
    logger.info(`_decodeJpegs() - took ${secondsElapsed} to decode ${jpegs[i].jpegFilename} / page ${i + 1}`)
  }
  return jpegs;
};
...
const _decodeJpeg = async (jpeg, logger) => {
  ...
  const patchSizes = ['small', 'medium', 'x-small', 'large', 'x-large'];

  if (!jpeg || typeof jpeg.pageNum != 'number' || !jpeg.jpegFilepath || !jpeg.workingDir) {
    throw Error('Invalid jpeg file or missing metadata');
  }
  const segmentFilepaths = await _segmentJpeg(jpeg.pageNum, jpeg.jpegFilepath, jpeg.workingDir, logger);
  ...
  for (let i = 0; i < segmentFilepaths.length; i++) {
    for (let j = 0; j < patchSizes.length; j++) {
      ...
      if (barcode) {
        logger.info('_decodeJpeg() found barcode', barcode);
        return barcode;
      }
    }
  }
 ...
 */
const _segmentJpeg = async (pageNum, jpegFilepath, workingDir, logger, algNum = 0) => {
  ...
  const algorithms = [
    ...
    // Priortize best algorithms to speed up process. Top-level decoded first. 
  ...
  logger.info(`Segmenting ${jpegFilepath} using imagemagick command ${algorithms[algNum].toString()} `)
  ...
  logger.info('_segmentJpeg() segmented jpeg into vertical slices', algorithms[algNum]);
  return _segmentJpeg(pageNum, jpegFilepath, workingDir, logger, algNum + 1);
};
...
const _convertPdf = async (numPages, data, originalName, logger) => {
  ...
  for (let i = 0; i < numPages; i++) {
    ...
    logger.info(`Created ${pdfFilename} for page ${i + 1}`);
    const { jpegFilename, jpegFilepath } = await _slicePageAsJpeg(i, pdfCollectionPath, workingDir);
    logger.info(`Created ${jpegFilename} for page ${i + 1}`);
    const secondsElapsed = (new Date() - start) / 1000;
