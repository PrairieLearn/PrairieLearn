...
const btoa = (s) =>
  Buffer.from(s.split('').map((c) => c.charCodeAt(0))).toString('base64');
...
module.exports.b64EncodeUnicode = function (str) {
  ...
  try {
    ...
    logger.error(
      `b64EncodeUnicode: returning empty string because failed to encode ${str}`,
    );
    return '';
  }
};
...
module.exports.b64DecodeUnicode = function (str) {
  ...
  try {
    ...
    logger.error(
      `b64DecodeUnicode: returning empty string because failed to decode ${str}`,
    );
    return '';
  }
};
