...
WITH submission_count_per_rubric_item AS (
    ...
    SELECT
        JSONB_AGG(
            TO_JSONB(ri) ||
            JSONB_BUILD_OBJECT('num_submissions', COALESCE(scpri.num_submissions, 0))
...
WITH rubric_items_data AS (
    SELECT JSONB_OBJECT_AGG(rgi.rubric_item_id, rgi) AS rubric_items
    FROM rubric_grading_items AS rgi
    WHERE rgi.rubric_grading_id = $rubric_grading_id
)
SELECT
    rg.*,
    rid.rubric_items
