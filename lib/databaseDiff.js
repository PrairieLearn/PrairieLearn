  ...
  async.series(
    [
      ...
      (callback) => {
        ...
        let missingFrom1 = _.difference(
          _.keys(description2.tables),
          _.keys(description1.tables),
        );
        let missingFrom2 = _.difference(
          _.keys(description1.tables),
          _.keys(description2.tables),
        );
        ...
        if (missingFrom1.length > 0) {
          result += formatText(
            `Tables added to ${db2NameBold} (${db2.type})\n`,
            colors.underline,
          );
          result += formatText(
      ...
      (callback) => {
        ...
        let missingFrom1 = _.difference(
          _.keys(description2.enums),
          _.keys(description1.enums),
        );
        let missingFrom2 = _.difference(
          _.keys(description1.enums),
          _.keys(description2.enums),
        ...
        if (missingFrom1.length > 0) {
          result += formatText(
            `Enums added to ${db2NameBold} (${db1.type})\n`,
            colors.underline,
          );
          result += formatText(
      ...
      (callback) => {
        ...
        _.forEach(intersection, (table) => {
          ...
          result += formatText(
            `Differences in ${boldTable} table\n`,
            colors.underline,
          );
      ...
      (callback) => {
        ...
        _.forEach(intersection, (enumName) => {
          ...
          if (
            description1.enums[enumName].trim() !==
            description2.enums[enumName].trim()
          ) {
    ...
    [
      ...
      (entries, callback) => {
        async.each(
          ...
          (entry, callback) => {
            fs.readFile(
              path.join(dirPath, 'tables', entry),
              'utf8',
              (err, data) => {
                if (ERR(err, callback)) return;
...
module.exports.diffDatabases = function (
  datbase1,
  database2,
  options,
  callback,
...
module.exports.diffDatabaseAndDirectory = function (
  database,
  directory,
  options,
  callback,
...
module.exports.diffDirectoryAndDatabase = function (
  directory,
  database,
  options,
  callback,
...
module.exports.diffDirectories = function (
  directory1,
  directory2,
  options,
  callback,
