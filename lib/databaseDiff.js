...
function diff(db1, db2, options, callback) {
  ...
  async.series(
    [
      ...
      (callback) => {
        // Determine if both databases have the same views
        let missingFrom1 = _.difference(_.keys(description2.views), _.keys(description1.views));
        let missingFrom2 = _.difference(_.keys(description1.views), _.keys(description2.views));

        let db2NameBold = formatText(db2Name, colors.bold);

        if (missingFrom1.length > 0) {
          result += formatText(`Views added to ${db2NameBold} (${db1.type})\n`, colors.underline);
          result += formatText(
            missingFrom1.map((viewName) => `+ ${viewName}`).join('\n') + '\n\n',
            colors.green
          );
        }
        ...
        if (missingFrom2.length > 0) {
          result += formatText(
            `Views missing from ${db2NameBold} (${db2.type})\n`,
            colors.underline
          );
          result += formatText(
            missingFrom2.map((viewsName) => `- ${viewsName}`).join('\n') + '\n\n',
            colors.red
          );
        }
      ...
      (callback) => {
        // Determine if the definitions of any views differ
        let intersection = _.intersection(_.keys(description1.views), _.keys(description2.views));
        _.forEach(intersection, (viewName) => {
          // We normalize each blob to end with a newline to make diffs print cleaner
          const diff = jsdiff.diffLines(
            description1.views[viewName].trim() + '\n',
            description2.views[viewName].trim() + '\n'
          );
          if (diff.length === 1) return;
          ...
          const boldView = formatText(viewName, colors.bold);
          result += formatText(`Differences in ${boldView} view\n`, colors.underline);
...
function loadDescriptionFromDisk(dirPath, callback) {
  ...
  async.waterfall(
    [
      ...
      (callback) => {
        fs.readdir(path.join(dirPath, 'views'), (err, entries) => {
          if (err) {
            if (err.code === 'ENOENT') {
              return callback(null);
            } else {
      ...
      (entries, callback) => {
        async.each(
          entries,
          (entry, callback) => {
            fs.readFile(path.join(dirPath, 'views', entry), 'utf8', (err, data) => {
              if (ERR(err, callback)) return;
              description.views[entry.replace('.pg', '')] = data;
              callback(null);
            });
          },
          (err) => {
