...
class PythonCaller {
    ...
    call(file, fcn, args, options, callback) {
        ...
        if (![CREATED, WAITING].includes(this.state)) return callback(new Error('invalid internal PythonCaller state for call()'));
        ...
        if (![CREATED, WAITING, EXITING, EXITED].includes(this.state)) return callback(new Error('invalid internal PythonCaller state for done()'));
