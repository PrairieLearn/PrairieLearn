...
class PythonCaller {
    ...
    _startChild() {
        ...
        if (config.useWorkers) {
            pythonTrampoline = path.join(__dirname, '..', 'python', 'python-caller-trampoline.py');
        } else {
            pythonTrampoline = path.join(__dirname, '..', 'python', 'python-caller-trampoline-no-fork.py');
        }
