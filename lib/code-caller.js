...
class PythonCaller {
    ...
    restart(callback) {
        debug('enter restart()');
        if (this.state == CREATED) return; // no need to restart if we don't have a worker
        this.call(null, 'restart', [], {}, (err, ret_val, consoleLog) => {
            if (ERR(err, callback)) return;
            if (ret_val != 'success') return callback(new Error(`Error while restarting: ${ret_val}`));
            debug('exit restart(), state: ' + String(this.state));
            callback(null);
        });
    }
    ...
    _startChild() {
        debug('enter _startChild(), state: ' + String(this.state));
        ...
        const options = {
            cwd: __dirname,
            stdio: ['pipe', 'pipe', 'pipe', 'pipe', 'pipe'], // stdin, stdout, stderr, and an extra one for data
            env,
        };
        ...
        this.state = WAITING;
        this._checkState();
        debug('exit _startChild(), state: ' + String(this.state));
    }

    _handleStderrData(data) {
        debug('enter _handleStderrData(), state: ' + String(this.state));
        ...
        debug('exit _handleStderrData(), state: ' + String(this.state));
    }

    _handleStdoutData(data) {
        debug('enter _handleStdoutData(), state: ' + String(this.state));
        this._checkState([IN_CALL, EXITING]);
        if (this.state == IN_CALL) {
            this.outputStdout += data;
            this.outputBoth += data;
        }
        debug('exit _handleStdoutData(), state: ' + String(this.state));
    }

    _handleStdio3Data(data) {
        debug('enter _handleStdio3Data(), state: ' + String(this.state));
        this._checkState([IN_CALL, EXITING]);
        if (this.state == IN_CALL) {
        ...
        debug('exit _handleStdio3Data(), state: ' + String(this.state));
    }

    _handleChildExit(code, signal) {
        debug('enter _handleChildExit(), state: ' + String(this.state));
        ...
        debug('exit _handleChildExit(), state: ' + String(this.state));
    }

    _handleChildError(error) {
        debug('enter _handleChildError(), state: ' + String(this.state));
        this._checkState([WAITING, IN_CALL, EXITING]);
        if (this.state == WAITING) {
        ...
        this._checkState();
        debug('exit _handleChildError(), state: ' + String(this.state));
    }

    _timeout() {
        debug('enter _timeout(), state: ' + String(this.state));
        ...
        this.state = EXITING;
        this._callCallback(err);
        debug('exit _timeout(), state: ' + String(this.state));
    }

    _clearTimeout() {
        debug('enter _clearTimeout(), state: ' + String(this.state));
        clearTimeout(this.timeoutID);
        this.timeoutID = null;
        debug('exit _clearTimeout(), state: ' + String(this.state));
    }

    _callCallback(err, data, output) {
        debug('enter _callCallback(), state: ' + String(this.state));
        if (err) err.data = this._errorData();
        const c = this.callback;
        this.callback = null;
        c(err, data, output);
        debug('exit _callCallback(), state: ' + String(this.state));
    }

    _callIsFinished() {
        debug('enter _callIsFinished(), state: ' + String(this.state));
        ...
        debug('exit _callIsFinished(), state: ' + String(this.state));
    }
    ...
    _logError(msg) {
        debug('enter _logError(), state: ' + String(this.state));
        const errData = this._errorData();
        logger.error(msg, errData);
        debug('exit _logError(), state: ' + String(this.state));
        return false;
    }
...
module.exports.waitForFinish = function(callback) {
    const testFinished = () => {
        if (_.size(activeCallers) == 0) return callback(null);
