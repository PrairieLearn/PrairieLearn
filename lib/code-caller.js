...
class PythonCaller {
    ...
    call(file, fcn, args, options, callback) {
        ...
        this.checkState([CREATED, WAITING, EXITING, EXITED]);
    ...
    callCallback(err, data, output) {
        ...
        if (err) err.data = this.errorData();
    ...
    callFinished() {
        ...
        try {
            ...
            err = new Error('Error decoding JSON: ' + e.message);
        }
        if (err) {
            this.state = EXITING;
            this.child.kill('SIGKILL');
    ...
    checkState(allowedStates) {
        ...
        if (this.state == CREATED) {
            ...
            return this.logError('Invalid state: ' + String(this.state));
        }
