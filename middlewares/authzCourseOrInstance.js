  ...
  async.series(
      ...
      (callback) => {
        ...
        sqldb.queryZeroOrOneRow(
          ...
          function (err, result) {
            ...
            const authn_course_instances =
              result.rows[0].course_instances || [];
            ...
            res.locals.authz_data = {
              ...
              authn_has_course_permission_preview:
                permissions_course.has_course_permission_preview,
              authn_has_course_permission_view:
                permissions_course.has_course_permission_view,
              authn_has_course_permission_edit:
                permissions_course.has_course_permission_edit,
              authn_has_course_permission_own:
              ...
              editable_courses: authn_courses.filter(
                (course) =>
                  course.permissions_course.has_course_permission_edit,
              ),
            };
            res.locals.user = res.locals.authz_data.user;
            if (isCourseInstance) {
              res.locals.course_instance = result.rows[0].course_instance;
              const permissions_course_instance =
                result.rows[0].permissions_course_instance;
    ...
    (err) => {
      ...
      if (
        ...
        debug(
          'requested overrides, but authn user does not have instructor permissions',
        );

        // If on a student page route, silently exit and ignore effective user requests
        if ((res.locals.viewType || 'none') == 'student') {
          debug(
            'on student page, so silently exit and ignore requested overrides',
          );
          return next();
        }

        debug(
          'not on student page, so clear all requested overrides and throw an error',
        );
        ...
        [
          ...
          (callback) => {
            ...
            const params = {
              ...
              course_instance_id: isCourseInstance
          ...
          (callback) => {
            ...
            if (req.cookies.pl_requested_date) {
              ...
              if (!req_date.isValid()) {
                debug(
                  `requested date is invalid: ${req.cookies.pl_requested_date}, ${req_date}`,
                );
                overrides.forEach((override) => {
                  debug(`clearing cookie: ${override.cookie}`);
                  res.clearCookie(override.cookie);
            ...
            const params = {
              ...
              req_date: req_date,
              req_mode:
                req.cookies.pl_requested_mode || res.locals.authz_data.mode,
              req_course_role: req.cookies.pl_requested_course_role || null,
              req_course_instance_role:
                req.cookies.pl_requested_course_instance_role || null,
            };
            ...
            sqldb.queryZeroOrOneRow(
              ...
              function (err, result) {
                ...
                if (result.rowCount == 0) {
                  ...
                  if (isCourseInstance) {
                    ...
                    if (
                      ...
                      res.locals.authz_data.authn_user.uid
                    ) {
                ...
                if (
                  !res.locals.authz_data.authn_has_course_permission_preview &&
                  result.rows[0].permissions_course
                    .has_course_permission_preview
                ) {
                  overrides.forEach((override) => {
                    debug(`clearing cookie: ${override.cookie}`);
                ...
                if (isCourseInstance) {
                  ...
                  if (
                    ...
                    result.rows[0].permissions_course_instance
                      .has_student_access_with_enrollment && // effective user is enrolled with access
                ...
                res.locals.authz_data.course_role =
                  result.rows[0].permissions_course.course_role;
                ...
                res.locals.authz_data.courses = (
                  result.rows[0].courses || []
                ).filter((course) => course.id == result.rows[0].course.id);
                res.locals.authz_data.courses.forEach(
                  (course) =>
                    (course.permissions_course = _.cloneDeep(
                      result.rows[0].permissions_course,
                    )),
                );
                res.locals.authz_data.editable_courses =
                  res.locals.authz_data.courses.filter(
                    (course) =>
                      course.permissions_course.has_course_permission_edit,
                  );
                ...
                res.locals.authz_data.course_instances =
                  res.locals.authz_data.course_instances.filter((ci) =>
                    res.locals.authz_data.authn_course_instances.some(
                      (authn_ci) => authn_ci.id == ci.id,
                    ),
                  );
