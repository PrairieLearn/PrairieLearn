...
- `workspaceOptions` contains the following properties:
  ...
  - `rewriteUrl` (optional): if true, the URL will be rewritten such that the workspace container will see all requests as originating from /. If false, the workspace container will see all requests as originating from /pl/workspace/&lt;workspace_id&gt;/container/. If not specified, the URL rewrite setting is retrieved from the workspace image label `com.prairielearn.workspace.url_rewrite`. This label is set to a value appropriate for the functionality of each workspace for PrairieLearn-maintained workspace images (or custom images using those as a base), so it doesn't need to be included in `info.json`.
...
- `com.prairielearn.workspace.url_rewrite`: whether to rewrite URLs such that the workspace container will see all requests as originating from /.
