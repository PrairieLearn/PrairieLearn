...
- Each table has an `id` number that is used for cross-referencing. For example, each row in the `questions` table has an `id` and other tables will refer to this as a `question_id`. For legacy reasons, the only exceptions are the `pl_courses` table that other tables refer to with `course_id` and `users` which has a `user_id`.
...
- To call our own library functions from async/await code, we provide a version of them with "Async" appended to the name:

  ```js
  import util from 'node:util';

  export function existingLibFun(x1, x2, callback) {
    callback(null, x1 * x2);
  }

  export const existingLibFunAsync = util.promisify(myFun);
...
- If our own library functions use multiple return values, then the async version of them should return an object:
  ...
  export function existingMultiFun(x, callback) {
    const y1 = x * x;
    const y2 = x * x * x;
    callback(null, y1, y2); // note the two return values here
  }

  export const existingMultiFunAsync = util.promisify((x, callback) =>
    existingMultiFun(x, (err, y1, y2) => callback(err, { y1, y2 })),
  );
...
- Use the [async-stacktrace library](https://github.com/Pita/async-stacktrace) for every callback-based error handler. That is, the top of every file should have `import ERR from 'async-stacktrace';` and wherever you would normally write `if (err) return callback(err);` you instead write `if (ERR(err, callback)) return;`. This does exactly the same thing, except that it modifies the `err` object's stack trace to include the current filename/linenumber, which greatly aids debugging. For example:
  ...
  // Instead do this:
  import ERR from 'async-stacktrace'; // at top of file
  function foo(p, callback) {
    bar(q, function (err, result) {
