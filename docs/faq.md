...
In these cases, you can store your files in the `tests` directory for the question, and reference them from the workspace by adding them to the `_workspace_files` list. For example, if files `xyz.c` and `xyz.h` should be included in both the workspace home folder and in `/grade/tests` in the autograder, save these files to `[QUESTION DIRECTORY]/tests`, then include, in server.py:
