...
| `partial-credit`      | string                  | Depends on `grading-method`    | For the `dag` grading method, you may specify `none` for no partial credit (defualt) or `lcs` for partial credit based on the LCS edit-distance from the student solution to some correct solution. For the other grading methods, using this property is not yet supported. Grading methods `unordered` and `ranking` will assign partial credit, and grading method `ordered` will not. |
