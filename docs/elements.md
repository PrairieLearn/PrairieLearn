...
| `ordering-feedback`   | string             | —       | Optional attribute used when `grading-method="dag"` and `correct=true` to provide specific feedback when the block is placed in the wrong position relative to the blocks it depends on (as defined by the `depends` attribute). This feedback is shown to the student after submission to help clarify ordering errors..                                              |
