...
| `partial-credit`      | string                  | Depends on `grading-method`    | Defaults to partial credit for grading methods `unordered` and `ranking`, and no partial credit for grading method `ordered`. For the `dag` grading method, you may specify `none` for no partial credit (defualt) or `lcs` for partial credit based on the LCS edit-distance from the student solution to some correct solution. |
