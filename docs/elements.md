...
| `display-simplified-expression` | boolean                 | `true`                  | Whether expressions submitted by students should be displayed in their simplified form. Setting `display-simplified-expression="false"` can prevent accidentally giving hints or revealing answers to students in the submission panel if they have multiple attempts to solve a question. Note that this setting does not affect grading behavior in any way. |
