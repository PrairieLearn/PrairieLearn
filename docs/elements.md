...
| `allow-blank`           | boolean | false   | Whether or not empty checkbox submissions are allowed. If `allow-blank` is set to `true`, the checkbox will be marked as incorrect instead of invalid if left empty. If `allow-blank` is enabled and `min-select` is specified, `allow-blank` takes precedence. |
