...
`depends` | string | "" | This attribute is used when `grading-method="dag"` to specify the directed acyclic graph relation among the blocks, with blocks being referred to by their `id`. For example, if `depends=1,3` for a particular block, it must appear later in the proof than the block with `id="1"` and the block with `id="3"`.
`subproof` | string | "" | Optional attribute when `grading-method="dag"`. Used to divide blocks into subsets that must appear contiguously with one another, for example in a proof by cases when all lines in the proof of one first case must appear before another case can be started.
...
* `dag`: in this method, the `depends` attibute of the `pl-answer` options are used to declare the directed acyclic graph relation between the blocks, and a correct answer is any topological sort of that directed acyclic graph. If the `subproof` option is also used to divide some of the answer choices into subproofs, then a correct answer is a topological sort of the lines of the proof with the added condition that the lines of each subproof must be listed contiguously.
