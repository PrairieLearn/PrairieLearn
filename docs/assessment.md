...
During a student's assessment instance, PrairieLearn has the ability to track the user's ip address, session id, and user agent (this includes the operating system, browser application, and version). These attributes together make up the client fingerprint. These fingerprints are then recorded during events while accessing the assessment instance. For example, when a student submits a question for grading, a record of the client fingerprint is saved along with the event. 

Each time the client fingerprint is recorded, PrairieLearn checks it against the previous client fingerprint used to access that assessment. PrairieLearn will display how many times the fingerprint has changed during the course of the assessment. You can see the amount of times a fingerprint changed by accessing the table in the students tab of an assessment. You may also see more detailed information about the client fingerprint by accessing the student's assessment instance and reviewing the Client Fingerprints column of the event log. 

Some fingerprint changes may occur during the course of an assessment, like a computer connecting to a different wi-fi access point or a student changing locations while working on homework. However, a high number of changes during an exam could be a possible indication of cheating. Many fingerprint changes could be an indication of multiple users or from accessessing the exam from a computer outside of the secure testing facility. While this is not an absolute method, it can be useful for an instructor that wishes to look further into a students legitimacy of an assessment instance. 
