...
During a student's assessment instance, PrairieLearn will track the user's IP address, session ID, and user agent (this includes the operating system, browser application, and version). These attributes together make up a client fingerprint. These fingerprints are then recorded during events while accessing the assessment instance. For example, when a student submits a question for grading, a record of the client fingerprint is saved along with the event.

Each time the client fingerprint is recorded, PrairieLearn checks it against the previous client fingerprint used to access that assessment. PrairieLearn will track and display how many times the fingerprint has changed during the course of the assessment. You can see the number of times a fingerprint changed on the Students tab of an assessment. You may also see more detailed information about the client fingerprint by accessing the student's assessment instance and reviewing the Fingerprint column of the event log.

Some fingerprint changes may occur during the course of an assessment, like a computer connecting to a different Wi-Fi access point or a student changing locations while working on homework. However, a high number of changes during an exam could be a possible indication of cheating. Many fingerprint changes could be an indication of multiple users or from accessessing the exam from a computer outside of the secure testing facility. While this is not an absolute method, it can be useful for an instructor that wishes to look further into the legitimacy of a student's work.
