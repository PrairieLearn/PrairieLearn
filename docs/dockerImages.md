...
!!! note "Testing natively on Mac OS"
    ...
    The configuration reference has a section on [running workspaces / external graders natively on Mac OS](./dev-guide/index.md#running-workspaces-external-graders-natively-on-mac-os). You will run into permissions errors if you don't do this. This does not apply to users running PrairieLearn via Docker.
