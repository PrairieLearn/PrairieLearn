...
!!! note "Testing natively on macOS"
    ...
    The configuration reference has a section on [running workspaces natively on macOS](./dev-guide/configJson.md#running-workspaces-natively-on-macos). You may run into permissions errors if you don't do this. This does not apply to users running PrairieLearn via Docker.
