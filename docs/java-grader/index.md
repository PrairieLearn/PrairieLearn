...
The Java autograder is based on [JUnit 5](https://docs.junit.org/5.14.3/user-guide/). In essence, individual tests correspond to individual annotated methods that run testing code, such as ([source](https://docs.junit.org/5.14.3/user-guide/#writing-tests)):
...
Each test found by JUnit will be provided as an individual result to the student. The name of the test result is based on the test's [display name](https://docs.junit.org/5.14.3/user-guide/#writing-tests-display-names). By default, each test will be worth one point. To change this default, a [tag](https://docs.junit.org/5.14.3/user-guide/#writing-tests-tagging-and-filtering) in the format `@Tag("points=XX")` (replacing `XX` with a number of points) must be provided. For example:
...
To change the order in which test results are shown to the user, you may use [the `@TestMethodOrder` annotation](https://docs.junit.org/5.14.3/user-guide/#writing-tests-test-execution-order).

To produce output that will be visible to students, tests may be optionally declared with [an argument of type `TestReporter`](https://docs.junit.org/5.14.3/user-guide/#writing-tests-dependency-injection). Calling [the `publishEntry` method for the test reporter](https://docs.junit.org/5.14.3/api/org.junit.jupiter.api/org/junit/jupiter/api/TestReporter.html) will cause the provided entry to be printed as the output of the test. For example:
...
JUnit 5 supports tests that are generated dynamically. These include [parameterized tests](https://docs.junit.org/5.14.3/user-guide/#writing-tests-parameterized-tests), [repeated tests](https://docs.junit.org/5.14.3/user-guide/#writing-tests-repeated-tests) and [test factories](https://docs.junit.org/5.14.3/user-guide/#writing-tests-dynamic-tests).
