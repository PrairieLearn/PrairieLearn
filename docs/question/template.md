...
!!! info "HTML rendering"
    ...
    By default, Mustache escapes special characters when using `{{..}}`. You can use triple-braces (e.g. `{{{params.html}}}`) to substitute raw HTML (and perform no escaping). This should only be used for content that is defined by the instructor. If you want to render student-provided HTML, you can use the [`<pl-xss-safe>` element](../elements.md#pl-xss-safe-element).
