...
**Note that time limits should not be set for exams in the CBTF (Computer-Based Testing Facility) or course-managed PrairieTest sessions. Instead, such exams should set `"mode": "Exam"`, in which case `timeLimitMin` will have no effect and the time limits will be enforced by PrairieTest.**
...
To require that students take a particular exam in the Computer-Based Testing Facility (CBTF) or a course-managed PrairieTest session, the `examUuid` should be set to the UUID value from PrairieTest. For example:
