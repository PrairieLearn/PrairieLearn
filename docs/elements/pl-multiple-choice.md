...
| `builtin-grading`            | boolean                                              | true                        | Whether the element performs its own grading. If `false`, grading must be handled by a `grade()` function in `server.py`. See [Disabling built-in grading](#disabling-built-in-grading) for details. |
