...
| `builtin-grading`            | boolean                                              | true                        | Whether the element performs its own grading. If `false`, grading must be handled externally (e.g., by `server.py`). The element will not validate that at least one correct answer exists, and will skip its built-in grading logic. |
