...
| `match-length`     | Checks whether all lines drawn with the specified tools have the specified length.                                           | `line`                                                              | `length`: reference length (in canvas units). `tolerance`: maximum allowed difference (in pixels) between the line's length and the expected length (default: 15).                                                                                                                                                                                                                                                                                                                             |
| `match-angle`      | Checks whether all lines drawn with the specified tools have the specified angle with respect to the x-axis.                 | `line`                                                              | `angle`: expected angle in degrees. `allow-flip`: If `true` (default: `false`), the angle and the angle + 180 degrees are considered correct (i.e., endpoints of the line can be flipped). `tolerance`: maximum allowed difference (in degrees) between the line's angle and the expected angle (default: 15).                                                                                                                                                                                 |
