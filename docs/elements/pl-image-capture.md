...
| `manual-upload-enabled`  | boolean | false   | When `true`, students can click "Upload image" to upload JPEG, PNG, HEIC, or HEIF images already stored on their device. HEIC/HEIF images are automatically converted to JPEG in the browser before submission. This is especially useful for students who write on a tablet and export their work as an image. In most cases, `manual-upload-enabled` should be `false` to ensure students submit original work. |
...
AI grading supports `pl-image-capture`. Handwritten mathematical work, graphs, diagrams, and any other free-form image submissions can be graded automatically. Questions can combine `pl-image-capture` with other supported elements such as [`pl-file-upload`](pl-file-upload.md); each distinct submitted file is sent to the model once. Learn more in the [AI grading docs](../aiGrading/index.md).
