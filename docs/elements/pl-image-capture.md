...
| `manual-upload-enabled`  | boolean | false   | When `true`, students can click "Upload image" to upload JPEG, PNG, HEIC, or HEIF images already stored on their device. HEIC and HEIF images are converted to JPEG when submitted. This is especially useful for students who write on a tablet and export their work as an image. In most cases, `manual-upload-enabled` should be `false` to ensure students submit original work. |
