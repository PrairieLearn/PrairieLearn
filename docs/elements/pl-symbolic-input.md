...
| `allowed-values`                | string                  | `all`                   | Comma-separated types accepted from a submitted answer: `expression`, `set`, or `interval`. `set` accepts only finite sets, including the empty set; `interval` accepts only intervals and the empty set. Use `all` to accept every type of submission.                                                                                         |
...
| `correct-answer`                | string                  | See description         | Correct answer for grading. Defaults to `data["correct_answers"][answers-name]`. A blank correct answer (`correct-answer=""`) is only valid when `allow-blank="true"` and `blank-value=""`.                                                                                                                                                     |
...
The `allowed-values` attribute can further restrict which types of parsed values are accepted.
