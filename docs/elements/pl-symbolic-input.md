...
| `allowed-types`                 | string                  | `expression`            | Comma-separated types accepted from a submitted answer: `expression`, `set`, or `interval`. `set` accepts only finite sets, including the empty set; `interval` accepts only intervals and the empty set. Use `all` to accept every type of submission.                                                                                         |
