...
| `display-blocks`      | `vertical` or `inline-wrap` or `inline-nowrap`                    | `vertical`                        | `vertical` stacks blocks vertically, `inline-wrap` and `inline-nowrap` position blocks in a single row with `inline-wrap` wrapping blocks onto multiple lines as needed, and `inline-nowrap` forcing all blocks onto a single line with horizontal scrolling.                                                                                                                                                                                                            |
...
| `partial-credit`      | `"none"` or `"lcs"`                                               | `"none"`                          | For the `"dag"`, `"ordered"`, and `"ranking"` grading methods, you may specify `"none"` for no partial credit or `"lcs"` for partial credit based on the LCS edit-distance from the student solution to some correct solution. `partial-credit` defaults to `"none"` for the `"dag"`, `"ordered"`, and `"ranking"` grading methods. Grading method `"unordered"` will always assign partial credit.                                                                      |
...
| `source-blocks-order` | `"random"`, `"ordered"`, `"alphabetized"`, or `"random-sections"` | `"alphabetized"`                  | The order of the blocks in the source area. See more details below.                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `indent`              | integer in [-1, 4] | -1      | Specifies the correct indentation level of the block. For example, `2` means the block should be indented twice, while `-1` means indentation is ignored for that block. When using `indentation="true"`, set `indent` on any block whose indentation should be graded. Omitting `indent` is equivalent to `-1`. This attribute can only be used when `indentation="true"`.                                                                                                  |
...
| `initially-placed`    | boolean            | false   | Optional attribute only allowed in correct blocks without distractors. When used, the block will initially appear in the dropzone.                                                                                                                                                                                                                                                                                                                                           |
...
| ---------------- | -------------------------------- |
| `inline="true"`  | `display-blocks="inline-nowrap"` |
| `inline="false"` | `display-blocks="vertical"`      |
...
- `random-sections`: the blocks are shuffled, but distractor groups always appear after all individual blocks.
