...
| `ace-mode`           | string  | —                    | Specifies an Ace editor mode to enable things like intelligent code indenting and syntax highlighting (see below).                                                                                                                                                                                                                                                                                                                                        |
...
The `ace-mode` attribute is typically set to `ace/mode/<language_name>`, where `<language_name>` is the name of a language supported by the Ace editor (e.g, `ace/mode/python`, `ace/mode/c_cpp`, `ace/mode/java`, etc.). Ace supports a number of built-in modes from the list of modes in the [Ace GitHub repository](https://github.com/ajaxorg/ace/tree/master/src/mode). Alternatively, instructors may define a custom mode, which can be an alternative for languages not natively supported by Ace, for course-created languages, or for cases where instructors may wish to limited the support for a language to a set of pre-determined features. The [Ace documentation](https://ace.c9.io/#nav=highlighter) provides more information on how to create custom modes. Once the mode is created, it can be saved as `ace/mode/mode-<language_name>.js` inside [`clientFilesQuestion` or `clientFilesCourse`](../clientServerFiles.md). The `pl-file-editor` element will automatically identify that a custom mode exists and load the custom mode file when the `ace-mode` attribute is set appropriately.
