...
| `ace-mode`           | string  | —                    | Specifies an Ace editor mode to enable things like intelligent code indenting and syntax highlighting. It can be set to a built-in mode from the list of modes in the [Ace GitHub repository](https://github.com/ajaxorg/ace/tree/master/src/mode). Alternatively, a custom mode can be created for the course, with the mode file saved as `mode-<mode-name>.js` inside [`clientFilesCourse`](../clientServerFiles.md); see the [Ace documentation](https://ace.c9.io/#nav=highlighter) for more information on creating custom modes. |
