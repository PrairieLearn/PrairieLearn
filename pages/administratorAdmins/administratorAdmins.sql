  ...
  select_question_render_cache_stats AS (
    SELECT
      jsonb_build_object(
        ...
        AVG(
          pvl.panel_render_cache_hit_count / pvl.panel_render_count
        ),
        'panel_cache_hit_rate',
        (
          CAST(SUM(pvl.panel_render_cache_hit_count) AS FLOAT) / SUM(pvl.panel_render_count)
        )
