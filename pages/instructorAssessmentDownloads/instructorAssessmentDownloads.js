...
const setFilenames = function (locals) {
  ...
  locals.submissionsForManualGradingCsvFilename =
    prefix + 'submissions_for_manual_grading.csv';
  ...
  locals.filesForManualGradingZipFilename =
    prefix + 'files_for_manual_grading.zip';
...
router.get('/:filename', function (req, res, next) {
  ...
  if (req.params.filename == res.locals.scoresCsvFilename) {
    ...
    req.params.filename == res.locals.submissionsForManualGradingCsvFilename
  ) {
    let params = {
      assessment_id: res.locals.assessment.id,
      group_work: res.locals.assessment.group_work,
    ...
    if (res.locals.assessment.group_work) {
      submissionColumn = identityColumn.concat([
        ['SubmitStudent', 'submission_user'],
      ]);
    }
    sqldb.query(
    ...
    req.params.filename == res.locals.filesForManualGradingZipFilename
  ) {
    const params = {
      assessment_id: res.locals.assessment.id,
      group_work: res.locals.assessment.group_work,
