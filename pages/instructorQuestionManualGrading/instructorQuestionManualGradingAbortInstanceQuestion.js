...
router.get('/', function(req, res, next) {
    ...
    sqldb.queryOneRow(sql.instance_question_abort_manual_grading, params, function(err, result) {
        if (ERR(err, next)) return callback(error.makeWithData(`Cannot find instance question: ${res.locals.instance_question_id}`));
        if (result.rowCount > 0) { res.redirect(redirectUrl)}
    });
});
