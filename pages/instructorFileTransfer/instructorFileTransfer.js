        ...
        sqldb.queryOneRow(
          ...
          (err, result) => {
            ...
            if (file_transfer.transfer_type != 'CopyQuestion') {
              return callback(
                new Error(`bad transfer_type: ${file_transfer.transfer_type}`),
              );
            }
            if (file_transfer.user_id != user_id) {
              return callback(
                new Error(
                  `must have same user_id: ${file_transfer.user_id} and ${user_id}`,
                ),
              );
            }
            callback(null);
...
router.get('/:file_transfer_id', function (req, res, next) {
  if (config.filesRoot == null)
    return next(new Error('config.filesRoot is null'));
  getFileTransfer(
    ...
    (err, file_transfer) => {
      ...
      const question_exploded = path
        .normalize(file_transfer.from_filename)
        .split(path.sep);
      const questions_dir_idx = question_exploded.findIndex(
        (x) => x == 'questions',
      );
