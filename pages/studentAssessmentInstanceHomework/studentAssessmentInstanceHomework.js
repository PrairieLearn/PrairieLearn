    ...
    (err, updated) => {
      ...
      sqldb.queryOneRow(
        ...
        (err, result) => {
          ...
          debug(
            'selected assessment_instance.id:',
            locals.assessment_instance.id,
          );
          callback(null);
        },
          ...
          if (res.locals.assessment.group_work) {
            sqldb.query(sql.get_group_info, params, function (err, result) {
              ...
              res.locals.join_code =
                res.locals.group_info[0].name +
                '-' +
                res.locals.group_info[0].join_code;
...
router.post('/', function (req, res, next) {
  ...
  if (req.body.__action == 'attach_file') {
    util.callbackify(studentAssessmentInstance.processFileUpload)(
      req,
      res,
      function (err) {
        if (ERR(err, next)) return;
    ...
    util.callbackify(studentAssessmentInstance.processTextUpload)(
      req,
      res,
      function (err) {
        if (ERR(err, next)) return;
    ...
    util.callbackify(studentAssessmentInstance.processDeleteFile)(
      req,
      res,
      function (err) {
        if (ERR(err, next)) return;
    ...
    if (!res.locals.authz_result.active)
      return next(error.make(400, 'Unauthorized request.'));
    const params = {
      assessment_instance_id: res.locals.assessment_instance.id,
      user_id: res.locals.user.user_id,
