...
router.post('/', (req, res, next) => {
  ...
  if (req.body.__action == 'course_permissions_insert_by_multi_user_uid') {
    ...
    if (
      !['None', 'Previewer', 'Viewer', 'Editor'].includes(req.body.course_role)
    ) {
      return next(
        error.make(
          400,
    ...
    if (
      course_instance &&
      !['Student Data Viewer', 'Student Data Editor'].includes(
        ...
        sqldb.call(
          ...
          (err, result) => {
            if (
              ERR(err, (e) =>
                logger.verbose(
                  `Failed to insert course permission for uid: ${uid}`,
                  e,
                ),
              )
            ) {
            ...
            sqldb.call(
              ...
              (err, _result) => {
                if (
                  ERR(err, (e) =>
                    logger.verbose(
                      `Failed to insert course instance permission for uid: ${uid}`,
                      e,
                    ),
                  )
                ) {
    ...
    if (
      ...
      return next(
        error.make(403, 'Owners cannot change their own course content access'),
      );
    }
    ...
    req.body.__action == 'course_instance_permissions_update_role_or_delete'
  ) {
    ...
    if (req.body.course_instance_id) {
      if (
        !res.locals.authz_data.course_instances.find(
          (ci) => ci.id == req.body.course_instance_id,
        )
      ) {
        return next(error.make(400, `Invalid requested course instance role`));
