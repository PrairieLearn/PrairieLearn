...
SELECT
    ...
    u.user_id, u.uid, ai.group_id AS gid, u.name, coalesce(e.role, 'None'::enum_role) AS role,
...
FROM
    ...
    LEFT OUTER JOIN group_users AS gu ON (ai.group_id = gu.group_id)
    LEFT OUTER JOIN users AS u ON (CASE WHEN $group_work THEN u.user_id = gu.user_id
                                        ELSE u.user_id = ai.user_id
                                   END)
    LEFT JOIN enrollments AS e ON (e.user_id = u.user_id AND e.course_instance_id = a.course_instance_id)
WHERE
    a.id = $assessment_id
ORDER BY
    e.role DESC, u.uid, gid, u.user_id, ai.number, ai.id;
