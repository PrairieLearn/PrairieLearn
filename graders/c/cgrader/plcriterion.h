...
static void pl_custom_output_provider(FILE *f, struct criterion_global_stats *stats)
{
  fprintf(f, "{\"test_suites\": [\n");
  for (struct criterion_suite_stats *ss = stats->suites; ss; ss = ss->next)
  {
    fprintf(f, "  {\"name\": \"%s\", \"tests\": [\n", ss->suite->name);
    ...
    for (struct criterion_test_stats *ts = ss->tests; ts; ts = ts->next)
    {
      ...
      for (struct criterion_assert_stats *asrt = ts->asserts; asrt; asrt = asrt->next)
      {
        if (!asrt->passed)
        {
          fprintf(f, "%s", comma);
          comma = ",\n";
          // TODO Escape message (cri_escape_str?)
          fprintf(f, "      \"%s\"", asrt->message && *asrt->message ? asrt->message : "(no message)");
        }
      }
      fprintf(f, "\n    ]}%s\n", ts->next ? "," : "");
    }
...
{
  (void)set;
  cr_log_info("Setting up sandboxed environment for tests.");
  criterion_register_output_provider("pl", pl_custom_output_provider);
}
