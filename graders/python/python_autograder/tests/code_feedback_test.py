...
import pytest
from python_autograder.code_feedback import Feedback, GradingComplete
...
class ErrorCodes:
    ...
    #     return f"{self.NAME} contains an extra key: {key}"
...
@patch("python_autograder.code_feedback.Feedback.add_feedback")
...
def test_check_dict_correct_check_only_1(mock_file, mock_add_feedback):
    ...
    ref_dict = {"a": 1, "b": 2, "c": 3}
    student_dict = {"a": 1, "b": 2, "c": 3}
    assert Feedback.check_dict(NAME, ref_dict, student_dict, check_only_keys=True, check_only_values=True)
    mock_add_feedback.assert_not_called()
...
def test_check_dict_extra_key(mock_file, mock_add_feedback):
    ...
    assert not Feedback.check_dict(NAME, ref_dict, student_dict)
    mock_add_feedback.assert_called_with(errorCodes.keysLengthMismatch(ref_dict, student_dict))
...
def test_check_dict_partial_keys_missing(mock_file, mock_add_feedback):
    ...
    mock_add_feedback.assert_called_with(errorCodes.missingPartialKey(student_dict, partial_keys))
...
def test_check_dict_check_only_values(mock_file, mock_add_feedback):
    ...
    assert Feedback.check_dict("test_dict", ref_dict, student_dict, check_only_values=True)
    mock_add_feedback.assert_not_called()
...
def test_check_dict_wrong_key_type(mock_file, mock_add_feedback):
    ...
    mock_add_feedback.assert_called_with(errorCodes.KeyTypeError(student_dict, entry_type_key = str))
...
def test_check_dict_wrong_key_type1_only_keys(mock_file, mock_add_feedback):
    ...
    ref_dict = {"1": 1, "2": 2, "3": 3}
    student_dict = {1: 10, 2: 20, 3: 30}
    assert not Feedback.check_dict(NAME, ref_dict, student_dict, check_only_keys=True, entry_type_key= str)
    mock_add_feedback.assert_called_with(errorCodes.KeyTypeError(student_dict, entry_type_key = str))
...
@patch("python_autograder.code_feedback.Feedback.add_feedback")
@patch("builtins.open", new_callable=mock_open)
def test_check_dict_wrong_key_type1_only_values(mock_file, mock_add_feedback):
    """Test when only keys are compared and they match."""
    ref_dict = {"1": 1, "2": 2, "3": 3}
    student_dict = {1: 10, 2: 20, 3: 30}
    assert not Feedback.check_dict(NAME, ref_dict, student_dict, check_only_values=True, entry_type_key= int)
...
def test_check_dict_wrong_key_type1_only_values(mock_file, mock_add_feedback):
    ...
    student_dict = {1: 10, 2: 20, 3: 30}
    assert not Feedback.check_dict(NAME, ref_dict, student_dict, check_only_values=True, entry_type_key= int)
    mock_add_feedback.assert_called_with(errorCodes.valuesLengthMismatch(ref_dict, student_dict))
...
def test_check_dict_wrong_key_type1_only_values_only_keys(mock_file, mock_add_feedback):
    ...
    ref_dict = {"1": 1, "2": 2, "3": 3, "4": 4}
    student_dict = {1: 10, 2: 20, 3: 30}
    assert not Feedback.check_dict(NAME, ref_dict, student_dict, check_only_keys= True, check_only_values=True, entry_type_key= int)
    mock_add_feedback.assert_called_with(errorCodes.valuesLengthMismatch(ref_dict, student_dict))
...
def test_check_dict_wrong_value_type(mock_file, mock_add_feedback):
    ...
    mock_add_feedback.assert_called_with(errorCodes.ValueTypeError(student_dict, entry_type_value = int))
...
def test_check_dict_with_type_mismatch(mock_file, mock_add_feedback):
    ...
    student_dict = {"a": 1, "b": 100, "c": ["1", "2", "3"]}  # Key 'c' still has the value of type 
    assert not Feedback.check_dict(NAME, ref_dict, student_dict, entry_type_value = int)
    mock_add_feedback.assert_called_with(errorCodes.ValueTypeError(student_dict, entry_type_value = int))
...
def test_check_dict_wrong_value_type1_only_values(mock_file, mock_add_feedback):
    ...
    assert not Feedback.check_dict(NAME, ref_dict, student_dict, check_only_values=True, entry_type_value = int)
    mock_add_feedback.assert_called_with(errorCodes.ValueTypeError(student_dict, entry_type_value = int))
