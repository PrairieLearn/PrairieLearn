...
Named arrays now use node-postgres array serialization, just like positional arrays. For JSON values, pass `JSON.stringify(value)` and use the appropriate `::json` or `::jsonb` cast. Positional parameter behavior is unchanged.
