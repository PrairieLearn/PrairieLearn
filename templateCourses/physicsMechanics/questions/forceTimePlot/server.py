...
def generate(data):
    ...
    if random.choice([0,1]): v2 = random.choice([60,80,100,120])
    v3 = 0
    ...
    A1 = v0*mid[0]*T/(ratiox**ratioy) + s1/2*( (mid[0]*T)**2 )/(ratiox**2)
    A2 = v1*(mid[1]-mid[0])*T/(ratiox*ratioy) + (s2/2)*( (mid[1]-mid[0])*T/ratiox )**2
    A3 = v2*(4-mid[1])*T/(ratiox*ratioy) + s3/2*( (4-mid[1])*T/ratiox )**2
    # impulse 
