...
def generate(data):
    ...
    mu = np.random.choice([0.1,0.2,0.3,0.4])
    a = (m2*g - m1*g*np.sin(theta*np.pi/180) - m1*g*np.cos(theta*np.pi/180)*mu )/(m1+m2)
    T = m2*(g-a)
    
    data["params"]["m1"] = float(m1)
    data["params"]["m2"] = float(m2)
    data["params"]["theta"] = float(theta)
    data["params"]["mu"] = float(mu)
    ...
    y3 = height_canvas - (
        (base_triangle - posbox) * np.tan(theta_rad)
        + (hbox / 2) * np.cos(theta_rad)
    )

    xcoord = x5 + posbox + (1.5 * hbox) * np.sin(theta_rad)
    ycoord = height_canvas - (
        (base_triangle - posbox) * np.tan(theta_rad)
        + (1.5 * hbox) * np.cos(theta_rad)
    )
