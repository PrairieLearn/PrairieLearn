...
def create_label_list(Ori,p,ratio,ox=0,oy=-10):
    x1 = int(Ori[0]+p[0])
    y1 = int(Ori[1]-p[1])
    return [x1,y1,"("+str(int(p[0]/ratio))+","+str(int(p[1]/ratio))+")", ox, oy]
...
def generate(data):
    ...
    problem = "accel"   # this variable can be "accel" or "vel"
    # this is the initial velocity or initial position of the vehicle
    initial = random.randint(10,20)
    ...
    if random.choice([0,1]): v2 = random.choice([-1,1])*random.choice([40,60,80,100,120])
    v3 = random.choice([-1,1])*random.choice([40,60,80,100,120])
    if v3 == v2: v3 = v2+40
    v4 = v3 # constant interval
    if random.choice([0,1]): v4 = random.choice([-1,1])*random.choice([40,60,80,100,120])
