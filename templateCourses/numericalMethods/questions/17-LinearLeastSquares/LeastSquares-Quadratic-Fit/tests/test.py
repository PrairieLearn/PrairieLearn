...
class Test(PLTestCaseWithPlot):
    ...
    def test_0(self):
        ...
        if feedback.check_scalar(
            "c0", self.ref.c0, self.st.c0, rtol=1e-2, atol=1e-5, accuracy_critical=False
        ):
            score = 1
    ...
    def test_1(self):
        score = 0
        if feedback.check_scalar(
            "c1", self.ref.c1, self.st.c1, rtol=1e-2, atol=1e-5, accuracy_critical=False
        ):
            score = 1
        feedback.set_score(score)
    ...
    def test_2(self):
        score = 0
        if feedback.check_scalar(
            "c2", self.ref.c2, self.st.c2, rtol=1e-2, atol=1e-8, accuracy_critical=False
        ):
            score = 1
        feedback.set_score(score)
    ...
    def test_3(self):
        ...
        line = (
            self.ref.data[0],
            self.ref.c0
            + self.ref.c1 * self.ref.data[0]
            + self.ref.c2 * (self.ref.data[0] ** 2),
        )
