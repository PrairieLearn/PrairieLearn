...
def generate(data):
	rOP = np.array(random.choice([[1, 1, 0], [0, 2, 0], [-1, 1, 0], [1, -1, 0], [0, -2, 0], [-1, -1, 0]]))
	...
	if np.dot(rOP, eQT) == 0:
		...
		alpha2 = (omega1**2 * np.dot(rOP, eQN) + omega2**2 * np.dot(rPQ, eQN))/np.dot(rPQ, eQT)
		alpha1 = randIntNonZero(-2, 2)
	else:
		omega2 = randIntNonZero(-2, 2)
		omega1 = -omega2 * np.dot(rPQ, eQT)/np.dot(rOP, eQT)
		alpha2 = randIntNonZero(-2, 2)
		alpha1 = (omega1**2 * np.dot(rOP, eQN) + omega2**2 * np.dot(rPQ, eQN) - alpha2 * np.dot(rPQ, eQT))/np.dot(rOP, eQT)
	...
	[bottomLeft, bottomRight, topLeft, topRight, center, extent] = boundingBox2D([point1, point2, point3, point4, g00, g01, g10, g11])
	...
	givenPotList = ["\\vec{\\alpha}_1", "\\vec{\\alpha}_2", "\\vec{a}_P", "\\vec{a}_Q", "\\vec{F}_P", "\\vec{F}_Q", "\\vec{M}"]

	vectorList = givenPotList[2:6]
	UnitsDictionary = {"\\vec{\\alpha}_1": "{\\rm\\ rad/s^2}", 
	...
	alpha1 = np.array(alpha1).astype(np.float64).reshape(3,)
	alpha2 = np.array(alpha2).astype(np.float64).reshape(3,)
	aP = np.array(aP).astype(np.float64).reshape(3,)
	aQ = np.array(aQ).astype(np.float64).reshape(3,)
	M = np.array(M).astype(np.float64).reshape(3,)
	Fp = np.array(Fp).astype(np.float64).reshape(3,)
	Fq = np.array(Fq).astype(np.float64).reshape(3,)

	valsDictionary = {"\\vec{\\alpha}_1": alpha1, 
					   "\\vec{\\alpha}_2": alpha2,
					   "\\vec{a}_P": aP,
					   "\\vec{a}_Q": aQ,
					   "\\vec{M}": M,
	...
	for i in range(len(givenPotList)):
		for j in range(len(givenList)):
			if givenList[j] == givenPotList[i]:
				...
				var_output = f'<variable params-name="{plVars[i]}">{plVars[i]}</variable>'
	...
	for i in range(len(givenPotList)):
		if givenPotList[i] in givenList:
			givenPotList[i] = givenPotList[i] + ' &amp;= ' + f'{cartesianVector(valsDictionary[givenPotList[i]])}' + UnitsDictionary[givenPotList[i]]
		else:
			givenPotList[i] = '&amp;'
