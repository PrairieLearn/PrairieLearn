...
def generate(data):
	...
	PList = [{"P": rectOffset, "desr": "one corner of the rectangle", "offsets": [[1, 0], [-1, 0], [0, -1]]}, 
	         {"P": [rectOffset[0] + width, rectOffset[1]], "desr": "one corner of the rectangle", "offsets": [[0, -1]]}, 
	         {"P": [rectOffset[0] + width, rectOffset[1] - height], "desr": "one corner of the rectangle", "offsets": [[0, 1]]}, 
	         {"P": [rectOffset[0], rectOffset[1] - height], "desr": "one corner of the rectangle", "offsets": [[1, 0], [-1, 0], [0, 1]]}, 
	         {"P": [diskOffset[0] - length, diskOffset[1]], "desr": "the edge of the disk", "offsets": edgeOffsets}, 
	         {"P": [diskOffset[0] - length - radius, diskOffset[1] + radius], "desr": "the edge of the disk", "offsets": [[0, -1]]}, 
	         {"P": [diskOffset[0] - length - radius, diskOffset[1] - radius], "desr": "the edge of the disk", "offsets": [[0, 1]]}, 
	         {"P": [diskOffset[0] - length - 2 * radius, diskOffset[1]], "desr": "the edge of the disk", "offsets": [[1, 0]]}]
	...
	C = (R @ (C[0:-1].reshape(2,1))).reshape(2,)
	O = np.zeros(2)

	# Disk
	CDisk = (R @ CDisk.reshape(2,1)).reshape(2,)
	...
	line_2 = (R @ line_2.reshape(2,1)).reshape(2,)
	...
	# Rectangle
	CRect = (R @ CRect.reshape(2,1)).reshape(2,)
