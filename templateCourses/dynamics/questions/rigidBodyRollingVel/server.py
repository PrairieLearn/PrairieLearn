...
def generate(data):
	pythTriples = [[3, 4, 5], [6, 8, 10], [5, 12, 13], [9, 12, 15], [8, 15, 17], [12, 16, 20], [7, 24, 25]]
	triple = random.choice(pythTriples)
	rCQchoice = triple[slice(0, 2)]
	rCQElem = random.sample(rCQchoice, 2)
	rCQ = np.array([random.choice([-1, 1])*rCQElem[0], random.choice([-1, 1])*rCQElem[1], 0])
	...
	drawGround = ground(np.array([300, 150 + data['params']['r']]), np.array([0, 1, 0]), 1000)
