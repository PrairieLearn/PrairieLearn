...
def generate(data):
	...
	if QPos == 'left':
		aQ = np.array([1, 1, 0]) * (-s * random.randint(2, 9)) + np.array([random.randint(2, 9), 0, 0])
		Qx = 90
		Qy = 95
		rCQHat = np.array([-1, 0, 0])
	elif QPos == 'top':
		aQ = np.array([-s * random.randint(2, 9), 0, 0]) + np.array([0, -random.randint(2,9), 0])
		Qx = 150
		Qy = 35
	...
	if givenExp == 'r':
		...
		omega = np.sqrt(np.dot(aQ, rAlphaDirPerp)/(-r)/np.dot(rCQHat, rAlphaDirPerp))
		questionStatement = random.choice(['angular acceleration', 'angular velocity magnitude'])
		...
		givenExp = "angular velocity $\\vec\\omega = " + cartesianVector(np.array([0, 0, omega])) + "\\rm\\ rad/s$"
		...
		givenExp = "angular acceleration $\\vec\\alpha = " + cartesianVector(np.array([0, 0, alpha])) + "\\rm\\ rad/s^2$"
