...
def generate(data):
	...
	alpha1 = int((-alpha2 * np.dot(rPQ, eQT) + omega1**2 * np.dot(rOP, eQN) + omega2**2 * np.dot(rPQ, eQN))/np.dot(rOP, eQT))

	aP = alpha1*rOPPerp - omega1**2 * rOP
	aQ = aP + alpha2*rPQPerp - omega2**2 * rPQ

	vars_given_list = ["\\vec{a}_Q", "\\vec{\\alpha}_1", "\\vec{\\alpha}_2", "\\vec{a}_P"]
	...
	if findVar == "\\vec{\\alpha}_1":
		...
		findUnits = "rad/s^2"
		data['params']['submitAnswer'] = '''<pl-number-input answers_name="ansScalar" comparison="relabs" rtol="1e-2" atol="1e-5" size="15" show-placeholder="false"></pl-number-input>$\\hat{k}$'''
		...
		findUnits = "m/s^2"
		data['params']['submitAnswer'] = '''<pl-number-input answers_name="ansValue1" comparison="relabs" rtol="1e-2" atol="1e-5" display="inline" size="15" show-placeholder="false"></pl-number-input> $\\hat{\\imath} + $\
