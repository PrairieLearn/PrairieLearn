...
def generate(data):
    ...
    y1 = p1[0] * x**2 + p1[1] * x + p1[2]
    y2 = p2[0] * x**2 + p2[1] * x + p2[2]

    m = integrate(rho * (y2 - y1), (x, x1, x2))

    rC = 1 / m * integrate(integrate(rho * Matrix([x, y, 0]), (y, y1, y2)), (x, x1, x2))

    IC = integrate(
        integrate(rho * ((x - rC[0]) ** 2 + (y - rC[1]) ** 2), (y, y1, y2)), (x, x1, x2)
    )

    data["params"]["rho"] = rho
    data["params"]["x1"] = x1
    data["params"]["x2"] = x2
    data["params"]["y1latex"] = latex(y1)
    data["params"]["y2latex"] = latex(y2)
    data["params"]["y1"] = str(y1)
    data["params"]["y2"] = str(y2)
    data["params"]["A1"] = p1[0]
    data["params"]["A2"] = p2[0]
    data["params"]["B1"] = p1[1]
    data["params"]["B2"] = p2[1]
    ...
    data["correct_answers"]["IC"] = float(IC)
