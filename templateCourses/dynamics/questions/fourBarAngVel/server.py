...
def generate(data):
	...
	[bottomLeft, bottomRight, topLeft, topRight, center, extent] = boundingBox2D(bboxlist)
	...
	if (rADAngle - theta) < (rADAngle + theta):
		drawOmega1 = arcArrow([Ax, Ay], rADAngle - theta, rADAngle + theta, scalar*r, label = "\\omega_1", offsetx = -25., offsety=5., color="#64B400")
	else:
		drawOmega1 = arcArrow([Ax, Ay], rADAngle + theta, rADAngle - theta, scalar*r, label = "\\omega_1", offsetx = -25., offsety=5., color="#64B400")

	if (rBCAngle - theta) < (rBCAngle + theta):
		drawOmega2 = arcArrow([Bx, By], rBCAngle - theta, rBCAngle + theta, scalar*r, label = "\\omega_2", color="#64B400")
	else:
		drawOmega2 = arcArrow([Bx, By], rBCAngle + theta, rBCAngle - theta, scalar*r, label = "\\omega_2", color="#64B400")

	if (rDCAngle - theta) < (rDCAngle + theta):
		drawOmega3 = arcArrow([Dx, Dy], rDCAngle - theta, rDCAngle + theta, scalar*r, label = "\\omega_3", color="#64B400", offsetx=10., offsety=-10.)
	else:
		drawOmega3 = arcArrow([Dx, Dy], rDCAngle + theta, rDCAngle - theta, scalar*r, label = "\\omega_3", color="#64B400", offsetx=10., offsety=-10.)
