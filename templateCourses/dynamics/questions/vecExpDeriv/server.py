...
def generate(data):
	...
	if case == 1:
		...
		expr = "\\frac{d}{dt} \\operatorname{Proj}\\left(\\vec{a}(t), \\hat{a}(t)\\right)"
		...
		expr = "\\frac{d}{dt} \\operatorname{Proj}\\left(\\hat{a}(t), \\vec{a}(t)\\right)"
		ansValue1 = float(aHatDot[0])
		ansValue2 = float(aHatDot[1])
		units = "{\\rm\\ s^{-1}}"
		data['correct_answers']['ansValue1'] = ansValue1
	...
	if case not in vector_list:
		data['params']['submitAnswer'] = '''<pl-number-input answers_name="ansScalar" comparison="relabs" rtol="1e-2" atol="1e-5" size="15" show-placeholder="false"></pl-number-input>'''
	else:
