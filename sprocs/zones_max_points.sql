...
BEGIN
    ...
    -- #########################################################################
    -- compute the max points by zone
    RETURN QUERY
        WITH all_questions AS (
                SELECT
                    iq.id AS qid,
                    z.id AS zid,
                    aq.max_points AS points,
                    row_number() OVER (PARTITION BY z.id ORDER BY aq.max_points DESC) AS points_rank,
                    z.number_grade,
                    z.max_points
                FROM
                ...
                WHERE
                    -- need zones_max_points.assessment_instance_id because just
                    -- assessment_instance_id would be ambiguous (it has already been
                    -- joined as a column to the table from which we are selecting)
                    iq.assessment_instance_id = zones_max_points.assessment_instance_id
