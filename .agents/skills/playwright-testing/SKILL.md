...
Prefer using selectors that use user-facing content, such as `getByRole`, `getByText`, and `getByLabel`. If that's impractical, consider using `getByTestId` instead. Add `data-testid` attributes or `aria-label` to page components when needed. Avoid using `locator()` with implementation details like CSS classes or IDs unless absolutely necessary.
