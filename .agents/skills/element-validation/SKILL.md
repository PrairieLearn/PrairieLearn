...
An element with a schema module should enforce it at runtime in `prepare()` rather than hand-maintaining `pl.check_attribs()` attribute lists, which duplicate the schema and drift from it. Both helpers load the generated JSON (the single source of truth) and raise linter-matching messages; they check attribute names, required attributes, and per-attribute formats/enums/patterns — a superset of `check_attribs`. `prepare()` receives post-mustache HTML, so templated typed attributes are already concrete when validated. Keep genuinely semantic checks (cross-attribute rules, cardinality, uniqueness) in Python.
