    ...
    if data['panel'] == 'question':
        ...
        for html_tags in pl_drag_drop_element: 
            if html_tags.tag == 'pl-answer':
                ...
                mcq_options.append(str.strip(html_tags.text)) # store the original specified
                                                              # ordering of all the MCQ options
        ...
        html_string += f"<li class='ui-sortable-handle info info-fixed'>Choose from these option:</li>"
...
def grade(element_html, data):
    ...
    if permutationMode == 'any':
        ...
        data['partial_scores'][answerName] = {'score': float(len(intersection)/len(true_answer)), 'feedback': f'' }
    elif permutationMode == 'html-order':
        if student_answer == true_answer:
            data['partial_scores'][answerName] = {'score': float(1.0), 'feedback': f'<strong> Note that you have to correctly select all of the answers, as well as place the answers in the correct order, for any credit to be granted. There is no partial credit for this question. </strong> <br>' }
        else:
            data['partial_scores'][answerName] = {'score': float(0.0), 'feedback': f'<strong> Note that you have to correctly select all of the answers, as well as place the answers in the correct order, for any credit to be granted. There is no partial credit for this question. </strong> <br>' }
        ...
        if len(ranking) != 0 and len(ranking) == len(true_answer):
            if ranking[0] == 1:
                partial_credit = 1 # student will at least get 1 point
                                   # for getting first element correct
        ...
        data['partial_scores'][answerName] = {'score': float(correctness/len(true_answer)), 'feedback': f'<strong>Note that ordering matters in your answer, and that correct ordering is necessary for credit to be given. </strong>' }
