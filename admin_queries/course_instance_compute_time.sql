...
workspace_durations AS (
    SELECT
        ...
        sum(extract(epoch from w.launching_duration + w.running_duration) / 3600.0) AS duration_hours
    FROM
        workspaces AS w
...
grading_job_durations AS (
    SELECT
        ...
        sum(extract(epoch from gj.grading_finished_at - gj.grading_received_at) / 3600.0) AS duration_hours
    FROM
        grading_jobs AS gj
