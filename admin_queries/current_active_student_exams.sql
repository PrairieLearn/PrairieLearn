...
SELECT
    ...
    EXTRACT(EPOCH FROM (now() - last_views.date)) AS _sortval_since_last_view,
    format_interval(now() - last_submissions.date) AS since_last_submission,
    EXTRACT(EPOCH FROM (now() - last_submissions.date)) AS _sortval_since_last_submission,
...
WHERE
    (
        ((last_submissions.date IS NULL) AND (ai.date > now() - interval '1 hour'))
