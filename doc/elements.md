...
The provided `script-name` corresponds to a file located within the director for the question. Parameter names are those stored in `data["params"]["m"]` in `server.py` and accessed as `{{params.m}}` in `question.html`.
