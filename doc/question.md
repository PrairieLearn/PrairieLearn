...
def generate(data):
    ...
    # Also compute the correct answer (if there is one) and store in the data["correct_answer"] dict:
    if data["params"]["operation"] == "double"
        data["correct_answer"]["y"] = 2 * data["params"]["x"]
    else:
        data["correct_answer"]["y"] = 3 * data["params"]["x"]
