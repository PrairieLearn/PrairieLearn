define(function() {
    ...
    Renderer.prototype.countsProgressBar = function(counts, objectType, toolTipTexts, placement, showCounts, percentages) {
        var total = counts.success + counts.warning + counts.danger;
        total = Math.max(1, total);
        var progressBar = function(type) {
            var tooltip;
            if (percentages) {
                tooltip = (counts[type] / total * 100).toFixed(0) + '% of ' + objectType + 's have ' + toolTipTexts[type];
            } else {
                tooltip = counts[type] + ' ' + objectType + ((counts[type] !== 1) ? 's have ' : ' has ') + toolTipTexts[type];
            }
            return '<div class="progress-bar progress-bar-' + type + '"'
                + ' data-toggle="tooltip"'
                + ' data-placement="' + placement + '"'
                + ' data-original-title="' + tooltip + '"'
                + ' style="width: ' + (counts[type] / total * 100).toFixed(3) + '%">'
                + (showCounts ? ('<div class="progress-num">' + ((counts[type] > 0) ? counts[type] : "") + '</div>') : '')
                + '</div>';
        };
        return '<div class="progress">' + progressBar("success") + progressBar("warning") + progressBar("danger") + '</div>';
    };
    ...
    */
    Renderer.prototype.answerList = function(type, answers) {
        var entries = [], i, answerHTML;
        if (type === "radio") {
            for (var i = 0; i < answers.length; i++) {
                answerHTML = '';
                answerHTML += '<div class="radio">';
                answerHTML += '<label>';
                answerHTML += '<input type="radio" name="selection" value="a' + i + '" data-checked="submittedAnswer.selection" />';
