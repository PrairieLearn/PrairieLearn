...
define(["underscore", "sha1"], function(_, Sha1) {
    ...
    */
    var makeTeXArray = function(entries) {
        return _.chain(entries).filter(function(row) {
            return _(row).some(function(e) {return (e.length > 0);});
        }).map(function(row, i, list) {
            return _(row).map(function(e) {
                return (e.length > 0) ? e : " &amp; ";
            }).join(" &amp; ");
        }).value().join(" \\\\ ");
    };
    ...
    */
    var polyString = function(poly, indVar) {
        var i, vec = [], basis = ["", "", "", "", "", ""];
        if (poly.length > 6)
            throw Error("poly length exceeds 6");
    ...
    */
    var fcnString = function(fcn, indVar) {
        /* jshint indent: false */
        switch (fcn.fcn) {
        case "poly": return polyString(fcn.data, indVar);
        case "exp": return expString(fcn.data, indVar);
        case "trig": return trigString(fcn.data, indVar);
        }
    };
    ...
    */
    var parenFcnString = function(fcn, indVar) {
        /* jshint indent: false */
        switch (fcn.fcn) {
        case "poly": return parenPolyString(fcn.data, indVar);
        case "exp": return expString(fcn.data, indVar);
        case "trig": return trigString(fcn.data, indVar);
        }
    };
